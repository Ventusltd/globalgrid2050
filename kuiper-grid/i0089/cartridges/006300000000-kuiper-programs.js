...
function makeSldNames(lay, net){
  ...
  if (net.kind === 'string') { const d = net.g.__draw || stringDrawing(net.g);
    const out = d.words.map(w => ({ x:w.x, y:w.y, dy:0, dx:0, text:w.text, small:w.small, warn:!!w.warnLine, note:!!w.note, danger:!!w.danger,
      wide:!!w.wide, narrow:!!w.narrow, pinned:w.pinned }));
...
function stringDrawing(g){
  ...
  for (let k = 1; k <= N; k++) {
    ...
    if (turned(k)) marks.push({ x:(x0 + x1) / 2, y:yb - 0.62, text:'turned', size:9, colour:SHAPE_EDGE, minPx:60, mid:true, dodge:true, rank:3 });
  }
  ...
  const stub = Math.max(0.10, wX * 0.20);
  g.links.forEach((l, i) => {
    const A = g.plusPt(l.from), B = g.minusPt(l.to);
    const outA = g.turned(l.from) ? -1 : 1, outB = g.turned(l.to) ? 1 : -1;
    const P = port ? [A[0] + stub * outA, A[1]] : [A[0], A[1] + stub * outA];
    const Q = port ? [B[0] + stub * outB, B[1]] : [B[0], B[1] + stub * outB];
    const dx = Q[0] - P[0], dy = Q[1] - P[1], d = Math.max(1e-6, Math.hypot(dx, dy));
    const sameRow = g.rowOf(l.from) === g.rowOf(l.to);
    const dir = l.back ? -1 : 1;
    let arc;
    if (sameRow) { const sag = rows === 1 ? (0.9 + d * 0.30 + (i % 3) * 0.22)
                                          : Math.min(wY * 0.42, boxH * 1.15 + d * 0.10 + (i % 3) * 0.07);
      const c1 = port ? [P[0] + dx * 0.22, P[1] + dir * sag * 1.25] : [P[0] + dir * sag * 1.25, P[1] + dy * 0.22];
      const c2 = port ? [Q[0] - dx * 0.22, Q[1] + dir * sag * 1.25] : [Q[0] - dir * sag * 1.25, Q[1] - dy * 0.22];
      arc = bez(P, c1, c2, Q, 26); }
    else { const nx = -dy / d, ny = dx / d, bow = 0.34 + 0.12 * (i % 3);
      arc = bez(P, [P[0] + nx * bow, P[1] + ny * bow], [Q[0] + nx * bow, Q[1] + ny * bow], Q, 24); }
    const pts = [A].concat(arc.slice(0)).concat([]);
    pts.splice(1, 0, P);
    for (const q of pts) { laneHi = Math.max(laneHi, q[1]); laneLo = Math.min(laneLo, q[1]); }
    const full = pts.concat([B]);
    ...
    const node = 'L' + i, mid = polyLen(full);
    conns.push({ node, from:fromId, to:toId, needed:l.need, pair:g.pair, reaches:l.reaches, jumper:l.jumper || 0 });
    const tAt = u2 => polyAt(full, mid.cum, mid.total, u2);
    if (l.reaches) { const t = Math.min(0.90, Math.max(0.10, lp / Math.max(1e-6, g.pair)));
      poly('lead', o, polySlice(full, 0, t), { node, pot, plus:true, link:l, says, hit:true, from:fromId, to:'mate' + i });
      poly('lead', o, polySlice(full, t, 1), { node, pot, plus:false, link:l, says, hit:true, from:'mate' + i, to:toId });
      const j = tAt(t), tan = tAt(Math.min(1, t + 0.02));
      put({ role:'mate', order:o, pot, node, pts:[[j[0], j[1]]], link:l, says, mated:true,
            along:[tan[0] - j[0], tan[1] - j[1]] }); }
    else { const a = Math.min(0.84, Math.max(0.08, lp / need)), b = Math.min(0.95, Math.max(a + 0.06, 1 - lm / need));
      poly('lead', o, polySlice(full, 0, a), { node, pot, plus:true, link:l, says, hit:true, open:true, from:fromId, to:'open' + i });
      poly('lead', o, polySlice(full, b, 1), { node, pot, plus:false, link:l, says, hit:true, open:true, from:'open' + i, to:toId });
      const p0 = tAt(a), p1 = tAt(b), t0 = tAt(Math.max(0, a - 0.02)), t1 = tAt(Math.min(1, b + 0.02));
      put({ role:'plug', order:o, pot, node, pts:[[p0[0], p0[1]]], male:true, along:[p0[0] - t0[0], p0[1] - t0[1]] });
      put({ role:'plug', order:o, pot, node, pts:[[p1[0], p1[1]]], male:false, along:[t1[0] - p1[0], t1[1] - p1[1]] });
  ...
  return { kind:'string', shapes, marks, words, conns, terminals, wiring, perM:S, open, g,
...
function drawSldShapes(){
  ...
  const fine = D.wiring ? 2.3 : 1.1, hair = D.wiring ? 0.9 : 0.7;
  ...
  for (const s of D.shapes) {
    if (s.role === 'module' || s.role === 'inverter') { if (!onScreen(s.pts)) continue;
      ...
      ctx.lineWidth = fine * 0.8; ctx.strokeStyle = s.role === 'inverter' ? SHAPE_INK : SHAPE_PENCIL; ctx.stroke(); continue; }
    ...
    if (s.role === 'cells' && s.grid) { if (!onScreen(s.pts)) continue;
      ...
      if (Math.min(cwPx, chPx) < 2.2) continue;
      const fade = Math.min(1, (Math.min(cwPx, chPx) - 2.2) / 3);
      ctx.globalAlpha = 0.85 * fade; ctx.lineWidth = hair; ctx.strokeStyle = SHAPE_FAINT;
      ctx.beginPath();
  ...
  for (const s of D.shapes) {
    ...
    if (D.wiring) {
      const tube = Math.max(5.2, Math.min(16, perM * 0.10));
      const edge = lit ? '#eaf6ff' : s.open ? SHAPE_WARN : (s.plus ? '#ff9c90' : '#c8d6e8');
      const core = lit ? '#ffffff' : s.open ? '#6b5118' : (s.plus ? '#ff3b2e' : '#0e1219');
      // the light edge is a hairline EACH SIDE, not most of the tube: what must read is a black cable
      // catching the light along its edges, never a white pipe with a dark seam
      path(s.pts); ctx.lineWidth = tube; ctx.strokeStyle = edge; ctx.stroke();
      path(s.pts); ctx.lineWidth = Math.max(2.6, tube - 2.0); ctx.strokeStyle = core; ctx.stroke();
      continue; }
  ...
  for (const s of D.shapes) {
    if (s.role === 'box') { if (!onScreen(s.pts)) continue;
      ...
      ctx.lineWidth = s.big ? fine + 0.4 : fine; ctx.strokeStyle = SHAPE_INK; ctx.stroke();
      ...
      if (s.diodeOnly) { const a = S(s.pts[0]), b = S(s.pts[2]);
        const cxm = (a[0] + b[0]) / 2, cym = (a[1] + b[1]) / 2, r2 = Math.min(Math.abs(b[0] - a[0]), Math.abs(b[1] - a[1])) * 0.30;
        if (r2 > 2) { ctx.beginPath(); ctx.moveTo(cxm - r2, cym - r2); ctx.lineTo(cxm - r2, cym + r2); ctx.lineTo(cxm + r2, cym);
          ctx.closePath(); ctx.lineWidth = Math.max(1, fine * 0.6); ctx.strokeStyle = SHAPE_EDGE; ctx.stroke();
    ...
    if (s.role === 'plug') { const q = S(s.pts[0]);
      ...
      ctx.lineWidth = Math.max(1, fine * 0.7); ctx.strokeStyle = SHAPE_INK; ctx.fillStyle = '#10151d';
      ctx.beginPath();
    ...
    if (s.role === 'mate') { const q = S(s.pts[0]);
      ...
      const a2 = s.along || [1, 0], L2 = Math.max(1e-6, Math.hypot(a2[0], a2[1]));
      const ux = a2[0] / L2, uy = -a2[1] / L2, r2 = Math.max(4.5, Math.min(16, perM * 0.09)), r3 = r2 * 0.62;
      ctx.beginPath();

...
(function (root) {
  ...
  // ===== SERIES RESISTANCE AT THREE SCALES =======================================================
  // IMPLANTED, function by function, from LIB/solar-resistance/resistance.mjs — a pure ES-module
  // library that passes 73 of 73 self-tests (node test.mjs) and reproduces the farm bench totals
  // exactly. What is copied here: the data tables (section 1), collectInputs / inputStatus
  // (section 2), r20PerKm / tempFactor / ohmPerKmAt (section 3), normalisePlan + segmentConductors
  // (section 4), connectorPairs (section 5), stringResistance (section 6), ampacity (section 7),
  // blockResistance + farmScale (section 8). Nothing is re-derived and nothing is improved on.
  // IT IS FED THE CONDUCTORS THE DRAWING ALREADY HAS — the module leads, the cable laid along the
  // table, the two home runs — so there is one geometry and not two. EVERY NUMBER FROM HERE GOES
  // TO THE ANSWER AND THE ONE LINE KEY. None of it is ever drawn.
  const RES = (function () {
  ...
  // WHAT THE ANSWER SAYS ABOUT RESISTANCE, at one string, one inverter block of 24 strings, and the
  // whole farm of 963 blocks and 20,222 strings. Fed from the drawing's own conductors. Never drawn.
  function resistanceAnswer(g, inp) {
    const c = g.c, u = inp || {};
    const wiring = g.routing === 'one-after-another' ? 'sequential'
      : (g.leap && c.turn_every_second) ? 'leapfrog-turned' : 'leapfrog';
    // THE CONDUCTORS THE DRAWING ALREADY HAS. The two home runs are the pair to the inverter; what
    // is left of the cable laid on site is the along-table run, the return if one is laid, and any
    // jumper spliced in. Nothing new is measured here.
    const homerun_m = 2 * c.near_end_m, site_m = Math.max(0, g.field_m - homerun_m);
    const conductors = [];
    for (let k = 1; k <= g.N; k++) conductors.push({ id:'lead' + k, kind:'lead', length_m:g.pair });
    conductors.push({ id:'site', kind:'site', length_m:site_m });
    conductors.push({ id:'homerun', kind:'homerun', length_m:homerun_m });
    // EVERY INPUT IS TAGGED. Typed means the reader typed it in the command; assumed means we chose it.
    const typedTemp = u.conductor_c !== undefined && u.conductor_c !== null || u.t_site_c !== undefined;
    const D = {
      modules_per_string:{ value:g.N, source:'BENCH: 30 modules to a string' },
      module_vmp_v:{ value:c.module_vmp, source:'module class, large-format bifacial' },
      module_imp_a:{ value:c.amps, source:'module class, large-format bifacial' },
      module_isc_a:{ value:18.45, source:'module class, large-format bifacial' },
      lead_mm2:{ value:c.lead_mm2, source:'module lead pair as supplied is 4 mm2' },
      site_mm2:{ value:c.cable_mm2, source:'the order log: 4 and 6 mm2 are a dead heat' },
      homerun_mm2:{ value:c.cable_mm2, source:'the order log: 4 and 6 mm2 are a dead heat' },
      conductor_temp_c:{ value:c.t_site_c, source:'BENCH: a conductor temperature is a RESULT, not a rating' },
      ambient_c:{ value:90, source:'the highest ambient the cable class is rated for — the honest worst case' },
      contact_mohm_per_pair:{ value:c.contact_milliohm, source:'BENCH: 0.35 milliohm a mated pair. The weakest number here — measure it before quoting anything that turns on it.' },
      n_parallel:{ value:1, source:'one cable a pole' },
      parallel_scope:{ value:'site+homerun', source:'n a pole applies to the cable laid on site, not to the module leads' },
      home_run_m:{ value:c.near_end_m, source:'the drawing: the near end of the table from the inverter' },
      design_current_factor:{ value:1.375, source:'the array standard 1.25 on Isc times an ASSUMED 1.1 for bifacial rear gain' },
      containment:{ value:'preset', source:'the proposal: leads frame-clipped, the along-table run on C rail, the home run 60% tray then 40% duct' },
      home_run_bands_m:{ value:'20/45/70, eight strings each', source:'BENCH block_plan; mean 45 m matches the 44.61 m a string the farm file implies' },
      group_factor:{ value:null, source:'NO DEFAULT, on purpose: it decides the ampacity check' },
    };
    const user = { modules_per_string:u.modules, module_vmp_v:u.module_vmp, module_imp_a:u.amps,
      module_isc_a:u.module_isc_a, lead_mm2:u.lead_mm2, site_mm2:u.cable_mm2, homerun_mm2:u.cable_mm2,
      conductor_temp_c: typedTemp ? c.t_site_c : undefined, ambient_c:u.ambient_c,
      contact_mohm_per_pair:u.contact_milliohm, n_parallel:u.n_parallel, parallel_scope:u.parallel_scope,
      home_run_m:u.near_end_m, design_current_factor:u.design_current_factor,
      containment:u.containment ? 'typed' : undefined, home_run_bands_m:u.home_run_bands ? 'typed' : undefined,
      group_factor:u.group_factor };
    const inputs = RES.collect(D, user), status = RES.inputStatus(inputs);
    const v = k => inputs[k].value;
    // CONTAINMENT PER SEGMENT is a declared input. Typing containment:{site:'conduit-duct'} or a
    // full plan per kind replaces the preset proposal for that kind and nothing else.
    const preset = Object.assign({}, RES.PRESET);
    if (u.containment && typeof u.containment === 'object') for (const k of Object.keys(u.containment)) {
      const val2 = u.containment[k];
      preset[k] = typeof val2 === 'string' ? [{ containment:val2, frac:1 }] : val2;
    }
    const bands = (u.home_run_bands && u.home_run_bands.length) ? u.home_run_bands : RES.BANDS;
    // THE WHOLE PIPELINE AS A FUNCTION OF THE DECLARED INPUT VALUES. The default run is this fired
    // once with the values as declared; the sensitivity below is this fired again with ONE value
    // nudged. There is no second model and no derivative: it is the same code path, enumerated.
    function scales(val) {
      const cond = [];
      for (let k = 1; k <= val.modules_per_string; k++) cond.push({ id:'lead' + k, kind:'lead', length_m:g.pair });
      cond.push({ id:'site', kind:'site', length_m:site_m });
      cond.push({ id:'homerun', kind:'homerun', length_m:2 * val.home_run_m });
      const S2 = RES.stringResistance({ conductors:cond, wiring, preset,
        modules_per_string:val.modules_per_string, module_vmp_v:val.module_vmp_v, module_imp_a:val.module_imp_a,
        lead_mm2:val.lead_mm2, site_mm2:val.site_mm2, homerun_mm2:val.homerun_mm2,
        conductor_temp_c:val.conductor_temp_c, contact_mohm_per_pair:val.contact_mohm_per_pair,
        n_parallel:val.n_parallel, parallel_scope:val.parallel_scope });
      // ONE INVERTER BLOCK: 24 strings, the home run declared band by band. Only the home run changes
      // from band to band, so the band's string is this string with its home run put back.
      const ohmHome = RES.ohmPerKmAt(val.homerun_mm2, val.conductor_temp_c) / 1000 / Math.max(1, val.n_parallel);
      const rest = S2.resistance.r_total_ohm - S2.resistance.r_homerun_ohm;
      const bandStrings = [];
      for (const b of bands) for (let i = 0; i < b.strings; i++) {
        const r_total = rest + 2 * b.home_run_m * ohmHome;
        bandStrings.push({ home_run_m:b.home_run_m, r_total_ohm:r_total, vdrop_v:S2.current_a * r_total,
                           vdrop_pct:S2.current_a * r_total / S2.vmp_string_v * 100, loss_w:S2.current_a * S2.current_a * r_total });
      }
      const nS = bandStrings.length, sum = f => bandStrings.reduce((t, x) => t + f(x), 0);
      const B2 = { n_strings:nS, home_run_bands:bands,
        r_total_ohm_mean:sum(x => x.r_total_ohm) / nS,
        r_total_ohm_min:Math.min.apply(null, bandStrings.map(x => x.r_total_ohm)),
        r_total_ohm_max:Math.max.apply(null, bandStrings.map(x => x.r_total_ohm)),
        vdrop_v_mean:sum(x => x.vdrop_v) / nS, vdrop_pct_mean:sum(x => x.vdrop_pct) / nS,
        vdrop_pct_max:Math.max.apply(null, bandStrings.map(x => x.vdrop_pct)),
        loss_w:sum(x => x.loss_w), p_w:nS * S2.p_string_w,
        connectors:nS * S2.connectors.total };
      B2.loss_pct = B2.loss_w / B2.p_w * 100;
      // THE FARM IS READ, NOT RECOMPUTED: the km were summed block by block by the bench and are
      // carried here as constants. The loss is this block's loss across 963 blocks.
      const fb = RES.FARM.builds[RES.FARM_BUILD[wiring]];
      const F2 = { blocks:RES.FARM.blocks, strings:RES.FARM.strings, build:RES.FARM_BUILD[wiring],
        lead_km:fb.lead_km, site_km:fb.site_km, total_km:fb.lead_km + fb.site_km,
        km_source:'BENCH constants, summed block by block — not recomputed from this drawing',
        r_total_ohm_mean:B2.r_total_ohm_mean, vdrop_v_mean:B2.vdrop_v_mean, vdrop_pct_mean:B2.vdrop_pct_mean,
        loss_w:B2.loss_w * RES.FARM.blocks, p_w:B2.p_w * RES.FARM.blocks,
        connectors:B2.connectors * RES.FARM.blocks };
      F2.loss_pct = F2.loss_w / F2.p_w * 100;
      return { S:S2, B:B2, F:F2 };
    }
    const V0 = {}; for (const k of Object.keys(inputs)) V0[k] = inputs[k].value;
    const base = scales(V0), S = base.S, B = base.B, F = base.F;
    // --- SENSITIVITY BY ENUMERATION. Every numeric input is nudged in turn and the three scales are
    // recomputed from scratch. A cross-section is a catalogue step, not a dial, so its nudge is the
    // NEXT LISTED SIZE and it is labelled as such; everything else gets +10%. An input that moves
    // nothing moves nothing — that is reported, not hidden.
    const NEXT_CSA = { 1.5:2.5, 2.5:4, 4:6, 6:10, 10:16, 16:null };
    const IS_CSA = { lead_mm2:1, site_mm2:1, homerun_mm2:1 };
    const SHORT = { modules_per_string:'modules', module_vmp_v:'Vmp', module_imp_a:'Imp', module_isc_a:'Isc',
      lead_mm2:'lead mm2', site_mm2:'site mm2', homerun_mm2:'homerun mm2', conductor_temp_c:'conductor C',
      ambient_c:'ambient C', contact_mohm_per_pair:'contact mOhm', n_parallel:'n a pole', home_run_m:'home run m',
      design_current_factor:'design factor' };
    const movers = [];
    for (const k of Object.keys(V0)) {
      const v0 = V0[k];
      if (typeof v0 !== 'number' || !isFinite(v0)) continue;
      const v1 = IS_CSA[k] ? NEXT_CSA[v0] : v0 * 1.1;
      if (v1 === null || v1 === undefined || v1 === v0) continue;
      const alt = Object.assign({}, V0); alt[k] = v1;
      let t; try { t = scales(alt); } catch (e) { continue; }
      const rel = (a, b2) => a === 0 ? 0 : (b2 - a) / a * 100;
      movers.push({ key:k, label:SHORT[k] || k, status:inputs[k].status,
        how: IS_CSA[k] ? ('to ' + v1 + ' mm2') : '+10%', from:v0, to:v1,
        string_pct:rel(base.S.loss_w, t.S.loss_w), block_pct:rel(base.B.loss_w, t.B.loss_w),
        farm_pct:rel(base.F.loss_w, t.F.loss_w) });
    }
    const topBy = f => movers.slice().sort((a, b2) => Math.abs(f(b2)) - Math.abs(f(a))).slice(0, 3);
    const sensitivity = { method:'each input nudged in turn, the whole answer fired again; no calculus',
      nudge:'+10%, except a cross-section which steps to the next listed size',
      measured:'change in I2R loss at that scale', all:movers,
      string:topBy(m => m.string_pct), block:topBy(m => m.block_pct), farm:topBy(m => m.farm_pct) };
    // THE AMPACITY SITUATION, run by run. The verdict is null everywhere until a group factor is typed.
    const seenRuns = {}, amps = [];
    for (const s of S.segments) {
      if (seenRuns[s.run_id]) continue; seenRuns[s.run_id] = 1;
      amps.push(RES.ampacity({ containment:s.containment, circuits_sharing:s.circuits_sharing,
        csa_mm2: s.kind === 'lead' ? v('lead_mm2') : s.kind === 'site' ? v('site_mm2') : v('homerun_mm2'),
        ambient_c:v('ambient_c'), module_isc_a:v('module_isc_a'), design_current_factor:v('design_current_factor'),
        n_parallel:v('n_parallel'), group_factor:v('group_factor') }));
    }
    // THE TABLE, three scales side by side, as fixed-width text for the pop-up.
    ...
    const row = (label, a, b2, d) => pad(label, 26) + rpad(a, 14) + rpad(b2, 16) + rpad(d, 18);
    const oh = x => x.toFixed(4) + ' Ω';
    // A MOVER CELL: the input's short name, how it was nudged, and what that did to this column.
    const sgn = x => (x >= 0 ? '+' : '') + x.toFixed(2) + '%';
    const mcell = (list, i, f) => list[i] ? list[i].label + ' ' + list[i].how + ' ' + sgn(f(list[i])) : '—';
    const mrow = (label, i) => pad(label, 26) + pad(mcell(sensitivity.string, i, m => m.string_pct), 24)
      + pad(mcell(sensitivity.block, i, m => m.block_pct), 24)
      + pad(mcell(sensitivity.farm, i, m => m.farm_pct), 24);
    const frow = (a, b2, d, e, f2) => pad(a, 30) + rpad(b2, 12) + rpad(d, 14) + rpad(e, 14) + '  ' + f2;
    const lines = [
      row('', 'PER STRING', 'PER BLOCK (24)', 'PER FARM (20,222)'),
      row('module leads', oh(S.resistance.r_lead_ohm), '—', '—'),
      row('cable along the table', oh(S.resistance.r_site_ohm), '—', '—'),
      row('home run (2 x ' + v('home_run_m') + ' m)', oh(S.resistance.r_homerun_ohm), '—', '—'),
      row('cable subtotal', oh(S.resistance.r_cable_ohm), '—', '—'),
      row(S.connectors.total + ' pairs x ' + v('contact_mohm_per_pair') + ' mΩ', oh(S.resistance.r_contact_ohm), '—', '—'),
      row('SERIES RESISTANCE', oh(S.resistance.r_total_ohm), oh(B.r_total_ohm_mean) + ' mean', oh(F.r_total_ohm_mean) + ' mean'),
      row('   range over the bands', '—', oh(B.r_total_ohm_min) + '–' + B.r_total_ohm_max.toFixed(4), '—'),
      row('volt drop', S.vdrop_v.toFixed(2) + ' V', B.vdrop_v_mean.toFixed(2) + ' V mean', F.vdrop_v_mean.toFixed(2) + ' V mean'),
      row('volt drop %', S.vdrop_pct.toFixed(3) + ' %', B.vdrop_pct_mean.toFixed(3) + ' %', F.vdrop_pct_mean.toFixed(3) + ' %'),
      row('loss', S.loss_w.toFixed(1) + ' W', (B.loss_w / 1000).toFixed(3) + ' kW', (F.loss_w / 1e6).toFixed(3) + ' MW'),
      row('loss % of DC', S.loss_pct.toFixed(3) + ' %', B.loss_pct.toFixed(3) + ' %', F.loss_pct.toFixed(3) + ' %'),
      row('conductor', (S.length_m).toFixed(1) + ' m', (S.length_m * 24 / 1000).toFixed(3) + ' km', (F.total_km).toFixed(3) + ' km'),
      row('mated pairs', S.connectors.total, B.connectors, F.connectors),
      '',
      // WHICH INPUTS MATTER, scale by scale, sitting under the column each one belongs to.
      'WHAT MOVES EACH COLUMN. ' + status.words + ', so here is which of them the answer actually',
      'turns on: each input nudged in turn and the whole answer fired again — enumeration, not calculus.',
      'Nudge: +10%, except a cross-section, which steps to the next listed size. Measured on I2R loss.',
      row('', 'PER STRING', 'PER BLOCK (24)', 'PER FARM (20,222)'),
      mrow('1st mover', 0), mrow('2nd mover', 1), mrow('3rd mover', 2),
      '   ' + (movers.filter(m => Math.abs(m.string_pct) < 1e-9).map(m => m.label).join(', ') || 'none') + ' move the loss not at all',
      '   the block and the farm cannot see the home run length: their home runs come from the bands',
      '',
      // THE FARM COLUMN, SAID PLAINLY: its km are constants, and all three builds are shown.
      'THE FARM COLUMN IS NOT COMPUTED FROM THIS DRAWING. Its conductor km are BENCH CONSTANTS, summed',
      'block by block over ' + F.blocks + ' blocks and ' + F.strings.toLocaleString('en-GB') + ' strings. The three builds as the bench left them:',
      frow('build', 'lead km', 'site km', 'total km', ''),
    ].concat(Object.keys(RES.FARM.builds).map(k =>
      frow('  ' + k, RES.FARM.builds[k].lead_km.toFixed(3), RES.FARM.builds[k].site_km.toFixed(3),
           (RES.FARM.builds[k].lead_km + RES.FARM.builds[k].site_km).toFixed(3),
           k === F.build ? '<-- this drawing' : ''))).concat([
      '   the loss and the resistance in that column ARE computed: this block, taken ' + F.blocks + ' times.',
      '',
      'n cables in parallel a pole: ' + v('n_parallel') + ' (' + inputs.n_parallel.status + '), applied to ' + v('parallel_scope'),
      'containment: ' + S.segments.map(s => s.kind + ' ' + s.containment + ' ' + s.length_m.toFixed(1) + ' m').filter((x, i, a) => a.indexOf(x) === i).join('; '),
      'ampacity: verdict null everywhere — group factor must be typed',
      status.words + ' — the movers above say which of those assumptions are worth typing first',
    ]);
    const key = 'STRING ' + S.resistance.r_total_ohm.toFixed(4) + ' Ω · ' + S.vdrop_v.toFixed(2) + ' V · '
      + S.vdrop_pct.toFixed(2) + '% · ' + S.loss_w.toFixed(0) + ' W | BLOCK ' + (B.loss_w / 1000).toFixed(2) + ' kW | FARM '
      + (F.loss_w / 1e6).toFixed(2) + ' MW · ampacity null, group factor must be typed · ' + status.words;
    return { wiring, wiring_label:S.wiring_label, string:S, block:B, farm:F, ampacity:amps,
             inputs, status, sensitivity, containment_preset:preset, table:lines, table_text:lines.join('\n'), key };
  }

  function run(inp) {
    const g = build(inp), c = g.c, worst = g.short.length ? Math.max(...g.short.map(l => l.need - g.pair)) : 0;
    const RE = resistanceAnswer(g, inp);
    ...
    return { connections, numbers: { circuit: open ? 'OPEN, both ends live' : 'closed', current_a: rnd(amps, 2),
               ...
               // THE THREE SCALES, from the implanted resistance library. NONE OF THIS IS DRAWN.
               series_resistance_ohm_string: rnd(RE.string.resistance.r_total_ohm, 4),
               series_resistance_ohm_block_mean: rnd(RE.block.r_total_ohm_mean, 4),
               volt_drop_v_string: rnd(RE.string.vdrop_v, 2), volt_drop_pct_string: rnd(RE.string.vdrop_pct, 3),
               loss_w_string: rnd(RE.string.loss_w, 1), loss_kw_block: rnd(RE.block.loss_w / 1000, 3),
               loss_mw_farm: rnd(RE.farm.loss_w / 1e6, 3), farm_blocks: RE.farm.blocks, farm_strings: RE.farm.strings,
               farm_lead_km: RE.farm.lead_km, farm_site_km: RE.farm.site_km,
               cables_in_parallel: RE.inputs.n_parallel.value,
               ampacity_verdict: null, ampacity_reason: 'group factor must be typed',
               inputs_assumed: RE.status.words },
             said, electrical_order: order, resistance: RE, resistance_table: RE.table, key: RE.key,

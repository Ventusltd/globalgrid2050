...
const BODY_RADIUS = 0.30, BODY_RADIUS_MAX = 0.92, DOT_MAX_PX = 3.5, DOT_MIN_PX = 1.0;
...
function makeBody(st){
  ...
  for (const step of seq) { const shape = step.shape === 'text' ? 'text:' + nameInLines(st.app.button) : step.shape;
    ...
    if (step.shape === 'text' && m < 6 * st.app.button.replace(/[^A-Za-z0-9]/g, '').length) continue;
...
function bodyJump(k){ if (!body) return; const stt = body.states[k]; body.pos.set(stt.to); body.k = k; body.gap = stt.gap; body.shown = true; bodyAnim = null; bodyTo(body.pos); dirty = true; }
function bodyMove(k){ if (!body) return; bodyAnim = { k, t0:performance.now(), from:new Float32Array(body.pos), gap0:body.shown ? body.gap : body.states[k].gap, phase:'move' };
...
function isolateNow(name, front){
  const prog = programs.find(x => x.name === name); if (!prog) return null;
...
function unisolate(quiet){ iso = null; isoAnim = null; gatherAnim = null; homeAnim = null; body = null; bodyAnim = null; pulse = -1;

// Copied from testcode/202609142202/proof/add_all_lines.mjs. Change: build_all_lines.mjs now writes straight into this stamp's data/, so SRC is data/
// itself (the copy step rewrites the same bytes) and paths are relative to this file. Otherwise unchanged.
// Generation 2 data step: copy the all-lines pack (built by build_all_lines.mjs from the public LINES.md) into data/
...
const DIR = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..').split(path.sep).join('/') + '/';
const SRC = DIR + 'data/';
...
prov.sources.push({ url: meta.source.url, bytes: meta.source.bytes, sha256: meta.source.sha256, fetched_utc: meta.built_utc, http_status: 200, attempts: 1, fetched_by: 'build_all_lines.mjs', header: meta.source.header });

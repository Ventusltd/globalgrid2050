...
(function (root) {
  ...
  const DEF = { modules: 30, width_m: 1.303, module_height_m: 2.384, cell_cols: 6, cell_rows: 22, module_vmp: 38.1, module_voc: 45.9,
                ...
                leads: 'reach', lead_slack_m: 0.20, lead_plus_m: null, lead_minus_m: null, lead_mm2: 4, cable_mm2: 6, near_end_m: 59, amps: 12.90,
                ...
                // ===== WHAT AN ELECTRICAL ENGINEER IS CHECKED ON, AND THE FIGURES IT TAKES TO ANSWER.
                // The model already held the geometry and the resistance; it did not hold the numbers a
                // designer is actually judged by - cold open circuit against the system limit, the hot
                // most-power voltage against the tracker's window, the design current against what a
                // connector and an input will carry, the power against the inverter. Every one is
                // DECLARED HERE with an assumption, and every one is marked ASSUMED in the answer until
                // somebody types it. An assumed figure that says it is assumed is honest; an assumed
                // figure that does not say so is a lie with a decimal point.
                module_isc: 13.65, module_imp: null,
                t_min_c: -10, voc_coeff_pct_per_k: -0.25, cell_hot_c: 70, pmax_coeff_pct_per_k: -0.30,
                system_max_v: 1500, mppt_min_v: 500,
                bifacial_factor: 1.10, connector_a: 30, input_a: 40, strings_per_tracker: 2,
                strings_per_block: 24, inverter_kva_30c: 352, inverter_kva_40c: 320, inverter_kva_50c: 295,
                // THE FUSE QUESTION HAS TWO WORLDS AND ONLY THE INVERTER MAKER CAN SAY WHICH ONE IT IS.
                // 0 means it is not known, and the answer says what must be asked of the maker; 1 means
                // the maker has stated in writing that the inputs block reverse current, and with two
                // strings per tracker no string fuse is needed. This one is never guessed quietly.
                inputs_block_reverse_current: 0,
                insulation_mohm_min: 1 };
  ...
  // ===== THE VALUES PANEL. EVERY ELECTRICAL FIGURE A DESIGNER IS ASKED FOR, ON THE FACE OF THE ANSWER.
  // A tester counted seven of sixteen: the model knew the cable and not the design. Each line is the
  // same five things and never fewer - LABEL, VALUE with its UNIT, the LIMIT it is held against, the
  // VERDICT IN WORDS (not a tick, not a colour: words a person can repeat in a meeting), and whether
  // the figure behind it was TYPED by a reader or is one of our declared ASSUMPTIONS. A panel that
  // hides which is which is worse than no panel, because it is believed.
  function valuePanel(g, typedKeys) {
    const c = g.c, N = g.N, V = [];
    const typed = k => typedKeys.indexOf(k) >= 0;
    // A LINE IS AS TYPED AS ITS LEAST TYPED INPUT. One assumed figure in a sum makes the sum assumed.
    const basis = keys => keys.every(typed) ? 'TYPED' : 'ASSUMED';
    const add = (label, value, unit, limit, verdict, keys, note) =>
      V.push({ label, value, unit, limit, verdict, basis: basis(keys), note: note || '' });
    const imp = (c.module_imp === null || c.module_imp === undefined || c.module_imp === '') ? +c.amps : +c.module_imp;
    const impKey = (c.module_imp === null || c.module_imp === undefined || c.module_imp === '') ? 'amps' : 'module_imp';
    const isc = +c.module_isc, vmax = +c.system_max_v;
    // 1-2. OPEN CIRCUIT. At the standard's 25 C, and then at the cold design temperature, which is the
    // one that decides how many modules may be put in series. The method is the standard's own:
    // N x Voc x (1 + coeff x (Tmin + 10 K - 25 C)). The 10 K is the allowance the method carries.
    const vocStc = N * c.module_voc;
    const tCold = +c.t_min_c + 10 - 25;
    const vocCold = vocStc * (1 + (+c.voc_coeff_pct_per_k / 100) * tCold);
    const margin = vmax - vocCold;
    add('string Voc at STC (25 C)', rnd(vocStc, 1), 'V', vmax + ' V system limit',
        vocStc <= vmax ? 'inside the system limit at 25 C, which is not the check that matters' : 'OVER the system limit even at 25 C',
        ['modules', 'module_voc', 'system_max_v']);
    add('string Voc at the cold design temperature (' + c.t_min_c + ' C + 10 K)', rnd(vocCold, 1), 'V', vmax + ' V system limit',
        vocCold <= vmax ? 'inside the system limit: this is the check that fixes the string length' : 'OVER THE SYSTEM LIMIT: the string is too long for ' + vmax + ' V and must be shortened',
        ['modules', 'module_voc', 'system_max_v', 't_min_c', 'voc_coeff_pct_per_k'],
        'N x Voc x (1 + ' + c.voc_coeff_pct_per_k + ' %/K x (' + c.t_min_c + ' + 10 - 25))');
    add('margin to ' + vmax + ' V', rnd(margin, 1), 'V', 'above 0 V',
        margin < 0 ? 'NEGATIVE: the limit is exceeded when it is cold' : margin < 25 ? 'positive but thin: less than one module of headroom' : 'positive, ' + Math.floor(margin / (c.module_voc * 1.09)) + ' more modules would still fit',
        ['modules', 'module_voc', 'system_max_v', 't_min_c', 'voc_coeff_pct_per_k']);
    // 3. THE HOT END OF THE WINDOW. The right coefficient here is the Vmp one and a module sheet gives
    // it; we do not have it, so the Pmax coefficient stands in for it AND THE ANSWER SAYS SO. It is a
    // proxy, it is roughly right, and calling it exact would be the dishonest bit.
    const vmpHot = N * c.module_vmp * (1 + (+c.pmax_coeff_pct_per_k / 100) * (+c.cell_hot_c - 25));
    add('string Vmp at ' + c.cell_hot_c + ' C cell', rnd(vmpHot, 1), 'V', 'at least ' + c.mppt_min_v + ' V at the tracker',
        vmpHot >= +c.mppt_min_v ? 'above the tracker minimum, so the string still tracks when it is hot' : 'BELOW THE TRACKER MINIMUM: the inverter drops off the knee on a hot day',
        ['modules', 'module_vmp', 'cell_hot_c', 'pmax_coeff_pct_per_k', 'mppt_min_v'],
        'the Pmax coefficient ' + c.pmax_coeff_pct_per_k + ' %/K is used as a PROXY for the Vmp coefficient, which a module sheet would give; it is close, not exact');
    // 4-7. CURRENT. Imp is what flows; Isc is what a fault or a bright edge gives; the design current is
    // the one every connector and cable is sized on, and bifacial gain goes INSIDE it, not after it.
    add('Imp, current at most power', rnd(imp, 2), 'A', 'the figure the loss is worked out at',
        'this is what flows in normal running', [impKey]);
    add('Isc, short circuit current', rnd(isc, 2), 'A', 'the figure sizing starts from',
        'what the string gives into a short at STC', ['module_isc']);
    const design = 1.25 * (+c.bifacial_factor) * isc;
    add('design current, 1.25 x ' + c.bifacial_factor + ' bifacial x Isc', rnd(design, 2), 'A', c.connector_a + ' A connector',
        design <= +c.connector_a ? 'within the connector rating' : 'OVER THE CONNECTOR RATING: a larger connector or a shorter string is needed',
        ['module_isc', 'bifacial_factor', 'connector_a']);
    add('current into one inverter input', rnd(design, 2), 'A', c.connector_a + ' A at the input connector',
        design <= +c.connector_a ? 'the input connector carries it' : 'OVER what the input connector carries',
        ['module_isc', 'bifacial_factor', 'connector_a']);
    const perInput = design * (+c.strings_per_tracker);
    add(c.strings_per_tracker + ' strings per tracker into one input', rnd(perInput, 2), 'A', c.input_a + ' A input rating',
        perInput <= +c.input_a ? 'the input carries both strings' : 'OVER THE INPUT RATING: the two strings must not share one input',
        ['module_isc', 'bifacial_factor', 'strings_per_tracker', 'input_a']);
    // 8-10. POWER, AND WHAT IT IS POURED INTO. A DC/AC ratio is not one number: the inverter's rating
    // falls as the air warms, so it is held against all three ratings and the hottest one decides.
    const pString = N * c.module_vmp * imp, pBlock = pString * (+c.strings_per_block);
    add('DC power, one string', rnd(pString / 1000, 2), 'kW', 'Vmp x Imp at STC',
        'the string at its most power point in standard conditions', ['modules', 'module_vmp', impKey]);
    add('DC power, one ' + c.strings_per_block + '-string block', rnd(pBlock / 1000, 1), 'kWp', 'the block the inverter sees',
        c.strings_per_block + ' strings on one inverter', ['modules', 'module_vmp', impKey, 'strings_per_block']);
    const r30 = pBlock / 1000 / (+c.inverter_kva_30c), r40 = pBlock / 1000 / (+c.inverter_kva_40c), r50 = pBlock / 1000 / (+c.inverter_kva_50c);
    add('DC/AC ratio at 30 / 40 / 50 C', rnd(r30, 2) + ' / ' + rnd(r40, 2) + ' / ' + rnd(r50, 2), '-',
        c.inverter_kva_30c + ' / ' + c.inverter_kva_40c + ' / ' + c.inverter_kva_50c + ' kVA',
        r50 <= 1.2 ? 'under 1.20 even on the 50 C rating, so little is clipped on a hot afternoon'
          : r50 <= 1.35 ? 'over 1.20 on the 50 C rating: some clipping on hot afternoons, which may well be the intention'
          : 'HIGH on the 50 C rating: the inverter is the limit for much of a hot day',
        ['modules', 'module_vmp', impKey, 'strings_per_block', 'inverter_kva_30c', 'inverter_kva_40c', 'inverter_kva_50c']);
    // 11-13. THE CABLE ITSELF, which the model always knew, now said in the same shape as the rest.
    const drop = imp * g.ohm, dropPct = 100 * drop / (N * c.module_vmp), loss = imp * imp * g.ohm, lossPct = 100 * loss / pString;
    add('string resistance, both conductors', rnd(g.ohm, 4), 'ohm', 'leads at ' + c.t_lead_c + ' C, site cable at ' + c.t_site_c + ' C',
        'lead ' + rnd(g.ohmLead, 4) + ' ohm, site cable ' + rnd(g.ohmSite, 4) + ' ohm, contacts the rest',
        ['lead_mm2', 'cable_mm2', 'near_end_m', 't_lead_c', 't_site_c']);
    add('volt drop at Imp', rnd(drop, 2) + ' V, ' + rnd(dropPct, 2) + ' % of Vmp', '-', 'commonly held to 1 % of Vmp',
        dropPct <= 1 ? 'under the 1 % usually asked of a string' : dropPct <= 1.5 ? 'over 1 %: a larger site cable or a shorter home run would bring it back' : 'WELL OVER 1 %: the home run or the cable size needs changing',
        ['lead_mm2', 'cable_mm2', 'near_end_m', impKey, 'module_vmp']);
    add('loss in the string', rnd(loss, 1) + ' W, ' + rnd(lossPct, 2) + ' % of string power', '-', 'the same 1 % test',
        lossPct <= 1 ? 'about one per cent or less of what the string makes' : 'over one per cent of what the string makes, and it is paid for every sunny hour',
        ['lead_mm2', 'cable_mm2', 'near_end_m', impKey, 'module_vmp']);
    // 14. THE FUSE QUESTION, IN BOTH WORLDS. Either the maker has said the inputs block reverse current,
    // or nobody has asked them yet - and if nobody has asked, the answer is the QUESTION, written out.
    add('string fuse', c.inputs_block_reverse_current ? 'not needed' : 'NOT SETTLED', '-',
        c.strings_per_tracker + ' strings per tracker',
        c.inputs_block_reverse_current
          ? 'the maker states the inputs block reverse current, and with only ' + c.strings_per_tracker + ' strings on an input no string can be fed by the others, so no string fuse is fitted'
          : 'ASK THE INVERTER MAKER IN WRITING: do the DC inputs block reverse current, and what reverse current may one string carry? Until that answer is on paper, fusing cannot be ruled out, whatever the string count',
        ['inputs_block_reverse_current', 'strings_per_tracker']);
    // 15. INSULATION RESISTANCE, which is what the block is actually tested against on the day.
    const riso = +c.insulation_mohm_min;
    add('insulation resistance, minimum to pass', rnd(riso, 2), 'Mohm', 'for a ' + rnd(pBlock / 1000, 1) + ' kWp block',
        'a block this size is held at ' + rnd(riso, 2) + ' Mohm at the test voltage; below it the block does not energise',
        ['insulation_mohm_min', 'strings_per_block']);
    // THE THREE THAT DECIDE WHETHER THE STRING IS LEGAL, SAFE AND WORTH BUILDING, in that order, kept
    // apart so the one line under the drawing can carry them without opening anything.
    return { V, key3: { margin_v: rnd(margin, 0), input_a: rnd(perInput, 1), input_limit_a: +c.input_a,
                        drop_pct: rnd(dropPct, 2), vmax } };
  }
  function run(inp) {
    const g = build(inp), c = g.c, worst = g.short.length ? Math.max(...g.short.map(l => l.need - g.pair)) : 0;
    const typedKeys = Object.keys(inp || {});
    const vp = valuePanel(g, typedKeys), values = vp.V;
    g.values = values; g.key3 = vp.key3;                       // the drawing's one-line key reads these
  ...
  const BOXES = [
    ...
    // EVERY FIGURE IN THE VALUES PANEL IS A BOX, because a figure a reader cannot change is a figure a
    // reader cannot check, and an unchecked assumption is the thing that puts 1500 V on a 1500 V limit.
    { key:'module_isc', label:'module short circuit', unit:'A', kind:'number', min:0.1, max:40, step:0.01 },
    { key:'module_imp', label:'module at most power', unit:'A', kind:'number', min:0.1, max:40, step:0.01, optional:true },
    { key:'t_min_c', label:'coldest design temperature', unit:'C', kind:'number', min:-45, max:15, step:1 },
    { key:'voc_coeff_pct_per_k', label:'Voc temperature coefficient', unit:'%/K', kind:'number', min:-0.6, max:0, step:0.01 },
    { key:'cell_hot_c', label:'hot cell temperature', unit:'C', kind:'number', min:30, max:95, step:1 },
    { key:'pmax_coeff_pct_per_k', label:'Pmax coefficient, used for Vmp', unit:'%/K', kind:'number', min:-0.6, max:0, step:0.01,
      hint:'a proxy for the Vmp coefficient until the module sheet gives it' },
    { key:'system_max_v', label:'system voltage limit', unit:'V', kind:'number', min:600, max:2000, step:50 },
    { key:'mppt_min_v', label:'tracker minimum', unit:'V', kind:'number', min:100, max:1200, step:10 },
    { key:'bifacial_factor', label:'bifacial factor', kind:'number', min:1, max:1.4, step:0.01 },
    { key:'connector_a', label:'connector rating', unit:'A', kind:'number', min:5, max:60, step:1 },
    { key:'input_a', label:'inverter input rating', unit:'A', kind:'number', min:5, max:120, step:1 },
    { key:'strings_per_tracker', label:'strings per input', kind:'number', min:1, max:6, step:1 },
    { key:'strings_per_block', label:'strings per block', kind:'number', min:1, max:120, step:1 },
    { key:'inverter_kva_30c', label:'inverter at 30 C', unit:'kVA', kind:'number', min:10, max:1000, step:1 },
    { key:'inverter_kva_40c', label:'inverter at 40 C', unit:'kVA', kind:'number', min:10, max:1000, step:1 },
    { key:'inverter_kva_50c', label:'inverter at 50 C', unit:'kVA', kind:'number', min:10, max:1000, step:1 },
    { key:'inputs_block_reverse_current', label:'inverter inputs block reverse current', kind:'choice',
      choices:[[0, 'not confirmed by the maker'], [1, 'the maker states they do']] },
    { key:'insulation_mohm_min', label:'insulation resistance to pass', unit:'Mohm', kind:'number', min:0.1, max:40, step:0.1 },
...
function fireCommand(text, rec, inPlace){
  ...
  const table = list => { const tb = document.createElement('table');
    ...
    return tb; };
  // ===== THE VALUES PANEL. Five columns because five is what an engineer needs to act: what it is,
  // what it came to, what it is held against, the verdict IN WORDS, and whether the figure was typed
  // by the reader or is one of our declared assumptions. A failing verdict is marked so it can be seen
  // from across a room, and the working note sits under the line it belongs to.
  const valuesPanel = out => { const box = el('div', 'values');
    if (!out.values || !out.values.length) return box;
    box.append(el('p', 'dimtext', 'EVERY ELECTRICAL VALUE, with the limit it is held against and where the figure came from. ASSUMED means we declared it; TYPED means you did.'));
    const tb = document.createElement('table');
    for (const v of out.values) { const tr = document.createElement('tr');
      const bad = /OVER|NEGATIVE|BELOW|NOT SETTLED|WELL OVER|HIGH on/.test(String(v.verdict) + ' ' + String(v.value));
      if (bad) tr.className = 'bad';
      tr.append(el('td', null, v.label), el('td', null, String(v.value) + (v.unit && v.unit !== '-' ? ' ' + v.unit : '')),
                el('td', null, v.limit), el('td', null, v.verdict), el('td', null, v.basis));
      tb.append(tr);
      if (v.note) { const nr = document.createElement('tr'); const td = el('td', 'dimtext', v.note);
        td.colSpan = 5; nr.append(td); tb.append(nr); } }
    box.append(tb); return box; };
  ...
  if (inPlace && fireLive && fireLive.name === out.name && fireLive.said && fireLive.said.isConnected) {
    ...
    if (fireLive.valuesBox) fireLive.valuesBox.replaceChildren(valuesPanel(out));
    ...
    const valuesBox = el('div'); valuesBox.append(valuesPanel(out)); nodes.push(valuesBox);
    ...
    open(title, nodes);
    fireLive = { name:out.name, said:saidP, leadBox, valuesBox, hidden, order:orderP, boxes, rec };
  }
...
function stringDrawing(g){
  ...
  // THE THREE FIGURES THAT DECIDE THE STRING, UNDER THE PICTURE, without opening a thing: whether it is
  // legal when it is cold, whether the input will carry it, and whether the cable is worth its copper.
  // Everything else is in the VALUES panel in the answer; these three are the ones a reader is stopped
  // on, so they are on the screen beside the drawing at every zoom and on every size of screen.
  if (g.key3) { const k3 = g.key3;
    const mOk = k3.margin_v >= 0, iOk = k3.input_a <= k3.input_limit_a, dOk = k3.drop_pct <= 1;
    words.push({ pinned:3, note:true, warnLine:!(mOk && iOk && dOk),
      text:'cold Voc margin ' + k3.margin_v + ' V to ' + k3.vmax + ' V (' + (mOk ? 'inside' : 'OVER THE LIMIT')
        + ') · ' + k3.input_a + ' A into the input against ' + k3.input_limit_a + ' A (' + (iOk ? 'carried' : 'OVER')
        + ') · volt drop ' + k3.drop_pct + ' % of Vmp (' + (dOk ? 'under 1 %' : 'over 1 %') + ')',
      small:'Voc margin ' + k3.margin_v + ' V' + (mOk ? '' : ' OVER') + ' · ' + k3.input_a + '/' + k3.input_limit_a
        + ' A' + (iOk ? '' : ' OVER') + ' · drop ' + k3.drop_pct + ' %' }); }

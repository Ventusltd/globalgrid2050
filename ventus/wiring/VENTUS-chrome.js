'use strict';
// VENTUS-chrome.js - a cartridge of the VENTUS shell: HOW THE PAGE IS DRESSED. What sits over the disc,
...
// commands. Keep the view minimalist like the VENTUS, Wafer and Gridatlas", and "Module measurements in
...
(function () {
  ...
  function boot() {
    ...
    const typed = (text) => { if (!pin) return false; pin.value = text;
      pin.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true })); openSheet(); return true; };
    ...
    function showSystems() {
      const S = window.VENTUS_SYSTEMS; if (!S) return false;
      if (!sysBox) {
    ...
    const dev = el('div', null, 'Not real cable or array sizing - consult a design engineer. DEVELOPMENT ENVIRONMENT: estimates from a model, nothing entered.');
    dev.id = 'kdev';
    ...
    if (foot) foot.addEventListener('keydown', e => {
      ...
      if (/^system\s+\S/.test(q) && window.VENTUS_SYSTEMS) {
        e.stopPropagation(); e.preventDefault();
        window.VENTUS_SYSTEMS.fire(q.replace(/^system\s+/, '')); shut(); pin.value = '';
        return;
      }
    ...
    try {
      if (typeof sayOpen === 'function') {
        ...
        sayOpen = function (title, nodes, beside) {
          ...
          if (!wasOpen) { sayEl.classList.remove('open', 'max', 'min'); relayout(); }
    ...
    try {
      if (typeof shapeBand === 'function') {
        ...
        window.shapeBand = function (reserve) {
          ...
          // THE DRAWING STARTS AT THE TOP OF THE SCREEN. The band used to begin under the chip row; the
          // row is one small button now, so the picture runs to just under the red line and the button
          // sits over its corner. Nothing else is up there to hide.
          const s = isOpen() ? sayEl.getBoundingClientRect() : null;
          const across = !!s && s.width > innerWidth * 0.7 && s.top < innerHeight * 0.5;
          if (!across) { const red = document.querySelector('#sldnames span.danger');
            const rb = red ? red.getBoundingClientRect().bottom : 0;
            b.top = Math.max(6, Math.round(rb) + 4); }
          // AT LEAST 85 PER CENT OF THE SCREEN IS THE DRAWING'S. What the key reserves under the picture
          // is given back, because the key is one line above the bar now and not a block under it.
          const want = Math.round(innerHeight * 0.85);
          if (b.bottom - b.top < want) b.top = Math.max(6, b.bottom - want);

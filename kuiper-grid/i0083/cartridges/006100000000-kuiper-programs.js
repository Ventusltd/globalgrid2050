...
const STRING_TESTS = [
  { family:'STRINGS', pin:-12, name:'A tracker, two in portrait, along and back', sentence:'Single axis tracker, two modules in portrait, out along the lower row and back along the upper: both ends at the near end and no return cable', command:'fire string {"mounting":"tracker","modules_high":2,"routing":"serpentine","turn_alternate_rows":1,"jumpers":1,"lead_plus_m":0.35,"lead_minus_m":0.28}' },
  { family:'STRINGS', pin:-11, name:'A tracker, two in portrait, leapfrog in each row', sentence:'Single axis tracker, two in portrait, every other module on the way out and the rest on the way back inside each row', command:'fire string {"mounting":"tracker","modules_high":2,"routing":"leapfrog","turn_every_second":1,"lead_plus_m":0.84,"lead_minus_m":1.13,"jumpers":1}' },
  { family:'STRINGS', pin:-10, name:'A tracker, two in portrait, one after another', sentence:'Single axis tracker, two in portrait, each module to the next with a cable to bring the far end home', command:'fire string {"mounting":"tracker","modules_high":2,"routing":"one-after-another","jumpers":1,"lead_plus_m":0.35,"lead_minus_m":0.28}' },
  { family:'STRINGS', pin:-9, name:'Fixed tilt, one in portrait, leapfrog', sentence:'A single row of thirty modules, every other one joined on the way out, the rest on the way back', command:'fire string {"mounting":"fixed","modules_high":1,"routing":"leapfrog"}' },
  { family:'STRINGS', pin:-8, name:'Fixed tilt, one in portrait, one after another', sentence:'A single row, each module to the next, and one cable the length of the row to bring the far end home', command:'fire string {"mounting":"fixed","modules_high":1,"routing":"one-after-another"}' },
  { family:'STRINGS', pin:-7, name:'Fixed tilt, two in portrait, along and back', sentence:'A table two modules high, out along the lower tier and back along the upper: no return cable is laid', command:'fire string {"mounting":"fixed","modules_high":2,"routing":"serpentine","turn_alternate_rows":1,"jumpers":1,"lead_plus_m":0.35,"lead_minus_m":0.28}' },
  { family:'STRINGS', pin:-6, name:'Fixed tilt, three in portrait, along and back', sentence:'A table three modules high, wired row by row, turning at the end of each', command:'fire string {"mounting":"fixed","modules_high":3,"routing":"serpentine","turn_alternate_rows":1,"jumpers":1,"lead_plus_m":0.35,"lead_minus_m":0.28}' },
  { family:'STRINGS', pin:-5, name:'Fixed tilt, four in portrait, along and back', sentence:'A table four modules high, wired row by row', command:'fire string {"mounting":"fixed","modules_high":4,"routing":"serpentine","turn_alternate_rows":1,"jumpers":1,"lead_plus_m":0.35,"lead_minus_m":0.28}' },
  { family:'STRINGS', pin:-4, name:'East and west facing, five in portrait each face', sentence:'Two faces back to back with a ridge between them, five modules in portrait on each, wired row by row', command:'fire string {"mounting":"east-west","modules_high":5,"routing":"serpentine","turn_alternate_rows":1,"jumpers":1,"lead_plus_m":0.35,"lead_minus_m":0.28}' },
  { family:'STRINGS', pin:-3, name:'Fixed tilt landscape, three high', sentence:'Modules laid on their side, three high: the two junction boxes now sit one above the other', command:'fire string {"mounting":"fixed","orientation":"landscape","modules_high":3,"routing":"serpentine","turn_alternate_rows":1,"jumpers":1,"lead_plus_m":0.35,"lead_minus_m":0.28}' },
  { family:'STRINGS', pin:-2, name:'Fixed tilt landscape, six high', sentence:'Modules on their side, six high, wired row by row', command:'fire string {"mounting":"fixed","orientation":"landscape","modules_high":6,"routing":"serpentine","turn_alternate_rows":1,"jumpers":1,"lead_plus_m":0.35,"lead_minus_m":0.28}' },
  { family:'STRINGS', pin:-1, name:'Leapfrog turned, every second module end for end', sentence:'Turning every second module end for end makes every link the short kind', command:'fire string {"wiring":"leapfrog","turn_every_second":1,"lead_plus_m":0.84,"lead_minus_m":1.13}' },
  { family:'STRINGS', pin:0, name:'A tracker, two in portrait, zigzag between the rows', sentence:'Single axis tracker, two in portrait, up one column and along, down the next and along', command:'fire string {"mounting":"tracker","modules_high":2,"routing":"zigzag","jumpers":1,"lead_plus_m":0.84,"lead_minus_m":1.13}' },
];
const FIRE_FALLBACK = [
...
function makeSldNames(lay, net){
  ...
  if (net.kind === 'string') { const d = net.g.__draw || stringDrawing(net.g);
    const out = d.words.map(w => ({ x:w.x, y:w.y, dy:0, dx:0, text:w.text, small:w.small, warn:!!w.warnLine, note:!!w.note, pinned:w.pinned }));
...
function placeSldNames(){
  ...
  if (el.childElementCount !== list.length || el.dataset.said !== String(said) || el.dataset.small !== String(small)) {
    ...
    el.replaceChildren(...list.map(n => { const sp = document.createElement('span');
      ...
      if (n.warn) sp.className = 'warn'; else if (n.note) sp.className = 'note';
...
function stringDrawing(g){
  ...
  const boxW = port ? 0.22 : 0.15, boxH = port ? 0.15 : 0.22, boxY = g.rowY[0] + wY / 2;
  ...
  for (let k = 1; k <= N; k++) {
    ...
    put({ role:'cells', order:o, pot, node:'M' + k, lines:cl, pts:[[x0, yb], [x1, yt]],
          split:port ? [[x0, yb + wY / 2], [x1, yb + wY / 2]] : [[x0 + wX / 2, yb], [x0 + wX / 2, yt]] });
    for (const [pt, sign] of [[g.minusPt(k), '−'], [g.plusPt(k), '+']]) {
      box('box', o, pt[0] - boxW / 2, pt[1] - boxH / 2, pt[0] + boxW / 2, pt[1] + boxH / 2, { fill:'#060a10', node:'M' + k, weight:1 });
      marks.push({ x:pt[0] + (port ? 0 : boxW * 0.5 + 0.17), y:pt[1] - (port ? boxH / 2 + 0.16 : 0),
        text:sign, size:11, colour:sign === '+' ? SHAPE_RED : SHAPE_INK, minPx:26, mid:port }); }
    marks.push({ x:(x0 + x1) / 2, y:yb - 0.34, text:'M' + k, size:11, colour:SHAPE_INK, minPx:18, mid:true });
  ...
  g.links.forEach((l, i) => {
    const A = [l.x_from, l.y_from], B = [l.x_to, l.y_to];
    const xa = A[0], xb = B[0], span = Math.hypot(B[0] - A[0], B[1] - A[1]), dir = l.back ? -1 : 1;
    let pts;
    if (rows === 1) { const lane = (l.back ? 2.30 : 2.05) + span * 0.30 + (i % 3) * 0.26;
      const ctrl = boxY + dir * lane, apex = boxY + dir * 0.75 * lane;
      laneHi = Math.max(laneHi, apex); laneLo = Math.min(laneLo, apex);
      pts = bez([xa, boxY + dir * boxH / 2], [xa, ctrl], [xb, ctrl], [xb, boxY + dir * boxH / 2], 26); }
    else {
      // ON A TABLE the cable is clipped to the structure and runs across the modules, so the line is
      // drawn where it really goes: a shallow curve from one box to the other, bowed a little to one
      // side so that two links between the same pair of columns can be told apart.
      const mx = (A[0] + B[0]) / 2, my = (A[1] + B[1]) / 2;
      const dx = B[0] - A[0], dy = B[1] - A[1], L = Math.max(1e-6, Math.hypot(dx, dy));
      const bow = (0.16 + 0.09 * (i % 3)) * Math.min(1.8, L) * (i % 2 ? -1 : 1);
      const cp = [mx - dy / L * bow, my + dx / L * bow];
      pts = bez(A, cp, cp, B, 20);
  ...
  const homeNeg = [[invX1 + 0.16, negY], [invX1 + 0.70, negY], [invX1 + 0.70, topLane], [xn, topLane], [xn, nP[1] + boxH / 2]];
  poly('cable', 0, homeNeg, { node:'home-', pot:0, plus:false, weight:1.2, says:'the home run to the string’s − end' });
  const homePos = [[invX1 + 0.16, posY], [invX1 + 0.55, posY], [invX1 + 0.55, botLane], [xp, botLane], [xp, pP[1] - boxH / 2]];
  ...
  words.push({ pinned:-1, note:true,
    ...
    small:'one current all the way round; the sweep is the instant of closure' });
  ...
  for (const m of marks) see(m.x, m.y); for (const w of words) if (w.pinned === undefined) see(w.x, w.y);

...
// THE CODE A PIECE OF WORK CHANGED, THE WAFER'S WAY. The table of what each piece of work changed is one
// small module, asked for the first time a dot is tapped. The file is read from the public repository at
// that exact commit. Nothing is fetched for a repository that is not provably public.
let WORK = null, workWhy = null; const rawCache = new Map();
{ const st = document.createElement('style');          // the look of the other-files buttons travels with this part
  st.textContent = '#card .cc-nav{display:flex;gap:.35rem;flex-wrap:wrap;margin:.3rem 0}'
    + '#card .cc-mini{background:#0a0a0a;color:#00ffff;border:1px solid #444;border-radius:3px;font:inherit;font-size:11px;padding:.3rem .5rem;cursor:pointer}'
    + '#card .cc-mini.cc-on{border-color:#00ffff;color:#fff}';
  document.head.appendChild(st); }
function workTable(){ if (WORK || workWhy) return Promise.resolve(WORK);
  // beside the shell, not beside this file (see the programs part for how the other way failed on the live address)
  return import(new URL('kuiper-work.mjs', document.baseURI).href).then(m => { WORK = m.WORK; return WORK; })
    .catch(e => { workWhy = String(e && e.message || e); console.error('[kuiper] what each piece of work changed could not be loaded:', workWhy); return null; }); }
function rawFile(owner, repo, sha, path){ const u = 'https://raw.githubusercontent.com/' + owner + '/' + repo + '/' + sha + '/' + path.split('/').map(encodeURIComponent).join('/');
  if (!rawCache.has(u)) { rawCache.set(u, fetch(u).then(x => { if (!x.ok) throw new Error('the file answered ' + x.status); return x.text(); })
      .then(t => t.split('\n').map(l => l.replace(/\r$/, ''))).catch(e => { rawCache.delete(u); throw e; }));
    if (rawCache.size > 8) rawCache.delete(rawCache.keys().next().value); }
  return rawCache.get(u); }
// THE SAME WINDOW, ASKED FOR BY PLACE. A runner holds this page's code window against the wafer's own card on
// the same lines of the same public files and fails unless the text is the same. This is not a second way of
// showing code: it is the one way (the same file reader, the same window) with the place given from outside.
window.__kuiperCode = async (owner, repo, commit, path, line) => { const lines = await rawFile(owner, repo, commit, path);
  const at = Math.min(Math.max(1, line | 0), lines.length), from = Math.max(1, at - CONTEXT), to = Math.min(lines.length, at + CONTEXT);
  const rows = codeWindow(lines, from, to, at).node.textContent.split(String.fromCharCode(10));
  return { from, to, at, rows:rows.map((t, i) => ({ n:from + i, text:t.slice(8) })) }; };
function workCode(a, r, s){
  if (!r.pub) { s.setTag('EMPTY', 'This work is not public, so its code is not shown.'); return; }
  const det = card.querySelector('details.cc-q');
  if (det && !det.open && !s.asked) { s.asked = true; s.setTag('WAIT', 'open this to read the code this piece of work changed');
    det.addEventListener('toggle', () => { if (det.open && s.live()) workCode(a, r, s); }, { once:true }); return; }
  s.setTag('LOAD', 'finding what this piece of work changed\u2026');
  workTable().then(table => { if (!s.live()) return;
    if (!table) { s.setTag('FAIL', 'The list of what each piece of work changed could not be read: ' + workWhy); return; }
    const row = table[a.c.sha];
    if (!row) { s.setTag('EMPTY', 'This piece of work changed no code a card can show: data, pictures, or files that were only removed.'); return; }
    const full = row[0], count = row[1], files = row.slice(2);
    const pick = el('div', 'cc-nav'), head = el('div', 'cc-codehead'), holder = el('div');
    s.body.append(head, pick, holder);
    const open = i => { const path = files[i][0], line = files[i][1], name = path.split('/').pop();
      for (const b of pick.children) b.classList.toggle('cc-on', b.getAttribute('data-i') === String(i));
      head.textContent = 'This piece of work changed ' + fmt(count) + (count === 1 ? ' file' : ' files') + '. Showing ' + path + '.';
      holder.replaceChildren(); cardShown = null;
      s.setTag('LOAD', 'reading ' + name + ' as it stood that day\u2026');
      rawFile(r.owner, r.name, full, path).then(lines => { if (!s.live()) return;
        const at = Math.min(Math.max(1, line), lines.length), from = Math.max(1, at - CONTEXT), to = Math.min(lines.length, at + CONTEXT);
        const w = codeWindow(lines, from, to, at); holder.append(w.node);
        const src = el('div', 'cc-source'), g = linkTo(gh({ owner:r.owner, repo:r.name, commit:full, path:path, line:at }), 'GitHub'); g.classList.add('cc-gh');
        src.append(g); holder.append(src);
        cardShown = { work:true, path, line:at, lines, window:w, sheet:s, from, to };
        s.setTag('OK', 'lines ' + fmt(from) + ' to ' + fmt(to) + ' of ' + fmt(lines.length) + ', around the first line this work changed (line ' + fmt(at) + ')');
        requestAnimationFrame(() => { if (s.live()) revealUnderHead(); }); revealUnderHead(); dirty = true;
      }).catch(e => { if (s.live()) s.setTag('FAIL', name + ' could not be read from the public repository: ' + (e && e.message || e) + '. Nothing is shown in its place.'); }); };
    files.forEach((f, i) => { const b = el('button', 'cc-mini', f[0].split('/').pop()); b.type = 'button'; b.setAttribute('data-i', i); b.title = f[0];
      b.addEventListener('click', e => { e.stopPropagation(); open(i); }); pick.append(b); });
    if (count > files.length) pick.append(el('span', 'cc-small', ' and ' + fmt(count - files.length) + ' more'));
    if (files.length < 2) pick.style.display = 'none';
    open(0);
  });
}
// THE CODE IS BROUGHT IN UNDER THE HEAD, never over it: the window's top is put just below the head's foot,
// and then the marked line is brought to the middle of the window.
function revealUnderHead(){ const w = cardShown && cardShown.window; if (!w) return;
  const head = card.querySelector('.cc-head'), floor = head ? head.getBoundingClientRect().bottom : card.getBoundingClientRect().top;
  card.scrollTop += w.node.getBoundingClientRect().top - (floor + 4);
  w.scrollToTarget(); }
{ const st = document.createElement('style');
  st.textContent = '#card .cc-head{position:sticky;top:-8px;z-index:2;background:#000;padding-top:8px;margin-top:-8px}'
    + '#card.codeopen .cc-tile-what,#card.codeopen .scada-brand,#card.codeopen .cc-plain,#card.codeopen .cc-links{display:none}'
    + '#card.codeopen .cc-tile{padding:8px 12px;margin:2px 0 6px}#card.codeopen .cc-tile-name{font-size:16px;margin-bottom:.35rem}'
    + '#card.codeopen .cc-tile-open{padding:.4rem 0}#card.codeopen .cc-tile-part{margin-bottom:.15rem}'
    + '@media (max-width:640px){#card.codeopen .cc-code{max-height:24vh}}';
  document.head.appendChild(st); }
function show(a){
  card.classList.remove('codeopen');
  cardShown = null;
  ...
  { const found = r.pub ? appFor(r.owner, r.name, c ? c.path : null) : null;
    ...
    // THE HEADER IS THE HEADER. It stays at the top of the card whatever is opened under it.
    const head = el('div', 'cc-head'); head.append(top, links, brand, plain);
    card.append(head, det);
    det.open = codeOpen; card.classList.toggle('codeopen', det.open);
    det.addEventListener('toggle', () => { card.classList.toggle('codeopen', det.open); });
    ...
    det.addEventListener('toggle', () => { if (det.open && cardShown && cardShown.window) revealUnderHead(); dirty = true; }); }
  ...
  const put = data => {
    ...
    requestAnimationFrame(() => { if (s.live()) revealUnderHead(); });
    revealUnderHead();
    dirty = true;
  };

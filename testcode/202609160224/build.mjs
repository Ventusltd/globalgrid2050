...
/* THE FRONT DOOR, GENERATED — because the chair replaced a hand-typed one with a
   hand-typed one and did not notice for five hours.
 *
 * testcode/index.html was 184 bytes naming two surfaces of ninety-six, the newest
 * eleven days stale. The Nest fixed that by generating its manifest from the
 * directory. The front door that LINKS to the Nest was then written by hand, and by
 * 06:50 it claimed "ninety-seven" surfaces when there were 119, named a "most recent"
 * list 22 surfaces out of date, and linked 202609151500 — a FRAGMENT with no
 * index.html, which the manifest had been correctly classifying as unopenable for
 * hours while the front door offered it as a link.
 *
 * Same defect, same file, by the seat that wrote the fix. So the front door is emitted
 * from the manifest and cannot disagree with it. No JavaScript: it is the plain-text
 * fallback, and a fallback that needs the thing it is a fallback for is not one. */
{
  const openable = surfaces.filter((s) => s.kind === 'surface' && s.entry);
  const recent = openable.slice(-6).reverse();
  const esc = (t) => String(t).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  const rows = recent.map((s) =>
    '<li><a href="' + s.stamp + '/">' + s.stamp + '</a>' +
    (s.title ? ' <span class="dim">— ' + esc(s.title.replace(/^CPU Wafer Galaxy - /, '')) + '</span>' : '') +
    '</li>').join(String.fromCharCode(10) + ' ');
  const doc = [
    '<!doctype html>',
    '<meta charset="utf-8">',
    '<meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover">',
    '<meta name="color-scheme" content="dark">',
    '<title>Test Code — the nest</title>',
    '<!-- GENERATED by testcode/202609160224/build.mjs. Do not hand-edit: the previous',
    '     two versions of this file were typed, and both went stale. -->',
    '<style>',
    ' body{margin:0;background:#0b0d12;color:#d8dee9;font:14px/1.6 ui-monospace,Menlo,Consolas,monospace;padding:2rem 1rem;max-width:44rem;margin-inline:auto}',
    ' h1{font-size:15px;letter-spacing:.14em;margin:0 0 .3rem}',
    ' .dim{color:#8b93a7} a{color:#5ec8f2}',
    ' .go{display:inline-block;margin:1.2rem 0;padding:.45rem 1rem;border:1px solid #5ec8f2;border-radius:.2rem;text-decoration:none}',
    ' ul{padding-left:1.1rem;margin:.6rem 0 0} li{margin:.15rem 0}',
    '</style>',
    '<h1>TEST CODE</h1>',
    '<p class="dim">' + openable.length + ' surfaces the estate has published, each one a page. ' +
      'This list is generated from the directory, so it cannot name a surface that is not ' +
      'there or omit one that is.</p>',
    '<a class="go" href="202609160224/">OPEN THE NEST &rarr;</a>',
    '<p class="dim">Most recent:</p>',
    '<ul>',
    ' ' + rows,
    '</ul>',
    '<p class="dim">' + surfaces.filter((s) => s.kind === 'fragment').length +
      ' directories hold code but no page, so they are listed in the nest and not linked here. ' +
      'Generated ' + out.generated_utc + '.</p>',
    '',

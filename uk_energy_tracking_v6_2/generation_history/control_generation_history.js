window.V6 22ControlGenerationHistory=(function(){
  var lastResult=null;
  ...
  function fillYears(){var sel=byId('generation-history-year');if(!sel)return;var now=new Date().getUTCFullYear(),first=(window.V6 22GenerationHistoryConfig.firstYear||2016);sel.innerHTML='';for(var y=now;y>=first;y--){var o=document.createElement('option');o.value=String(y);o.textContent=String(y);sel.appendChild(o)}}
  ...
  function drawCached(){var canvas=byId('generation-history-canvas');if(canvas&&lastResult)window.V6 22RenderGenerationHistoryChart.render(canvas,lastResult)}
...
document.addEventListener('DOMContentLoaded',function(){window.V6 22ControlGenerationHistory.init()});

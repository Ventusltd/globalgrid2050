...
function getLookupKey(voltageKey, csaMm2, selectedCore) {
    ...
    return `${prefix}_${voltageKey}_${csaMm2}`;
}
...
function lookupOD(voltageKey, csaMm2, selectedCore) {
    ...
    const isMVorHV = ["uk_11kv_sc", "uk_33kv_sc", "iec_110kv_sc"].includes(voltageKey);
    if (selectedCore === "single" && isMVorHV) {
        const od = OD_A + OD_B * Math.sqrt(csaMm2) + OD_C * runtime.Uo;
        const od_r = Math.round(od / 2.5) * 2.5;
        const minOD = 25 + (runtime.Uo * 0.8);
        const finalOD = Math.max(od_r, minOD);
        return {
            od: finalOD,
            mbr: finalOD * runtime.mbr_factor,
            src: "Catalogue model with lower bound",
            estimated: true
        };
    }

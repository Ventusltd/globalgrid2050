/* wafer.mjs - the wafer law, generated. Variation wafer-lit.
 *
 * r = sqrt(key): the order things were written. Permanent, linkable, unbounded.
 *
...
function draw() {
  ...
  footEl.textContent = 'law: wafer - Chronology. ' + shown.toLocaleString() +
...
fetch(LAWS_INDEX).then(r => r.ok ? r.json() : null).then(ix => {
  ...
  for (const law of ix.laws) {
    ...
    if (law.id === 'wafer') a.setAttribute('aria-current', 'page');
    nav.append(a);
  }
  nav.hidden = false;
 ...
 */
window.__wafer = {
  law: 'wafer', variant: 'wafer-lit', place, lawsIndex: LAWS_INDEX,

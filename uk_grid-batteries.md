---
layout: page
title: UK Grid Scale Batteries (BESS) Atlas
permalink: /uk_grid-batteries/
---
...
<style>
    ...
    #map { height: 750px; width: 100%; border-radius: 12px; background: #0b0e14; border: 2px solid #2a2f3a; margin-bottom: 20px; }
    #repd-table-container { background: #fff; padding: 20px; border-radius: 12px; border: 1px solid #e1e4e8; color: #333; }
...
<script>
    ...
    // Linked to the BESS CSV shown in your image
    ...
    function updateDisplay(minMW) {
        ...
        allData.forEach(row => {
            ...
            if (capacity >= minMW) {
                const x = parseFloat(row['X-coordinate']), y = parseFloat(row['Y-coordinate']);
                if (x && y) {
                    try {
                        const coords = proj4("EPSG:27700", "WGS84", [x, y]);
                        const isOp = status === 'Operational';
                        const marker = L.circleMarker([coords[1], coords[0]], {
                            radius: Math.max(5, Math.sqrt(capacity)),
                            fillColor: isOp ? '#00f2ff' : '#ff9d00',
                            color: "#fff", weight: 1, fillOpacity: 0.8
                        }).bindPopup(`<b>${row['Site Name']}</b><br>${capacity} MW<br>Status: ${status}`);
                        markers.addLayer(marker);
                        filteredTableData.push([row['Site Name'], capacity, status, row['County']]);
                    } catch (e) {}
                }
            }
        });
        ...
        else { dataTable = $('#repd-table').DataTable({ data: filteredTableData, order: [[1, 'desc']] }); }
    }
</script>

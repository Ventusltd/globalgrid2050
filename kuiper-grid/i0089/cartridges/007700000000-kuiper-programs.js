...
// ===== THE SAME ROW WIRED BOTH WAYS, ONE ABOVE THE OTHER: fire compare ==========================
// A reader cannot hold two drawings in their head and subtract them. So the SAME physical row is drawn
// twice at the SAME scale in one view: one after another on top, leapfrog underneath, and under both a
// small table of what each one costs and what the difference is. Nothing here works anything out: the
// string model is fired twice, once with each wiring, and every figure below is lifted out of its own
// answer. The drawing is not forked either - the one drawing maker is called twice and the second
// picture is moved down the page.
(function (root) {
  const F = root.FIRE; if (!F) return;
  const rnd = (x, n) => { const k = Math.pow(10, n); return Math.round(x * k) / k; };
  // MOVE A FINISHED DRAWING: the same scale change and the same drop applied to every last point of it,
  // so the two pictures can be read against each other module for module. The left edge is the near end
  // and it is held still, because that is the end a telephone opens on.
  function place(d, f, dy, tag) {
  ...
  function run(inp) {
    // THE SAME ROW BOTH TIMES. Whatever the reader typed is kept, and only the wiring is changed, so
    // the two bands can differ in nothing else.
    const base = Object.assign({}, inp || {});
    delete base.routing; delete base.wiring;
    const fire1 = r => F.fire('fire string ' + JSON.stringify(Object.assign({}, base, { routing: r })));
    const A = fire1('one-after-another'), B = fire1('leapfrog');
    const na = A.numbers, nb = B.numbers;
    // ONE DRAWING MAKER, CALLED TWICE. Both pictures are then brought to one scale and stacked.
    const dA = stringDrawing(A.string), dB = stringDrawing(B.string);
    const half = 0.40, fA = half, fB = half * (dA.perM / dB.perM);
    place(dA, fA, 0.74, 'a~'); place(dB, fB, -0.10, 'b~');
    const shapes = dA.shapes.concat(dB.shapes), marks = dA.marks.concat(dB.marks);
    const terminals = dA.terminals.concat(dB.terminals);
    const crossings = (dA.crossings || []).concat(dB.crossings || []);
    // Only the first band's pinned lines are kept: the same grey note twice is the same note.
    const words = dA.words.filter(w => w.pinned !== undefined)
      .concat(dA.words.filter(w => w.pinned === undefined), dB.words.filter(w => w.pinned === undefined));
    const INK = typeof SHAPE_INK === 'string' ? SHAPE_INK : '#dfe6ef';
    const EDGE = typeof SHAPE_EDGE === 'string' ? SHAPE_EDGE : '#8b97a6';
    const RED = typeof SHAPE_RED === 'string' ? SHAPE_RED : '#e8503a';
    const say = (x, y, text, size, colour, minPx) => marks.push({ x, y, text, size: size || 10, colour: colour || INK, minPx: minPx === undefined ? 0 : minPx, rank: 0 });
    // BAND TITLES IN PLAIN WORDS, not a legend somewhere else.
    say(-0.92, 1.30, 'ONE AFTER ANOTHER, with a cable to bring the far end home', 12, INK);
    say(-0.92, 0.34, 'LEAPFROG, out on every other module and back on the ones between', 12, RED);
    // THE TABLE, drawn as text marks under the two bands. Every figure is read out of the two answers.
    const dif = (a, b, n) => { const v = b - a; return (v > 0 ? '+' : v < 0 ? '−' : '') + Math.abs(rnd(v, n)).toFixed(n); };
    const vA = na.volt_drop_v, vB = nb.volt_drop_v, vmp = na.string_volts_at_vmp;
    const pA = vmp * na.current_a, pB = vmp * nb.current_a;
    // THREE COLUMNS, NOT FOUR. On a 390 px telephone a fourth column of differences
    // squeezes every figure below reading size; the differences are said in words on
    // the last line instead, where they belong. Nothing is dropped, only moved.
    const rows = [
      ['lead needed / fitted, m', na.longest_link_m.toFixed(2) + ' / ' + na.lead_pair_m.toFixed(2),
        nb.longest_link_m.toFixed(2) + ' / ' + nb.lead_pair_m.toFixed(2)],
      ['factory lead, m', na.module_lead_m.toFixed(1), nb.module_lead_m.toFixed(1)],
      ['site cable, m', na.cable_laid_on_site_m.toFixed(1), nb.cable_laid_on_site_m.toFixed(1)],
      ['total conductor, m', na.total_conductor_m.toFixed(1), nb.total_conductor_m.toFixed(1)],
      ['string resistance, ohm', na.string_resistance_ohm.toFixed(3), nb.string_resistance_ohm.toFixed(3)],
      ['volt drop, V', vA.toFixed(2), vB.toFixed(2)],
      ['volt drop, % of Vmp', pct(vA, vmp).toFixed(2), pct(vB, vmp).toFixed(2)],
      ['loss, W', na.loss_w.toFixed(1), nb.loss_w.toFixed(1)],
      ['loss, % of the string', pct(na.loss_w, pA).toFixed(2), pct(nb.loss_w, pB).toFixed(2)],
      ['joints made on site', String(na.connectors_fitted_on_site), String(nb.connectors_fitted_on_site)]
    ];
    // The table sits INSIDE the width the two bands already occupy, so it cannot push
    // the view wider and shrink the drawing; and no line of it is allowed under 9 px.
    const MINPX = 9;
    const cX = [-0.92, 0.34, 0.86], top = -0.62, step = 0.085;
    say(cX[0], top, 'the same row, both ways', 11, INK, MINPX);
    say(cX[1], top, 'sequential', 11, EDGE, MINPX); say(cX[2], top, 'leapfrog', 11, RED, MINPX);
    rows.forEach((r, i) => { const y = top - step * (i + 1);
      say(cX[0], y, r[0], 10, EDGE, MINPX); say(cX[1], y, r[1], 10, INK, MINPX); say(cX[2], y, r[2], 10, INK, MINPX); });
    // THE LAST LINE IN WORDS, because a table is not an answer until somebody says what it means.
    const lessCable = na.cable_laid_on_site_m - nb.cable_laid_on_site_m;
    const fewerJoints = na.connectors_fitted_on_site - nb.connectors_fitted_on_site;
    const lossPct = pct(nb.loss_w - na.loss_w, na.loss_w);
    const verdict = 'leapfrog lays ' + Math.abs(rnd(lessCable, 1)).toFixed(1) + ' m ' + (lessCable >= 0 ? 'less' : 'more')
      + ' site cable, ' + Math.abs(fewerJoints) + ' ' + (fewerJoints >= 0 ? 'fewer' : 'more') + ' site joints, and '
      + Math.abs(rnd(lossPct, 1)).toFixed(1) + ' % ' + (lossPct >= 0 ? 'more' : 'less') + ' loss';
    say(cX[0], top - step * (rows.length + 2), verdict, 11, INK, MINPX);
    // The drawing that goes to the page is the two bands and the table, handed over as the first band's.
    const g = A.string;
    const xs = [], ys = [];
    for (const s of shapes) { if (s.pts) for (const p of s.pts) { xs.push(p[0]); ys.push(p[1]); } }
    for (const m of marks) { xs.push(m.x); ys.push(m.y); }
    g.__draw = Object.assign({}, dA, { shapes, marks, words, terminals, crossings, g,
      conns: (dA.conns || []).concat(dB.conns || []),
      bounds: { x0: Math.min.apply(null, xs), y0: Math.min.apply(null, ys), x1: Math.max.apply(null, xs), y1: Math.max.apply(null, ys) } });
    const said = 'The same row drawn twice at one scale: one after another above, leapfrog below. '
      + verdict + '. Sequential lays ' + na.cable_laid_on_site_m.toFixed(1) + ' m of site cable at ' + na.string_resistance_ohm.toFixed(3)
      + ' ohm, losing ' + na.loss_w.toFixed(0) + ' W (' + pct(na.loss_w, pA).toFixed(2) + ' % of the string) with a drop of '
      + vA.toFixed(2) + ' V, ' + pct(vA, vmp).toFixed(2) + ' % of Vmp; leapfrog lays ' + nb.cable_laid_on_site_m.toFixed(1)
      + ' m at ' + nb.string_resistance_ohm.toFixed(3) + ' ohm, losing ' + nb.loss_w.toFixed(0) + ' W ('
      + pct(nb.loss_w, pB).toFixed(2) + ' %) with a drop of ' + vB.toFixed(2) + ' V, ' + pct(vB, vmp).toFixed(2) + ' % of Vmp.';
    const numbers = {};
    const keep = ['cable_laid_on_site_m', 'module_lead_m', 'total_conductor_m', 'string_resistance_ohm', 'volt_drop_v',
                  'loss_w', 'connectors_fitted_on_site', 'longest_link_m', 'lead_pair_m', 'links_that_do_not_reach'];
    for (const k of keep) { numbers['sequential_' + k] = na[k]; numbers['leapfrog_' + k] = nb[k]; }
    numbers.sequential_volt_drop_pct_of_vmp = rnd(pct(vA, vmp), 3);
    numbers.leapfrog_volt_drop_pct_of_vmp = rnd(pct(vB, vmp), 3);
    numbers.sequential_loss_pct_of_the_string = rnd(pct(na.loss_w, pA), 3);
    numbers.leapfrog_loss_pct_of_the_string = rnd(pct(nb.loss_w, pB), 3);
    numbers.site_cable_leapfrog_saves_m = rnd(lessCable, 2);
    numbers.site_joints_leapfrog_saves = fewerJoints;
    numbers.loss_leapfrog_adds_pct = rnd(lossPct, 2);
    return { numbers, said, string: g,
             how: 'the string model fired twice over the same row, once one after another and once leapfrog, and nothing added: '
               + 'every figure above is lifted out of one of those two answers, and the two pictures are the same drawing maker '
               + 'called twice and stacked at one scale. ' + A.how, inputs_used: Object.assign({}, A.inputs_used) };
  }
  F.register('compare', 'STRINGS', run, F.string.BOXES.filter(b => b.key !== 'routing' && b.key !== 'turn_every_second'));
})(typeof window !== 'undefined' ? window : globalThis);
...
const STRING_TESTS = [
  ...
  { family:'STRINGS', pin:-13, name:'The same row wired both ways, one above the other', chip:'compare', sentence:'One after another on top, leapfrog underneath, both at the same scale, and under them what each one costs in cable, joints and loss', command:'fire compare {"mounting":"fixed","modules_high":1,"jumpers":1}' },
...
function fireCommand(text, rec, inPlace){
  ...
  try { const fp = FIRE_PROGRAM;
    if (out.name === 'site-pulse' || out.name === 'network' || out.name === 'string' || out.name === 'compare') {
      fp.sld = (out.name === 'string' || out.name === 'compare') ? { kind:'string', g:out.string } : out.name === 'network' ? { kind:'network', net:out.net, solved:out.solved, inputs:c0(text) } : c0(text);

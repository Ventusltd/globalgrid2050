...
[span_1](start_span)Large scale price estimator for global 33 kV cable supply delivered to site with typical manufacturing lead times of 10 to 30 weeks.[span_1](end_span)
...
<div style="margin-bottom:20px; padding:15px; background:#f9f9f9; border: 1px solid #ddd; border-radius: 8px;">
  ...
  <p style="font-size:0.85em; color:#666; margin-bottom:15px;">
    <strong>Correlation:</strong> The green input boxes (Metals & FX) are the primary drivers. Changing these values instantly updates the green <strong>Net Price</strong> column in the table below.
  </p>
...
function calc() {
  ...
  // Supporting currency boxes updated from Green drivers
...
## Disclaimer
These values are derived from live market data feeds. Actual cable pricing varies based on project volume, factory loading, and specific utility requirements. [span_7](start_span)No warranty is given for data accuracy.[span_7](end_span)

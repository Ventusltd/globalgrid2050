'use strict';
// VENTUS-card.js - a cartridge of the VENTUS shell. Cut from the whole page with no change of behaviour;
...
// Historical source browsing is disabled in this generic drawing release.
function workTable(){return Promise.resolve({});}
function rawFile(){return Promise.reject(new Error('Source browsing is not part of this drawing release.'));}
...
window.__VENTUSCode = async (owner, repo, commit, path, line) => { const lines = await rawFile(owner, repo, commit, path);
...
// SHOW NEW CODE. "New" is entered from the newest commit, not from the wall clock, so the same

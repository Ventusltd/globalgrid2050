...
function makeFireBar(){
  ...
  st.textContent = ':root{--cmd:56px;--footh:34px;--stack:calc(var(--cmd) + var(--footh) + 8px);--egh:0px;--above:calc(var(--stack) + 8px)}'
    ...
    + '#eg{margin:0 0 6px auto;width:fit-content;max-width:min(30rem,88vw);background:#0e121b;border:1px solid #1b2030;'
    ...
    + '#eg{margin-left:0;width:auto;max-width:none;padding:.35rem .6rem}#eg label{font-size:10px}#eg select{max-width:none;min-height:40px}'
  ...
  const showSay = (title, nodes) => { $('saytitle').textContent = title;
  ...
  egLabel.textContent = 'examples: pick one, it fires, the bar keeps its name';
  ...
  addEventListener('resize', setStack); addEventListener('orientationchange', setStack);
  ...
  let ALL = dedupeCommands(FIRE_FALLBACK.filter(runsHere)), PINS = curateCommands(ALL, PICKER_SHOWN);
  ...
  loadFireConfig(FIRE_CONFIG).then(r => {
    if (r.ok) {
      ...
      if (listed.length) {
        ...
        PINS = pinned.length ? pinned : curateCommands(listed, PICKER_SHOWN);
...
function fireCommand(text, rec){
  ...
  const open = (title, nodes) => { if (sayOpen) sayOpen(title, nodes); };
  let out;

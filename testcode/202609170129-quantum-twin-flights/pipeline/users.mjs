...
const SEED = +(process.argv[3] || 0), COUNT = +(process.argv[4] || 0); // seed > 0: deterministic shuffle; count > 0: only that many
...
const T = [ // [what a user types, what must appear (regex) in the card or the log]
  ...
  ['measure 39885', /Draw [0-9]+/], ['connect 39885 1217', /connect 39885 → 1217/], ['apps', /19 apps/], ['app gridatlas', /app gridatlas/], ['app globalgrid2050', /app globalgrid2050/],
  ...
  ['state 39885', /K 1 same directory/], ['measure 39885 alpha', /seed "alpha"/], ['measure 39885', /Draw [0-9]+/], ['connect 2 39885', /connect 2 → 39885/], ['help', /block/],
  ...
  ['land gridatlas', /landed on gridatlas/], ['space', /back in space/], ['take me to pipeline news', /landed on pipelinenews/], ['back to space', /back in space/], ['show me the versions', /no command matched|versions/], ['block 39885', /beam/], ['reset', /reset/],
];
function mulberry32(a){ return function(){ a |= 0; a = a + 0x6D2B79F5 | 0; let t = Math.imul(a ^ a >>> 15, 1 | a); t = t + Math.imul(t ^ t >>> 7, 61 | t) ^ t; return ((t ^ t >>> 14) >>> 0) / 4294967296; }; }
if (SEED > 0) { const rnd = mulberry32(SEED); for (let i = T.length - 1; i > 0; i--) { const j = Math.floor(rnd() * (i + 1)); [T[i], T[j]] = [T[j], T[i]]; } }
if (COUNT > 0) T.length = Math.min(T.length, COUNT);
console.log(`users: ${T.length} actions · seed ${SEED} · ${URL}`);
const SLOW = +(process.env.SLOW || 0); // ms to hold after each action so a person can read it
...
try {
  ...
  for (let i = 0; i < T.length; i++) {
    ...
    await page.click('#pin'); await page.evaluate(() => { document.getElementById('pin').value = ''; }); await page.type('#pin', typed, { delay: SLOW ? 30 : 4 }); await page.keyboard.press('Enter');
    ...
    lines.push(line); console.log(line); if (SLOW) await new Promise(r => setTimeout(r, SLOW));

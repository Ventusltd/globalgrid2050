#!/usr/bin/env python3
"""
GridBot Interconnector Mobile UI Repair.

Target:
  /uk_energy_tracking_v6/generation_history/

Purpose:
  The first granular interconnector split passed the data audit but produced a bad mobile UI.
  This repair keeps the granular JSON outputs untouched and changes only the renderer/status
  layout so the annual MWh card remains readable on phone screens.

Audit mode:
  Builds the proposed JS patch in memory and writes reports only.

Apply mode:
  Writes only the renderer, controller and report files.
"""
...
REPORT_MD = REPORT_DIR / "INTERCONNECTOR_MOBILE_UI_REPAIR_LATEST.md"
REPORT_JSON = REPORT_JSON_DIR / "INTERCONNECTOR_MOBILE_UI_REPAIR_LATEST.json"
ROUTE = "/uk_energy_tracking_v6/generation_history/"
SCRIPT_NAME = "scripts/gridbot_interconnector_mobile_ui_repair.py"
WORKFLOW_NAME = "GridBot Interconnector Mobile UI Repair"
...
RENDER_JS = """window.V6RenderGenerationMwhAggregates=(function(){
  ...
  function annual(el,rows,icIndex,totals){
    ...
    var h='<div class="mwh-aggregate-head"><strong>Annual MWh by technology</strong><span>'+y+' - generation shown; interconnectors below</span></div><div class="mwh-bars">';
    ...
    if(links.length){
      h+='<details class="mwh-interconnector-details"><summary>Interconnectors · imports positive · exports negative</summary><div class="mwh-interconnector-list">';
      links.forEach(function(r){
        var imp=Number(r.importMWh||0), exp=Number(r.exportMWh||0), net=Number(r.netMWh||0);
        h+='<div class="mwh-ic-item"><div class="mwh-ic-name">'+r.label+'</div><div class="mwh-ic-values"><span>Import '+fmt(imp/1000000,2)+' TWh</span><span>Export '+fmt(exp/1000000,2)+' TWh</span><span>Net '+fmt(net/1000000,2)+' TWh</span></div></div>';
      });
      h+='</div></details>';
    }else{
...
def checks():
    ...
    return {
        ...
        'interconnectors_collapsed_into_details': 'mwh-interconnector-details' in render_text and '<summary>Interconnectors' in render_text,
        'total_check_reformatted_as_grid': 'mwh-total-grid' in render_text,
        'long_interconnector_rows_removed_from_main_bar_layout': 'mwh-interconnector-row' not in render_text,
...
def write_report(report):
    ...
    lines=['# Interconnector Mobile UI Repair','',f"Generated UTC: `{report['generatedUTC']}`",f"Mode: `{report['mode']}`",f"Pass: `{report['pass']}`",'',report['executiveSummary'],'','## Planned changed files','']

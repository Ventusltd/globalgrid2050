...
// ===== THE COMMAND BAR ============================================================================
// The wafer's bar, on the Kuiper: choose a sentence or type a command, it RUNS ON THIS DEVICE (the FIRE commands above are
// part of this same hashed file), the pulse sweeps, the work that holds those tests lights, and a card gives the answer.
// THE LIST OF SENTENCES IS A CONFIG FILE, fetched like GridAtlas fetches its own: change the file and the list changes; nothing
// about the list is written here except three sentences to fall back on, and the bar SAYS SO when it had to fall back.
const FIRE_CONFIG = 'https://ventusltd.github.io/faraday/bench/results/commands.json';
...
function makeFireBar(){
  ...
  st.textContent = '#firebar{display:flex;gap:6px;margin-bottom:6px;align-items:stretch}'
    ...
    + '@media (max-width:700px){#firebar{flex-wrap:wrap}#firebar select{flex:1 1 100%}#firecard{left:8px;right:8px;top:auto;bottom:150px;width:auto;max-height:34vh}}';
  ...
  bar.append(sel, box, go); foot.prepend(bar);
  const card = document.createElement('div'); card.id = 'firecard'; document.body.append(card);
  const fill = (list, note) => { sel.replaceChildren(new Option(note, ''));
    const groups = new Map(); for (const c of list) { if (!groups.has(c.family)) groups.set(c.family, []); groups.get(c.family).push(c); }
    for (const [fam, cs] of groups) { const g = document.createElement('optgroup'); g.label = fam; for (const c of cs) g.append(new Option(c.sentence, c.command)); sel.append(g); } };
  fill(FIRE_FALLBACK, '- choose an example sentence -');
  fetch(FIRE_CONFIG).then(r => { if (!r.ok) throw new Error(r.status); return r.json(); })
    .then(j => { if (!(j && Array.isArray(j.commands))) throw new Error('shape');
      ...
      const listed = j.commands.filter(c => { try { return FIRE_DRAWS.includes(window.FIRE.parse(c.command).name); } catch (_) { return false; } });
      const runs = FIRE_FALLBACK.filter(c => c.family === 'NETWORKS').concat(listed);
      if (!runs.length) throw new Error('none'); fill(runs, '- choose an example sentence (' + runs.length + ' that run on this device) -'); })
    .catch(() => fill(FIRE_FALLBACK, '- the list could not be fetched: three sentences kept here -'));
  const stop = e => e.stopPropagation();
  for (const x of [sel, box, go, card]) for (const ev of ['keydown', 'keyup', 'keypress', 'pointerdown', 'mousedown', 'touchstart', 'wheel', 'click']) x.addEventListener(ev, stop);
...
function makeSldBody(st){
  ...
  // st.idx[j]; a diagram has more dots than the application has pieces of work, and a tap found nothing, failed, and never let the

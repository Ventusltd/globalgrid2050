...
function renderTable() {
  ...
  const firstCount = Math.min(FIRST_PAINT_ROWS, total);

  const head = new Array(firstCount);
  for (let i = 0; i < firstCount; i += 1) head[i] = projectRowHtml(filtered[i]);
  body.innerHTML = head.join("");
  body.dataset.rendered = String(firstCount);
  body.dataset.total = String(total);
  if (total <= firstCount) { body.dataset.complete = "true"; return; }
  body.dataset.complete = "false";

  let next = firstCount;
  const step = () => {
    if (token !== renderToken) return;
    const end = Math.min(next + CHUNK_ROWS, total);
    const chunk = new Array(end - next);
    for (let i = next; i < end; i += 1) chunk[i - next] = projectRowHtml(filtered[i]);
    body.insertAdjacentHTML("beforeend", chunk.join(""));
    next = end;
    body.dataset.rendered = String(next);
    if (next < total) schedule(step);
    else body.dataset.complete = "true";
  };
  schedule(step);
}

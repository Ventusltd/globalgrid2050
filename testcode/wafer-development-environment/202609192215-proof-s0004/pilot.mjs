...
/* The bottom stack: the command bar, then the footer above it, then everything else above both.
   The two heights are measured on the laid out page (setStack, below) because a guessed 58px is
   what put the log and the example chooser on top of the footer in the first place. */
:root{--cmd:56px;--footh:34px;--stack:calc(var(--cmd) + var(--footh) + 8px);--statush:0px;--above:calc(var(--stack) + var(--statush) + 8px);--egh:0px}
...
#say{position:fixed;left:50%;top:calc(var(--toph) + 8px);transform:translateX(-50%);width:min(72ch,84vw);max-height:calc(100vh - var(--toph) - var(--above) - var(--beamh) - var(--egh) - 32px);min-width:280px;min-height:64px;background:#0e121bf2;border:1px solid #1b2030;border-radius:8px;padding:0;font:14px/1.5 ui-monospace,Menlo,Consolas,monospace;color:#cfe3f2;display:none;z-index:20;white-space:pre-wrap;resize:both;overflow:hidden;display:none;flex-direction:column}
#say.open{display:flex} #say.max{left:12px;top:12px;right:12px;bottom:var(--above);transform:none;width:auto;max-height:none;height:auto}
...
#eg{position:fixed;right:12px;bottom:calc(var(--above) + var(--beamh) + 8px);background:#0e121bf2;border:1px solid #1b2030;border-radius:8px;padding:.5rem .7rem;display:flex;flex-direction:column;gap:4px;max-width:46vw;z-index:20}
...
#plog{position:fixed;left:12px;bottom:var(--stack);width:max-content;max-width:calc(100% - 24px);background:#11151fef;border:1px solid #1b2030;border-radius:6px;padding:.3rem .6rem;font:12px/1.5 ui-monospace,Menlo,Consolas,monospace;color:#cfe3f2;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;pointer-events:none;z-index:19}
...
/* THE APP PANEL IS NOT THERE UNTIL IT IS OPENED. This was inside the phone media query, so above
   640px it was a real full width box on the page with a link in it, invisible only because the
   canvas is painted over it - and a link nobody can press is worse than no link at all. */
#earth{position:fixed;inset:0;z-index:30;display:none;flex-direction:column;background:#0b0e15}
#earth.open{display:flex}
/* and its bar and frame are styled at every width, for the same reason: this whole block used to
   sit inside the phone media query, which left the panel unstyled anywhere else. */
...
/* THE BOTTOM STACK, MEASURED. The command bar's height depends on the font a reader's browser gives
   it and on how the footer wraps at their width, so both heights are read off the laid out page and
   published as --cmd and --footh. A guessed 58px is what buried the line that says what this page is
   not. Measured again whenever either box changes shape, which includes a phone turning sideways. */
function setStack() {
  const r = document.documentElement.style;
  const bar = document.getElementById('pilot'), foot = document.getElementById('foot');
  if (bar) r.setProperty('--cmd', Math.ceil(bar.getBoundingClientRect().height) + 'px');
  if (foot) r.setProperty('--footh', Math.ceil(foot.getBoundingClientRect().height) + 'px');
  const st = document.getElementById('plog');
  if (st) r.setProperty('--statush', (st.textContent.trim() ? Math.ceil(st.getBoundingClientRect().height) : 0) + 'px');
  const top = document.getElementById('bar'), menu = document.getElementById('presets'),
        fly = document.getElementById('beam');
  if (top) r.setProperty('--barh', Math.ceil(top.getBoundingClientRect().height) + 'px');
  if (menu) r.setProperty('--toph', Math.ceil(menu.getBoundingClientRect().bottom) + 'px');
  if (fly) r.setProperty('--beamh', Math.ceil(fly.getBoundingClientRect().height) + 'px');
  const egs = document.getElementById('eg');
  if (egs) r.setProperty('--egh', Math.ceil(egs.getBoundingClientRect().height) + 'px');
}
setStack();
addEventListener('resize', setStack);
if (window.ResizeObserver) {
  const ro = new ResizeObserver(setStack);
  ['pilot', 'foot', 'plog', 'bar', 'presets', 'beam', 'eg'].forEach(id => { const e = document.getElementById(id); if (e) ro.observe(e); });
}
...
/* THE RUNNING COMMENTARY IS NOT FOR THE FACE. It was printed straight onto the drawing, twelve lines
   at a time, one of them 1530 pixels wide inside a 1366 pixel window and off the edge of every phone,
   and it explained the machine: "245,170 lines placed by: edge weight = the line's initial symmetrical
   short-circuit current". A page carries what this is, what it is for, and what it is not. Every line
   is still written, in full and unchanged, to the browser's own console and to window.__log, which is
   where a reader checking a claim can read it. The face gets one plain sentence. */
const lines = [];
const log = s => {
  ...
  if (plain) { $('plog').textContent = plain; setStack(); }
};
/* Nothing reaches the face unless it is named here, in plain words. A sentence the machine invents
   tomorrow leaves the status line alone rather than quietly putting the machinery back on the page. */
function status(s) {
  let m;
  const n = x => Number(String(x).replace(/,/g, '')).toLocaleString('en-GB');
  if ((m = /^draw \w+: ([\d,]+) nodes?, ([\d,]+) edges?/.exec(s))) return `Loaded. ${n(m[1])} places, ${n(m[2])} connections.`;
  if (/^clock: /.test(s)) return 'Drawn from the time on this computer.';
  if (/^logo \S+: assembled$/.test(s)) return 'Drawn.';
  if ((m = /showing ([\d,]+) of ([\d,]+) lines/.exec(s))) return `Showing ${m[1]} of ${m[2]} lines.`;
  if (/^error: /.test(s)) return 'That did not work. Try one of the buttons at the top.';
  if (/not ready$|not loaded$|missing$/.test(s)) return 'Still loading. One moment.';
  return null;
}
...
document.body.insertAdjacentHTML('beforeend', '<div id="hud" style="position:fixed;right:12px;top:calc(var(--barh) + 8px);font:11px/1.5 ui-monospace,Menlo,Consolas,monospace;color:#8b93a7;pointer-events:none;z-index:19"></div>');
...
cmds.logo = async function(spec){ if (!W()) return log('the wafer is not ready'); const sc = scopeKeys(spec || scopeName); if (!sc) return scopeFail(spec); const r = await W().scope(sc.keys); scopeName = sc.name; const L = rasterLogo(); const m = W().state().scopeN;
  log(`logo ${sc.name}: ${fmtN(m)} lines → "${LOGO.lines.join(' / ')}" (${fmtN(L.n)} glyph samples, ${LOGO.font}, grid ${LOGO.step} px)`); say(`<b>The wordmark</b>\nThe same dots as the drawings, arranged into the name of this project. It shows that any one of them can be put anywhere on demand. A demonstration, not a measurement of anything.`);
  await W().blend(logoTargets(m)); hud(sc.name + ' · logo');
  log(`logo ${sc.name}: assembled`); return r; };
...
(async () => {
  ...
  const qd = new URLSearchParams(location.search).get('draw'); const start = qd ? (['logo', 'clock'].includes(qd) ? qd : 'draw ' + qd) : document.querySelector('meta[name="wafer-start"]')?.content; if (start) { await new Promise(r => setTimeout(r, 1500)); const m = /^(land|visit|go|open)\s+(\S+)/.exec(start); if (m) { await run('app ' + m[2]); log(`This page stops here. To enter ${m[2]}, type: land ${m[2]} — space brings you back to the wafer.`); } else await run(start); /* THE CARD OPENS. It used to fold itself to its title bar the moment a view had drawn, so all
     twelve views offered a frame with a cut off title and not one sentence under it. */ }
})();

// ---------- NETWORK: a whole system drawn from a typed command, on the wafer's own dust (no new looks) ----------
// draw underground: the London Underground's topology (Transport for London open data; stations and the order of
...
const netCache = {};
/* WHAT EACH DRAWING IS, IN THREE SENTENCES: what it is, what it is for, and what it is not. The
   reader is the person who signs a project, not a coder, and he has about ten seconds. The title is
   short on purpose: it has to fit its own bar on a phone without being cut off mid word. */
const NETWORKS = {
  grid:        ['The grid', 'Britain’s 400 kV and 132 kV lines and its substations, drawn from public data.', 'See what runs near a site and where it could connect.'],
  grid400:     ['The 400 kV grid', 'Britain’s highest voltage transmission lines, drawn from public data.', 'See how far a site sits from the main network.'],
  grid132:     ['The 132 kV network', 'Britain’s 132 kV lines, drawn from public data.', 'See the voltage level most new projects connect at.'],
  substations: ['Substations', 'Every substation in the public map of Britain’s network.', 'See which ones are near a site before asking for a connection.'],
  shotwick:    ['Shotwick solar farm', 'A built solar farm and the network around it, drawn from public data.', 'See what a finished connection looks like on the map.'],
  underground: ['The London Underground', 'Every line and station of the London Underground, drawn from public data.', 'See how a dense network reads at the same scale as the grid.'],
  uk:          ['The British Isles', 'The coastline of the British Isles, drawn from public data.', 'The outline the network drawings sit on.'],
  world:       ['The world', 'The coastlines of the world, drawn from public data.', 'The scale everything else on this page sits in.'],
  trench:      ['A cable trench', 'Four circuits in flat formation, at a stated spacing and depth.', 'See the arrangement a cable rating is worked out from.'],
  fault:       ['A fault study', 'The fault current paths in a published example network.', 'See which lines carry the most current when a fault happens.'],
};
const NOT_A_STUDY = 'An estimate from public data, not a study.';
...
function networkTargets(N, m){
  ...
  for (let s = 0; j < m; s++, j++) { const q = s % st.length, c = Math.floor(s / st.length), th = c * GA, r = 0.004 * S * Math.sqrt(c);
    t[2 * j] = S * st[q][0] + r * Math.cos(th); t[2 * j + 1] = S * st[q][1] + r * Math.sin(th); }
  return t;
}
cmds.draw = async function(spec){
  let what = (spec || '').trim().toLowerCase().split(/\s+/)[0] || ''; if (/^(tube|london)$/.test(what)) what = 'underground'; 
  if (!NETWORKS[what]) return log('draw <system> — one of: ' + Object.keys(NETWORKS).join(', '));
  if (!W()) return log('the wafer is not ready');
  const N = await loadNet(what); if (!N) return log(`draw ${what}: ${what}-network.json missing`);
  ...
  const [title, isWhat, isFor] = NETWORKS[what];
  /* The sentences go up BEFORE the drawing assembles, not after it: they are what the reader came
     for, and they do not depend on the animation having finished. */
  say(`<b>${title}</b>
${isWhat} ${isFor} ${NOT_A_STUDY}
...
cmds.clock = async function(){
  ...
  say(`<b>A clock</b>\nThe time on this computer at ${d.toTimeString().slice(0, 8)}, drawn with the same dots as the networks. It shows that every drawing here is worked out at the moment you ask for it. A demonstration, not a measurement of anything.`);

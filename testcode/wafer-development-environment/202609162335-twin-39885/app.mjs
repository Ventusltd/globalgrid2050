 ...
 * where it focuses the structure resolves â€” one line, its length, whether any
 ...
 * line carried by exactly one family is perfectly connectable â€” to another line
...
/* â”€â”€ the surface â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
async function tier1() {
  ...
  $('count').textContent =
    `${fmt(U.n)} numbered lines Â· 1 to ${fmt(meta.max)} Â· ${fmt(carried)} carried by a family`;
  $('prov').textContent =
    `numbered database built ${meta.built_utc.slice(0, 16).replace('T', ' ')} UTC Â· pack ${meta.source.sha256.slice(0, 12)}`;
}
...
/* If anything after acquiring the context fails â€” a shader that will not compile
   on some driver, a program that will not link â€” the page must fall back, not
...
/* â”€â”€ the beam: steering and focusing â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
/* The line under the finger. Until iteration 49 this compared the tap with
   ...
   answer â€” proved against the full scan in proof/pick-index.check.mjs â€” about
...
/* â”€â”€ reading the line itself â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
async function fillCode(el, key) {
  ...
  const s = store.stats();
  setStoreLine(`${fmt(s.bytes)} bytes read in ${s.requests} request${s.requests === 1 ? '' : 's'} Â· ` +
    `${s.blocksHeld} block${s.blocksHeld === 1 ? '' : 's'} held Â· ${store.drifted ? 'offsets by search' : 'offsets exact'}`);
}
...
/* â”€â”€ the panel: what the beam found â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
function paintPanel(key) {
  ...
  let body =
    `<h2>Line <span class="n">${fmt(key)}</span></h2>
     <pre class="code loading" data-key="${key}">reading this line from the numbered databaseâ€¦</pre>
     <dl>
       <dt>characters</dt><dd>${chars === 65535
          ? 'at least 65,535 â€” the pack stores this in sixteen bits and this line reaches the ceiling, so its true length is not known here'
    ...
    body += `<p>Carried by <span class="fam">${fmt(fams.length)}</span> ${fams.length === 1 ? 'family' : 'families'}${fams.length > 12 ? ', first twelve' : ''}:</p><ul>` +
      show.map(f => {
        ...
        return `<li><button type="button" class="famopen" data-fam="${f}">` +
          `<span class="fam">${esc(fa.name)}</span> <span class="dim">#${fmt(fa.n)} Â· ${esc(fa.kind)} Â· ` +
          `${esc(cat)} Â· ${fmt(fa.lineCount)} lines</span></button></li>`;
      }).join('') + `</ul>`;
    if (fams.length > 60) {
...
/* â”€â”€ a family, as a block of code â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function openFamily(fIdx) {
  ...
  host.innerHTML = '<p class="dim">reading ' + fmt(keys.length) + ' linesâ€¦</p>';
  const store = await reader();
  ...
  host.innerHTML =
    ...
    ' Â· ' + fmt(fa.lineCount) + ' lines Â· in ' + fmt(fa.files) + ' files across ' + fmt(fa.repos) + ' repos</span></div>' +
  ...
  setStoreLine(fmt(st.bytes) + ' bytes read in ' + st.requests + ' request' + (st.requests === 1 ? '' : 's') +
    ' Â· ' + st.blocksHeld + ' block' + (st.blocksHeld === 1 ? '' : 's') + ' held');
}
...
function connect(ka, kb) {
  ...
  const famLine = f => {
    const fa = U.families[f];
    return `<li><span class="fam">${esc(fa.name)}</span> <span class="dim">#${fmt(fa.n)} Â· ${esc(fa.category ?? 'no category recorded')}</span></li>`;
  };
  let body = head;
...
/* â”€â”€ gestures â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
/* â”€â”€ URL state: the whole of it, in permanent numbers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */

...
Direct V6 port of the proven V5 collector. It writes only V6 frequency data files
and leaves the V5 collector untouched for rollback and comparison.
"""
...
REPORT_FILE = REPORT_DIR / "uk_frequency_v6_report.md"
...
USER_AGENT = "GlobalGrid2050 V6 frequency collector using public Elexon data"
...
def weekly_summary_from_rows(rows: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    ...
    for key, values in buckets.items():
        ...
        out[key] = {
            ...
            "samples_below_49_9": sum(1 for v in values if v < 49.9),
            "samples_above_50_1": sum(1 for v in values if v > 50.1),
            "largest_deviation_hz": round(max(abs(v - 50.0) for v in values), 4),

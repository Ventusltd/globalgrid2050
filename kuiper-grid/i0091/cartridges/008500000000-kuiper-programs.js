...
function systemCard(st, sys){ appCard(st);
  ...
  put(sys.system, 'cc-title'); put(sys.kind + ' Â· ' + fmt(st.lit) + ' pieces of work lit Â· the run counted ' + fmt(sys.files_total) + ' files in all');
  for (const r of sys.repos) put(r.repo + ' Â· ' + fmt(r.files) + ' files Â· ' + r.prefixes.join('  '));
...
function systemsList(){ return SYSTEMS.map(s => s.id + '  ' + s.system + '  Â· ' + s.files_total + ' files').join(String.fromCharCode(10)); }
if (typeof window !== 'undefined') { window.KUIPER_SYSTEMS = { all:SYSTEMS, list:systemsList, fire:fireSystem, find:systemFind };
  try { if (window.FIRE && window.FIRE.register) {
    ...
    window.FIRE.register('system', 'THE ESTATE', i => { const s = systemFind(i && (i.id || i.name)); if (!s) throw new Error('no such system; type "systems" for the list');
      fireSystem(s.id); return { numbers:{ files:s.files_total }, said:s.system + ' â€” its pulse is running on the disc', how:'the work of ' + s.repos.map(r => r.repo).join(', ') + ' under the folders of this system' }; }); } } catch (_) {}
}
...
(function (root) {
  ...
  function build(inp) {
    ...
    (() => { if (Math.round(+c.n_boxes) === 1 && Math.abs(fP0 - fM0) > 1e-6)
        return no_('lead_only_from_terminal_box', 'this module has ONE junction box, so both leads leave that box and the box spacing is zero; the + and the âˆ’ are asked for ' + boxSpanM.toFixed(3) + ' m apart. A module lead can only leave a junction box that carries one.');
    ...
    // 4
    (() => { const count = new Map(); for (let k = 1; k <= N; k++) { count.set('M' + k + 'âˆ’', 0); count.set('M' + k + '+', 0); }
      for (const l of links) { count.set('M' + l.from + '+', (count.get('M' + l.from + '+') || 0) + 1); count.set('M' + l.to + 'âˆ’', (count.get('M' + l.to + 'âˆ’') || 0) + 1); }
      count.set('M' + order[0] + 'âˆ’', (count.get('M' + order[0] + 'âˆ’') || 0) + 1);
    ...
    if (short.length) no_('every_link_reaches', short.length + ' of ' + links.length + ' links do not reach with a lead pair of ' + rnd(pair, 3) + ' m. First: M' + short[0].from + '+ to M' + short[0].to + 'âˆ’ needs ' + rnd(short[0].need, 3) + ' m.');
  ...
  function run(inp) {
    ...
    const order = g.order.map(k => 'M' + k).join(' â†’ ');
    ...
    return { connections, refused: false, checks: g.checks,
             numbers: { circuit: open ? 'OPEN, both ends live' : 'closed', current_a: rnd(amps, 2),
               ...
               volts_to_earth_each_end: 'âˆ’' + rnd(g.N * c.module_vmp / 2, 0) + ' to +' + rnd(g.N * c.module_vmp / 2, 0),
  ...
  const BOXES = [
    ...
    { key:'lead_minus_m', label:'âˆ’ lead', unit:'m', kind:'number', min:0.05, max:8, step:0.01 },
    ...
    { key:'box_across_frac_minus', label:'âˆ’ box across the module', unit:'frac', kind:'number', group:'MODULE', min:0, max:1, step:0.0001, optional:true, hint:'blank takes it from the box spacing' },
    ...
    { key:'clip_offset_m', label:'clip line off the box line', unit:'m', kind:'number', group:'STRUCTURE', min:-2, max:2, step:0.01, hint:'the library assumes âˆ’0.120; zero is the straight geometric case' },
...
(function (root) {
  ...
  // light for âˆ’, and each one ends ON ITS OWN NAMED INVERTER INPUT TERMINAL, which is drawn and named.
...
const STRING_TESTS = [
  ...
  { family:'THE GRID', pin:-30, name:'One substation and every way out of it', chip:'Barking', sentence:'Barking as a station on a map: one coloured line for every circuit route out of it, the voltage the colour, the number of circuits the weight', command:'fire substation {"site":"Barking"}' },
  { family:'THE GRID', pin:-29, name:'The same map with the distances to scale', chip:'neighbours', sentence:'The far stations pushed out to their real distance, so the shape of the map is the shape of the network', command:'fire substation {"site":"Barking","neighbours":1}' },
  { family:'THE GRID', pin:-28, name:'The pulse out from the station', chip:'pulse', sentence:'The front runs out from the station along every line in order of electrical distance, shortest first: this shows CONNECTIVITY, not power', command:'fire substation {"site":"Barking","pulse":1}' },
  { family:'THE GRID', pin:-27, name:'One way out taken off', chip:'outage', sentence:'One circuit route open: it carries nothing and its far station goes dark, and every other way out still lights', command:'fire substation {"site":"Barking","outage":"West Ham"}' },
...
function chipWords(c){
  ...
  return head.length > 30 ? head.slice(0, 29) + 'â€¦' : head;
}
...
function makeFireBar(){
  ...
  sayEl.innerHTML = '<div id="saybar"><span id="saytitle">card</span>'
    + '<button id="saymin" type="button" title="minimise" aria-label="minimise">â€“</button>'
    + '<button id="saymax" type="button" title="maximise" aria-label="maximise">â–¢</button>'
    + '<button id="sayclose" type="button" title="close" aria-label="close">Ã—</button></div>'
    + '<div id="saybody"></div>';
  ...
  pin.placeholder = 'type a sentence or a command: tree of 12 Â· on its export limit Â· the most solar that fits Â· apps Â· help';
  ...
  const fill = list => { egs.replaceChildren(new Option(list.length ? 'â€” choose an example sentence â€”' : 'â€” nothing matches â€”', ''));
...
function stringDrawing(g){
  ...
  for (const [y, sign] of [[negY, 'âˆ’'], [posY, '+']]) {
  ...
  for (let k = 1; k <= N; k++) {
    ...
    for (const [pt, sign] of [[g.minusPt(k), 'âˆ’'], [g.plusPt(k), '+']]) {
  ...
  poly('cable', 0, homeNeg, { node:'home-', pot:0, plus:false, weight:1.2, from:'INV-', to:'M' + first + '-', says:'the home run to the stringâ€™s âˆ’ end' });
  ...
  poly('cable', 1, homePos, { node:'home+', pot:1, plus:true, weight:1.2, from:'M' + last + '+', to:'INV+',
    says:g.bothNear ? 'the home run from the stringâ€™s + end' : 'the far end return: one more cable, ' + g.ret.toFixed(1) + ' m of it' });
  ...
  words.push({ x:xn, y:topLane + 0.40, text:'inverter âˆ’ to M' + first + 'âˆ’' });
  ...
  words.push({ pinned:1, note:true, narrow:true,
    text:CABLE_NOTE + ' Â· red +, black âˆ’ Â· the back of the modules, boxes and cables enlarged' });
...
function stringPieces(g){
  const P0 = [];
  // A MAP'S DOTS SETTLE ALONG ITS OWN LINES AND RINGS: the same description, so they can never disagree.
  if ((g.__draw||{}).substation) { const d = g.__draw, P = P0; for (const s of d.shapes) { if (!s.pts || s.pts.length < 2) continue;
      const m = polyLen(s.pts); if (m.total > 1e-7) P.push({ tag:s.role === 'sldstation' ? 'inv' : ('line' + s.id), d:s.order || 0, flat:true, kind:'path', pts:s.pts, cum:m.cum, len:m.total }); }
    return P; }
...
function drawSldShapes(){
  ...
  // 3a. THE MAP. A station, its interchange ring, and one coloured line for every circuit route
  //     out of it. Colour is the voltage level, weight is how many parallel circuits run that way.
  //     DASHED where the value was ASSUMED and SOLID where it came from the data, so provenance is in
  //     the drawing itself and not in a footnote. A line marked OPEN by an outage is drawn dead grey.
  //     The length in km is a quiet label and it appears only when the map is zoomed in far enough to
  //     read it: at map distance the shape of the network is the thing, not the numbers on it.
  if (D.substation) {
    for (const s of D.shapes) {
      if (!s.pts || s.pts.length < 2 || !onScreen(s.pts)) continue;
      const lit = shapeHover && shapeHover === s.node;
      const done = front >= 0 && (s.order === undefined ? 0.5 : s.order) <= 0.5 + front * 0.5 + 1e-6;
      const dead = !!s.dark;
      const col = dead ? (SHEET ? '#c3c8cf' : '#39424e') : lit ? '#eaf6ff' : (s.colour || INK);
      const w = Math.max(1, fine * (s.weight || 1));
      ctx.setLineDash(s.assumed ? [5.5, 4.2] : []);
      if (s.role === 'sldstation') {
        path(s.pts); ctx.closePath();
        ctx.fillStyle = dead ? (SHEET ? '#dfe3e8' : '#0a0e14') : (SHEET ? '#ffffff' : '#05080d'); ctx.fill();
        ctx.lineWidth = w; ctx.strokeStyle = dead ? col : (SHEET ? '#10151c' : INK); ctx.stroke();
        ctx.setLineDash([]); continue; }
      // a line is drawn with a dark casing under it so two lines that meet are still two lines
      path(s.pts); ctx.lineWidth = w + 3.0; ctx.strokeStyle = SHEET ? '#f2f4f7' : '#05080d'; ctx.setLineDash([]); ctx.stroke();
      ctx.setLineDash(s.assumed ? [5.5, 4.2] : []);
      path(s.pts); ctx.lineWidth = w + (lit ? 1.8 : 0); ctx.strokeStyle = done && !dead ? '#eaf6ff' : col; ctx.stroke();
      ctx.setLineDash([]);
      if (s.km && px > 900) {
        const mid = s.pts[Math.max(0, s.pts.length - 2)], end = s.pts[s.pts.length - 1];
        const a = S([(mid[0] + end[0]) / 2, (mid[1] + end[1]) / 2]);
        ctx.font = '600 10px ui-monospace,SFMono-Regular,Menlo,monospace';
        ctx.fillStyle = SHEET ? '#55606e' : '#7f8899'; ctx.textAlign = 'center';
        ctx.fillText(s.km + ' km', Math.round(a[0]), Math.round(a[1]) - 7); } }
  }
...
// ===== ONE SUBSTATION AND ALL ITS POSSIBLE CONNECTIONS: fire substation {"site":"Barking"} ========
// NOT AN ENGINEER'S SINGLE LINE DIAGRAM. A reader does not need to know how a train works to use an
// underground map, and a politician or a banker does not need inverters, transformers or surge
// arresters to see where the power can go. So a substation is a STATION: a mark, an interchange ring
// when more than one voltage level meets there, and one coloured LINE for every circuit that leaves
// it, at 0, 45 or 90 degrees and nothing between. Colour is the voltage level. Weight is how many
// parallel circuits run that way. The far end is another station, named, and tapping it walks there.
// WHAT IS ASSUMED IS DRAWN ASSUMED: solid came from the data, dashed was assumed.
/* GRID-DATA:START */ const GRID_DATA = {"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","made":"E:\\swarm\\grid\\build_grid_data.py - topology from circuit names, geometry for length only","netlists":{"S-barking":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-barking.BB400","circuit":"C-barking--littlebrook--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Barking - Littlebrook 1"},"far_end":{"id":"S-littlebrook","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Littlebrook"},"status":"ASSUMED"},"id":"S-barking.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-barking.BAY01.T1"},{"busbar":"S-barking.BB400","circuit":"C-barking--littlebrook--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Barking - Littlebrook 2"},"far_end":{"id":"S-littlebrook","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Littlebrook"},"status":"ASSUMED"},"id":"S-barking.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-barking.BAY02.T1"},{"busbar":"S-barking.BB400","circuit":"C-barking--northfleet-east--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Barking - Northfleet East 1"},"far_end":{"id":"S-northfleet-east","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Northfleet East"},"status":"ASSUMED"},"id":"S-barking.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-barking.BAY03.T1"},{"busbar":"S-barking.BB400","circuit":"C-barking--northfleet-east--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Barking - Northfleet East 2"},"far_end":{"id":"S-northfleet-east","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Northfleet East"},"status":"ASSUMED"},"id":"S-barking.BAY04","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-barking.BAY04.T1"},{"busbar":"S-barking.BB400","circuit":"C-barking--west-ham--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Barking - West Ham"},"far_end":{"id":"S-west-ham","label":{"source":"the other half of the circuit name","status":"TYPED","value":"West Ham"},"status":"ASSUMED"},"id":"S-barking.BAY05","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-barking.BAY05.T1"},{"busbar":"S-barking.BB400","circuit":"C-barking--west-thurrock--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Barking - West Thurrock"},"far_end":{"id":"S-west-thurrock","label":{"source":"the other half of the circuit name","status":"TYPED","value":"West Thurrock"},"status":"ASSUMED"},"id":"S-barking.BAY06","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-barking.BAY06.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-barking.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-barking.BAY01.T1","from_site":"S-barking","id":"C-barking--littlebrook--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features / 2 (mean of the parallel circuits); 23 feature(s)","status":"TYPED","value":4.425},"name":{"source":"name tag on the line features","status":"TYPED","value":"Barking/Littlebrook - Tilbury"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.1328},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-littlebrook.BAY?.T1","to_site":"S-littlebrook","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":1.3276}},{"from":"S-barking.BAY02.T1","from_site":"S-barking","id":"C-barking--littlebrook--400#2","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features / 2 (mean of the parallel circuits); 23 feature(s)","status":"TYPED","value":4.425},"name":{"source":"name tag on the line features","status":"TYPED","value":"Barking/Littlebrook - Tilbury"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.1328},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-littlebrook.BAY?.T1","to_site":"S-littlebrook","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":1.3276}},{"from":"S-barking.BAY03.T1","from_site":"S-barking","id":"C-barking--northfleet-east--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features / 2 (mean of the parallel circuits); 8 feature(s)","status":"TYPED","value":5.381},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Barking - Northfleet East - West Thurrock"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.1614},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-northfleet-east.BAY?.T1","to_site":"S-northfleet-east","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":1.6144}},{"from":"S-barking.BAY04.T1","from_site":"S-barking","id":"C-barking--northfleet-east--400#2","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features / 2 (mean of the parallel circuits); 8 feature(s)","status":"TYPED","value":5.381},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Barking - Northfleet East - West Thurrock"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.1614},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-northfleet-east.BAY?.T1","to_site":"S-northfleet-east","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":1.6144}},{"from":"S-barking.BAY05.T1","from_site":"S-barking","id":"C-barking--west-ham--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 7 feature(s)","status":"TYPED","value":9.883},"name":{"source":"name tag on the line features","status":"TYPED","value":"West Ham–Barking"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.2965},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-west-ham.BAY?.T1","to_site":"S-west-ham","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":2.9649}},{"from":"S-barking.BAY06.T1","from_site":"S-barking","id":"C-barking--west-thurrock--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 10 feature(s)","status":"TYPED","value":3.412},"name":{"source":"name tag on the line features","status":"TYPED","value":"West Thurrock - Barking"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.1024},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-west-thurrock.BAY?.T1","to_site":"S-west-thurrock","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":1.0237}}],"counts":{"bays":6,"busbars":1,"circuits":4,"conductors":6,"km_total":32.907,"neighbours":4},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":6,"id":"S-littlebrook","kv":[400],"label":"Littlebrook","lat":51.442903,"lon":0.092319,"status":"ASSUMED","walk_to":"substation.py \"Littlebrook\""},{"bays":6,"id":"S-northfleet-east","kv":[400],"label":"Northfleet East","lat":51.429089,"lon":0.329869,"status":"ASSUMED","walk_to":"substation.py \"Northfleet East\""},{"bays":2,"id":"S-west-ham","kv":[400],"label":"West Ham","lat":51.519582,"lon":0.001102,"status":"ASSUMED","walk_to":"substation.py \"West Ham\""},{"bays":4,"id":"S-west-thurrock","kv":[400],"label":"West Thurrock","lat":51.457348,"lon":0.301643,"status":"ASSUMED","walk_to":"substation.py \"West Thurrock\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"no exact normalised match in the register - this is a name off the map, not a registered site","status":"ASSUMED","value":null},"id":"S-barking","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Barking"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":51.523135},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":0.115747},"register":null,"status":"ASSUMED"},"transformers":[]},"S-bramford":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-bramford.BB400","circuit":"C-bramford--norwich-main--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Bramford - Norwich Main"},"far_end":{"id":"S-norwich-main","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Norwich Main"},"status":"MATCHED"},"id":"S-bramford.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-bramford.BAY01.T1"},{"busbar":"S-bramford.BB400","circuit":"C-bramford--sizewell--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Bramford - Sizewell 1"},"far_end":{"id":"S-sizewell","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Sizewell"},"status":"ASSUMED"},"id":"S-bramford.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-bramford.BAY02.T1"},{"busbar":"S-bramford.BB400","circuit":"C-bramford--sizewell--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Bramford - Sizewell 2"},"far_end":{"id":"S-sizewell","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Sizewell"},"status":"ASSUMED"},"id":"S-bramford.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-bramford.BAY03.T1"},{"busbar":"S-bramford.BB400","circuit":"C-bramford--stocking-pelham--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Bramford - Stocking Pelham"},"far_end":{"id":"S-stocking-pelham","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Stocking Pelham"},"status":"ASSUMED"},"id":"S-bramford.BAY04","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-bramford.BAY04.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-bramford.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-bramford.BAY01.T1","from_site":"S-bramford","id":"C-bramford--norwich-main--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":61.469},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Norwich Main-Bramford"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.8441},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-norwich-main.BAY?.T1","to_site":"S-norwich-main","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":18.4406}},{"from":"S-bramford.BAY02.T1","from_site":"S-bramford","id":"C-bramford--sizewell--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features / 2 (mean of the parallel circuits); 2 feature(s)","status":"TYPED","value":43.139},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Bramford-Sizewell"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.2942},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-sizewell.BAY?.T1","to_site":"S-sizewell","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":12.9417}},{"from":"S-bramford.BAY03.T1","from_site":"S-bramford","id":"C-bramford--sizewell--400#2","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features / 2 (mean of the parallel circuits); 2 feature(s)","status":"TYPED","value":43.139},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Bramford-Sizewell"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.2942},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-sizewell.BAY?.T1","to_site":"S-sizewell","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":12.9417}},{"from":"S-bramford.BAY04.T1","from_site":"S-bramford","id":"C-bramford--stocking-pelham--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 6 feature(s)","status":"TYPED","value":68.904},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Bramford-Stocking Pelham"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":2.0671},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-stocking-pelham.BAY?.T1","to_site":"S-stocking-pelham","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":20.6713}}],"counts":{"bays":4,"busbars":1,"circuits":3,"conductors":4,"km_total":216.651,"neighbours":3},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":1,"id":"S-norwich-main","kv":[400],"label":"Norwich Main","lat":52.572736,"lon":1.271911,"status":"MATCHED","walk_to":"substation.py \"Norwich Main\""},{"bays":2,"id":"S-sizewell","kv":[400],"label":"Sizewell","lat":52.215025,"lon":1.616575,"status":"ASSUMED","walk_to":"substation.py \"Sizewell\""},{"bays":3,"id":"S-stocking-pelham","kv":[400],"label":"Stocking Pelham","lat":51.932743,"lon":0.119123,"status":"ASSUMED","walk_to":"substation.py \"Stocking Pelham\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"exact normalised match to the NESO connection-site register (R10)","status":"TYPED","value":"Bramford 400kV Substation"},"id":"S-bramford","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Bramford"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":52.071892},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":1.060841},"register":{"neso_id":"7479","placeable":true,"register":"TEC","register_name":"Bramford 400kV Substation","status":"Built"},"status":"MATCHED"},"transformers":[]},"S-bramley":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-bramley.BB400","circuit":"C-bramley--didcot--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Bramley - Didcot"},"far_end":{"id":"S-didcot","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Didcot"},"status":"ASSUMED"},"id":"S-bramley.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-bramley.BAY01.T1"},{"busbar":"S-bramley.BB400","circuit":"C-bramley--fleet--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Bramley - Fleet"},"far_end":{"id":"S-fleet","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Fleet"},"status":"ASSUMED"},"id":"S-bramley.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-bramley.BAY02.T1"},{"busbar":"S-bramley.BB400","circuit":"C-bramley--melksham--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Bramley - Melksham"},"far_end":{"id":"S-melksham","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Melksham"},"status":"ASSUMED"},"id":"S-bramley.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-bramley.BAY03.T1"},{"busbar":"S-bramley.BB400","circuit":"C-bramley--west-weybridge--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Bramley - West Weybridge"},"far_end":{"id":"S-west-weybridge","label":{"source":"the other half of the circuit name","status":"TYPED","value":"West Weybridge"},"status":"ASSUMED"},"id":"S-bramley.BAY04","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-bramley.BAY04.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-bramley.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-bramley.BAY01.T1","from_site":"S-bramley","id":"C-bramley--didcot--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 9 feature(s)","status":"TYPED","value":48.103},"name":{"source":"name tag on the line features","status":"TYPED","value":"Bramley - Didcot Circuit No. 1"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.4431},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-didcot.BAY?.T1","to_site":"S-didcot","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":14.431}},{"from":"S-bramley.BAY02.T1","from_site":"S-bramley","id":"C-bramley--fleet--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 3 feature(s)","status":"TYPED","value":17.936},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Fleet-Bramley"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.5381},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-fleet.BAY?.T1","to_site":"S-fleet","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":5.3808}},{"from":"S-bramley.BAY03.T1","from_site":"S-bramley","id":"C-bramley--melksham--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 6 feature(s)","status":"TYPED","value":80.948},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Bramley-Melksham"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":2.4284},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-melksham.BAY?.T1","to_site":"S-melksham","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":24.2845}},{"from":"S-bramley.BAY04.T1","from_site":"S-bramley","id":"C-bramley--west-weybridge--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 6 feature(s)","status":"TYPED","value":49.724},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Bramley-West Weybridge"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.4917},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-west-weybridge.BAY?.T1","to_site":"S-west-weybridge","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":14.9173}}],"counts":{"bays":4,"busbars":1,"circuits":4,"conductors":4,"km_total":196.711,"neighbours":4},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":2,"id":"S-didcot","kv":[400],"label":"Didcot","lat":51.626679,"lon":-1.259541,"status":"ASSUMED","walk_to":"substation.py \"Didcot\""},{"bays":2,"id":"S-fleet","kv":[400],"label":"Fleet","lat":51.251101,"lon":-0.884454,"status":"ASSUMED","walk_to":"substation.py \"Fleet\""},{"bays":5,"id":"S-melksham","kv":[400],"label":"Melksham","lat":51.395294,"lon":-2.150761,"status":"ASSUMED","walk_to":"substation.py \"Melksham\""},{"bays":1,"id":"S-west-weybridge","kv":[400],"label":"West Weybridge","lat":51.354147,"lon":-0.479109,"status":"ASSUMED","walk_to":"substation.py \"West Weybridge\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"no exact normalised match in the register - this is a name off the map, not a registered site","status":"ASSUMED","value":null},"id":"S-bramley","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Bramley"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":51.336444},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-1.077429},"register":null,"status":"ASSUMED"},"transformers":[]},"S-canterbury-north":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-canterbury-north.BB400","circuit":"C-canterbury-north--cleve-hill--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Canterbury North - Cleve Hill"},"far_end":{"id":"S-cleve-hill","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Cleve Hill"},"status":"MATCHED"},"id":"S-canterbury-north.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-canterbury-north.BAY01.T1"},{"busbar":"S-canterbury-north.BB400","circuit":"C-canterbury-north--kemsley--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Canterbury North - Kemsley"},"far_end":{"id":"S-kemsley","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Kemsley"},"status":"MATCHED"},"id":"S-canterbury-north.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-canterbury-north.BAY02.T1"},{"busbar":"S-canterbury-north.BB400","circuit":"C-canterbury-north--sellindge--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Canterbury North - Sellindge"},"far_end":{"id":"S-sellindge","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Sellindge"},"status":"MATCHED"},"id":"S-canterbury-north.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-canterbury-north.BAY03.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-canterbury-north.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-canterbury-north.BAY01.T1","from_site":"S-canterbury-north","id":"C-canterbury-north--cleve-hill--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 3 feature(s)","status":"TYPED","value":0.397},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Cleve Hill-Canterbury North"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.0119},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-cleve-hill.BAY?.T1","to_site":"S-cleve-hill","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":0.1192}},{"from":"S-canterbury-north.BAY02.T1","from_site":"S-canterbury-north","id":"C-canterbury-north--kemsley--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 7 feature(s)","status":"TYPED","value":28.901},"name":{"source":"name tag on the line features","status":"TYPED","value":"Canterbury North-Kemsley"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.867},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-kemsley.BAY?.T1","to_site":"S-kemsley","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":8.6704}},{"from":"S-canterbury-north.BAY03.T1","from_site":"S-canterbury-north","id":"C-canterbury-north--sellindge--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":26.974},"name":{"source":"name tag on the line features","status":"TYPED","value":"Canterbury North-Sellindge"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.8092},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-sellindge.BAY?.T1","to_site":"S-sellindge","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":8.0921}}],"counts":{"bays":3,"busbars":1,"circuits":3,"conductors":3,"km_total":56.272,"neighbours":3},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":2,"id":"S-cleve-hill","kv":[400],"label":"Cleve Hill","lat":51.338198,"lon":0.94049,"status":"MATCHED","walk_to":"substation.py \"Cleve Hill\""},{"bays":4,"id":"S-kemsley","kv":[400],"label":"Kemsley","lat":51.360788,"lon":0.790835,"status":"MATCHED","walk_to":"substation.py \"Kemsley\""},{"bays":2,"id":"S-sellindge","kv":[400],"label":"Sellindge","lat":51.105708,"lon":0.972205,"status":"MATCHED","walk_to":"substation.py \"Sellindge\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"no exact normalised match in the register - this is a name off the map, not a registered site","status":"ASSUMED","value":null},"id":"S-canterbury-north","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Canterbury North"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":51.309337},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":1.046038},"register":null,"status":"ASSUMED"},"transformers":[]},"S-carrington":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-carrington.BB400","circuit":"C-carrington--daines--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Carrington - Daines"},"far_end":{"id":"S-daines","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Daines"},"status":"ASSUMED"},"id":"S-carrington.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-carrington.BAY01.T1"},{"busbar":"S-carrington.BB400","circuit":"C-carrington--kearsley--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Carrington - Kearsley"},"far_end":{"id":"S-kearsley","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Kearsley"},"status":"ASSUMED"},"id":"S-carrington.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-carrington.BAY02.T1"},{"busbar":"S-carrington.BB400","circuit":"C-carrington--south-manchester--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Carrington - South Manchester"},"far_end":{"id":"S-south-manchester","label":{"source":"the other half of the circuit name","status":"TYPED","value":"South Manchester"},"status":"ASSUMED"},"id":"S-carrington.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-carrington.BAY03.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-carrington.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-carrington.BAY01.T1","from_site":"S-carrington","id":"C-carrington--daines--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":0.494},"name":{"source":"name tag on the line features","status":"TYPED","value":"Daines to Carrington / South Manchester"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.0148},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-daines.BAY?.T1","to_site":"S-daines","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":0.1481}},{"from":"S-carrington.BAY02.T1","from_site":"S-carrington","id":"C-carrington--kearsley--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":4.321},"name":{"source":"name tag on the line features","status":"TYPED","value":"Penwortham to Danes / Kearsley to Carrington"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.1296},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-kearsley.BAY?.T1","to_site":"S-kearsley","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":1.2961}},{"from":"S-carrington.BAY03.T1","from_site":"S-carrington","id":"C-carrington--south-manchester--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":2.346},"name":{"source":"name tag on the line features","status":"TYPED","value":"Carrington to South Manchester"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.0704},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-south-manchester.BAY?.T1","to_site":"S-south-manchester","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":0.7038}}],"counts":{"bays":3,"busbars":1,"circuits":3,"conductors":3,"km_total":7.161,"neighbours":3},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":2,"id":"S-daines","kv":[400],"label":"Daines","lat":53.426061,"lon":-2.37788,"status":"ASSUMED","walk_to":"substation.py \"Daines\""},{"bays":2,"id":"S-kearsley","kv":[400],"label":"Kearsley","lat":53.436898,"lon":-2.406618,"status":"ASSUMED","walk_to":"substation.py \"Kearsley\""},{"bays":1,"id":"S-south-manchester","kv":[400],"label":"South Manchester","lat":53.435408,"lon":-2.373216,"status":"ASSUMED","walk_to":"substation.py \"South Manchester\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"exact normalised match to the NESO connection-site register (R10)","status":"TYPED","value":"Carrington 275kV Substation"},"id":"S-carrington","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Carrington"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":53.433249},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-2.395335},"register":{"neso_id":"357","placeable":false,"register":"TEC","register_name":"Carrington 275kV Substation","status":"Consents Approved"},"status":"MATCHED"},"transformers":[]},"S-chickerell":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-chickerell.BB400","circuit":"C-axminster--chickerell--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Axminster - Chickerell"},"far_end":{"id":"S-axminster","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Axminster"},"status":"ASSUMED"},"id":"S-chickerell.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-chickerell.BAY01.T1"},{"busbar":"S-chickerell.BB400","circuit":"C-chickerell--exeter--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Chickerell - Exeter"},"far_end":{"id":"S-exeter","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Exeter"},"status":"MATCHED"},"id":"S-chickerell.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-chickerell.BAY02.T1"},{"busbar":"S-chickerell.BB400","circuit":"C-chickerell--mannington--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Chickerell - Mannington"},"far_end":{"id":"S-mannington","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Mannington"},"status":"ASSUMED"},"id":"S-chickerell.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-chickerell.BAY03.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-chickerell.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-chickerell.BAY01.T1","from_site":"S-chickerell","id":"C-axminster--chickerell--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 12 feature(s)","status":"TYPED","value":40.561},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Chickerell-Axminster"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.2168},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-axminster.BAY?.T1","to_site":"S-axminster","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":12.1684}},{"from":"S-chickerell.BAY02.T1","from_site":"S-chickerell","id":"C-chickerell--exeter--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":18.047},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Axminster- Exeter (Circuit 1);Chickerell - Exeter (Circuit 2)"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.5414},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-exeter.BAY?.T1","to_site":"S-exeter","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":5.4142}},{"from":"S-chickerell.BAY03.T1","from_site":"S-chickerell","id":"C-chickerell--mannington--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 4 feature(s)","status":"TYPED","value":54.002},"name":{"source":"name tag on the line features","status":"TYPED","value":"Mannington-Chickerell"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.6201},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-mannington.BAY?.T1","to_site":"S-mannington","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":16.2007}}],"counts":{"bays":3,"busbars":1,"circuits":3,"conductors":3,"km_total":112.61,"neighbours":3},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":2,"id":"S-axminster","kv":[400],"label":"Axminster","lat":50.786831,"lon":-2.924451,"status":"ASSUMED","walk_to":"substation.py \"Axminster\""},{"bays":6,"id":"S-exeter","kv":[400],"label":"Exeter","lat":50.767877,"lon":-3.404542,"status":"MATCHED","walk_to":"substation.py \"Exeter\""},{"bays":2,"id":"S-mannington","kv":[400],"label":"Mannington","lat":50.847454,"lon":-1.895839,"status":"ASSUMED","walk_to":"substation.py \"Mannington\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"no exact normalised match in the register - this is a name off the map, not a registered site","status":"ASSUMED","value":null},"id":"S-chickerell","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Chickerell"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":50.625021},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-2.486765},"register":null,"status":"ASSUMED"},"transformers":[]},"S-cilfynydd":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-cilfynydd.BB400","circuit":"C-cilfynydd--imperial-park--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Cilfynydd - Imperial Park"},"far_end":{"id":"S-imperial-park","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Imperial Park"},"status":"MATCHED"},"id":"S-cilfynydd.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-cilfynydd.BAY01.T1"},{"busbar":"S-cilfynydd.BB400","circuit":"C-cilfynydd--melksham--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Cilfynydd - Melksham"},"far_end":{"id":"S-melksham","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Melksham"},"status":"ASSUMED"},"id":"S-cilfynydd.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-cilfynydd.BAY02.T1"},{"busbar":"S-cilfynydd.BB400","circuit":"C-cilfynydd--swansea-north--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Cilfynydd - Swansea North"},"far_end":{"id":"S-swansea-north","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Swansea North"},"status":"MATCHED"},"id":"S-cilfynydd.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-cilfynydd.BAY03.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-cilfynydd.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-cilfynydd.BAY01.T1","from_site":"S-cilfynydd","id":"C-cilfynydd--imperial-park--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":25.115},"name":{"source":"name tag on the line features","status":"TYPED","value":"Imperial Park - Cilfynydd"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.7534},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-imperial-park.BAY?.T1","to_site":"S-imperial-park","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":7.5345}},{"from":"S-cilfynydd.BAY02.T1","from_site":"S-cilfynydd","id":"C-cilfynydd--melksham--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":50.091},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Melksham-Cilfynydd"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.5027},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-melksham.BAY?.T1","to_site":"S-melksham","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":15.0273}},{"from":"S-cilfynydd.BAY03.T1","from_site":"S-cilfynydd","id":"C-cilfynydd--swansea-north--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 3 feature(s)","status":"TYPED","value":61.19},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Cilfynydd-Swansea North"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.8357},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-swansea-north.BAY?.T1","to_site":"S-swansea-north","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":18.3571}}],"counts":{"bays":3,"busbars":1,"circuits":3,"conductors":3,"km_total":136.396,"neighbours":3},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":1,"id":"S-imperial-park","kv":[400],"label":"Imperial Park","lat":51.547823,"lon":-3.031015,"status":"MATCHED","walk_to":"substation.py \"Imperial Park\""},{"bays":5,"id":"S-melksham","kv":[400],"label":"Melksham","lat":51.395294,"lon":-2.150761,"status":"ASSUMED","walk_to":"substation.py \"Melksham\""},{"bays":2,"id":"S-swansea-north","kv":[400],"label":"Swansea North","lat":51.688628,"lon":-3.957389,"status":"MATCHED","walk_to":"substation.py \"Swansea North\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"exact normalised match to the NESO connection-site register (R10)","status":"TYPED","value":"Cilfynydd 400kV Substation"},"id":"S-cilfynydd","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Cilfynydd"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":51.633523},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-3.301687},"register":{"neso_id":"9468","placeable":false,"register":"TEC","register_name":"Cilfynydd 400kV Substation","status":"Scoping"},"status":"MATCHED"},"transformers":[]},"S-cottam":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-cottam.BB400","circuit":"C-cottam--eaton-socon--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Cottam - Eaton Socon"},"far_end":{"id":"S-eaton-socon","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Eaton Socon"},"status":"ASSUMED"},"id":"S-cottam.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-cottam.BAY01.T1"},{"busbar":"S-cottam.BB400","circuit":"C-cottam--high-marnham--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Cottam - High Marnham"},"far_end":{"id":"S-high-marnham","label":{"source":"the other half of the circuit name","status":"TYPED","value":"High Marnham"},"status":"MATCHED"},"id":"S-cottam.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-cottam.BAY02.T1"},{"busbar":"S-cottam.BB400","circuit":"C-cottam--keadby--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Cottam - Keadby"},"far_end":{"id":"S-keadby","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Keadby"},"status":"MATCHED"},"id":"S-cottam.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-cottam.BAY03.T1"},{"busbar":"S-cottam.BB400","circuit":"C-cottam--wymondley--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Cottam - Wymondley"},"far_end":{"id":"S-wymondley","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Wymondley"},"status":"ASSUMED"},"id":"S-cottam.BAY04","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-cottam.BAY04.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-cottam.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-cottam.BAY01.T1","from_site":"S-cottam","id":"C-cottam--eaton-socon--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 3 feature(s)","status":"TYPED","value":84.794},"name":{"source":"name tag on the line features","status":"TYPED","value":"Cottam - Eaton Socon - Wymondley"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":2.5438},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-eaton-socon.BAY?.T1","to_site":"S-eaton-socon","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":25.4383}},{"from":"S-cottam.BAY02.T1","from_site":"S-cottam","id":"C-cottam--high-marnham--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":0.505},"name":{"source":"name tag on the line features","status":"TYPED","value":"High Marnham-Cottam"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.0152},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-high-marnham.BAY?.T1","to_site":"S-high-marnham","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":0.1516}},{"from":"S-cottam.BAY03.T1","from_site":"S-cottam","id":"C-cottam--keadby--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":13.249},"name":{"source":"name tag on the line features","status":"TYPED","value":"Cottam - Keadby"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.3975},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-keadby.BAY?.T1","to_site":"S-keadby","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":3.9747}},{"from":"S-cottam.BAY04.T1","from_site":"S-cottam","id":"C-cottam--wymondley--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":0.381},"name":{"source":"name tag on the line features","status":"TYPED","value":"Wymondley - Cottam Power Station, Notts"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.0114},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-wymondley.BAY?.T1","to_site":"S-wymondley","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":0.1144}}],"counts":{"bays":4,"busbars":1,"circuits":4,"conductors":4,"km_total":98.929,"neighbours":4},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":2,"id":"S-eaton-socon","kv":[400],"label":"Eaton Socon","lat":53.300213,"lon":-0.77856,"status":"ASSUMED","walk_to":"substation.py \"Eaton Socon\""},{"bays":2,"id":"S-high-marnham","kv":[400],"label":"High Marnham","lat":53.240038,"lon":-0.796262,"status":"MATCHED","walk_to":"substation.py \"High Marnham\""},{"bays":1,"id":"S-keadby","kv":[400],"label":"Keadby","lat":53.415098,"lon":-0.818876,"status":"MATCHED","walk_to":"substation.py \"Keadby\""},{"bays":4,"id":"S-wymondley","kv":[400],"label":"Wymondley","lat":51.96587,"lon":-0.243667,"status":"ASSUMED","walk_to":"substation.py \"Wymondley\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"exact normalised match to the NESO connection-site register (R10)","status":"TYPED","value":"Cottam 400kV Substation"},"id":"S-cottam","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Cottam"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":53.280435},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-0.786124},"register":{"neso_id":"7066","placeable":true,"register":"TEC","register_name":"Cottam 400kV Substation","status":"Consents Approved"},"status":"MATCHED"},"transformers":[]},"S-cowley":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-cowley.BB400","circuit":"C-cowley--didcot--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Cowley - Didcot"},"far_end":{"id":"S-didcot","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Didcot"},"status":"ASSUMED"},"id":"S-cowley.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-cowley.BAY01.T1"},{"busbar":"S-cowley.BB400","circuit":"C-cowley--east-claydon--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Cowley - East Claydon"},"far_end":{"id":"S-east-claydon","label":{"source":"the other half of the circuit name","status":"TYPED","value":"East Claydon"},"status":"ASSUMED"},"id":"S-cowley.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-cowley.BAY02.T1"},{"busbar":"S-cowley.BB400","circuit":"C-cowley--seven-springs--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Cowley - Seven Springs"},"far_end":{"id":"S-seven-springs","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Seven Springs"},"status":"ASSUMED"},"id":"S-cowley.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-cowley.BAY03.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-cowley.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-cowley.BAY01.T1","from_site":"S-cowley","id":"C-cowley--didcot--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":11.392},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Didcot-Cowley"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.3417},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-didcot.BAY?.T1","to_site":"S-didcot","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":3.4175}},{"from":"S-cowley.BAY02.T1","from_site":"S-cowley","id":"C-cowley--east-claydon--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 3 feature(s)","status":"TYPED","value":36.252},"name":{"source":"name tag on the line features","status":"TYPED","value":"East Claydon-Cowley"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.0876},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-east-claydon.BAY?.T1","to_site":"S-east-claydon","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":10.8757}},{"from":"S-cowley.BAY03.T1","from_site":"S-cowley","id":"C-cowley--seven-springs--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 3 feature(s)","status":"TYPED","value":72.223},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Cowley-Seven Springs"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":2.1667},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-seven-springs.BAY?.T1","to_site":"S-seven-springs","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":21.667}}],"counts":{"bays":3,"busbars":1,"circuits":3,"conductors":3,"km_total":119.867,"neighbours":3},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":2,"id":"S-didcot","kv":[400],"label":"Didcot","lat":51.626679,"lon":-1.259541,"status":"ASSUMED","walk_to":"substation.py \"Didcot\""},{"bays":4,"id":"S-east-claydon","kv":[400],"label":"East Claydon","lat":51.925667,"lon":-0.906669,"status":"ASSUMED","walk_to":"substation.py \"East Claydon\""},{"bays":4,"id":"S-seven-springs","kv":[400],"label":"Seven Springs","lat":51.8495,"lon":-2.028484,"status":"ASSUMED","walk_to":"substation.py \"Seven Springs\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"no exact normalised match in the register - this is a name off the map, not a registered site","status":"ASSUMED","value":null},"id":"S-cowley","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Cowley"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":51.712491},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-1.190827},"register":null,"status":"ASSUMED"},"transformers":[]},"S-drakelow":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-drakelow.BB400","circuit":"C-bustleholm--drakelow--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Bustleholm - Drakelow"},"far_end":{"id":"S-bustleholm","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Bustleholm"},"status":"MATCHED"},"id":"S-drakelow.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-drakelow.BAY01.T1"},{"busbar":"S-drakelow.BB400","circuit":"C-cellarhead--drakelow--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Cellarhead - Drakelow"},"far_end":{"id":"S-cellarhead","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Cellarhead"},"status":"ASSUMED"},"id":"S-drakelow.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-drakelow.BAY02.T1"},{"busbar":"S-drakelow.BB400","circuit":"C-drakelow--hams-hall--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Drakelow - Hams Hall"},"far_end":{"id":"S-hams-hall","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Hams Hall"},"status":"ASSUMED"},"id":"S-drakelow.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-drakelow.BAY03.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-drakelow.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-drakelow.BAY01.T1","from_site":"S-drakelow","id":"C-bustleholm--drakelow--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 4 feature(s)","status":"TYPED","value":34.578},"name":{"source":"name tag on the line features","status":"TYPED","value":"Drakelow-Bustleholm"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.0373},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-bustleholm.BAY?.T1","to_site":"S-bustleholm","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":10.3735}},{"from":"S-drakelow.BAY02.T1","from_site":"S-drakelow","id":"C-cellarhead--drakelow--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 4 feature(s)","status":"TYPED","value":45.366},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Cellarhead-Drakelow"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.361},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-cellarhead.BAY?.T1","to_site":"S-cellarhead","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":13.6099}},{"from":"S-drakelow.BAY03.T1","from_site":"S-drakelow","id":"C-drakelow--hams-hall--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 3 feature(s)","status":"TYPED","value":29.412},"name":{"source":"name tag on the line features","status":"TYPED","value":"Hams Hall-Drakelow"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.8824},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-hams-hall.BAY?.T1","to_site":"S-hams-hall","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":8.8236}}],"counts":{"bays":3,"busbars":1,"circuits":3,"conductors":3,"km_total":109.356,"neighbours":3},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":2,"id":"S-bustleholm","kv":[400],"label":"Bustleholm","lat":52.55227,"lon":-1.95935,"status":"MATCHED","walk_to":"substation.py \"Bustleholm\""},{"bays":2,"id":"S-cellarhead","kv":[400],"label":"Cellarhead","lat":53.040916,"lon":-2.081563,"status":"ASSUMED","walk_to":"substation.py \"Cellarhead\""},{"bays":3,"id":"S-hams-hall","kv":[400],"label":"Hams Hall","lat":52.532237,"lon":-1.714148,"status":"ASSUMED","walk_to":"substation.py \"Hams Hall\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"exact normalised match to the NESO connection-site register (R10)","status":"TYPED","value":"Drakelow 132kV Substation"},"id":"S-drakelow","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Drakelow"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":52.772292},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-1.653001},"register":{"neso_id":"1925","placeable":true,"register":"TEC","register_name":"Drakelow 132kV Substation","status":"Scoping"},"status":"MATCHED"},"transformers":[]},"S-drax":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-drax.BB400","circuit":"C-drax--eggborough--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Drax - Eggborough"},"far_end":{"id":"S-eggborough","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Eggborough"},"status":"ASSUMED"},"id":"S-drax.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-drax.BAY01.T1"},{"busbar":"S-drax.BB400","circuit":"C-drax--thornton--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Drax - Thornton"},"far_end":{"id":"S-thornton","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Thornton"},"status":"MATCHED"},"id":"S-drax.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-drax.BAY02.T1"},{"busbar":"S-drax.BB400","circuit":"C-drax--thorpe-marsh--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Drax - Thorpe Marsh"},"far_end":{"id":"S-thorpe-marsh","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Thorpe Marsh"},"status":"ASSUMED"},"id":"S-drax.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-drax.BAY03.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-drax.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-drax.BAY01.T1","from_site":"S-drax","id":"C-drax--eggborough--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":11.169},"name":{"source":"name tag on the line features","status":"TYPED","value":"Drax to Eggborough Line"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.3351},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-eggborough.BAY?.T1","to_site":"S-eggborough","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":3.3507}},{"from":"S-drax.BAY02.T1","from_site":"S-drax","id":"C-drax--thornton--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":22.658},"name":{"source":"name tag on the line features","status":"TYPED","value":"Drax to Thornton Line"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.6797},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-thornton.BAY?.T1","to_site":"S-thornton","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":6.7973}},{"from":"S-drax.BAY03.T1","from_site":"S-drax","id":"C-drax--thorpe-marsh--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":13.432},"name":{"source":"name tag on the line features","status":"TYPED","value":"Drax to Thorpe Marsh Line"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.403},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-thorpe-marsh.BAY?.T1","to_site":"S-thorpe-marsh","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":4.0295}}],"counts":{"bays":3,"busbars":1,"circuits":3,"conductors":3,"km_total":47.259,"neighbours":3},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":1,"id":"S-eggborough","kv":[400],"label":"Eggborough","lat":53.715098,"lon":-1.130246,"status":"ASSUMED","walk_to":"substation.py \"Eggborough\""},{"bays":1,"id":"S-thornton","kv":[400],"label":"Thornton","lat":53.900344,"lon":-0.822132,"status":"MATCHED","walk_to":"substation.py \"Thornton\""},{"bays":1,"id":"S-thorpe-marsh","kv":[400],"label":"Thorpe Marsh","lat":53.636587,"lon":-1.06925,"status":"ASSUMED","walk_to":"substation.py \"Thorpe Marsh\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"exact normalised match to the NESO connection-site register (R10)","status":"TYPED","value":"Drax 132kV Substation"},"id":"S-drax","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Drax"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":53.736019},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-0.990522},"register":{"neso_id":"7017","placeable":true,"register":"TEC","register_name":"Drax 132kV Substation","status":"Scoping"},"status":"MATCHED"},"transformers":[]},"S-east-claydon":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-east-claydon.BB400","circuit":"C-cowley--east-claydon--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Cowley - East Claydon"},"far_end":{"id":"S-cowley","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Cowley"},"status":"ASSUMED"},"id":"S-east-claydon.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-east-claydon.BAY01.T1"},{"busbar":"S-east-claydon.BB400","circuit":"C-east-claydon--enderby--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"East Claydon - Enderby"},"far_end":{"id":"S-enderby","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Enderby"},"status":"MATCHED"},"id":"S-east-claydon.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-east-claydon.BAY02.T1"},{"busbar":"S-east-claydon.BB400","circuit":"C-east-claydon--mop-end--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"East Claydon - Mop End"},"far_end":{"id":"S-mop-end","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Mop End"},"status":"ASSUMED"},"id":"S-east-claydon.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-east-claydon.BAY03.T1"},{"busbar":"S-east-claydon.BB400","circuit":"C-east-claydon--sundon--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"East Claydon - Sundon"},"far_end":{"id":"S-sundon","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Sundon"},"status":"MATCHED"},"id":"S-east-claydon.BAY04","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-east-claydon.BAY04.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-east-claydon.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-east-claydon.BAY01.T1","from_site":"S-east-claydon","id":"C-cowley--east-claydon--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 3 feature(s)","status":"TYPED","value":36.252},"name":{"source":"name tag on the line features","status":"TYPED","value":"East Claydon-Cowley"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.0876},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-cowley.BAY?.T1","to_site":"S-cowley","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":10.8757}},{"from":"S-east-claydon.BAY02.T1","from_site":"S-east-claydon","id":"C-east-claydon--enderby--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":85.763},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid East Claydon-Enderby"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":2.5729},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-enderby.BAY?.T1","to_site":"S-enderby","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":25.729}},{"from":"S-east-claydon.BAY03.T1","from_site":"S-east-claydon","id":"C-east-claydon--mop-end--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":35.316},"name":{"source":"name tag on the line features","status":"TYPED","value":"East Claydon - Mop End"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.0595},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-mop-end.BAY?.T1","to_site":"S-mop-end","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":10.5949}},{"from":"S-east-claydon.BAY04.T1","from_site":"S-east-claydon","id":"C-east-claydon--sundon--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 4 feature(s)","status":"TYPED","value":29.216},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Sundon-East Claydon"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.8765},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-sundon.BAY?.T1","to_site":"S-sundon","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":8.7649}}],"counts":{"bays":4,"busbars":1,"circuits":4,"conductors":4,"km_total":186.547,"neighbours":4},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":3,"id":"S-cowley","kv":[400],"label":"Cowley","lat":51.712491,"lon":-1.190827,"status":"ASSUMED","walk_to":"substation.py \"Cowley\""},{"bays":3,"id":"S-enderby","kv":[400],"label":"Enderby","lat":52.862214,"lon":-1.256813,"status":"MATCHED","walk_to":"substation.py \"Enderby\""},{"bays":2,"id":"S-mop-end","kv":[400],"label":"Mop End","lat":51.665542,"lon":-0.656221,"status":"ASSUMED","walk_to":"substation.py \"Mop End\""},{"bays":4,"id":"S-sundon","kv":[400],"label":"Sundon","lat":51.935297,"lon":-0.503612,"status":"MATCHED","walk_to":"substation.py \"Sundon\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"no exact normalised match in the register - this is a name off the map, not a registered site","status":"ASSUMED","value":null},"id":"S-east-claydon","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"East Claydon"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":51.925667},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-0.906669},"register":null,"status":"ASSUMED"},"transformers":[]},"S-enderby":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-enderby.BB400","circuit":"C-east-claydon--enderby--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"East Claydon - Enderby"},"far_end":{"id":"S-east-claydon","label":{"source":"the other half of the circuit name","status":"TYPED","value":"East Claydon"},"status":"ASSUMED"},"id":"S-enderby.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-enderby.BAY01.T1"},{"busbar":"S-enderby.BB400","circuit":"C-enderby--ratcliffe--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Enderby - Ratcliffe"},"far_end":{"id":"S-ratcliffe","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Ratcliffe"},"status":"ASSUMED"},"id":"S-enderby.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-enderby.BAY02.T1"},{"busbar":"S-enderby.BB400","circuit":"C-enderby--soar--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Enderby - Soar"},"far_end":{"id":"S-soar","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Soar"},"status":"ASSUMED"},"id":"S-enderby.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-enderby.BAY03.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-enderby.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-enderby.BAY01.T1","from_site":"S-enderby","id":"C-east-claydon--enderby--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":85.763},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid East Claydon-Enderby"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":2.5729},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-east-claydon.BAY?.T1","to_site":"S-east-claydon","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":25.729}},{"from":"S-enderby.BAY02.T1","from_site":"S-enderby","id":"C-enderby--ratcliffe--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":36.846},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Ratcliffe-Enderby"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.1054},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-ratcliffe.BAY?.T1","to_site":"S-ratcliffe","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":11.0539}},{"from":"S-enderby.BAY03.T1","from_site":"S-enderby","id":"C-enderby--soar--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 4 feature(s)","status":"TYPED","value":0.024},"name":{"source":"name tag on the line features","status":"TYPED","value":"Ratcliffe-on-Soar - Enderby 1"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.0007},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-soar.BAY?.T1","to_site":"S-soar","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":0.007}}],"counts":{"bays":3,"busbars":1,"circuits":3,"conductors":3,"km_total":122.633,"neighbours":3},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":4,"id":"S-east-claydon","kv":[400],"label":"East Claydon","lat":51.925667,"lon":-0.906669,"status":"ASSUMED","walk_to":"substation.py \"East Claydon\""},{"bays":4,"id":"S-ratcliffe","kv":[400],"label":"Ratcliffe","lat":52.860768,"lon":-1.329285,"status":"ASSUMED","walk_to":"substation.py \"Ratcliffe\""},{"bays":3,"id":"S-soar","kv":[400],"label":"Soar","lat":52.857567,"lon":-1.451303,"status":"ASSUMED","walk_to":"substation.py \"Soar\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"exact normalised match to the NESO connection-site register (R10)","status":"TYPED","value":"Enderby 400kV Substation"},"id":"S-enderby","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Enderby"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":52.862214},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-1.256813},"register":{"neso_id":"539","placeable":true,"register":"TEC","register_name":"Enderby 400kV Substation","status":"Built"},"status":"MATCHED"},"transformers":[]},"S-exeter":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-exeter.BB400","circuit":"C-abham--exeter--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Abham - Exeter"},"far_end":{"id":"S-abham","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Abham"},"status":"ASSUMED"},"id":"S-exeter.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-exeter.BAY01.T1"},{"busbar":"S-exeter.BB400","circuit":"C-axminster--exeter--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Axminster - Exeter"},"far_end":{"id":"S-axminster","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Axminster"},"status":"ASSUMED"},"id":"S-exeter.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-exeter.BAY02.T1"},{"busbar":"S-exeter.BB400","circuit":"C-chickerell--exeter--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Chickerell - Exeter"},"far_end":{"id":"S-chickerell","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Chickerell"},"status":"ASSUMED"},"id":"S-exeter.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-exeter.BAY03.T1"},{"busbar":"S-exeter.BB400","circuit":"C-exeter--langage--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Exeter - Langage"},"far_end":{"id":"S-langage","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Langage"},"status":"MATCHED"},"id":"S-exeter.BAY04","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-exeter.BAY04.T1"},{"busbar":"S-exeter.BB400","circuit":"C-exeter--taunton--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Exeter - Taunton 1"},"far_end":{"id":"S-taunton","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Taunton"},"status":"MATCHED"},"id":"S-exeter.BAY05","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-exeter.BAY05.T1"},{"busbar":"S-exeter.BB400","circuit":"C-exeter--taunton--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Exeter - Taunton 2"},"far_end":{"id":"S-taunton","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Taunton"},"status":"MATCHED"},"id":"S-exeter.BAY06","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-exeter.BAY06.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-exeter.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-exeter.BAY01.T1","from_site":"S-exeter","id":"C-abham--exeter--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":24.46},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Abham - Exeter - Langage"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.7338},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-abham.BAY?.T1","to_site":"S-abham","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":7.3381}},{"from":"S-exeter.BAY02.T1","from_site":"S-exeter","id":"C-axminster--exeter--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":18.199},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Axminster- Exeter (Circuit 1);Chickerell - Exeter (Circuit 2)"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.546},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-axminster.BAY?.T1","to_site":"S-axminster","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":5.4596}},{"from":"S-exeter.BAY03.T1","from_site":"S-exeter","id":"C-chickerell--exeter--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":18.047},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Axminster- Exeter (Circuit 1);Chickerell - Exeter (Circuit 2)"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.5414},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-chickerell.BAY?.T1","to_site":"S-chickerell","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":5.4142}},{"from":"S-exeter.BAY04.T1","from_site":"S-exeter","id":"C-exeter--langage--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":24.46},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Abham - Exeter - Langage"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.7338},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-langage.BAY?.T1","to_site":"S-langage","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":7.3381}},{"from":"S-exeter.BAY05.T1","from_site":"S-exeter","id":"C-exeter--taunton--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features / 2 (mean of the parallel circuits); 5 feature(s)","status":"TYPED","value":19.361},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Exeter-Taunton"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.5808},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-taunton.BAY?.T1","to_site":"S-taunton","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":5.8083}},{"from":"S-exeter.BAY06.T1","from_site":"S-exeter","id":"C-exeter--taunton--400#2","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features / 2 (mean of the parallel circuits); 5 feature(s)","status":"TYPED","value":19.361},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Exeter-Taunton"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.5808},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-taunton.BAY?.T1","to_site":"S-taunton","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":5.8083}}],"counts":{"bays":6,"busbars":1,"circuits":5,"conductors":6,"km_total":123.888,"neighbours":5},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":2,"id":"S-abham","kv":[400],"label":"Abham","lat":50.47107,"lon":-3.730746,"status":"ASSUMED","walk_to":"substation.py \"Abham\""},{"bays":2,"id":"S-axminster","kv":[400],"label":"Axminster","lat":50.786831,"lon":-2.924451,"status":"ASSUMED","walk_to":"substation.py \"Axminster\""},{"bays":3,"id":"S-chickerell","kv":[400],"label":"Chickerell","lat":50.625021,"lon":-2.486765,"status":"ASSUMED","walk_to":"substation.py \"Chickerell\""},{"bays":1,"id":"S-langage","kv":[400],"label":"Langage","lat":50.471679,"lon":-3.730355,"status":"MATCHED","walk_to":"substation.py \"Langage\""},{"bays":5,"id":"S-taunton","kv":[400],"label":"Taunton","lat":51.063392,"lon":-3.148434,"status":"MATCHED","walk_to":"substation.py \"Taunton\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"exact normalised match to the NESO connection-site register (R10)","status":"TYPED","value":"Exeter 400kV Substation"},"id":"S-exeter","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Exeter"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":50.767877},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-3.404542},"register":{"neso_id":"939","placeable":true,"register":"TEC","register_name":"Exeter 400kV Substation","status":"Awaiting Consents"},"status":"MATCHED"},"transformers":[]},"S-hams-hall":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-hams-hall.BB400","circuit":"C-coventry--hams-hall--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Coventry - Hams Hall"},"far_end":{"id":"S-coventry","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Coventry"},"status":"MATCHED"},"id":"S-hams-hall.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-hams-hall.BAY01.T1"},{"busbar":"S-hams-hall.BB400","circuit":"C-drakelow--hams-hall--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Drakelow - Hams Hall"},"far_end":{"id":"S-drakelow","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Drakelow"},"status":"MATCHED"},"id":"S-hams-hall.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-hams-hall.BAY02.T1"},{"busbar":"S-hams-hall.BB400","circuit":"C-feckenham--hams-hall--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Feckenham - Hams Hall"},"far_end":{"id":"S-feckenham","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Feckenham"},"status":"ASSUMED"},"id":"S-hams-hall.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-hams-hall.BAY03.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-hams-hall.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-hams-hall.BAY01.T1","from_site":"S-hams-hall","id":"C-coventry--hams-hall--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":19.592},"name":{"source":"name tag on the line features","status":"TYPED","value":"Hams Hall-Coventry"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.5878},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-coventry.BAY?.T1","to_site":"S-coventry","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":5.8775}},{"from":"S-hams-hall.BAY02.T1","from_site":"S-hams-hall","id":"C-drakelow--hams-hall--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 3 feature(s)","status":"TYPED","value":29.412},"name":{"source":"name tag on the line features","status":"TYPED","value":"Hams Hall-Drakelow"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.8824},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-drakelow.BAY?.T1","to_site":"S-drakelow","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":8.8236}},{"from":"S-hams-hall.BAY03.T1","from_site":"S-hams-hall","id":"C-feckenham--hams-hall--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":9.351},"name":{"source":"name tag on the line features","status":"TYPED","value":"Feckenham-Hams Hall"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.2805},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-feckenham.BAY?.T1","to_site":"S-feckenham","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":2.8052}}],"counts":{"bays":3,"busbars":1,"circuits":3,"conductors":3,"km_total":58.355,"neighbours":3},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":2,"id":"S-coventry","kv":[400],"label":"Coventry","lat":52.457793,"lon":-1.465279,"status":"MATCHED","walk_to":"substation.py \"Coventry\""},{"bays":3,"id":"S-drakelow","kv":[400],"label":"Drakelow","lat":52.772292,"lon":-1.653001,"status":"MATCHED","walk_to":"substation.py \"Drakelow\""},{"bays":2,"id":"S-feckenham","kv":[400],"label":"Feckenham","lat":52.242171,"lon":-1.932057,"status":"ASSUMED","walk_to":"substation.py \"Feckenham\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"no exact normalised match in the register - this is a name off the map, not a registered site","status":"ASSUMED","value":null},"id":"S-hams-hall","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Hams Hall"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":52.532237},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-1.714148},"register":null,"status":"ASSUMED"},"transformers":[]},"S-hinkley-point":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-hinkley-point.BB400","circuit":"C-hinkley-point--melksham--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Hinkley Point - Melksham"},"far_end":{"id":"S-melksham","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Melksham"},"status":"ASSUMED"},"id":"S-hinkley-point.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-hinkley-point.BAY01.T1"},{"busbar":"S-hinkley-point.BB400","circuit":"C-hinkley-point--sandford--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Hinkley Point - Sandford"},"far_end":{"id":"S-sandford","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Sandford"},"status":"ASSUMED"},"id":"S-hinkley-point.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-hinkley-point.BAY02.T1"},{"busbar":"S-hinkley-point.BB400","circuit":"C-hinkley-point--shurton--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Hinkley Point - Shurton"},"far_end":{"id":"S-shurton","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Shurton"},"status":"ASSUMED"},"id":"S-hinkley-point.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-hinkley-point.BAY03.T1"},{"busbar":"S-hinkley-point.BB400","circuit":"C-hinkley-point--taunton--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Hinkley Point - Taunton"},"far_end":{"id":"S-taunton","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Taunton"},"status":"MATCHED"},"id":"S-hinkley-point.BAY04","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-hinkley-point.BAY04.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-hinkley-point.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-hinkley-point.BAY01.T1","from_site":"S-hinkley-point","id":"C-hinkley-point--melksham--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":0.891},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Melksham-Hinkley Point"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.0267},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-melksham.BAY?.T1","to_site":"S-melksham","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":0.2674}},{"from":"S-hinkley-point.BAY02.T1","from_site":"S-hinkley-point","id":"C-hinkley-point--sandford--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":25.198},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Sandford-Hinkley Point"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.7559},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-sandford.BAY?.T1","to_site":"S-sandford","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":7.5593}},{"from":"S-hinkley-point.BAY03.T1","from_site":"S-hinkley-point","id":"C-hinkley-point--shurton--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":1.102},"name":{"source":"name tag on the line features","status":"TYPED","value":"Hinkley Point - Shurton"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.0331},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-shurton.BAY?.T1","to_site":"S-shurton","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":0.3307}},{"from":"S-hinkley-point.BAY04.T1","from_site":"S-hinkley-point","id":"C-hinkley-point--taunton--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":0.505},"name":{"source":"name tag on the line features","status":"TYPED","value":"Taunton - Hinkley Point"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.0152},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-taunton.BAY?.T1","to_site":"S-taunton","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":0.1516}}],"counts":{"bays":4,"busbars":1,"circuits":4,"conductors":4,"km_total":27.696,"neighbours":4},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":5,"id":"S-melksham","kv":[400],"label":"Melksham","lat":51.395294,"lon":-2.150761,"status":"ASSUMED","walk_to":"substation.py \"Melksham\""},{"bays":2,"id":"S-sandford","kv":[400],"label":"Sandford","lat":51.313541,"lon":-2.870022,"status":"ASSUMED","walk_to":"substation.py \"Sandford\""},{"bays":3,"id":"S-shurton","kv":[400],"label":"Shurton","lat":51.203085,"lon":-3.137775,"status":"ASSUMED","walk_to":"substation.py \"Shurton\""},{"bays":5,"id":"S-taunton","kv":[400],"label":"Taunton","lat":51.063392,"lon":-3.148434,"status":"MATCHED","walk_to":"substation.py \"Taunton\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"no exact normalised match in the register - this is a name off the map, not a registered site","status":"ASSUMED","value":null},"id":"S-hinkley-point","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Hinkley Point"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":51.205812},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-3.127443},"register":null,"status":"ASSUMED"},"transformers":[]},"S-kemsley":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-kemsley.BB400","circuit":"C-canterbury-north--kemsley--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Canterbury North - Kemsley"},"far_end":{"id":"S-canterbury-north","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Canterbury North"},"status":"ASSUMED"},"id":"S-kemsley.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-kemsley.BAY01.T1"},{"busbar":"S-kemsley.BB400","circuit":"C-cleve-hill--kemsley--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Cleve Hill - Kemsley"},"far_end":{"id":"S-cleve-hill","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Cleve Hill"},"status":"MATCHED"},"id":"S-kemsley.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-kemsley.BAY02.T1"},{"busbar":"S-kemsley.BB400","circuit":"C-grain--kemsley--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Grain - Kemsley"},"far_end":{"id":"S-grain","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Grain"},"status":"MATCHED"},"id":"S-kemsley.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-kemsley.BAY03.T1"},{"busbar":"S-kemsley.BB400","circuit":"C-kemsley--northfleet-west--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Kemsley - Northfleet West"},"far_end":{"id":"S-northfleet-west","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Northfleet West"},"status":"ASSUMED"},"id":"S-kemsley.BAY04","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-kemsley.BAY04.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-kemsley.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-kemsley.BAY01.T1","from_site":"S-kemsley","id":"C-canterbury-north--kemsley--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 7 feature(s)","status":"TYPED","value":28.901},"name":{"source":"name tag on the line features","status":"TYPED","value":"Canterbury North-Kemsley"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.867},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-canterbury-north.BAY?.T1","to_site":"S-canterbury-north","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":8.6704}},{"from":"S-kemsley.BAY02.T1","from_site":"S-kemsley","id":"C-cleve-hill--kemsley--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 3 feature(s)","status":"TYPED","value":0.439},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Cleve Hill-Kemsley"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.0132},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-cleve-hill.BAY?.T1","to_site":"S-cleve-hill","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":0.1316}},{"from":"S-kemsley.BAY03.T1","from_site":"S-kemsley","id":"C-grain--kemsley--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 15 feature(s)","status":"TYPED","value":10.506},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Grain-Kemsley"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.3152},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-grain.BAY?.T1","to_site":"S-grain","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":3.1517}},{"from":"S-kemsley.BAY04.T1","from_site":"S-kemsley","id":"C-kemsley--northfleet-west--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":0.409},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Kemsley-Northfleet West"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.0123},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-northfleet-west.BAY?.T1","to_site":"S-northfleet-west","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":0.1228}}],"counts":{"bays":4,"busbars":1,"circuits":4,"conductors":4,"km_total":40.255,"neighbours":4},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":3,"id":"S-canterbury-north","kv":[400],"label":"Canterbury North","lat":51.309337,"lon":1.046038,"status":"ASSUMED","walk_to":"substation.py \"Canterbury North\""},{"bays":2,"id":"S-cleve-hill","kv":[400],"label":"Cleve Hill","lat":51.338198,"lon":0.94049,"status":"MATCHED","walk_to":"substation.py \"Cleve Hill\""},{"bays":2,"id":"S-grain","kv":[400],"label":"Grain","lat":51.44584,"lon":0.717476,"status":"MATCHED","walk_to":"substation.py \"Grain\""},{"bays":2,"id":"S-northfleet-west","kv":[400],"label":"Northfleet West","lat":51.351885,"lon":-0.003773,"status":"ASSUMED","walk_to":"substation.py \"Northfleet West\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"exact normalised match to the NESO connection-site register (R10)","status":"TYPED","value":"Kemsley 400kV Substation"},"id":"S-kemsley","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Kemsley"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":51.360788},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":0.790835},"register":{"neso_id":"7368","placeable":true,"register":"TEC","register_name":"Kemsley 400kV Substation","status":"Built"},"status":"MATCHED"},"transformers":[]},"S-kensal-green":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-kensal-green.BB400","circuit":"C-kensal-green--saint-johns-wood--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Kensal Green - St John's Wood"},"far_end":{"id":"S-saint-johns-wood","label":{"source":"the other half of the circuit name","status":"TYPED","value":"St John's Wood"},"status":"ASSUMED"},"id":"S-kensal-green.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-kensal-green.BAY01.T1"},{"busbar":"S-kensal-green.BB400","circuit":"C-kensal-green--willesden--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Kensal Green - Willesden"},"far_end":{"id":"S-willesden","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Willesden"},"status":"ASSUMED"},"id":"S-kensal-green.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-kensal-green.BAY02.T1"},{"busbar":"S-kensal-green.BB400","circuit":"C-kensal-green--wimbledon--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Kensal Green - Wimbledon"},"far_end":{"id":"S-wimbledon","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Wimbledon"},"status":"ASSUMED"},"id":"S-kensal-green.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-kensal-green.BAY03.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-kensal-green.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-kensal-green.BAY01.T1","from_site":"S-kensal-green","id":"C-kensal-green--saint-johns-wood--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":4.462},"name":{"source":"name tag on the line features","status":"TYPED","value":"London Power Tunnels St John's Wood - Kensal Green"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.1339},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-saint-johns-wood.BAY?.T1","to_site":"S-saint-johns-wood","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":1.3387}},{"from":"S-kensal-green.BAY02.T1","from_site":"S-kensal-green","id":"C-kensal-green--willesden--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":2.808},"name":{"source":"name tag on the line features","status":"TYPED","value":"Kensal Green–Willesden Cable Tunnel (LPT1)"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.0842},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-willesden.BAY?.T1","to_site":"S-willesden","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":0.8424}},{"from":"S-kensal-green.BAY03.T1","from_site":"S-kensal-green","id":"C-kensal-green--wimbledon--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":12.509},"name":{"source":"name tag on the line features","status":"TYPED","value":"Kensal Green–Wimbledon Cable Tunnel"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.3753},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-wimbledon.BAY?.T1","to_site":"S-wimbledon","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":3.7527}}],"counts":{"bays":3,"busbars":1,"circuits":3,"conductors":3,"km_total":19.779,"neighbours":3},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":4,"id":"S-saint-johns-wood","kv":[400],"label":"St John's Wood","lat":51.52772,"lon":-0.16906,"status":"ASSUMED","walk_to":"substation.py \"St John's Wood\""},{"bays":1,"id":"S-willesden","kv":[400],"label":"Willesden","lat":51.534531,"lon":-0.259319,"status":"ASSUMED","walk_to":"substation.py \"Willesden\""},{"bays":1,"id":"S-wimbledon","kv":[400],"label":"Wimbledon","lat":51.430873,"lon":-0.188182,"status":"ASSUMED","walk_to":"substation.py \"Wimbledon\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"no exact normalised match in the register - this is a name off the map, not a registered site","status":"ASSUMED","value":null},"id":"S-kensal-green","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Kensal Green"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":51.526235},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-0.224156},"register":null,"status":"ASSUMED"},"transformers":[]},"S-kingsnorth":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-kingsnorth.BB400","circuit":"C-grain--kingsnorth--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Grain - Kingsnorth"},"far_end":{"id":"S-grain","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Grain"},"status":"MATCHED"},"id":"S-kingsnorth.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-kingsnorth.BAY01.T1"},{"busbar":"S-kingsnorth.BB400","circuit":"C-kingsnorth--northfleet-east--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Kingsnorth - Northfleet East"},"far_end":{"id":"S-northfleet-east","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Northfleet East"},"status":"ASSUMED"},"id":"S-kingsnorth.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-kingsnorth.BAY02.T1"},{"busbar":"S-kingsnorth.BB400","circuit":"C-kingsnorth--singlewell--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Kingsnorth - Singlewell"},"far_end":{"id":"S-singlewell","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Singlewell"},"status":"ASSUMED"},"id":"S-kingsnorth.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-kingsnorth.BAY03.T1"},{"busbar":"S-kingsnorth.BB400","circuit":"C-kingsnorth--tilbury--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Kingsnorth - Tilbury"},"far_end":{"id":"S-tilbury","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Tilbury"},"status":"MATCHED"},"id":"S-kingsnorth.BAY04","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-kingsnorth.BAY04.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-kingsnorth.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-kingsnorth.BAY01.T1","from_site":"S-kingsnorth","id":"C-grain--kingsnorth--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":11.351},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Kingsnorth-Grain"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.3405},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-grain.BAY?.T1","to_site":"S-grain","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":3.4052}},{"from":"S-kingsnorth.BAY02.T1","from_site":"S-kingsnorth","id":"C-kingsnorth--northfleet-east--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":5.995},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Kingsnorth - Northfleet East"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.1799},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-northfleet-east.BAY?.T1","to_site":"S-northfleet-east","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":1.7986}},{"from":"S-kingsnorth.BAY03.T1","from_site":"S-kingsnorth","id":"C-kingsnorth--singlewell--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":17.673},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Singlewell-Kingsnorth"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.5302},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-singlewell.BAY?.T1","to_site":"S-singlewell","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":5.3019}},{"from":"S-kingsnorth.BAY04.T1","from_site":"S-kingsnorth","id":"C-kingsnorth--tilbury--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 3 feature(s)","status":"TYPED","value":14.61},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Kingsnorth-Tilbury"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.4383},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-tilbury.BAY?.T1","to_site":"S-tilbury","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":4.3829}}],"counts":{"bays":4,"busbars":1,"circuits":4,"conductors":4,"km_total":49.629,"neighbours":4},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":2,"id":"S-grain","kv":[400],"label":"Grain","lat":51.44584,"lon":0.717476,"status":"MATCHED","walk_to":"substation.py \"Grain\""},{"bays":6,"id":"S-northfleet-east","kv":[400],"label":"Northfleet East","lat":51.429089,"lon":0.329869,"status":"ASSUMED","walk_to":"substation.py \"Northfleet East\""},{"bays":2,"id":"S-singlewell","kv":[400],"label":"Singlewell","lat":51.416663,"lon":0.36115,"status":"ASSUMED","walk_to":"substation.py \"Singlewell\""},{"bays":4,"id":"S-tilbury","kv":[400],"label":"Tilbury","lat":51.462315,"lon":0.368538,"status":"MATCHED","walk_to":"substation.py \"Tilbury\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"no exact normalised match in the register - this is a name off the map, not a registered site","status":"ASSUMED","value":null},"id":"S-kingsnorth","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Kingsnorth"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":51.415979},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":0.543739},"register":null,"status":"ASSUMED"},"transformers":[]},"S-littlebrook":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-littlebrook.BB400","circuit":"C-barking--littlebrook--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Barking - Littlebrook 1"},"far_end":{"id":"S-barking","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Barking"},"status":"ASSUMED"},"id":"S-littlebrook.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-littlebrook.BAY01.T1"},{"busbar":"S-littlebrook.BB400","circuit":"C-barking--littlebrook--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Barking - Littlebrook 2"},"far_end":{"id":"S-barking","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Barking"},"status":"ASSUMED"},"id":"S-littlebrook.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-littlebrook.BAY02.T1"},{"busbar":"S-littlebrook.BB400","circuit":"C-kelmsley--littlebrook--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Kelmsley - Littlebrook"},"far_end":{"id":"S-kelmsley","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Kelmsley"},"status":"ASSUMED"},"id":"S-littlebrook.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-littlebrook.BAY03.T1"},{"busbar":"S-littlebrook.BB400","circuit":"C-littlebrook--rowdown--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Littlebrook - Rowdown"},"far_end":{"id":"S-rowdown","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Rowdown"},"status":"ASSUMED"},"id":"S-littlebrook.BAY04","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-littlebrook.BAY04.T1"},{"busbar":"S-littlebrook.BB400","circuit":"C-littlebrook--tilbury--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Littlebrook - Tilbury"},"far_end":{"id":"S-tilbury","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Tilbury"},"status":"MATCHED"},"id":"S-littlebrook.BAY05","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-littlebrook.BAY05.T1"},{"busbar":"S-littlebrook.BB400","circuit":"C-littlebrook--west-thurrock--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Littlebrook - West Thurrock"},"far_end":{"id":"S-west-thurrock","label":{"source":"the other half of the circuit name","status":"TYPED","value":"West Thurrock"},"status":"ASSUMED"},"id":"S-littlebrook.BAY06","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-littlebrook.BAY06.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-littlebrook.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-littlebrook.BAY01.T1","from_site":"S-littlebrook","id":"C-barking--littlebrook--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features / 2 (mean of the parallel circuits); 23 feature(s)","status":"TYPED","value":4.425},"name":{"source":"name tag on the line features","status":"TYPED","value":"Barking/Littlebrook - Tilbury"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.1328},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-barking.BAY?.T1","to_site":"S-barking","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":1.3276}},{"from":"S-littlebrook.BAY02.T1","from_site":"S-littlebrook","id":"C-barking--littlebrook--400#2","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features / 2 (mean of the parallel circuits); 23 feature(s)","status":"TYPED","value":4.425},"name":{"source":"name tag on the line features","status":"TYPED","value":"Barking/Littlebrook - Tilbury"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.1328},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-barking.BAY?.T1","to_site":"S-barking","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":1.3276}},{"from":"S-littlebrook.BAY03.T1","from_site":"S-littlebrook","id":"C-kelmsley--littlebrook--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 12 feature(s)","status":"TYPED","value":37.33},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Kelmsley - Littlebrook - Rowdown"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.1199},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-kelmsley.BAY?.T1","to_site":"S-kelmsley","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":11.1991}},{"from":"S-littlebrook.BAY04.T1","from_site":"S-littlebrook","id":"C-littlebrook--rowdown--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 12 feature(s)","status":"TYPED","value":37.33},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Kelmsley - Littlebrook - Rowdown"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.1199},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-rowdown.BAY?.T1","to_site":"S-rowdown","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":11.1991}},{"from":"S-littlebrook.BAY05.T1","from_site":"S-littlebrook","id":"C-littlebrook--tilbury--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 7 feature(s)","status":"TYPED","value":6.905},"name":{"source":"name tag on the line features","status":"TYPED","value":"Barking/Littlebrook - Tilbury"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.2072},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-tilbury.BAY?.T1","to_site":"S-tilbury","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":2.0716}},{"from":"S-littlebrook.BAY06.T1","from_site":"S-littlebrook","id":"C-littlebrook--west-thurrock--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 20 feature(s)","status":"TYPED","value":2.523},"name":{"source":"name tag on the line features","status":"TYPED","value":"Littlebrook - West Thurrock"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.0757},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-west-thurrock.BAY?.T1","to_site":"S-west-thurrock","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":0.7568}}],"counts":{"bays":6,"busbars":1,"circuits":5,"conductors":6,"km_total":92.938,"neighbours":5},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":6,"id":"S-barking","kv":[400],"label":"Barking","lat":51.523135,"lon":0.115747,"status":"ASSUMED","walk_to":"substation.py \"Barking\""},{"bays":1,"id":"S-kelmsley","kv":[400],"label":"Kelmsley","lat":51.369612,"lon":0.740718,"status":"ASSUMED","walk_to":"substation.py \"Kelmsley\""},{"bays":3,"id":"S-rowdown","kv":[400],"label":"Rowdown","lat":51.351885,"lon":-0.003773,"status":"ASSUMED","walk_to":"substation.py \"Rowdown\""},{"bays":4,"id":"S-tilbury","kv":[400],"label":"Tilbury","lat":51.462315,"lon":0.368538,"status":"MATCHED","walk_to":"substation.py \"Tilbury\""},{"bays":4,"id":"S-west-thurrock","kv":[400],"label":"West Thurrock","lat":51.457348,"lon":0.301643,"status":"ASSUMED","walk_to":"substation.py \"West Thurrock\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"no exact normalised match in the register - this is a name off the map, not a registered site","status":"ASSUMED","value":null},"id":"S-littlebrook","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Littlebrook"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":51.442903},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":0.092319},"register":null,"status":"ASSUMED"},"transformers":[]},"S-lovedean":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-lovedean.BB400","circuit":"C-bolney--lovedean--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Bolney - Lovedean"},"far_end":{"id":"S-bolney","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Bolney"},"status":"MATCHED"},"id":"S-lovedean.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-lovedean.BAY01.T1"},{"busbar":"S-lovedean.BB400","circuit":"C-botley-wood--lovedean--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Botley Wood - Lovedean"},"far_end":{"id":"S-botley-wood","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Botley Wood"},"status":"ASSUMED"},"id":"S-lovedean.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-lovedean.BAY02.T1"},{"busbar":"S-lovedean.BB400","circuit":"C-fleet--lovedean--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Fleet - Lovedean"},"far_end":{"id":"S-fleet","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Fleet"},"status":"ASSUMED"},"id":"S-lovedean.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-lovedean.BAY03.T1"},{"busbar":"S-lovedean.BB400","circuit":"C-lovedean--nursling--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Lovedean - Nursling"},"far_end":{"id":"S-nursling","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Nursling"},"status":"ASSUMED"},"id":"S-lovedean.BAY04","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-lovedean.BAY04.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-lovedean.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-lovedean.BAY01.T1","from_site":"S-lovedean","id":"C-bolney--lovedean--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":64.694},"name":{"source":"name tag on the line features","status":"TYPED","value":"Bolney-Lovedean"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.9408},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-bolney.BAY?.T1","to_site":"S-bolney","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":19.4083}},{"from":"S-lovedean.BAY02.T1","from_site":"S-lovedean","id":"C-botley-wood--lovedean--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":14.811},"name":{"source":"name tag on the line features","status":"TYPED","value":"Botley Wood-Lovedean"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.4443},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-botley-wood.BAY?.T1","to_site":"S-botley-wood","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":4.4433}},{"from":"S-lovedean.BAY03.T1","from_site":"S-lovedean","id":"C-fleet--lovedean--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":41.082},"name":{"source":"name tag on the line features","status":"TYPED","value":"Lovedean-Fleet"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.2324},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-fleet.BAY?.T1","to_site":"S-fleet","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":12.3245}},{"from":"S-lovedean.BAY04.T1","from_site":"S-lovedean","id":"C-lovedean--nursling--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":36.41},"name":{"source":"name tag on the line features","status":"TYPED","value":"Lovedean-Nursling"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.0923},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-nursling.BAY?.T1","to_site":"S-nursling","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":10.9231}}],"counts":{"bays":4,"busbars":1,"circuits":4,"conductors":4,"km_total":156.997,"neighbours":4},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":2,"id":"S-bolney","kv":[400],"label":"Bolney","lat":50.97635,"lon":-0.234077,"status":"MATCHED","walk_to":"substation.py \"Bolney\""},{"bays":2,"id":"S-botley-wood","kv":[400],"label":"Botley Wood","lat":50.884795,"lon":-1.23254,"status":"ASSUMED","walk_to":"substation.py \"Botley Wood\""},{"bays":2,"id":"S-fleet","kv":[400],"label":"Fleet","lat":51.251101,"lon":-0.884454,"status":"ASSUMED","walk_to":"substation.py \"Fleet\""},{"bays":3,"id":"S-nursling","kv":[400],"label":"Nursling","lat":50.942876,"lon":-1.48721,"status":"ASSUMED","walk_to":"substation.py \"Nursling\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"exact normalised match to the NESO connection-site register (R10)","status":"TYPED","value":"Lovedean 400kV Substation"},"id":"S-lovedean","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Lovedean"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":50.917971},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-1.03998},"register":{"neso_id":"1449","placeable":true,"register":"TEC","register_name":"Lovedean 400kV Substation","status":"Scoping"},"status":"MATCHED"},"transformers":[]},"S-lower-shelf":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-lower-shelf.BB400","circuit":"C-bradford-west--lower-shelf--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Bradford West - Lower Shelf"},"far_end":{"id":"S-bradford-west","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Bradford West"},"status":"ASSUMED"},"id":"S-lower-shelf.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-lower-shelf.BAY01.T1"},{"busbar":"S-lower-shelf.BB400","circuit":"C-lower-shelf--monk-fryston--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Lower Shelf - Monk Fryston"},"far_end":{"id":"S-monk-fryston","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Monk Fryston"},"status":"ASSUMED"},"id":"S-lower-shelf.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-lower-shelf.BAY02.T1"},{"busbar":"S-lower-shelf.BB400","circuit":"C-lower-shelf--padiham--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Lower Shelf - Padiham"},"far_end":{"id":"S-padiham","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Padiham"},"status":"ASSUMED"},"id":"S-lower-shelf.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-lower-shelf.BAY03.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-lower-shelf.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-lower-shelf.BAY01.T1","from_site":"S-lower-shelf","id":"C-bradford-west--lower-shelf--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":11.355},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Lower Shelf-Bradford West"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.3407},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-bradford-west.BAY?.T1","to_site":"S-bradford-west","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":3.4066}},{"from":"S-lower-shelf.BAY02.T1","from_site":"S-lower-shelf","id":"C-lower-shelf--monk-fryston--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":39.66},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Lower Shelf-Monk Fryston"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.1898},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-monk-fryston.BAY?.T1","to_site":"S-monk-fryston","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":11.8981}},{"from":"S-lower-shelf.BAY03.T1","from_site":"S-lower-shelf","id":"C-lower-shelf--padiham--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":47.94},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Padiham-Lower Shelf"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.4382},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-padiham.BAY?.T1","to_site":"S-padiham","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":14.382}}],"counts":{"bays":3,"busbars":1,"circuits":3,"conductors":3,"km_total":98.955,"neighbours":3},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":1,"id":"S-bradford-west","kv":[400],"label":"Bradford West","lat":53.810556,"lon":-1.86394,"status":"ASSUMED","walk_to":"substation.py \"Bradford West\""},{"bays":1,"id":"S-monk-fryston","kv":[400],"label":"Monk Fryston","lat":53.756698,"lon":-1.268433,"status":"ASSUMED","walk_to":"substation.py \"Monk Fryston\""},{"bays":2,"id":"S-padiham","kv":[400],"label":"Padiham","lat":53.79586,"lon":-2.320692,"status":"ASSUMED","walk_to":"substation.py \"Padiham\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"no exact normalised match in the register - this is a name off the map, not a registered site","status":"ASSUMED","value":null},"id":"S-lower-shelf","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Lower Shelf"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":53.743731},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-1.805312},"register":null,"status":"ASSUMED"},"transformers":[]},"S-macclesfield":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-macclesfield.BB400","circuit":"C-cellarhead--macclesfield--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Cellarhead - Macclesfield"},"far_end":{"id":"S-cellarhead","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Cellarhead"},"status":"ASSUMED"},"id":"S-macclesfield.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-macclesfield.BAY01.T1"},{"busbar":"S-macclesfield.BB400","circuit":"C-daines--macclesfield--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Daines - Macclesfield"},"far_end":{"id":"S-daines","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Daines"},"status":"ASSUMED"},"id":"S-macclesfield.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-macclesfield.BAY02.T1"},{"busbar":"S-macclesfield.BB400","circuit":"C-macclesfield--ze--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Macclesfield - ZE"},"far_end":{"id":"S-ze","label":{"source":"the other half of the circuit name","status":"TYPED","value":"ZE"},"status":"ASSUMED"},"id":"S-macclesfield.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-macclesfield.BAY03.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-macclesfield.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-macclesfield.BAY01.T1","from_site":"S-macclesfield","id":"C-cellarhead--macclesfield--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 3 feature(s)","status":"TYPED","value":25.877},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Macclesfield-Cellarhead"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.7763},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-cellarhead.BAY?.T1","to_site":"S-cellarhead","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":7.7631}},{"from":"S-macclesfield.BAY02.T1","from_site":"S-macclesfield","id":"C-daines--macclesfield--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 3 feature(s)","status":"TYPED","value":27.666},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Daines-Macclesfield"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.83},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-daines.BAY?.T1","to_site":"S-daines","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":8.2997}},{"from":"S-macclesfield.BAY03.T1","from_site":"S-macclesfield","id":"C-macclesfield--ze--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":5.91},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Macclesfield-ZE"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.1773},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-ze.BAY?.T1","to_site":"S-ze","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":1.7732}}],"counts":{"bays":3,"busbars":1,"circuits":3,"conductors":3,"km_total":59.453,"neighbours":3},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":2,"id":"S-cellarhead","kv":[400],"label":"Cellarhead","lat":53.040916,"lon":-2.081563,"status":"ASSUMED","walk_to":"substation.py \"Cellarhead\""},{"bays":2,"id":"S-daines","kv":[400],"label":"Daines","lat":53.426061,"lon":-2.37788,"status":"ASSUMED","walk_to":"substation.py \"Daines\""},{"bays":1,"id":"S-ze","kv":[400],"label":"ZE","lat":53.268628,"lon":-2.120901,"status":"ASSUMED","walk_to":"substation.py \"ZE\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"no exact normalised match in the register - this is a name off the map, not a registered site","status":"ASSUMED","value":null},"id":"S-macclesfield","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Macclesfield"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":53.255907},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-2.184005},"register":null,"status":"ASSUMED"},"transformers":[]},"S-melksham":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-melksham.BB400","circuit":"C-bramley--melksham--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Bramley - Melksham"},"far_end":{"id":"S-bramley","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Bramley"},"status":"ASSUMED"},"id":"S-melksham.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-melksham.BAY01.T1"},{"busbar":"S-melksham.BB400","circuit":"C-cilfynydd--melksham--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Cilfynydd - Melksham"},"far_end":{"id":"S-cilfynydd","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Cilfynydd"},"status":"MATCHED"},"id":"S-melksham.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-melksham.BAY02.T1"},{"busbar":"S-melksham.BB400","circuit":"C-hinkley-point--melksham--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Hinkley Point - Melksham"},"far_end":{"id":"S-hinkley-point","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Hinkley Point"},"status":"ASSUMED"},"id":"S-melksham.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-melksham.BAY03.T1"},{"busbar":"S-melksham.BB400","circuit":"C-horsey-levels--melksham--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Horsey Levels - Melksham"},"far_end":{"id":"S-horsey-levels","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Horsey Levels"},"status":"ASSUMED"},"id":"S-melksham.BAY04","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-melksham.BAY04.T1"},{"busbar":"S-melksham.BB400","circuit":"C-melksham--minety--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Melksham - Minety"},"far_end":{"id":"S-minety","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Minety"},"status":"MATCHED"},"id":"S-melksham.BAY05","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-melksham.BAY05.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-melksham.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-melksham.BAY01.T1","from_site":"S-melksham","id":"C-bramley--melksham--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 6 feature(s)","status":"TYPED","value":80.948},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Bramley-Melksham"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":2.4284},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-bramley.BAY?.T1","to_site":"S-bramley","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":24.2845}},{"from":"S-melksham.BAY02.T1","from_site":"S-melksham","id":"C-cilfynydd--melksham--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":50.091},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Melksham-Cilfynydd"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.5027},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-cilfynydd.BAY?.T1","to_site":"S-cilfynydd","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":15.0273}},{"from":"S-melksham.BAY03.T1","from_site":"S-melksham","id":"C-hinkley-point--melksham--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":0.891},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Melksham-Hinkley Point"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.0267},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-hinkley-point.BAY?.T1","to_site":"S-hinkley-point","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":0.2674}},{"from":"S-melksham.BAY04.T1","from_site":"S-melksham","id":"C-horsey-levels--melksham--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":71.457},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Melksham-Horsey Levels"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":2.1437},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-horsey-levels.BAY?.T1","to_site":"S-horsey-levels","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":21.437}},{"from":"S-melksham.BAY05.T1","from_site":"S-melksham","id":"C-melksham--minety--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":27.799},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Minety - Melksham"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.834},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-minety.BAY?.T1","to_site":"S-minety","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":8.3396}}],"counts":{"bays":5,"busbars":1,"circuits":5,"conductors":5,"km_total":231.186,"neighbours":5},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":4,"id":"S-bramley","kv":[400],"label":"Bramley","lat":51.336444,"lon":-1.077429,"status":"ASSUMED","walk_to":"substation.py \"Bramley\""},{"bays":3,"id":"S-cilfynydd","kv":[400],"label":"Cilfynydd","lat":51.633523,"lon":-3.301687,"status":"MATCHED","walk_to":"substation.py \"Cilfynydd\""},{"bays":4,"id":"S-hinkley-point","kv":[400],"label":"Hinkley Point","lat":51.205812,"lon":-3.127443,"status":"ASSUMED","walk_to":"substation.py \"Hinkley Point\""},{"bays":1,"id":"S-horsey-levels","kv":[400],"label":"Horsey Levels","lat":51.181981,"lon":-2.929649,"status":"ASSUMED","walk_to":"substation.py \"Horsey Levels\""},{"bays":2,"id":"S-minety","kv":[400],"label":"Minety","lat":51.607495,"lon":-2.002626,"status":"MATCHED","walk_to":"substation.py \"Minety\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"no exact normalised match in the register - this is a name off the map, not a registered site","status":"ASSUMED","value":null},"id":"S-melksham","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Melksham"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":51.395294},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-2.150761},"register":null,"status":"ASSUMED"},"transformers":[]},"S-northfleet-east":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-northfleet-east.BB400","circuit":"C-barking--northfleet-east--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Barking - Northfleet East 1"},"far_end":{"id":"S-barking","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Barking"},"status":"ASSUMED"},"id":"S-northfleet-east.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-northfleet-east.BAY01.T1"},{"busbar":"S-northfleet-east.BB400","circuit":"C-barking--northfleet-east--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Barking - Northfleet East 2"},"far_end":{"id":"S-barking","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Barking"},"status":"ASSUMED"},"id":"S-northfleet-east.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-northfleet-east.BAY02.T1"},{"busbar":"S-northfleet-east.BB400","circuit":"C-kingsnorth--northfleet-east--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Kingsnorth - Northfleet East"},"far_end":{"id":"S-kingsnorth","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Kingsnorth"},"status":"ASSUMED"},"id":"S-northfleet-east.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-northfleet-east.BAY03.T1"},{"busbar":"S-northfleet-east.BB400","circuit":"C-northfleet-east--singlewell--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Northfleet East - Singlewell"},"far_end":{"id":"S-singlewell","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Singlewell"},"status":"ASSUMED"},"id":"S-northfleet-east.BAY04","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-northfleet-east.BAY04.T1"},{"busbar":"S-northfleet-east.BB400","circuit":"C-northfleet-east--west-thurrock--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Northfleet East - West Thurrock 1"},"far_end":{"id":"S-west-thurrock","label":{"source":"the other half of the circuit name","status":"TYPED","value":"West Thurrock"},"status":"ASSUMED"},"id":"S-northfleet-east.BAY05","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-northfleet-east.BAY05.T1"},{"busbar":"S-northfleet-east.BB400","circuit":"C-northfleet-east--west-thurrock--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Northfleet East - West Thurrock 2"},"far_end":{"id":"S-west-thurrock","label":{"source":"the other half of the circuit name","status":"TYPED","value":"West Thurrock"},"status":"ASSUMED"},"id":"S-northfleet-east.BAY06","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-northfleet-east.BAY06.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-northfleet-east.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-northfleet-east.BAY01.T1","from_site":"S-northfleet-east","id":"C-barking--northfleet-east--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features / 2 (mean of the parallel circuits); 8 feature(s)","status":"TYPED","value":5.381},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Barking - Northfleet East - West Thurrock"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.1614},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-barking.BAY?.T1","to_site":"S-barking","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":1.6144}},{"from":"S-northfleet-east.BAY02.T1","from_site":"S-northfleet-east","id":"C-barking--northfleet-east--400#2","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features / 2 (mean of the parallel circuits); 8 feature(s)","status":"TYPED","value":5.381},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Barking - Northfleet East - West Thurrock"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.1614},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-barking.BAY?.T1","to_site":"S-barking","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":1.6144}},{"from":"S-northfleet-east.BAY03.T1","from_site":"S-northfleet-east","id":"C-kingsnorth--northfleet-east--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":5.995},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Kingsnorth - Northfleet East"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.1799},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-kingsnorth.BAY?.T1","to_site":"S-kingsnorth","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":1.7986}},{"from":"S-northfleet-east.BAY04.T1","from_site":"S-northfleet-east","id":"C-northfleet-east--singlewell--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 3 feature(s)","status":"TYPED","value":0.231},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Northfleet East-Singlewell"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.0069},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-singlewell.BAY?.T1","to_site":"S-singlewell","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":0.0693}},{"from":"S-northfleet-east.BAY05.T1","from_site":"S-northfleet-east","id":"C-northfleet-east--west-thurrock--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features / 2 (mean of the parallel circuits); 8 feature(s)","status":"TYPED","value":5.381},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Barking - Northfleet East - West Thurrock"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.1614},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-west-thurrock.BAY?.T1","to_site":"S-west-thurrock","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":1.6144}},{"from":"S-northfleet-east.BAY06.T1","from_site":"S-northfleet-east","id":"C-northfleet-east--west-thurrock--400#2","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features / 2 (mean of the parallel circuits); 8 feature(s)","status":"TYPED","value":5.381},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Barking - Northfleet East - West Thurrock"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.1614},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-west-thurrock.BAY?.T1","to_site":"S-west-thurrock","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":1.6144}}],"counts":{"bays":6,"busbars":1,"circuits":4,"conductors":6,"km_total":27.75,"neighbours":4},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":6,"id":"S-barking","kv":[400],"label":"Barking","lat":51.523135,"lon":0.115747,"status":"ASSUMED","walk_to":"substation.py \"Barking\""},{"bays":4,"id":"S-kingsnorth","kv":[400],"label":"Kingsnorth","lat":51.415979,"lon":0.543739,"status":"ASSUMED","walk_to":"substation.py \"Kingsnorth\""},{"bays":2,"id":"S-singlewell","kv":[400],"label":"Singlewell","lat":51.416663,"lon":0.36115,"status":"ASSUMED","walk_to":"substation.py \"Singlewell\""},{"bays":4,"id":"S-west-thurrock","kv":[400],"label":"West Thurrock","lat":51.457348,"lon":0.301643,"status":"ASSUMED","walk_to":"substation.py \"West Thurrock\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"no exact normalised match in the register - this is a name off the map, not a registered site","status":"ASSUMED","value":null},"id":"S-northfleet-east","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Northfleet East"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":51.429089},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":0.329869},"register":null,"status":"ASSUMED"},"transformers":[]},"S-nursling":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-nursling.BB400","circuit":"C-fawley--nursling--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Fawley - Nursling"},"far_end":{"id":"S-fawley","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Fawley"},"status":"ASSUMED"},"id":"S-nursling.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-nursling.BAY01.T1"},{"busbar":"S-nursling.BB400","circuit":"C-lovedean--nursling--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Lovedean - Nursling"},"far_end":{"id":"S-lovedean","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Lovedean"},"status":"MATCHED"},"id":"S-nursling.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-nursling.BAY02.T1"},{"busbar":"S-nursling.BB400","circuit":"C-mannington--nursling--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Mannington - Nursling"},"far_end":{"id":"S-mannington","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Mannington"},"status":"ASSUMED"},"id":"S-nursling.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-nursling.BAY03.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-nursling.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-nursling.BAY01.T1","from_site":"S-nursling","id":"C-fawley--nursling--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":20.627},"name":{"source":"name tag on the line features","status":"TYPED","value":"Fawley-Nursling"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.6188},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-fawley.BAY?.T1","to_site":"S-fawley","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":6.1879}},{"from":"S-nursling.BAY02.T1","from_site":"S-nursling","id":"C-lovedean--nursling--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":36.41},"name":{"source":"name tag on the line features","status":"TYPED","value":"Lovedean-Nursling"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.0923},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-lovedean.BAY?.T1","to_site":"S-lovedean","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":10.9231}},{"from":"S-nursling.BAY03.T1","from_site":"S-nursling","id":"C-mannington--nursling--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 4 feature(s)","status":"TYPED","value":39.984},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Nursling-Mannington"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.1995},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-mannington.BAY?.T1","to_site":"S-mannington","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":11.9951}}],"counts":{"bays":3,"busbars":1,"circuits":3,"conductors":3,"km_total":97.021,"neighbours":3},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":2,"id":"S-fawley","kv":[400],"label":"Fawley","lat":50.821691,"lon":-1.328668,"status":"ASSUMED","walk_to":"substation.py \"Fawley\""},{"bays":4,"id":"S-lovedean","kv":[400],"label":"Lovedean","lat":50.917971,"lon":-1.03998,"status":"MATCHED","walk_to":"substation.py \"Lovedean\""},{"bays":2,"id":"S-mannington","kv":[400],"label":"Mannington","lat":50.847454,"lon":-1.895839,"status":"ASSUMED","walk_to":"substation.py \"Mannington\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"no exact normalised match in the register - this is a name off the map, not a registered site","status":"ASSUMED","value":null},"id":"S-nursling","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Nursling"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":50.942876},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-1.48721},"register":null,"status":"ASSUMED"},"transformers":[]},"S-pentir":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-pentir.BB400","circuit":"C-brynrefair--pentir--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Brynrefair - Pentir"},"far_end":{"id":"S-brynrefair","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Brynrefair"},"status":"ASSUMED"},"id":"S-pentir.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-pentir.BAY01.T1"},{"busbar":"S-pentir.BB400","circuit":"C-deeside--pentir--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Deeside - Pentir"},"far_end":{"id":"S-deeside","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Deeside"},"status":"ASSUMED"},"id":"S-pentir.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-pentir.BAY02.T1"},{"busbar":"S-pentir.BB400","circuit":"C-pentir--trawsfynydd--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Pentir - Trawsfynydd"},"far_end":{"id":"S-trawsfynydd","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Trawsfynydd"},"status":"MATCHED"},"id":"S-pentir.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-pentir.BAY03.T1"},{"busbar":"S-pentir.BB400","circuit":"C-pentir--wylfa--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Pentir - Wylfa"},"far_end":{"id":"S-wylfa","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Wylfa"},"status":"ASSUMED"},"id":"S-pentir.BAY04","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-pentir.BAY04.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-pentir.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-pentir.BAY01.T1","from_site":"S-pentir","id":"C-brynrefair--pentir--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":6.11},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Pentir-Brynrefair (underground)"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.1833},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-brynrefair.BAY?.T1","to_site":"S-brynrefair","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":1.8331}},{"from":"S-pentir.BAY02.T1","from_site":"S-pentir","id":"C-deeside--pentir--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":79.138},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Deeside - Pentir"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":2.3741},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-deeside.BAY?.T1","to_site":"S-deeside","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":23.7413}},{"from":"S-pentir.BAY03.T1","from_site":"S-pentir","id":"C-pentir--trawsfynydd--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 7 feature(s)","status":"TYPED","value":43.445},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Pentir-Trawsfynydd"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.3034},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-trawsfynydd.BAY?.T1","to_site":"S-trawsfynydd","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":13.0336}},{"from":"S-pentir.BAY04.T1","from_site":"S-pentir","id":"C-pentir--wylfa--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 3 feature(s)","status":"TYPED","value":35.502},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Wylfa-Pentir"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.0651},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-wylfa.BAY?.T1","to_site":"S-wylfa","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":10.6505}}],"counts":{"bays":4,"busbars":1,"circuits":4,"conductors":4,"km_total":164.195,"neighbours":4},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":1,"id":"S-brynrefair","kv":[400],"label":"Brynrefair","lat":53.140687,"lon":-4.156151,"status":"ASSUMED","walk_to":"substation.py \"Brynrefair\""},{"bays":2,"id":"S-deeside","kv":[400],"label":"Deeside","lat":53.229782,"lon":-3.07522,"status":"ASSUMED","walk_to":"substation.py \"Deeside\""},{"bays":3,"id":"S-trawsfynydd","kv":[400],"label":"Trawsfynydd","lat":52.928004,"lon":-3.947009,"status":"MATCHED","walk_to":"substation.py \"Trawsfynydd\""},{"bays":1,"id":"S-wylfa","kv":[400],"label":"Wylfa","lat":53.415208,"lon":-4.480276,"status":"ASSUMED","walk_to":"substation.py \"Wylfa\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"exact normalised match to the NESO connection-site register (R10)","status":"TYPED","value":"Pentir GSP"},"id":"S-pentir","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Pentir"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":53.187728},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-4.157672},"register":{"neso_id":"4881","placeable":true,"register":"TEC","register_name":"Pentir GSP","status":"Built"},"status":"MATCHED"},"transformers":[]},"S-penwortham":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-penwortham.BB400","circuit":"C-danes--penwortham--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Danes - Penwortham"},"far_end":{"id":"S-danes","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Danes"},"status":"ASSUMED"},"id":"S-penwortham.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-penwortham.BAY01.T1"},{"busbar":"S-penwortham.BB400","circuit":"C-eccleshill--penwortham--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Eccleshill - Penwortham"},"far_end":{"id":"S-eccleshill","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Eccleshill"},"status":"ASSUMED"},"id":"S-penwortham.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-penwortham.BAY02.T1"},{"busbar":"S-penwortham.BB400","circuit":"C-penwortham--quernmore--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Penwortham - Quernmore"},"far_end":{"id":"S-quernmore","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Quernmore"},"status":"ASSUMED"},"id":"S-penwortham.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-penwortham.BAY03.T1"},{"busbar":"S-penwortham.BB400","circuit":"C-penwortham--stanah--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Penwortham - Stanah"},"far_end":{"id":"S-stanah","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Stanah"},"status":"ASSUMED"},"id":"S-penwortham.BAY04","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-penwortham.BAY04.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-penwortham.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-penwortham.BAY01.T1","from_site":"S-penwortham","id":"C-danes--penwortham--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":4.321},"name":{"source":"name tag on the line features","status":"TYPED","value":"Penwortham to Danes / Kearsley to Carrington"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.1296},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-danes.BAY?.T1","to_site":"S-danes","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":1.2961}},{"from":"S-penwortham.BAY02.T1","from_site":"S-penwortham","id":"C-eccleshill--penwortham--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":21.162},"name":{"source":"name tag on the line features","status":"TYPED","value":"Penwortham-Eccleshill"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.6348},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-eccleshill.BAY?.T1","to_site":"S-eccleshill","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":6.3485}},{"from":"S-penwortham.BAY03.T1","from_site":"S-penwortham","id":"C-penwortham--quernmore--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":31.957},"name":{"source":"name tag on the line features","status":"TYPED","value":"Quernmore-Penwortham"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.9587},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-quernmore.BAY?.T1","to_site":"S-quernmore","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":9.5872}},{"from":"S-penwortham.BAY04.T1","from_site":"S-penwortham","id":"C-penwortham--stanah--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 8 feature(s)","status":"TYPED","value":24.072},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Penwortham-Stanah"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.7222},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-stanah.BAY?.T1","to_site":"S-stanah","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":7.2216}}],"counts":{"bays":4,"busbars":1,"circuits":4,"conductors":4,"km_total":81.512,"neighbours":4},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":2,"id":"S-danes","kv":[400],"label":"Danes","lat":53.436898,"lon":-2.406618,"status":"ASSUMED","walk_to":"substation.py \"Danes\""},{"bays":2,"id":"S-eccleshill","kv":[400],"label":"Eccleshill","lat":53.710158,"lon":-2.453481,"status":"ASSUMED","walk_to":"substation.py \"Eccleshill\""},{"bays":3,"id":"S-quernmore","kv":[400],"label":"Quernmore","lat":54.012314,"lon":-2.753414,"status":"ASSUMED","walk_to":"substation.py \"Quernmore\""},{"bays":2,"id":"S-stanah","kv":[400],"label":"Stanah","lat":53.877586,"lon":-2.960329,"status":"ASSUMED","walk_to":"substation.py \"Stanah\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"exact normalised match to the NESO connection-site register (R10)","status":"TYPED","value":"Penwortham 400kV Substation"},"id":"S-penwortham","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Penwortham"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":53.744057},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-2.756716},"register":{"neso_id":"10919","placeable":true,"register":"TEC","register_name":"Penwortham 400kV Substation","status":"Scoping"},"status":"MATCHED"},"transformers":[]},"S-quernmore":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-quernmore.BB400","circuit":"C-heysham--quernmore--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Heysham - Quernmore"},"far_end":{"id":"S-heysham","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Heysham"},"status":"ASSUMED"},"id":"S-quernmore.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-quernmore.BAY01.T1"},{"busbar":"S-quernmore.BB400","circuit":"C-hutton-zx--quernmore--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Hutton ZX - Quernmore"},"far_end":{"id":"S-hutton-zx","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Hutton ZX"},"status":"ASSUMED"},"id":"S-quernmore.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-quernmore.BAY02.T1"},{"busbar":"S-quernmore.BB400","circuit":"C-penwortham--quernmore--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Penwortham - Quernmore"},"far_end":{"id":"S-penwortham","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Penwortham"},"status":"MATCHED"},"id":"S-quernmore.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-quernmore.BAY03.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-quernmore.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-quernmore.BAY01.T1","from_site":"S-quernmore","id":"C-heysham--quernmore--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 5 feature(s)","status":"TYPED","value":9.995},"name":{"source":"name tag on the line features","status":"TYPED","value":"Quernmore-Heysham"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.2999},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-heysham.BAY?.T1","to_site":"S-heysham","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":2.9986}},{"from":"S-quernmore.BAY02.T1","from_site":"S-quernmore","id":"C-hutton-zx--quernmore--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":34.463},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Quernmore-Hutton ZX"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.0339},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-hutton-zx.BAY?.T1","to_site":"S-hutton-zx","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":10.339}},{"from":"S-quernmore.BAY03.T1","from_site":"S-quernmore","id":"C-penwortham--quernmore--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":31.957},"name":{"source":"name tag on the line features","status":"TYPED","value":"Quernmore-Penwortham"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.9587},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-penwortham.BAY?.T1","to_site":"S-penwortham","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":9.5872}}],"counts":{"bays":3,"busbars":1,"circuits":3,"conductors":3,"km_total":76.415,"neighbours":3},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":2,"id":"S-heysham","kv":[400],"label":"Heysham","lat":54.032924,"lon":-2.890022,"status":"ASSUMED","walk_to":"substation.py \"Heysham\""},{"bays":1,"id":"S-hutton-zx","kv":[400],"label":"Hutton ZX","lat":54.297737,"lon":-2.670316,"status":"ASSUMED","walk_to":"substation.py \"Hutton ZX\""},{"bays":4,"id":"S-penwortham","kv":[400],"label":"Penwortham","lat":53.744057,"lon":-2.756716,"status":"MATCHED","walk_to":"substation.py \"Penwortham\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"no exact normalised match in the register - this is a name off the map, not a registered site","status":"ASSUMED","value":null},"id":"S-quernmore","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Quernmore"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":54.012314},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-2.753414},"register":null,"status":"ASSUMED"},"transformers":[]},"S-ratcliffe":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-ratcliffe.BB400","circuit":"C-castle-donington--ratcliffe--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Castle Donington - Ratcliffe"},"far_end":{"id":"S-castle-donington","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Castle Donington"},"status":"ASSUMED"},"id":"S-ratcliffe.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-ratcliffe.BAY01.T1"},{"busbar":"S-ratcliffe.BB400","circuit":"C-enderby--ratcliffe--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Enderby - Ratcliffe"},"far_end":{"id":"S-enderby","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Enderby"},"status":"MATCHED"},"id":"S-ratcliffe.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-ratcliffe.BAY02.T1"},{"busbar":"S-ratcliffe.BB400","circuit":"C-on--ratcliffe--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"on - Ratcliffe"},"far_end":{"id":"S-on","label":{"source":"the other half of the circuit name","status":"TYPED","value":"on"},"status":"ASSUMED"},"id":"S-ratcliffe.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-ratcliffe.BAY03.T1"},{"busbar":"S-ratcliffe.BB400","circuit":"C-ratcliffe--staythorpe--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Ratcliffe - Staythorpe"},"far_end":{"id":"S-staythorpe","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Staythorpe"},"status":"ASSUMED"},"id":"S-ratcliffe.BAY04","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-ratcliffe.BAY04.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-ratcliffe.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-ratcliffe.BAY01.T1","from_site":"S-ratcliffe","id":"C-castle-donington--ratcliffe--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 3 feature(s)","status":"TYPED","value":7.99},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Ratcliffe-Castle Donington"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.2397},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-castle-donington.BAY?.T1","to_site":"S-castle-donington","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":2.3971}},{"from":"S-ratcliffe.BAY02.T1","from_site":"S-ratcliffe","id":"C-enderby--ratcliffe--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":36.846},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Ratcliffe-Enderby"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.1054},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-enderby.BAY?.T1","to_site":"S-enderby","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":11.0539}},{"from":"S-ratcliffe.BAY03.T1","from_site":"S-ratcliffe","id":"C-on--ratcliffe--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 7 feature(s)","status":"TYPED","value":21.644},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Ratcliffe-on-Soar - Willington"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.6493},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-on.BAY?.T1","to_site":"S-on","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":6.4933}},{"from":"S-ratcliffe.BAY04.T1","from_site":"S-ratcliffe","id":"C-ratcliffe--staythorpe--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":14.45},"name":{"source":"name tag on the line features","status":"TYPED","value":"Staythorpe - Ratcliffe-on-Soar"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.4335},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-staythorpe.BAY?.T1","to_site":"S-staythorpe","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":4.3349}}],"counts":{"bays":4,"busbars":1,"circuits":4,"conductors":4,"km_total":80.93,"neighbours":4},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":2,"id":"S-castle-donington","kv":[400],"label":"Castle Donington","lat":52.846943,"lon":-1.355539,"status":"ASSUMED","walk_to":"substation.py \"Castle Donington\""},{"bays":3,"id":"S-enderby","kv":[400],"label":"Enderby","lat":52.862214,"lon":-1.256813,"status":"MATCHED","walk_to":"substation.py \"Enderby\""},{"bays":2,"id":"S-on","kv":[400],"label":"on","lat":52.855162,"lon":-1.548435,"status":"ASSUMED","walk_to":"substation.py \"on\""},{"bays":1,"id":"S-staythorpe","kv":[400],"label":"Staythorpe","lat":53.079294,"lon":-0.873628,"status":"ASSUMED","walk_to":"substation.py \"Staythorpe\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"no exact normalised match in the register - this is a name off the map, not a registered site","status":"ASSUMED","value":null},"id":"S-ratcliffe","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Ratcliffe"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":52.860768},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-1.329285},"register":null,"status":"ASSUMED"},"transformers":[]},"S-rowdown":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-rowdown.BB400","circuit":"C-beddington--rowdown--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Beddington - Rowdown"},"far_end":{"id":"S-beddington","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Beddington"},"status":"ASSUMED"},"id":"S-rowdown.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-rowdown.BAY01.T1"},{"busbar":"S-rowdown.BB400","circuit":"C-littlebrook--rowdown--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Littlebrook - Rowdown"},"far_end":{"id":"S-littlebrook","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Littlebrook"},"status":"ASSUMED"},"id":"S-rowdown.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-rowdown.BAY02.T1"},{"busbar":"S-rowdown.BB400","circuit":"C-northfleet-west--rowdown--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Northfleet West - Rowdown"},"far_end":{"id":"S-northfleet-west","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Northfleet West"},"status":"ASSUMED"},"id":"S-rowdown.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-rowdown.BAY03.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-rowdown.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-rowdown.BAY01.T1","from_site":"S-rowdown","id":"C-beddington--rowdown--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":11.578},"name":{"source":"name tag on the line features","status":"TYPED","value":"Beddington–Rowdown 2"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.3474},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-beddington.BAY?.T1","to_site":"S-beddington","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":3.4735}},{"from":"S-rowdown.BAY02.T1","from_site":"S-rowdown","id":"C-littlebrook--rowdown--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 12 feature(s)","status":"TYPED","value":37.33},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Kelmsley - Littlebrook - Rowdown"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.1199},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-littlebrook.BAY?.T1","to_site":"S-littlebrook","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":11.1991}},{"from":"S-rowdown.BAY03.T1","from_site":"S-rowdown","id":"C-northfleet-west--rowdown--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":0.0},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Northfleet West-Rowdown"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.0},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-northfleet-west.BAY?.T1","to_site":"S-northfleet-west","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":0.0}}],"counts":{"bays":3,"busbars":1,"circuits":3,"conductors":3,"km_total":48.908,"neighbours":3},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":1,"id":"S-beddington","kv":[400],"label":"Beddington","lat":51.372628,"lon":-0.126223,"status":"ASSUMED","walk_to":"substation.py \"Beddington\""},{"bays":6,"id":"S-littlebrook","kv":[400],"label":"Littlebrook","lat":51.442903,"lon":0.092319,"status":"ASSUMED","walk_to":"substation.py \"Littlebrook\""},{"bays":2,"id":"S-northfleet-west","kv":[400],"label":"Northfleet West","lat":51.351885,"lon":-0.003773,"status":"ASSUMED","walk_to":"substation.py \"Northfleet West\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"no exact normalised match in the register - this is a name off the map, not a registered site","status":"ASSUMED","value":null},"id":"S-rowdown","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Rowdown"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":51.351885},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-0.003773},"register":null,"status":"ASSUMED"},"transformers":[]},"S-rye-house":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-rye-house.BB400","circuit":"C-pelham--rye-house--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Pelham - Rye House"},"far_end":{"id":"S-pelham","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Pelham"},"status":"ASSUMED"},"id":"S-rye-house.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-rye-house.BAY01.T1"},{"busbar":"S-rye-house.BB400","circuit":"C-rye-house--stocking-pelham--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Rye House - Stocking Pelham"},"far_end":{"id":"S-stocking-pelham","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Stocking Pelham"},"status":"ASSUMED"},"id":"S-rye-house.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-rye-house.BAY02.T1"},{"busbar":"S-rye-house.BB400","circuit":"C-rye-house--waltham-cross--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Rye House - Waltham Cross"},"far_end":{"id":"S-waltham-cross","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Waltham Cross"},"status":"ASSUMED"},"id":"S-rye-house.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-rye-house.BAY03.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-rye-house.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-rye-house.BAY01.T1","from_site":"S-rye-house","id":"C-pelham--rye-house--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 7 feature(s)","status":"TYPED","value":3.083},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Pelham-Rye House-Waltham Cross"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.0925},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-pelham.BAY?.T1","to_site":"S-pelham","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":0.925}},{"from":"S-rye-house.BAY02.T1","from_site":"S-rye-house","id":"C-rye-house--stocking-pelham--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":22.561},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Rye House-Stocking Pelham"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.6768},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-stocking-pelham.BAY?.T1","to_site":"S-stocking-pelham","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":6.7684}},{"from":"S-rye-house.BAY03.T1","from_site":"S-rye-house","id":"C-rye-house--waltham-cross--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 7 feature(s)","status":"TYPED","value":3.083},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Pelham-Rye House-Waltham Cross"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.0925},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-waltham-cross.BAY?.T1","to_site":"S-waltham-cross","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":0.925}}],"counts":{"bays":3,"busbars":1,"circuits":3,"conductors":3,"km_total":28.727,"neighbours":3},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":2,"id":"S-pelham","kv":[400],"label":"Pelham","lat":51.847217,"lon":0.067304,"status":"ASSUMED","walk_to":"substation.py \"Pelham\""},{"bays":3,"id":"S-stocking-pelham","kv":[400],"label":"Stocking Pelham","lat":51.932743,"lon":0.119123,"status":"ASSUMED","walk_to":"substation.py \"Stocking Pelham\""},{"bays":1,"id":"S-waltham-cross","kv":[400],"label":"Waltham Cross","lat":51.71491,"lon":-0.012634,"status":"ASSUMED","walk_to":"substation.py \"Waltham Cross\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"no exact normalised match in the register - this is a name off the map, not a registered site","status":"ASSUMED","value":null},"id":"S-rye-house","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Rye House"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":51.758619},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":0.010528},"register":null,"status":"ASSUMED"},"transformers":[]},"S-saint-johns-wood":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-saint-johns-wood.BB400","circuit":"C-city-road--saint-johns-wood--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"City Road - St John's Wood"},"far_end":{"id":"S-city-road","label":{"source":"the other half of the circuit name","status":"TYPED","value":"City Road"},"status":"ASSUMED"},"id":"S-saint-johns-wood.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-saint-johns-wood.BAY01.T1"},{"busbar":"S-saint-johns-wood.BB400","circuit":"C-elstree--saint-johns-wood--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Elstree - St John's Wood"},"far_end":{"id":"S-elstree","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Elstree"},"status":"ASSUMED"},"id":"S-saint-johns-wood.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-saint-johns-wood.BAY02.T1"},{"busbar":"S-saint-johns-wood.BB400","circuit":"C-hackney--saint-johns-wood--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Hackney - St John's Wood"},"far_end":{"id":"S-hackney","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Hackney"},"status":"ASSUMED"},"id":"S-saint-johns-wood.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-saint-johns-wood.BAY03.T1"},{"busbar":"S-saint-johns-wood.BB400","circuit":"C-kensal-green--saint-johns-wood--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Kensal Green - St John's Wood"},"far_end":{"id":"S-kensal-green","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Kensal Green"},"status":"ASSUMED"},"id":"S-saint-johns-wood.BAY04","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-saint-johns-wood.BAY04.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-saint-johns-wood.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-saint-johns-wood.BAY01.T1","from_site":"S-saint-johns-wood","id":"C-city-road--saint-johns-wood--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":6.436},"name":{"source":"name tag on the line features","status":"TYPED","value":"St John's Wood - City Road 2x 400kV"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.1931},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-city-road.BAY?.T1","to_site":"S-city-road","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":1.9307}},{"from":"S-saint-johns-wood.BAY02.T1","from_site":"S-saint-johns-wood","id":"C-elstree--saint-johns-wood--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":19.767},"name":{"source":"name tag on the line features","status":"TYPED","value":"Elstree-St John's Wood Deep Cable Tunnel"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.593},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-elstree.BAY?.T1","to_site":"S-elstree","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":5.9302}},{"from":"S-saint-johns-wood.BAY03.T1","from_site":"S-saint-johns-wood","id":"C-hackney--saint-johns-wood--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 4 feature(s)","status":"TYPED","value":12.157},"name":{"source":"name tag on the line features","status":"TYPED","value":"London Power Tunnels St John's Wood - Hackney"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.3647},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-hackney.BAY?.T1","to_site":"S-hackney","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":3.6472}},{"from":"S-saint-johns-wood.BAY04.T1","from_site":"S-saint-johns-wood","id":"C-kensal-green--saint-johns-wood--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":4.462},"name":{"source":"name tag on the line features","status":"TYPED","value":"London Power Tunnels St John's Wood - Kensal Green"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.1339},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-kensal-green.BAY?.T1","to_site":"S-kensal-green","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":1.3387}}],"counts":{"bays":4,"busbars":1,"circuits":4,"conductors":4,"km_total":42.822,"neighbours":4},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":2,"id":"S-city-road","kv":[400],"label":"City Road","lat":51.530405,"lon":-0.095849,"status":"ASSUMED","walk_to":"substation.py \"City Road\""},{"bays":2,"id":"S-elstree","kv":[400],"label":"Elstree","lat":51.660521,"lon":-0.344047,"status":"ASSUMED","walk_to":"substation.py \"Elstree\""},{"bays":1,"id":"S-hackney","kv":[400],"label":"Hackney","lat":51.559009,"lon":-0.042622,"status":"ASSUMED","walk_to":"substation.py \"Hackney\""},{"bays":3,"id":"S-kensal-green","kv":[400],"label":"Kensal Green","lat":51.526235,"lon":-0.224156,"status":"ASSUMED","walk_to":"substation.py \"Kensal Green\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"no exact normalised match in the register - this is a name off the map, not a registered site","status":"ASSUMED","value":null},"id":"S-saint-johns-wood","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"St John's Wood"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":51.52772},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-0.16906},"register":null,"status":"ASSUMED"},"transformers":[]},"S-seven-springs":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-seven-springs.BB400","circuit":"C-cowley--seven-springs--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Cowley - Seven Springs"},"far_end":{"id":"S-cowley","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Cowley"},"status":"ASSUMED"},"id":"S-seven-springs.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-seven-springs.BAY01.T1"},{"busbar":"S-seven-springs.BB400","circuit":"C-feckenham--seven-springs--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Feckenham - Seven Springs"},"far_end":{"id":"S-feckenham","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Feckenham"},"status":"ASSUMED"},"id":"S-seven-springs.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-seven-springs.BAY02.T1"},{"busbar":"S-seven-springs.BB400","circuit":"C-minety--seven-springs--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Minety - Seven Springs"},"far_end":{"id":"S-minety","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Minety"},"status":"MATCHED"},"id":"S-seven-springs.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-seven-springs.BAY03.T1"},{"busbar":"S-seven-springs.BB400","circuit":"C-seven-springs--walham--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Seven Springs - Walham"},"far_end":{"id":"S-walham","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Walham"},"status":"ASSUMED"},"id":"S-seven-springs.BAY04","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-seven-springs.BAY04.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-seven-springs.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-seven-springs.BAY01.T1","from_site":"S-seven-springs","id":"C-cowley--seven-springs--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 3 feature(s)","status":"TYPED","value":72.223},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Cowley-Seven Springs"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":2.1667},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-cowley.BAY?.T1","to_site":"S-cowley","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":21.667}},{"from":"S-seven-springs.BAY02.T1","from_site":"S-seven-springs","id":"C-feckenham--seven-springs--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":45.741},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Seven Springs - Feckenham"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.3722},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-feckenham.BAY?.T1","to_site":"S-feckenham","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":13.7222}},{"from":"S-seven-springs.BAY03.T1","from_site":"S-seven-springs","id":"C-minety--seven-springs--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":29.77},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Seven Springs - Minety"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.8931},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-minety.BAY?.T1","to_site":"S-minety","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":8.9309}},{"from":"S-seven-springs.BAY04.T1","from_site":"S-seven-springs","id":"C-seven-springs--walham--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":19.227},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Seven Springs - Walham"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.5768},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-walham.BAY?.T1","to_site":"S-walham","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":5.7682}}],"counts":{"bays":4,"busbars":1,"circuits":4,"conductors":4,"km_total":166.961,"neighbours":4},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":3,"id":"S-cowley","kv":[400],"label":"Cowley","lat":51.712491,"lon":-1.190827,"status":"ASSUMED","walk_to":"substation.py \"Cowley\""},{"bays":2,"id":"S-feckenham","kv":[400],"label":"Feckenham","lat":52.242171,"lon":-1.932057,"status":"ASSUMED","walk_to":"substation.py \"Feckenham\""},{"bays":2,"id":"S-minety","kv":[400],"label":"Minety","lat":51.607495,"lon":-2.002626,"status":"MATCHED","walk_to":"substation.py \"Minety\""},{"bays":2,"id":"S-walham","kv":[400],"label":"Walham","lat":51.8792,"lon":-2.254317,"status":"ASSUMED","walk_to":"substation.py \"Walham\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"no exact normalised match in the register - this is a name off the map, not a registered site","status":"ASSUMED","value":null},"id":"S-seven-springs","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Seven Springs"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":51.8495},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-2.028484},"register":null,"status":"ASSUMED"},"transformers":[]},"S-shurton":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-shurton.BB400","circuit":"C-bridgwater--shurton--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Bridgwater - Shurton"},"far_end":{"id":"S-bridgwater","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Bridgwater"},"status":"ASSUMED"},"id":"S-shurton.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-shurton.BAY01.T1"},{"busbar":"S-shurton.BB400","circuit":"C-hinkley-point--shurton--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Hinkley Point - Shurton"},"far_end":{"id":"S-hinkley-point","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Hinkley Point"},"status":"ASSUMED"},"id":"S-shurton.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-shurton.BAY02.T1"},{"busbar":"S-shurton.BB400","circuit":"C-shurton--taunton--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Shurton - Taunton"},"far_end":{"id":"S-taunton","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Taunton"},"status":"MATCHED"},"id":"S-shurton.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-shurton.BAY03.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-shurton.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-shurton.BAY01.T1","from_site":"S-shurton","id":"C-bridgwater--shurton--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 3 feature(s)","status":"TYPED","value":18.641},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Shurton-Bridgwater"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.5592},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-bridgwater.BAY?.T1","to_site":"S-bridgwater","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":5.5924}},{"from":"S-shurton.BAY02.T1","from_site":"S-shurton","id":"C-hinkley-point--shurton--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 2 feature(s)","status":"TYPED","value":1.102},"name":{"source":"name tag on the line features","status":"TYPED","value":"Hinkley Point - Shurton"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.0331},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-hinkley-point.BAY?.T1","to_site":"S-hinkley-point","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":0.3307}},{"from":"S-shurton.BAY03.T1","from_site":"S-shurton","id":"C-shurton--taunton--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":27.092},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Shurton-Taunton"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.8128},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-taunton.BAY?.T1","to_site":"S-taunton","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":8.1276}}],"counts":{"bays":3,"busbars":1,"circuits":3,"conductors":3,"km_total":46.835,"neighbours":3},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":1,"id":"S-bridgwater","kv":[400],"label":"Bridgwater","lat":51.118605,"lon":-2.97199,"status":"ASSUMED","walk_to":"substation.py \"Bridgwater\""},{"bays":4,"id":"S-hinkley-point","kv":[400],"label":"Hinkley Point","lat":51.205812,"lon":-3.127443,"status":"ASSUMED","walk_to":"substation.py \"Hinkley Point\""},{"bays":5,"id":"S-taunton","kv":[400],"label":"Taunton","lat":51.063392,"lon":-3.148434,"status":"MATCHED","walk_to":"substation.py \"Taunton\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"no exact normalised match in the register - this is a name off the map, not a registered site","status":"ASSUMED","value":null},"id":"S-shurton","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Shurton"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":51.203085},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-3.137775},"register":null,"status":"ASSUMED"},"transformers":[]},"S-sundon":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-sundon.BB400","circuit":"C-east-claydon--sundon--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"East Claydon - Sundon"},"far_end":{"id":"S-east-claydon","label":{"source":"the other half of the circuit name","status":"TYPED","value":"East Claydon"},"status":"ASSUMED"},"id":"S-sundon.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-sundon.BAY01.T1"},{"busbar":"S-sundon.BB400","circuit":"C-elstree--sundon--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Elstree - Sundon"},"far_end":{"id":"S-elstree","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Elstree"},"status":"ASSUMED"},"id":"S-sundon.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-sundon.BAY02.T1"},{"busbar":"S-sundon.BB400","circuit":"C-grendon--sundon--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Grendon - Sundon"},"far_end":{"id":"S-grendon","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Grendon"},"status":"ASSUMED"},"id":"S-sundon.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-sundon.BAY03.T1"},{"busbar":"S-sundon.BB400","circuit":"C-sundon--wymondley--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Sundon - Wymondley"},"far_end":{"id":"S-wymondley","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Wymondley"},"status":"ASSUMED"},"id":"S-sundon.BAY04","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-sundon.BAY04.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-sundon.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-sundon.BAY01.T1","from_site":"S-sundon","id":"C-east-claydon--sundon--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 4 feature(s)","status":"TYPED","value":29.216},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Sundon-East Claydon"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.8765},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-east-claydon.BAY?.T1","to_site":"S-east-claydon","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":8.7649}},{"from":"S-sundon.BAY02.T1","from_site":"S-sundon","id":"C-elstree--sundon--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":34.414},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Sundon-Elstree"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.0324},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-elstree.BAY?.T1","to_site":"S-elstree","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":10.3242}},{"from":"S-sundon.BAY03.T1","from_site":"S-sundon","id":"C-grendon--sundon--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":39.708},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Sundon-Grendon"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":1.1912},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-grendon.BAY?.T1","to_site":"S-grendon","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":11.9123}},{"from":"S-sundon.BAY04.T1","from_site":"S-sundon","id":"C-sundon--wymondley--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":17.725},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Sundon-Wymondley"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.5318},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-wymondley.BAY?.T1","to_site":"S-wymondley","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":5.3176}}],"counts":{"bays":4,"busbars":1,"circuits":4,"conductors":4,"km_total":121.063,"neighbours":4},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":4,"id":"S-east-claydon","kv":[400],"label":"East Claydon","lat":51.925667,"lon":-0.906669,"status":"ASSUMED","walk_to":"substation.py \"East Claydon\""},{"bays":2,"id":"S-elstree","kv":[400],"label":"Elstree","lat":51.660521,"lon":-0.344047,"status":"ASSUMED","walk_to":"substation.py \"Elstree\""},{"bays":1,"id":"S-grendon","kv":[400],"label":"Grendon","lat":52.243082,"lon":-0.727703,"status":"ASSUMED","walk_to":"substation.py \"Grendon\""},{"bays":4,"id":"S-wymondley","kv":[400],"label":"Wymondley","lat":51.96587,"lon":-0.243667,"status":"ASSUMED","walk_to":"substation.py \"Wymondley\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"exact normalised match to the NESO connection-site register (R10)","status":"TYPED","value":"Sundon 400kV Substation"},"id":"S-sundon","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Sundon"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":51.935297},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-0.503612},"register":{"neso_id":"513","placeable":true,"register":"TEC","register_name":"Sundon 400kV Substation","status":"Built"},"status":"MATCHED"},"transformers":[]},"S-taunton":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-taunton.BB400","circuit":"C-alverdiscott--taunton--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Alverdiscott - Taunton"},"far_end":{"id":"S-alverdiscott","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Alverdiscott"},"status":"ASSUMED"},"id":"S-taunton.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-taunton.BAY01.T1"},{"busbar":"S-taunton.BB400","circuit":"C-exeter--taunton--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Exeter - Taunton 1"},"far_end":{"id":"S-exeter","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Exeter"},"status":"MATCHED"},"id":"S-taunton.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-taunton.BAY02.T1"},{"busbar":"S-taunton.BB400","circuit":"C-exeter--taunton--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Exeter - Taunton 2"},"far_end":{"id":"S-exeter","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Exeter"},"status":"MATCHED"},"id":"S-taunton.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-taunton.BAY03.T1"},{"busbar":"S-taunton.BB400","circuit":"C-hinkley-point--taunton--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Hinkley Point - Taunton"},"far_end":{"id":"S-hinkley-point","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Hinkley Point"},"status":"ASSUMED"},"id":"S-taunton.BAY04","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-taunton.BAY04.T1"},{"busbar":"S-taunton.BB400","circuit":"C-shurton--taunton--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Shurton - Taunton"},"far_end":{"id":"S-shurton","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Shurton"},"status":"ASSUMED"},"id":"S-taunton.BAY05","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-taunton.BAY05.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-taunton.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-taunton.BAY01.T1","from_site":"S-taunton","id":"C-alverdiscott--taunton--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 5 feature(s)","status":"TYPED","value":73.841},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Taunton-Alverdiscott"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":2.2152},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-alverdiscott.BAY?.T1","to_site":"S-alverdiscott","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":22.1522}},{"from":"S-taunton.BAY02.T1","from_site":"S-taunton","id":"C-exeter--taunton--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features / 2 (mean of the parallel circuits); 5 feature(s)","status":"TYPED","value":19.361},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Exeter-Taunton"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.5808},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-exeter.BAY?.T1","to_site":"S-exeter","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":5.8083}},{"from":"S-taunton.BAY03.T1","from_site":"S-taunton","id":"C-exeter--taunton--400#2","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features / 2 (mean of the parallel circuits); 5 feature(s)","status":"TYPED","value":19.361},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Exeter-Taunton"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.5808},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-exeter.BAY?.T1","to_site":"S-exeter","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":5.8083}},{"from":"S-taunton.BAY04.T1","from_site":"S-taunton","id":"C-hinkley-point--taunton--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":0.505},"name":{"source":"name tag on the line features","status":"TYPED","value":"Taunton - Hinkley Point"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.0152},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-hinkley-point.BAY?.T1","to_site":"S-hinkley-point","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":0.1516}},{"from":"S-taunton.BAY05.T1","from_site":"S-taunton","id":"C-shurton--taunton--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":27.092},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Shurton-Taunton"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.8128},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-shurton.BAY?.T1","to_site":"S-shurton","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":8.1276}}],"counts":{"bays":5,"busbars":1,"circuits":4,"conductors":5,"km_total":140.16,"neighbours":4},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":2,"id":"S-alverdiscott","kv":[400],"label":"Alverdiscott","lat":51.006954,"lon":-4.134685,"status":"ASSUMED","walk_to":"substation.py \"Alverdiscott\""},{"bays":6,"id":"S-exeter","kv":[400],"label":"Exeter","lat":50.767877,"lon":-3.404542,"status":"MATCHED","walk_to":"substation.py \"Exeter\""},{"bays":4,"id":"S-hinkley-point","kv":[400],"label":"Hinkley Point","lat":51.205812,"lon":-3.127443,"status":"ASSUMED","walk_to":"substation.py \"Hinkley Point\""},{"bays":3,"id":"S-shurton","kv":[400],"label":"Shurton","lat":51.203085,"lon":-3.137775,"status":"ASSUMED","walk_to":"substation.py \"Shurton\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"exact normalised match to the NESO connection-site register (R10)","status":"TYPED","value":"Taunton 400kV Substation"},"id":"S-taunton","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Taunton"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":51.063392},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-3.148434},"register":{"neso_id":"1509","placeable":true,"register":"TEC","register_name":"Taunton 400kV Substation","status":"Awaiting Consents"},"status":"MATCHED"},"transformers":[]},"S-tilbury":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-tilbury.BB400","circuit":"C-chafford-hundred--tilbury--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Chafford Hundred - Tilbury"},"far_end":{"id":"S-chafford-hundred","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Chafford Hundred"},"status":"ASSUMED"},"id":"S-tilbury.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-tilbury.BAY01.T1"},{"busbar":"S-tilbury.BB400","circuit":"C-kingsnorth--tilbury--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Kingsnorth - Tilbury"},"far_end":{"id":"S-kingsnorth","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Kingsnorth"},"status":"ASSUMED"},"id":"S-tilbury.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-tilbury.BAY02.T1"},{"busbar":"S-tilbury.BB400","circuit":"C-littlebrook--tilbury--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Littlebrook - Tilbury"},"far_end":{"id":"S-littlebrook","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Littlebrook"},"status":"ASSUMED"},"id":"S-tilbury.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-tilbury.BAY03.T1"},{"busbar":"S-tilbury.BB400","circuit":"C-rayleigh-main--tilbury--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Rayleigh Main - Tilbury"},"far_end":{"id":"S-rayleigh-main","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Rayleigh Main"},"status":"MATCHED"},"id":"S-tilbury.BAY04","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-tilbury.BAY04.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-tilbury.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-tilbury.BAY01.T1","from_site":"S-tilbury","id":"C-chafford-hundred--tilbury--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":13.038},"name":{"source":"name tag on the line features","status":"TYPED","value":"Chafford Hundred- Tilbury"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.3911},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-chafford-hundred.BAY?.T1","to_site":"S-chafford-hundred","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":3.9114}},{"from":"S-tilbury.BAY02.T1","from_site":"S-tilbury","id":"C-kingsnorth--tilbury--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 3 feature(s)","status":"TYPED","value":14.61},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Kingsnorth-Tilbury"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.4383},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-kingsnorth.BAY?.T1","to_site":"S-kingsnorth","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":4.3829}},{"from":"S-tilbury.BAY03.T1","from_site":"S-tilbury","id":"C-littlebrook--tilbury--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 7 feature(s)","status":"TYPED","value":6.905},"name":{"source":"name tag on the line features","status":"TYPED","value":"Barking/Littlebrook - Tilbury"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.2072},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-littlebrook.BAY?.T1","to_site":"S-littlebrook","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":2.0716}},{"from":"S-tilbury.BAY04.T1","from_site":"S-tilbury","id":"C-rayleigh-main--tilbury--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 5 feature(s)","status":"TYPED","value":21.058},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Rayleigh Main-Tilbury"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.6317},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-rayleigh-main.BAY?.T1","to_site":"S-rayleigh-main","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":6.3174}}],"counts":{"bays":4,"busbars":1,"circuits":4,"conductors":4,"km_total":55.611,"neighbours":4},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":1,"id":"S-chafford-hundred","kv":[400],"label":"Chafford Hundred","lat":51.491957,"lon":0.286465,"status":"ASSUMED","walk_to":"substation.py \"Chafford Hundred\""},{"bays":4,"id":"S-kingsnorth","kv":[400],"label":"Kingsnorth","lat":51.415979,"lon":0.543739,"status":"ASSUMED","walk_to":"substation.py \"Kingsnorth\""},{"bays":6,"id":"S-littlebrook","kv":[400],"label":"Littlebrook","lat":51.442903,"lon":0.092319,"status":"ASSUMED","walk_to":"substation.py \"Littlebrook\""},{"bays":2,"id":"S-rayleigh-main","kv":[400],"label":"Rayleigh Main","lat":51.593572,"lon":0.565512,"status":"MATCHED","walk_to":"substation.py \"Rayleigh Main\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"exact normalised match to the NESO connection-site register (R10)","status":"TYPED","value":"Tilbury 132kV Substation"},"id":"S-tilbury","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Tilbury"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":51.462315},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":0.368538},"register":{"neso_id":"152","placeable":true,"register":"TEC","register_name":"Tilbury 132kV Substation","status":"Built"},"status":"MATCHED"},"transformers":[]},"S-west-thurrock":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-west-thurrock.BB400","circuit":"C-barking--west-thurrock--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Barking - West Thurrock"},"far_end":{"id":"S-barking","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Barking"},"status":"ASSUMED"},"id":"S-west-thurrock.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-west-thurrock.BAY01.T1"},{"busbar":"S-west-thurrock.BB400","circuit":"C-littlebrook--west-thurrock--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Littlebrook - West Thurrock"},"far_end":{"id":"S-littlebrook","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Littlebrook"},"status":"ASSUMED"},"id":"S-west-thurrock.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-west-thurrock.BAY02.T1"},{"busbar":"S-west-thurrock.BB400","circuit":"C-northfleet-east--west-thurrock--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Northfleet East - West Thurrock 1"},"far_end":{"id":"S-northfleet-east","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Northfleet East"},"status":"ASSUMED"},"id":"S-west-thurrock.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-west-thurrock.BAY03.T1"},{"busbar":"S-west-thurrock.BB400","circuit":"C-northfleet-east--west-thurrock--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Northfleet East - West Thurrock 2"},"far_end":{"id":"S-northfleet-east","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Northfleet East"},"status":"ASSUMED"},"id":"S-west-thurrock.BAY04","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-west-thurrock.BAY04.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-west-thurrock.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-west-thurrock.BAY01.T1","from_site":"S-west-thurrock","id":"C-barking--west-thurrock--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 10 feature(s)","status":"TYPED","value":3.412},"name":{"source":"name tag on the line features","status":"TYPED","value":"West Thurrock - Barking"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.1024},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-barking.BAY?.T1","to_site":"S-barking","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":1.0237}},{"from":"S-west-thurrock.BAY02.T1","from_site":"S-west-thurrock","id":"C-littlebrook--west-thurrock--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 20 feature(s)","status":"TYPED","value":2.523},"name":{"source":"name tag on the line features","status":"TYPED","value":"Littlebrook - West Thurrock"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.0757},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-littlebrook.BAY?.T1","to_site":"S-littlebrook","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":0.7568}},{"from":"S-west-thurrock.BAY03.T1","from_site":"S-west-thurrock","id":"C-northfleet-east--west-thurrock--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features / 2 (mean of the parallel circuits); 8 feature(s)","status":"TYPED","value":5.381},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Barking - Northfleet East - West Thurrock"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.1614},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-northfleet-east.BAY?.T1","to_site":"S-northfleet-east","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":1.6144}},{"from":"S-west-thurrock.BAY04.T1","from_site":"S-west-thurrock","id":"C-northfleet-east--west-thurrock--400#2","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features / 2 (mean of the parallel circuits); 8 feature(s)","status":"TYPED","value":5.381},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Barking - Northfleet East - West Thurrock"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.1614},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-northfleet-east.BAY?.T1","to_site":"S-northfleet-east","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":1.6144}}],"counts":{"bays":4,"busbars":1,"circuits":3,"conductors":4,"km_total":16.697,"neighbours":3},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":6,"id":"S-barking","kv":[400],"label":"Barking","lat":51.523135,"lon":0.115747,"status":"ASSUMED","walk_to":"substation.py \"Barking\""},{"bays":6,"id":"S-littlebrook","kv":[400],"label":"Littlebrook","lat":51.442903,"lon":0.092319,"status":"ASSUMED","walk_to":"substation.py \"Littlebrook\""},{"bays":6,"id":"S-northfleet-east","kv":[400],"label":"Northfleet East","lat":51.429089,"lon":0.329869,"status":"ASSUMED","walk_to":"substation.py \"Northfleet East\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"no exact normalised match in the register - this is a name off the map, not a registered site","status":"ASSUMED","value":null},"id":"S-west-thurrock","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"West Thurrock"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":51.457348},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":0.301643},"register":null,"status":"ASSUMED"},"transformers":[]},"S-wymondley":{"assumed_tables":{"bay_template":"single busbar, one breaker per bay","geometry_cross_check_km":10.0,"per_kv":{"132":{"r_ohm_per_km":0.1,"rating_mva":200,"x_ohm_per_km":0.4},"275":{"r_ohm_per_km":0.04,"rating_mva":1000,"x_ohm_per_km":0.32},"400":{"r_ohm_per_km":0.03,"rating_mva":2000,"x_ohm_per_km":0.3}},"transformer_mva":1000},"attribution":"line geometry, circuit names and voltage tags: open map data, (c) OpenStreetMap contributors, ODbL, via GridAtlas; connection site names: NESO open data portal register (contractual names, not measurements). Sandbox only: no confidential data, no real busbar arrangement, no real ratings.","bays":[{"busbar":"S-wymondley.BB400","circuit":"C-cottam--wymondley--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Cottam - Wymondley"},"far_end":{"id":"S-cottam","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Cottam"},"status":"MATCHED"},"id":"S-wymondley.BAY01","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-wymondley.BAY01.T1"},{"busbar":"S-wymondley.BB400","circuit":"C-eaton-socon--wymondley--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Eaton Socon - Wymondley"},"far_end":{"id":"S-eaton-socon","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Eaton Socon"},"status":"ASSUMED"},"id":"S-wymondley.BAY02","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-wymondley.BAY02.T1"},{"busbar":"S-wymondley.BB400","circuit":"C-stocking-pelham--wymondley--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Stocking Pelham - Wymondley"},"far_end":{"id":"S-stocking-pelham","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Stocking Pelham"},"status":"ASSUMED"},"id":"S-wymondley.BAY03","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-wymondley.BAY03.T1"},{"busbar":"S-wymondley.BB400","circuit":"C-sundon--wymondley--400","circuit_label":{"source":"circuit name on the line features","status":"TYPED","value":"Sundon - Wymondley"},"far_end":{"id":"S-sundon","label":{"source":"the other half of the circuit name","status":"TYPED","value":"Sundon"},"status":"MATCHED"},"id":"S-wymondley.BAY04","role":{"source":"a named circuit to another site is a feeder bay","status":"TYPED","value":"feeder"},"template":{"source":"drawing template, one breaker and two isolators per bay","status":"ASSUMED","value":"single busbar, one breaker per bay"},"terminal":"S-wymondley.BAY04.T1"}],"busbars":[{"arrangement":{"source":"drawing template; the real arrangement is not in open data","status":"ASSUMED","value":"single busbar, one breaker per bay"},"id":"S-wymondley.BB400","kv":{"source":"voltage tag of the circuits naming this site","status":"TYPED","value":400}}],"conductors":[{"from":"S-wymondley.BAY01.T1","from_site":"S-wymondley","id":"C-cottam--wymondley--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":0.381},"name":{"source":"name tag on the line features","status":"TYPED","value":"Wymondley - Cottam Power Station, Notts"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.0114},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-cottam.BAY?.T1","to_site":"S-cottam","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":0.1144}},{"from":"S-wymondley.BAY02.T1","from_site":"S-wymondley","id":"C-eaton-socon--wymondley--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 3 feature(s)","status":"TYPED","value":84.794},"name":{"source":"name tag on the line features","status":"TYPED","value":"Cottam - Eaton Socon - Wymondley"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":2.5438},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-eaton-socon.BAY?.T1","to_site":"S-eaton-socon","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":25.4383}},{"from":"S-wymondley.BAY03.T1","from_site":"S-wymondley","id":"C-stocking-pelham--wymondley--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":26.515},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Stocking Pelham-Wymondley"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.7954},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-stocking-pelham.BAY?.T1","to_site":"S-stocking-pelham","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":7.9545}},{"from":"S-wymondley.BAY04.T1","from_site":"S-wymondley","id":"C-sundon--wymondley--400#1","kv":{"source":"voltage tag on the line features","status":"TYPED","value":400},"length_km":{"source":"sum of great-circle segment lengths of the named features; 1 feature(s)","status":"TYPED","value":17.725},"name":{"source":"name tag on the line features","status":"TYPED","value":"National Grid Sundon-Wymondley"},"r_ohm":{"source":"declared per-voltage table, 0.03 ohm/km at 400 kV","status":"ASSUMED","value":0.5318},"rating_mva":{"source":"declared per-voltage table; ratings are never added together","status":"ASSUMED","value":2000},"to":"S-sundon.BAY?.T1","to_site":"S-sundon","x_ohm":{"source":"declared per-voltage table, 0.3 ohm/km at 400 kV","status":"ASSUMED","value":5.3176}}],"counts":{"bays":4,"busbars":1,"circuits":4,"conductors":4,"km_total":129.415,"neighbours":4},"made_by":"E:\\swarm\\grid\\substation.py (topology from circuit names, geometry only for length)","neighbours":[{"bays":4,"id":"S-cottam","kv":[400],"label":"Cottam","lat":53.280435,"lon":-0.786124,"status":"MATCHED","walk_to":"substation.py \"Cottam\""},{"bays":2,"id":"S-eaton-socon","kv":[400],"label":"Eaton Socon","lat":53.300213,"lon":-0.77856,"status":"ASSUMED","walk_to":"substation.py \"Eaton Socon\""},{"bays":3,"id":"S-stocking-pelham","kv":[400],"label":"Stocking Pelham","lat":51.932743,"lon":0.119123,"status":"ASSUMED","walk_to":"substation.py \"Stocking Pelham\""},{"bays":4,"id":"S-sundon","kv":[400],"label":"Sundon","lat":51.935297,"lon":-0.503612,"status":"MATCHED","walk_to":"substation.py \"Sundon\""}],"not":"a real busbar arrangement, breaker position, impedance, rating or transformer","schema":"ventus.kuiper.netlist.v1","substation":{"connection_site_match":{"source":"no exact normalised match in the register - this is a name off the map, not a registered site","status":"ASSUMED","value":null},"id":"S-wymondley","label":{"source":"the site's half of the circuit names","status":"TYPED","value":"Wymondley"},"lat":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":51.96587},"lon":{"source":"centroid of the ends of the circuits naming this site (R12)","status":"ASSUMED","value":-0.243667},"register":null,"status":"ASSUMED"},"transformers":[]}},"schema":"ventus.kuiper.griddata.v1","sites":[{"bays":6,"id":"S-barking","kv":[400],"label":"Barking","lat":51.523135,"lon":0.115747,"status":"ASSUMED"},{"bays":6,"id":"S-exeter","kv":[400],"label":"Exeter","lat":50.767877,"lon":-3.404542,"status":"MATCHED"},{"bays":6,"id":"S-littlebrook","kv":[400],"label":"Littlebrook","lat":51.442903,"lon":0.092319,"status":"ASSUMED"},{"bays":6,"id":"S-northfleet-east","kv":[400],"label":"Northfleet East","lat":51.429089,"lon":0.329869,"status":"ASSUMED"},{"bays":5,"id":"S-melksham","kv":[400],"label":"Melksham","lat":51.395294,"lon":-2.150761,"status":"ASSUMED"},{"bays":5,"id":"S-taunton","kv":[400],"label":"Taunton","lat":51.063392,"lon":-3.148434,"status":"MATCHED"},{"bays":4,"id":"S-bramford","kv":[400],"label":"Bramford","lat":52.071892,"lon":1.060841,"status":"MATCHED"},{"bays":4,"id":"S-bramley","kv":[400],"label":"Bramley","lat":51.336444,"lon":-1.077429,"status":"ASSUMED"},{"bays":4,"id":"S-cottam","kv":[400],"label":"Cottam","lat":53.280435,"lon":-0.786124,"status":"MATCHED"},{"bays":4,"id":"S-east-claydon","kv":[400],"label":"East Claydon","lat":51.925667,"lon":-0.906669,"status":"ASSUMED"},{"bays":4,"id":"S-hinkley-point","kv":[400],"label":"Hinkley Point","lat":51.205812,"lon":-3.127443,"status":"ASSUMED"},{"bays":4,"id":"S-kemsley","kv":[400],"label":"Kemsley","lat":51.360788,"lon":0.790835,"status":"MATCHED"},{"bays":4,"id":"S-kingsnorth","kv":[400],"label":"Kingsnorth","lat":51.415979,"lon":0.543739,"status":"ASSUMED"},{"bays":4,"id":"S-lovedean","kv":[400],"label":"Lovedean","lat":50.917971,"lon":-1.03998,"status":"MATCHED"},{"bays":4,"id":"S-pentir","kv":[400],"label":"Pentir","lat":53.187728,"lon":-4.157672,"status":"MATCHED"},{"bays":4,"id":"S-penwortham","kv":[400],"label":"Penwortham","lat":53.744057,"lon":-2.756716,"status":"MATCHED"},{"bays":4,"id":"S-ratcliffe","kv":[400],"label":"Ratcliffe","lat":52.860768,"lon":-1.329285,"status":"ASSUMED"},{"bays":4,"id":"S-saint-johns-wood","kv":[400],"label":"St John's Wood","lat":51.52772,"lon":-0.16906,"status":"ASSUMED"},{"bays":4,"id":"S-seven-springs","kv":[400],"label":"Seven Springs","lat":51.8495,"lon":-2.028484,"status":"ASSUMED"},{"bays":4,"id":"S-sundon","kv":[400],"label":"Sundon","lat":51.935297,"lon":-0.503612,"status":"MATCHED"},{"bays":4,"id":"S-tilbury","kv":[400],"label":"Tilbury","lat":51.462315,"lon":0.368538,"status":"MATCHED"},{"bays":4,"id":"S-west-thurrock","kv":[400],"label":"West Thurrock","lat":51.457348,"lon":0.301643,"status":"ASSUMED"},{"bays":4,"id":"S-wymondley","kv":[400],"label":"Wymondley","lat":51.96587,"lon":-0.243667,"status":"ASSUMED"},{"bays":3,"id":"S-canterbury-north","kv":[400],"label":"Canterbury North","lat":51.309337,"lon":1.046038,"status":"ASSUMED"},{"bays":3,"id":"S-carrington","kv":[400],"label":"Carrington","lat":53.433249,"lon":-2.395335,"status":"MATCHED"},{"bays":3,"id":"S-chickerell","kv":[400],"label":"Chickerell","lat":50.625021,"lon":-2.486765,"status":"ASSUMED"},{"bays":3,"id":"S-cilfynydd","kv":[400],"label":"Cilfynydd","lat":51.633523,"lon":-3.301687,"status":"MATCHED"},{"bays":3,"id":"S-cowley","kv":[400],"label":"Cowley","lat":51.712491,"lon":-1.190827,"status":"ASSUMED"},{"bays":3,"id":"S-drakelow","kv":[400],"label":"Drakelow","lat":52.772292,"lon":-1.653001,"status":"MATCHED"},{"bays":3,"id":"S-drax","kv":[400],"label":"Drax","lat":53.736019,"lon":-0.990522,"status":"MATCHED"},{"bays":3,"id":"S-enderby","kv":[400],"label":"Enderby","lat":52.862214,"lon":-1.256813,"status":"MATCHED"},{"bays":3,"id":"S-hams-hall","kv":[400],"label":"Hams Hall","lat":52.532237,"lon":-1.714148,"status":"ASSUMED"},{"bays":3,"id":"S-kensal-green","kv":[400],"label":"Kensal Green","lat":51.526235,"lon":-0.224156,"status":"ASSUMED"},{"bays":3,"id":"S-lower-shelf","kv":[400],"label":"Lower Shelf","lat":53.743731,"lon":-1.805312,"status":"ASSUMED"},{"bays":3,"id":"S-macclesfield","kv":[400],"label":"Macclesfield","lat":53.255907,"lon":-2.184005,"status":"ASSUMED"},{"bays":3,"id":"S-nursling","kv":[400],"label":"Nursling","lat":50.942876,"lon":-1.48721,"status":"ASSUMED"},{"bays":3,"id":"S-quernmore","kv":[400],"label":"Quernmore","lat":54.012314,"lon":-2.753414,"status":"ASSUMED"},{"bays":3,"id":"S-rowdown","kv":[400],"label":"Rowdown","lat":51.351885,"lon":-0.003773,"status":"ASSUMED"},{"bays":3,"id":"S-rye-house","kv":[400],"label":"Rye House","lat":51.758619,"lon":0.010528,"status":"ASSUMED"},{"bays":3,"id":"S-shurton","kv":[400],"label":"Shurton","lat":51.203085,"lon":-3.137775,"status":"ASSUMED"},{"bays":3,"id":"S-soar","kv":[400],"label":"Soar","lat":52.857567,"lon":-1.451303,"status":"ASSUMED"},{"bays":3,"id":"S-stocking-pelham","kv":[400],"label":"Stocking Pelham","lat":51.932743,"lon":0.119123,"status":"ASSUMED"},{"bays":3,"id":"S-strathaven","kv":[400],"label":"Strathaven","lat":55.752393,"lon":-4.080375,"status":"ASSUMED"},{"bays":3,"id":"S-trawsfynydd","kv":[400],"label":"Trawsfynydd","lat":52.928004,"lon":-3.947009,"status":"MATCHED"},{"bays":3,"id":"S-treuddyn","kv":[400],"label":"Treuddyn","lat":53.11675,"lon":-3.147947,"status":"ASSUMED"},{"bays":3,"id":"S-walpole","kv":[400],"label":"Walpole","lat":52.725678,"lon":0.197564,"status":"MATCHED"},{"bays":2,"id":"S-abham","kv":[400],"label":"Abham","lat":50.47107,"lon":-3.730746,"status":"ASSUMED"},{"bays":2,"id":"S-alverdiscott","kv":[400],"label":"Alverdiscott","lat":51.006954,"lon":-4.134685,"status":"ASSUMED"},{"bays":2,"id":"S-axminster","kv":[400],"label":"Axminster","lat":50.786831,"lon":-2.924451,"status":"ASSUMED"},{"bays":2,"id":"S-bicker-fen","kv":[400],"label":"Bicker Fen","lat":52.930762,"lon":-0.220556,"status":"MATCHED"},{"bays":2,"id":"S-bolney","kv":[400],"label":"Bolney","lat":50.97635,"lon":-0.234077,"status":"MATCHED"},{"bays":2,"id":"S-botley-wood","kv":[400],"label":"Botley Wood","lat":50.884795,"lon":-1.23254,"status":"ASSUMED"},{"bays":2,"id":"S-braintree","kv":[400],"label":"Braintree","lat":51.862124,"lon":0.576302,"status":"ASSUMED"},{"bays":2,"id":"S-burwell-main","kv":[400],"label":"Burwell Main","lat":52.280464,"lon":0.315481,"status":"MATCHED"},{"bays":2,"id":"S-bustleholm","kv":[400],"label":"Bustleholm","lat":52.55227,"lon":-1.95935,"status":"MATCHED"},{"bays":2,"id":"S-capenhurst","kv":[400],"label":"Capenhurst","lat":53.268661,"lon":-2.949549,"status":"MATCHED"},{"bays":2,"id":"S-castle-donington","kv":[400],"label":"Castle Donington","lat":52.846943,"lon":-1.355539,"status":"ASSUMED"},{"bays":2,"id":"S-cellarhead","kv":[400],"label":"Cellarhead","lat":53.040916,"lon":-2.081563,"status":"ASSUMED"},{"bays":2,"id":"S-chilling","kv":[400],"label":"Chilling","lat":50.835641,"lon":-1.285028,"status":"ASSUMED"},{"bays":2,"id":"S-city-road","kv":[400],"label":"City Road","lat":51.530405,"lon":-0.095849,"status":"ASSUMED"},{"bays":2,"id":"S-cleve-hill","kv":[400],"label":"Cleve Hill","lat":51.338198,"lon":0.94049,"status":"MATCHED"},{"bays":2,"id":"S-coalburn","kv":[400],"label":"Coalburn","lat":55.616522,"lon":-3.866747,"status":"MATCHED"},{"bays":2,"id":"S-coventry","kv":[400],"label":"Coventry","lat":52.457793,"lon":-1.465279,"status":"MATCHED"},{"bays":2,"id":"S-daines","kv":[400],"label":"Daines","lat":53.426061,"lon":-2.37788,"status":"ASSUMED"},{"bays":2,"id":"S-danes","kv":[400],"label":"Danes","lat":53.436898,"lon":-2.406618,"status":"ASSUMED"},{"bays":2,"id":"S-deeside","kv":[400],"label":"Deeside","lat":53.229782,"lon":-3.07522,"status":"ASSUMED"},{"bays":2,"id":"S-didcot","kv":[400],"label":"Didcot","lat":51.626679,"lon":-1.259541,"status":"ASSUMED"},{"bays":2,"id":"S-dungeness","kv":[400],"label":"Dungeness","lat":50.916528,"lon":0.941325,"status":"ASSUMED"},{"bays":2,"id":"S-eaton-socon","kv":[400],"label":"Eaton Socon","lat":53.300213,"lon":-0.77856,"status":"ASSUMED"},{"bays":2,"id":"S-eccleshill","kv":[400],"label":"Eccleshill","lat":53.710158,"lon":-2.453481,"status":"ASSUMED"},{"bays":2,"id":"S-elstree","kv":[400],"label":"Elstree","lat":51.660521,"lon":-0.344047,"status":"ASSUMED"},{"bays":2,"id":"S-elvanfoot","kv":[400],"label":"Elvanfoot","lat":55.43684,"lon":-3.672813,"status":"MATCHED"},{"bays":2,"id":"S-fawley","kv":[400],"label":"Fawley","lat":50.821691,"lon":-1.328668,"status":"ASSUMED"},{"bays":2,"id":"S-feckenham","kv":[400],"label":"Feckenham","lat":52.242171,"lon":-1.932057,"status":"ASSUMED"},{"bays":2,"id":"S-fleet","kv":[400],"label":"Fleet","lat":51.251101,"lon":-0.884454,"status":"ASSUMED"},{"bays":2,"id":"S-grain","kv":[400],"label":"Grain","lat":51.44584,"lon":0.717476,"status":"MATCHED"},{"bays":2,"id":"S-gretna","kv":[400],"label":"Gretna","lat":55.031456,"lon":-3.052216,"status":"ASSUMED"},{"bays":2,"id":"S-harker","kv":[400],"label":"Harker","lat":54.942883,"lon":-2.962254,"status":"MATCHED"},{"bays":2,"id":"S-heysham","kv":[400],"label":"Heysham","lat":54.032924,"lon":-2.890022,"status":"ASSUMED"},{"bays":2,"id":"S-high-marnham","kv":[400],"label":"High Marnham","lat":53.240038,"lon":-0.796262,"status":"MATCHED"},{"bays":2,"id":"S-indian-queens","kv":[400],"label":"Indian Queens","lat":50.394827,"lon":-4.898126,"status":"ASSUMED"},{"bays":2,"id":"S-kearsley","kv":[400],"label":"Kearsley","lat":53.436898,"lon":-2.406618,"status":"ASSUMED"},{"bays":2,"id":"S-kilmarnock-south","kv":[400],"label":"Kilmarnock South","lat":55.576297,"lon":-4.463312,"status":"MATCHED"},{"bays":2,"id":"S-landulph","kv":[400],"label":"Landulph","lat":50.445223,"lon":-4.243029,"status":"ASSUMED"},{"bays":2,"id":"S-legacy","kv":[400],"label":"Legacy","lat":53.028681,"lon":-3.055378,"status":"ASSUMED"},{"bays":2,"id":"S-mannington","kv":[400],"label":"Mannington","lat":50.847454,"lon":-1.895839,"status":"ASSUMED"},{"bays":2,"id":"S-minety","kv":[400],"label":"Minety","lat":51.607495,"lon":-2.002626,"status":"MATCHED"},{"bays":2,"id":"S-moffat","kv":[400],"label":"Moffat","lat":55.301934,"lon":-3.426364,"status":"MATCHED"},{"bays":2,"id":"S-mop-end","kv":[400],"label":"Mop End","lat":51.665542,"lon":-0.656221,"status":"ASSUMED"},{"bays":2,"id":"S-neilston","kv":[400],"label":"Neilston","lat":55.806195,"lon":-4.481608,"status":"MATCHED"},{"bays":2,"id":"S-ninfield","kv":[400],"label":"Ninfield","lat":50.882002,"lon":0.446852,"status":"ASSUMED"},{"bays":2,"id":"S-northfleet-west","kv":[400],"label":"Northfleet West","lat":51.351885,"lon":-0.003773,"status":"ASSUMED"},{"bays":2,"id":"S-norton","kv":[400],"label":"Norton","lat":54.59341,"lon":-1.36809,"status":"MATCHED"},{"bays":2,"id":"S-on","kv":[400],"label":"on","lat":52.855162,"lon":-1.548435,"status":"ASSUMED"},{"bays":2,"id":"S-padiham","kv":[400],"label":"Padiham","lat":53.79586,"lon":-2.320692,"status":"ASSUMED"},{"bays":2,"id":"S-pelham","kv":[400],"label":"Pelham","lat":51.847217,"lon":0.067304,"status":"ASSUMED"},{"bays":2,"id":"S-rayleigh-main","kv":[400],"label":"Rayleigh Main","lat":51.593572,"lon":0.565512,"status":"MATCHED"},{"bays":2,"id":"S-sandford","kv":[400],"label":"Sandford","lat":51.313541,"lon":-2.870022,"status":"ASSUMED"},{"bays":2,"id":"S-seabank","kv":[400],"label":"Seabank","lat":51.342637,"lon":-2.840294,"status":"MATCHED"},{"bays":2,"id":"S-sellindge","kv":[400],"label":"Sellindge","lat":51.105708,"lon":0.972205,"status":"MATCHED"},{"bays":2,"id":"S-shrewsbury","kv":[400],"label":"Shrewsbury","lat":52.708683,"lon":-2.691387,"status":"ASSUMED"},{"bays":2,"id":"S-singlewell","kv":[400],"label":"Singlewell","lat":51.416663,"lon":0.36115,"status":"ASSUMED"},{"bays":2,"id":"S-stanah","kv":[400],"label":"Stanah","lat":53.877586,"lon":-2.960329,"status":"ASSUMED"},{"bays":2,"id":"S-swansea-north","kv":[400],"label":"Swansea North","lat":51.688628,"lon":-3.957389,"status":"MATCHED"},{"bays":2,"id":"S-walham","kv":[400],"label":"Walham","lat":51.8792,"lon":-2.254317,"status":"ASSUMED"},{"bays":2,"id":"S-west-burton","kv":[400],"label":"West Burton","lat":53.299335,"lon":-0.801982,"status":"MATCHED"},{"bays":2,"id":"S-west-ham","kv":[400],"label":"West Ham","lat":51.519582,"lon":0.001102,"status":"ASSUMED"}]}; /* GRID-DATA:END */

// THE RED LINE FOR THIS FAMILY. Cable sizing is not what is drawn here, so the string's red line is
// not apt: this is the development-environment warning and what an open-data map actually is.
const DEV_WARNING_SUB = 'DEVELOPMENT ENVIRONMENT: estimates from a model, nothing measured. An ASSUMED arrangement drawn from open data: not the operator\'s diagram.';
// THE LINE COLOURS ARE THE VOLTAGE LEVELS, and they are the only colours on the map.
const KV_COLOUR = { 400:'#ffb454', 275:'#5ec8f2', 132:'#7ee081', 66:'#c58cf5', 33:'#f28fb2' };
function kvColour(kv){ return KV_COLOUR[kv] || '#9fb0c4'; }

(function (root) {
  const F = root.FIRE; if (!F) return;
  // A VALUE IS A VALUE AND ITS PROVENANCE. A bare number that arrives without provenance is read as
  // ASSUMED and never as typed: the safe way to be wrong about a value is to say you assumed it.
  const V = x => (x && typeof x === 'object' && 'value' in x) ? x : { value:x, status:'ASSUMED', source:'not stated' };
  const val = x => V(x).value;
  const typed = x => V(x).status === 'TYPED';
  const DATA = () => (typeof GRID_DATA !== 'undefined' && GRID_DATA) ? GRID_DATA : { sites:[], netlists:{} };
  const slug = s => String(s || '').toLowerCase().replace(/[^a-z0-9]/g, '');
  function findSite(want){ const D = DATA(), w = slug(want);
    if (!w) return D.sites && D.sites[0] ? D.sites[0] : null;
    for (const s of (D.sites || [])) if (slug(s.id) === w || slug(s.label) === w) return s;
    for (const s of (D.sites || [])) if (slug(s.label).indexOf(w) === 0) return s;
    return null; }
  function fiveNames(){ const D = DATA(), n = [];
    for (const s of (D.sites || [])) { if (n.length >= 5) break; if (D.netlists && D.netlists[s.id]) n.push(s.label); }
    for (const s of (D.sites || [])) { if (n.length >= 5) break; if (n.indexOf(s.label) < 0) n.push(s.label); }
    return n.join(', '); }
  // ---- THE RULES THAT REFUSE IN WORDS -----------------------------------------------------------
  function checkNetlist(nl){
    const bus = new Set((nl.busbars || []).map(b => b.id));
    for (const b of (nl.bays || [])) if (b.busbar && !bus.has(b.busbar))
      throw new Error('bay "' + b.id + '" hangs off busbar "' + b.busbar + '", and this netlist has no such busbar (it has ' + [...bus].join(', ') + '): the map is refused rather than drawn wrong');
    const kvOf = {}; for (const b of (nl.busbars || [])) kvOf[b.id] = val(b.kv);
    const hasTx = (nl.transformers || []).length > 0;
    for (const c of (nl.conductors || [])) {
      const bay = (nl.bays || []).find(b => val(b.circuit) === val(c.name)); if (!bay) continue;
      const bkv = kvOf[bay.busbar];
      if (bkv !== undefined && val(c.kv) !== bkv && !hasTx)
        throw new Error('circuit "' + val(c.name) + '" is ' + val(c.kv) + ' kV and leaves a ' + bkv + ' kV busbar with no transformer between them: a netlist cannot change voltage without one');
    }
    return true; }
  // ---- THE MAP ----------------------------------------------------------------------------------
  // Eight directions and nothing between them. A line leaves the station along one of them; where two
  // lines would share a direction the second steps sideways on a 90 degree jog and runs on parallel,
  // which is exactly what an underground map does and keeps every angle at 0, 45 or 90.
  const DIRS = [ [1,0], [0,1], [-1,0], [0,-1], [0.7071,0.7071], [-0.7071,0.7071], [-0.7071,-0.7071], [0.7071,-0.7071] ];
  function buildMap(nl, site, opts){
    const shapes = [], words = [], marks = [], conns = [], terminals = {};
    const neigh = !!opts.neighbours, out = String(opts.outage || '').toLowerCase();
    const levels = (nl.busbars || []).map(b => val(b.kv)).filter(v => v !== undefined);
    const uniqKv = [...new Set(levels)].sort((a, b) => b - a);
    let assumed = 0, total = 0; const tables = new Set();
    const count = (x, table) => { total++; if (!typed(x)) { assumed++; if (table) tables.add(table); } return val(x); };
    // ONE LINE PER FAR END, however many circuits run that way: the WEIGHT says how many.
    const groups = new Map();
    // WHICH WAY IS OUT. A circuit names a terminal at each end and the site that terminal is in; the
    // far end is whichever of the two is not this station. A reader is shown the NAME, never the id.
    const me = String((nl.substation && nl.substation.id) || '');
    const tidy = id => String(id).replace(/^S-/, '').split('.')[0].replace(/-/g, ' ').replace(/\b\w/g, ch => ch.toUpperCase());
    for (const c of (nl.conductors || [])) {
      const toS = String(c.to_site || c.to || ''), fromS = String(c.from_site || c.from || '');
      const far = (toS === me || toS.indexOf(me + '.') === 0) ? fromS : toS;
      const nb = (nl.neighbours || []).find(x => String(x.id) === String(far));
      const key = String(far);
      if (!groups.has(key)) { total++; if (!nb || nb.status === 'ASSUMED') { assumed++; tables.add('far end identity'); }
        groups.set(key, { id:key, label:(nb && (nb.label && nb.label.value !== undefined ? nb.label.value : nb.label)) || tidy(far),
                          farAssumed:!nb || nb.status === 'ASSUMED', kv:count(c.kv, 'circuit voltages'), circuits:[], km:0 }); }
      const g = groups.get(key);
      const km = count(c.length_km, 'circuit lengths');
      count(c.name, 'circuit names');
      g.circuits.push({ id:c.id, name:val(c.name), km, typed:typed(c.length_km) });
      g.km = Math.max(g.km, km || 0); }
    const list = [...groups.values()].sort((a, b) => (a.km || 0) - (b.km || 0));
    let maxKm = 1e-6; for (const g of list) maxKm = Math.max(maxKm, g.km || 0);
    const used = {};
    const STATION = 0.052;
    list.forEach((g, i) => {
      const di = i % DIRS.length, d = DIRS[di], lane = used[di] || 0; used[di] = lane + 1;
      const perp = [-d[1], d[0]], off = lane * 0.16;
      const len = neigh ? (0.30 + 0.62 * (g.km / maxKm)) : 0.46 + 0.05 * lane;
      const a = [d[0] * (STATION + 0.03), d[1] * (STATION + 0.03)];
      const b = [a[0] + d[0] * 0.10, a[1] + d[1] * 0.10];
      const c2 = [b[0] + perp[0] * off, b[1] + perp[1] * off];
      const e = [c2[0] + d[0] * len, c2[1] + d[1] * len];
      const pts = off ? [a, b, c2, e] : [a, e];
      const allTyped = g.circuits.every(c => c.typed);
      const dark = !!(out && (slug(out) === slug(g.id) || slug(out) === slug(g.label) || g.circuits.some(c => slug(c.id) === slug(out))));
      shapes.push({ role:'sldline', node:'line:' + g.id, id:site.id + '.line.' + g.id, pts, dist:g.km || 0,
                    colour:kvColour(g.kv), weight:1.0 + 1.5 * Math.min(4, g.circuits.length), assumed:!allTyped, dark,
                    km:g.km, n:g.circuits.length, far:g.id, farLabel:g.label,
                    says:g.label + ': ' + g.circuits.length + ' circuit' + (g.circuits.length === 1 ? '' : 's') + ' at ' + g.kv + ' kV, ' + (g.km || '?') + ' km' });
      // THE FAR STATION, a mark at the end of its line, and its name beside it.
      const r = 0.030, ring = [];
      for (let k = 0; k <= 28; k++) { const t = 2 * Math.PI * k / 28; ring.push([e[0] + r * Math.cos(t), e[1] + r * Math.sin(t)]); }
      shapes.push({ role:'sldstation', node:'line:' + g.id, id:site.id + '.station.' + g.id, pts:ring, closed:true,
                    colour:kvColour(g.kv), weight:1.4, assumed:!!g.farAssumed, dark, far:g.id, farLabel:g.label, dist:(g.km || 0) + 0.01,
                    says:g.label });
      words.push({ x:e[0], y:e[1] + (d[1] < -0.5 ? -0.062 : 0.062), text:g.label, small:g.label.length > 13 ? g.label.split(' ')[0] : null });
      conns.push({ from:site.label, to:g.label, node:'line:' + g.id, home:true,
                   metres:g.circuits.length + ' x ' + g.kv + ' kV, ' + (g.km || '?') + ' km' + (dark ? ', OPEN' : '') });
    });
    // THE STATION ITSELF: one mark, and an INTERCHANGE ring when more than one voltage level meets.
    const inter = uniqKv.length > 1;
    const ringAt = (rad, tag, w) => { const pts = [];
      for (let k = 0; k <= 40; k++) { const t = 2 * Math.PI * k / 40; pts.push([rad * Math.cos(t), rad * Math.sin(t)]); }
      shapes.push({ role:'sldstation', node:'site', id:site.id + '.station.' + tag, pts, closed:tag === 'core',
                    colour:kvColour(uniqKv[0]), weight:w, assumed:false, dist:0, says:site.label + ' - ' + uniqKv.map(k => k + ' kV').join(', ') }); };
    ringAt(STATION, 'core', 2.2);
    if (inter) ringAt(STATION + 0.026, 'interchange', 1.4);
    words.push({ x:0, y:-STATION - (inter ? 0.10 : 0.075), text:site.label });
    // THE PULSE: out from the station along every line, shortest first. The engine's own neon front
    // walks these legs. It shows CONNECTIVITY, not power: no load flow is run here.
    const legs = [];
    for (const s of shapes) if (s.role === 'sldline' && !s.dark) legs.push({ s, len:Math.max(1e-6, polyLen(s.pts).total) });
    let tot = 0; for (const L of legs) tot += L.len;
    let c3 = 0; for (const L of legs) { L.u0 = c3 / Math.max(1e-9, tot); c3 += L.len; L.u1 = c3 / Math.max(1e-9, tot); }
    let maxD = 1e-6; for (const s of shapes) maxD = Math.max(maxD, s.dist || 0);
    for (const s of shapes) s.order = 0.5 + 0.5 * ((s.dist || 0) / maxD);
    let x0 = 1e9, x1 = -1e9, y0 = 1e9, y1 = -1e9;
    for (const s of shapes) for (const p of s.pts) { x0 = Math.min(x0, p[0]); x1 = Math.max(x1, p[0]); y0 = Math.min(y0, p[1]); y1 = Math.max(y1, p[1]); }
    const midX = (x0 + x1) / 2;
    words.push({ x:midX, y:y1 + 0.135, text:site.label + '  -  ' + uniqKv.map(k => k + ' kV').join(' / ') + ', ' + list.length + ' ways out' });
    words.push({ x:midX, y:y0 - 0.085, text:'A line is a circuit route, its colour the voltage and its weight how many circuits run that way.  SOLID = from the data.  DASHED = assumed.', note:true });
    words.push({ x:midX, y:y0 - 0.125, text:String(DATA().attribution || ''), note:true });
    words.push({ x:midX, y:y0 - 0.168, text:DEV_WARNING_SUB, danger:true, pinned:true });
    return { kind:'string', substation:true, tube:true, shapes, marks, words, conns, terminals, wiring:false, crossings:[],
             perM:1, open:false, g:null, assumed, total, tables:[...tables], site, nl, groups:list, outage:out,
             bounds:{ x0:x0 - 0.20, y0:y0 - 0.20, x1:x1 + 0.20, y1:y1 + 0.17 }, near:{ x:midX, y:(y0 + y1) / 2 }, mPerUnit:1,
             firstModuleX:midX, amps:12, front:-1, head:-1, walk:(legs.length ? { legs, total:tot } : false), t0:0 };
  }
  // ---- THE COMMAND ------------------------------------------------------------------------------
  const INPUTS = [
    { key:'site', label:'Substation', kind:'choice', choices:(DATA().sites || []).slice(0, 40).map(s => [s.label, s.label]) },
    { key:'neighbours', label:'Far ends', kind:'choice', choices:[[0, 'even spacing'], [1, 'lengths to scale']] },
    { key:'pulse', label:'Pulse', kind:'choice', choices:[[0, 'still'], [1, 'run the front out']] },
    { key:'outage', label:'Line out', kind:'choice', choices:[['', 'none']] }
  ];
  F.register('substation', 'THE GRID', function (i) {
    const D = DATA();
    const site = findSite(i.site);
    if (!site) throw new Error('no substation called "' + i.site + '" in this sandbox. Try one of: ' + fiveNames());
    const nl = (D.netlists || {})[site.id];
    if (!nl) throw new Error(site.label + ' is named at the end of a circuit but has no netlist here yet, so it cannot be opened. Try one of: ' + fiveNames());
    checkNetlist(nl);
    const g = { c:{ view:'macro', sheet:i.sheet ? 1 : 0 }, pitch:0.30 };
    const d = buildMap(nl, site, { neighbours:!!i.neighbours, outage:i.outage || '' });
    d.g = g; g.__draw = d;
    const k = d.assumed, n = d.total, outLine = i.outage ? String(i.outage) : '';
    const drawable = d.groups.filter(x => (D.netlists || {})[x.id]).map(x => x.label);
    const notDrawable = d.groups.filter(x => !(D.netlists || {})[x.id]).map(x => x.label);
    const said = String(site.label) + ': ' + d.groups.length + ' ways out, ' + (nl.conductors || []).length + ' circuits, '
      + [...new Set((nl.busbars || []).map(b => val(b.kv)))].sort((a, b) => b - a).map(v => v + ' kV').join(' and ') + '. '
      + k + ' of ' + n + ' values assumed (' + (d.tables.join(', ') || 'none') + '), and those lines are drawn dashed. '
      + (outLine ? outLine + ' is OPEN: that line carries nothing and its far station goes dark; every other way out still lights. ' : '')
      + 'The pulse shows CONNECTIVITY, not power: no load flow is run here.';
    return { numbers:{ ways_out:d.groups.length, circuits:(nl.conductors || []).length,
                       voltage_levels:[...new Set((nl.busbars || []).map(b => val(b.kv)))].length,
                       values_assumed:k, values_typed:n - k },
             said,
             how:'the netlist (' + (nl.schema || 'netlist') + ') drawn as a map, not as the diagram an engineer would draw: a station, an interchange ring where two voltages meet, one coloured line a circuit route. '
               + String(D.attribution || '')
               + ' Far ends that can be opened with  fire substation {"site":"..."} : ' + (drawable.join(', ') || 'none in this sandbox yet')
               + (notDrawable.length ? '. Named but not yet in this sandbox: ' + notDrawable.join(', ') : ''),
             substation:g, connections:d.conns, electrical_order:'out from the station, shortest circuit first' };
  }, INPUTS);
  try { if (root.FIRE_SLD_KIND) root.FIRE_SLD_KIND['substation'] = 'string'; } catch (_) {}
})(typeof window !== 'undefined' ? window : globalThis);
try { FIRE_SLD_KIND['substation'] = 'string'; } catch (_) {}

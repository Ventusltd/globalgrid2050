...
# Explicit priority-source sweeps so the newspaper does not depend only on generic discovery.
PRIORITY_SOURCES={
 'BBC':'bbc.co.uk',
...
for domain in PRIORITY_SOURCES.values():
    SOURCE_QUERIES += [
        f'site:{domain} UK solar farm MW',
        f'site:{domain} UK solar park MW',
        f'site:{domain} UK battery storage MW',
        f'site:{domain} UK BESS MW',
...
def fetch_rss(q):
    ...
    r=requests.get(url,headers={'User-Agent':'GlobalGrid2050/5.1 (+https://globalgrid2050.com/)'},timeout=25)
    r.raise_for_status()
...
def source_bonus(source,url):
    ...
    if any(x in s for x in ['solar power portal','energy storage news','energy storage news uk','pv magazine','bbc']): return 20
...
def batched_queries(projects):
    ...
    for cat in ('solar','bess'):
        ...
        for n in range(0,len(names),BATCH_SIZE):
            ...
            # Repeat each project-name batch against every priority publication.
            for domain in PRIORITY_SOURCES.values():
                qs.append('('+ors+') '+suffix+' site:'+domain)
    return qs
    ...
    for n,q in enumerate(queries):
        try:
            for s in fetch_rss(q):
                ...
                if s['link'] not in seen and key not in seen:
                    seen.add(s['link']); seen.add(key); raw.append(s)
...
def main():
    ...
    NEWS_OUT.write_text(json.dumps({
        'schema':'globalgrid2050.major-project-news.v5','updated':now,
        'thresholds':{'solar_mw_exclusive':SOLAR_MIN_EXCLUSIVE,'bess_mw_exclusive':BESS_MIN_EXCLUSIVE},
        'eligible_projects':len(projects),'headline_count':len(headlines),
        'priority_sources':list(PRIORITY_SOURCES.keys()),
        'method':'REPD eligibility -> broad + explicit BBC/Solar Power Portal/Energy-Storage.News/PV Magazine sweeps -> batched project matching -> deterministic scoring -> headline dedupe',
        'items':headlines
    },indent=2),encoding='utf-8')
    print('eligible',len(projects),'headlines',len(headlines),'priority_sources',','.join(PRIORITY_SOURCES))

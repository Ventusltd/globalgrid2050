'use strict';
// VENTUS-programs.js - a cartridge of the VENTUS shell. Cut from the whole page with no change of behaviour;
...
// ===== THE BODY: a smaller VENTUS of one application's own work =====================================
...
void main(){ if (length(gl_PointCoord - 0.5) > 0.5) discard; vec3 c = u_on;
  ...
  o = vec4(c, 1.0); }`;
// THE GAP, entered. The nearest two centres among the places the module returned, found with a grid
...
function makeBody(st){
  ...
  // the middle, so the oldest work is the middle of the small VENTUS and the newest is its rim.
  ...
  for (const step of seq) { const shape = step.shape === 'text' ? 'text:' + nameInLines(st.app.button) : step.shape;
    ...
    // two words, and a scatter that means nothing is worse than the small VENTUS it would replace.
...
// the dot's radius ON THE SCREEN, from the entered gap and the zoom of this moment: a third of the
...
// point size is already in the units the gap is entered in. Multiplying by the screen's pixel ratio
...
// A THIRD OF THE GAP AS THE DOTS ARE NOW. At rest the gap is the state's own; in flight it is entered
...
function bodyDotPx(){ const gap = body.sld ? body.sldGap : body.live !== undefined ? body.live : body.gap;   // a diagram's dot is sized by the spacing along its lines, where a junction would make every dot a speck      // entered every time the dots are placed
...
const asmReady = import(new URL('assemble.mjs', document.baseURI).href).then(m => { ASM = m; })
  .catch(e => { ASM = null; asmWhy = String(e && e.message || e); console.error('[VENTUS] the gathering animation could not be loaded:', asmWhy); });
...
(function (root) {
  ...
  const DEF = { modules: 30, width_m: 1.303, module_height_m: 2.384, cell_cols: 6, cell_rows: 22, module_vmp: 38.1, module_voc: 45.9,
                ...
                // BUT A NARROW TELEPHONE OPENS ON MACRO. Below 500 px the micro view is set by the width
                // of four modules, which on a 390 px screen leaves a fifth of the screen dark under the
                // table and shows the reader an inverter and one module instead of a string. So the
                // narrow screen opens on the WHOLE string, fitted to the width, and the near end becomes
                // what the reader asks for - by the chip, by the pinch, or by typing view micro.
                mounting: 'fixed', orientation: 'portrait', modules_high: 1, routing: '',
                view: (typeof innerWidth === 'number' && innerWidth >= 500 && innerWidth < 700) ? 'micro' : 'macro', sheet: 0,
  ...
  function build(inp) {
    ...
    // needs are entered first, and only then is the lead pair settled.
    const links = [];
    ...
    // NO SILENT SUBSTITUTE: a size the table has no number for is refused in words, never costed as another size's copper.
    const sizeWords = Object.keys(R20).map(Number).sort((a, b) => a - b).join(', ') + ' mm2';
    const needSize = (what, mm2, give) => { if (R20[mm2] === undefined) throw new Error(what + ' ' + mm2 + ' mm2 has no resistance in the table: the sizes with a number are ' + sizeWords + ', or give "' + give + '" yourself'); };
    const leadGiven = !(c.ohm_per_km_lead === null || c.ohm_per_km_lead === undefined), siteGiven = !(c.ohm_per_km_site === null || c.ohm_per_km_site === undefined);
    if (!leadGiven) needSize('the lead size', c.lead_mm2, 'ohm_per_km_lead');
    if (!siteGiven) needSize('the site cable size', c.cable_mm2, 'ohm_per_km_site');
    const rLead = leadGiven ? +c.ohm_per_km_lead : R20[c.lead_mm2];
    const rSite = siteGiven ? +c.ohm_per_km_site : R20[c.cable_mm2];
    const hrSize = (c.home_run_mm2 === null || c.home_run_mm2 === undefined) ? c.cable_mm2 : c.home_run_mm2;
    if (!siteGiven) needSize('the home run size', hrSize, 'ohm_per_km_site');
    const rHome = siteGiven ? rSite : R20[hrSize];
  ...
  // Before a single coordinate is worked out, every value that was TYPED is read and entered. A
  ...
  function run(inp) {
    ...
    if (stopped.length) {
      ...
      return { refused: true, string: null, connections: [],
               ...
               how: 'every value typed into this command is read and entered before a single coordinate is worked out. A value that is not a number, or a number outside what that input can mean, would put a NaN into the drawing, so it is refused in words instead. Change the input named above and fire again.',
...
//   solar-block/block.mjs  (solar-block/1.0.0, 68 of 68 assertions pass in
...
(function (root) {
  ...
  function buildBlock(typed = {}) {
    ...
    push('insulation resistance threshold for this block', insulation.value_Mohm, 'Mohm', insulation.value_Mohm,
      ...
      `Block is ${r(kWp, 2)} kWp; the clause is entered per string or sub-array, not across the block.`);
  ...
  function blockDrawing(BK) {
    ...
    // ---- entered, then laid out the way the row is: as tall as the disc, as wide as it needs to be
...
// ===== THE WAFER'S OWN BAR AND WINDOW, LIFTED ONTO THE VENTUS ====================================
...
const FIRE_CONFIG = './'; // Generic release: remote command catalogue disabled.
...
const STRING_TESTS = [
  ...
  // THE PLANT DRAWING HAD NO WAY IN. The `site` verb has existed since i0092
  // and draws the whole plant, and it appeared in no menu, no chip, no example
  // list and no help text - so the only way to reach it was to be told the
  // command. A reader who is not told sees an empty prompt and leaves. This
  // list is the one guaranteed to render: it feeds both the chips and the
  // examples and is never filtered or replaced by the remote fetch.
  { family:'STRINGS', pin:2, name:'The whole plant in one drawing', chip:'whole plant', sentence:'The whole plant in one drawing: the grid, the interface, one transformer down to the collection busbar, and every inverter block hanging off it', command:'fire site {}' },
...
function makeFireBar(){
  ...
  st.textContent = ':root{--cmd:56px;--footh:34px;--stack:calc(var(--cmd) + var(--footh) + 8px);--egh:0px;--above:calc(var(--stack) + 8px)}'
    ...
    // the bottom of its page, under the one line that says what the page is not, and the VENTUS's shell
    ...
    // ON A PHONE THE WINDOW COVERED THE THING IT DESCRIBED. It opens
    // minimised there, so the button labelled minimise GREW it on the first
    // press, and un-minimised it took the screen. Docked low at 38vh the
    // numbers and the drawing are visible together, which is the only state
    // in which either is worth reading.
    + '@media (max-width:700px){#say.open:not(.min):not(.max){top:auto!important;bottom:calc(var(--footh) + 8px)!important;left:8px!important;width:calc(100vw - 16px)!important;max-height:38vh!important}}'
    ...
    // THE BOTTOM OF THE SCREEN IS A STACK, NOT A PILE - the wafer's own rule, and the VENTUS's card has to
    ...
    // off the screen. The card is lifted to the top of the entered stack instead.
    ...
    + '#saybar button{width:44px;height:40px}}';
  ...
  sayOpen = showSay;
  const setMin = on => { sayEl.classList.toggle('min', !!on);
    // THE LABEL FOLLOWS THE STATE. It said minimise while it expanded,
    // because the window is born minimised on a phone and the handler only
    // toggled. One setter owns the class and the wording.
    const b = $('saymin'); if (!b) return;
    b.textContent = on ? '+' : '–';
    b.title = on ? 'expand' : 'minimise';
    b.setAttribute('aria-label', b.title); };
  $('saymin').onclick = e => { e.stopPropagation();
    setMin(!sayEl.classList.contains('min')); };
  ...
  // THE STACK IS entered, NOT GUESSED, exactly as the wafer measures it, so the picker never sits on
  ...
  Promise.resolve({ok:false,reason:'Generic drawing uses local controls only'}).then(r => {
    if (r.ok) {
...
function placeSldNames(){
  ...
  // AN "off" LINE IS THE LONG ARITHMETIC KEY. It is held back on a telephone, where it would wrap
  // over the drawing, but a desk has the room and the reader came for the numbers: at desk width it
  // shows, in its long form, with the rest of the pinned band.
  if (sldNames.some(n => n.wide || n.narrow || n.off)) sldNames = sldNames.filter(n => !(n.off && small) && (small ? !n.wide : !n.narrow));
  ...
  // THE PINNED LINES ARE STACKED FROM THE BOTTOM UP, each one entered after it has wrapped.
...
// of the screen is entered - what the foot takes, what the chips take, and where the window is docked -
...
// THE ELECTRICAL WALK, READ OFF THE DRAWING ITSELF. Nothing is entered or placed here: every
...
const DEV_WARNING = 'Not real cable or array sizing - consult a design engineer. DEVELOPMENT ENVIRONMENT: estimates from a model, nothing entered.';
...
function stringDrawing(g){
  ...
  // ---- everything entered, then put into the page's units with the row laid out LARGE
  let x0 = 1e9, x1 = -1e9, y0 = 1e9, y1 = -1e9;
  const see = (x, y) => { if (x < x0) x0 = x; if (x > x1) x1 = x; if (y < y0) y0 = y; if (y > y1) y1 = y; };
...
function drawSldShapes(){
  ...
  // WORDS ARE DROPPED, NEVER OVERcableED. The polarity marks and the module names go down first, then
...
/* ===========================================================================
   SITE - the whole plant as one drawing, anonymised
   ---------------------------------------------------------------------------
   A TUBE MAP, NOT A SINGLE LINE DIAGRAM. A single line diagram is drawn where
   the plant is; a tube map is drawn where the reader's eye goes. Nothing here
   carries a coordinate, a length, a bearing or a name, so nothing here can
   identify a site: the spacing is the VENTUS's, not the ground's.

   WHAT IT DRAWS, top to bottom: the grid, the interface, one transformer down
   to the collection voltage, the collection busbar, and one column per block -
   feeder, switch, skid transformer, inverter, and the DC that feeds it.

   IT DOES NOT DRAW EVERY STRING. A tube map does not draw every train. The
   strings arrive as a comb whose teeth are GROUPS, and the real count is said
   in words. Drawing two dozen lines per block eight times over would break the
   one rule this renderer does not bend: a dot's nearest neighbour is three dot
   radii away, or the sky stops being black.

   THE COLOUR IS THE ARITHMETIC, NOT A MOOD. A column goes amber when a test
   says that design is outside a rating. Nothing is coloured by hand.

   EVERY NUMBER IS CARRIED, NOT REMEMBERED. An earlier draft of this program
   hard-coded one temperature coefficient and used it with another class's
   open-circuit voltage. It was wrong by 0.03 %/K, which is 14.5 V at thirty in
   series - on a design whose entire margin is a couple of volts. The bins now
   travel with the program and the coefficient comes from the bin.
   =========================================================================== */
(function (root) {

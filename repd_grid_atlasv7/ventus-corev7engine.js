...
// ── NAEI Emitter News/Images threshold ──────────────────────────────────────
const NAEI_SEARCH_THRESHOLD_TCO2E = 50000; // Only show buttons on major emitters
...
// ── NAEI Emitter search buttons (Google News + Images) ──────────────────────
function buildNaeiSearchButtons(name, operator, tonnes) {
    if (!tonnes || tonnes < NAEI_SEARCH_THRESHOLD_TCO2E) return '';
    // Build a specific query: site name + operator helps disambiguate common names
    // (e.g. there are multiple "Saltend" sites, operator pins it to the right one)
    const opClean = operator && operator.toLowerCase() !== 'unknown' ? ` ${operator}` : '';
    const q = encodeURIComponent(`${name}${opClean} UK`);
    const newsUrl  = `https://news.google.com/search?q=${q}`;
    const imageUrl = `https://www.google.com/search?q=${q}&tbm=isch`;
    return `<div class="popup-search-btns">
        <a class="popup-btn popup-btn-news" href="${newsUrl}" target="_blank" rel="noopener noreferrer">📰 NEWS</a>
...
map.on('load', () => {
    ...
    map.on('click', e => {
        ...
        if (p.type === 'naei_emitter') {
            const tonnesNum = parseFloat(p.emission_tco2e) || 0;
            ...
            const naeiSearchBtns = buildNaeiSearchButtons(name, p.operator, tonnesNum);

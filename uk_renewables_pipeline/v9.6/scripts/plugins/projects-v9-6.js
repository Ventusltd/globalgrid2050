...
} from "../core/project-filter-v9-6.js";
import { loadCanonicalProjectsV9_6 } from "../data/canonical-projects-v9-6.js";
import {
  formatCapacityV9_2,
  formatLargestV9_2,
...
import { signalForProjectV9_6 } from "./newspaper-v9-6.js";
...
export function compareProjectUpdatesV9_6(left, right, direction = "desc") {
...
function updateSortHeader() {
  ...
  const mobileButton = document.getElementById("mobileSortUpdated");
  ...
  if (sortMode === "updated_desc") {
    ...
    if (mobileButton) mobileButton.textContent = "REPD UPDATED: NEWEST ▼";
    return;
  }
  if (sortMode === "updated_asc") {
    ...
    if (mobileButton) mobileButton.textContent = "REPD UPDATED: OLDEST ▲";
    return;
  }
  ...
  if (mobileButton) mobileButton.textContent = "SORT REPD UPDATED ↕";
}
...
function syncFilterUrl() {
  ...
  for (const parameter of ["technology", "status", "county", "q", "sort", "min_mw", "max_mw"]) url.searchParams.delete(parameter);
  ...
  if (range.valid && range.minMW !== null) url.searchParams.set("min_mw", String(range.minMW));
  if (range.valid && range.maxMW !== null) url.searchParams.set("max_mw", String(range.maxMW));
  history.replaceState(null, "", url);
}

function updateCapacityStatus(range) {
  const meta = document.getElementById("capacityMeta");
  const minInput = document.getElementById("minCapacity");
  const maxInput = document.getElementById("maxCapacity");
  if (meta) {
    meta.textContent = capacityRangeLabelV9_6(range);
    meta.classList.toggle("invalid", !range.valid);
  }
  for (const input of [minInput, maxInput]) {
    if (input) input.setAttribute("aria-invalid", String(!range.valid));
  }
}
...
function apply({ syncUrl = true, resetVisible = true } = {}) {
  ...
  filtered = range.valid ? all.filter((project) => projectMatchesV9_2(project, {
    ...
    county,
    tokens,
  }, searchIndex.get(project.repd_ref)) && projectMatchesOfficialCapacityV9_6(project, range)) : [];
  if (sortMode === "updated_desc") filtered.sort((left, right) => compareProjectUpdatesV9_6(left, right, "desc"));
  if (sortMode === "updated_asc") filtered.sort((left, right) => compareProjectUpdatesV9_6(left, right, "asc"));
  state.filtered = filtered;
  updateGaugesV9_2(filtered);
  renderTable();
  updateResultSummary();
  updateSortHeader();
  updateCapacityStatus(range);
...
function hydrateFiltersFromUrl() {
  ...
  minCapacity = parameters.get("min_mw") || "";
  maxCapacity = parameters.get("max_mw") || "";
...
function downloadCsv(event) {
  ...
  anchor.href = url;
  anchor.download = `globalgrid2050_uk_renewables_pipeline_v9_6_${new Date().toISOString().slice(0, 10)}.csv`;
  anchor.click();
...
export async function loadProjectsV9_6() {
  try {
    const model = await loadCanonicalProjectsV9_6();
    all = [...model.projects];
    filtered = all;
    ...
    apply({ syncUrl: false });
    document.getElementById("releaseMeta").textContent = `V9.6 interface · V${release.data_parent.release} canonical data spine · all ${all.length.toLocaleString("en-GB")} qualifying records loaded`;
  } catch (error) {
    console.error(error);
    document.getElementById("tbody").innerHTML = '<tr><td colspan="11" style="text-align:center;color:#ff6666">Canonical Q2 REPD data unavailable. V9.6 has failed closed.</td></tr>';
    document.getElementById("resultsMeta").textContent = "canonical data unavailable";
  }
}

export function refreshProjectsV9_6() {
  if (metadata) renderTable();
}

export function bindProjectControlsV9_6() {
  ...
  document.getElementById("mobileSortUpdated").onclick = () => {
    sortMode = sortMode === "updated_desc" ? "updated_asc" : "updated_desc";
    document.getElementById("sortProjects").value = sortMode;
    apply();
  };
  document.getElementById("search").oninput = (event) => { query = event.target.value.trim(); apply(); };
  const scheduleCapacityApply = () => {
    clearTimeout(capacityTimer);
    capacityTimer = setTimeout(() => apply(), 160);
  };
  document.getElementById("minCapacity").oninput = (event) => {
    minCapacity = event.target.value.trim();
    scheduleCapacityApply();
  };
  document.getElementById("maxCapacity").oninput = (event) => {
    maxCapacity = event.target.value.trim();
    scheduleCapacityApply();
  };

...
export function buildStation({kva=352,lengths={},ambient=30,baseAmpacity=null,derating=1,r20=.0778,conductorTemp=20,alpha=.00403,racFactor=null,reactance=null,pf=1}={}) {
 ...
 for(let s=0;s<2;s++){
  ...
  for(let i=0;i<14;i++){
   const id='INV-'+String(s*14+i+1).padStart(2,'0');ids.push(id);inverters.push({id,sectionId,model:'352 kVA inverter',kva,lineCurrentA:lineCurrent});
   for(let p=1;p<=3;p++){const cableId=id+'-L'+p,value=lengths[cableId]??0;if(!Number.isFinite(value)||value<0||value>10000)throw Error('Cable length outside range');const rdc=value?rTheta*value/1000:null;cables.push({id:cableId,inverterId:id,sectionId,phase:'L'+p,areaMm2:400,shieldMm2:39,material:'Al',part:'reference',lengthM:value||null,ambientC:ambient,baseAmpacityA:baseAmpacity,derating,deratedAmpacityA:baseAmpacity===null?null:baseAmpacity*derating,currentA:lineCurrent,resistanceDcOhm:rdc,dcResistanceLossW:rdc===null?null:lineCurrent**2*rdc,acResistanceLossW:rdc===null||racFactor===null?null:lineCurrent**2*rdc*racFactor});}
  }
  sections.push({id:sectionId,inverters:ids,phases:['L1','L2','L3'],busRatingA:4000,lineCurrentA:14*lineCurrent,kva:14*kva,transformer:'TX-'+(s+1),transformerRatingKva:null});
 }
 ...
 return {schema:'VENTUS.inverter-station/1',voltageLL:800,kvaPerInverter:kva,inverters,cables,feeders,sections,transformers:sections.map(s=>({id:s.transformer,from:s.id,ratedKva:null,connectedKva:s.kva})),totalMva:28*kva/1000,lineCurrentA:lineCurrent,ambientC:ambient,baseAmpacityA:baseAmpacity,derating,electricalInputs:{r20,conductorTemp,alpha,racFactor,reactance,pf,rTheta},sources,cableSpecificationStatus:'Entered generic 400 mm² aluminium phase-cable scenario; validate actual cable construction and installation before use',scope:'Phase connectivity, nameplate arithmetic and entered-length conductor scenario. No protection or cable suitability verdict.'};
}
let entered={},playing=false,model=buildStation();
...
function render(){
 ...
 txt(svg,'28 × 352 kVA inverter  /  800 V THREE-PHASE AC',40,32,18,'#8edfe7');txt(svg,'NOT TO SCALE',1730,32,23,'#b9b6ab');
 const colours=['#ff927a','#c6ceda','#81cce8'];
 model.sections.forEach((section,s)=>{
  const top=75+s*415;txt(svg,'SECTION '+(s+1)+'  /  14 INVERTERS  /  42 INDIVIDUAL PHASE CABLES',40,top,18,'#95e3e7');
  ...
  section.inverters.forEach((id,i)=>{
   const x=45+i*112,y=top+35;svg.append(el('rect',{x,y,width:92,height:83,rx:2,fill:'#111c28',stroke:'#8ba6bb','data-inverter':id}));
  ...
  txt(svg,(section.kva/1000).toFixed(3)+' MVA',1786,ty+64,16);txt(svg,'CONNECTED INPUT',1786,ty+82,11);txt(svg,'RATING / HV / VECTOR',1727,ty+130,11);txt(svg,'GROUP: UNASSIGNED',1727,ty+145,11);
 });
 ...
 $('total-rating').textContent=model.totalMva.toFixed(3)+' MVA';
 $('arithmetic').textContent='I = S / (√3 × 800 V) = '+model.lineCurrentA.toFixed(3)+' A per inverter; 14 × I = '+model.sections[0].lineCurrentA.toFixed(2)+' A per phase per section. Each section: '+(model.sections[0].kva/1000).toFixed(3)+' MVA.';

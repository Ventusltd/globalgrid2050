...
(function () {
  ...
  // ---- THE SEQUENCE: CINEMATIC ON ARRIVAL, NEAR-INSTANT AFTER IT ---------------------------------
  // THE DELAY IS DESIGNED, NOT SLOWNESS. Nothing blocks the thread while it runs; TWO NUMBERS hold the
  // clock, and until now both were fixed for every command of a visit:
  //   the ring sweep   isoAnim.ms, set in isolate() from the fired program's own pulse.seconds
  //                    (FIRE_PROGRAM in kuiper-programs.js: seconds 1.0 -> 1000 ms; a pressed program 1.5 -> 1500 ms)
  //   the gathering    ASM.MOVE_MS, 2000 ms, declared in assemble.mjs beside the shell
  // 1000 + 2000 = 3.0 s before a fired drawing can be read, every single time.
  // ARRIVAL IS WORTH THREE SECONDS; THE SECOND COMMAND IS NOT. The first pulse of a visit keeps the
  // cinematic exactly as it was. Every command after it uses the short pair below. Both numbers are
  // here, named, in one object, so the distinction is a number to change and not a rewrite.
  const SEQUENCE = {
    arrival: { sweepMs: null, gatherMs: null },   // null: whatever the program and the module already say
    after:   { sweepMs: 140,  gatherMs: 220 }     // near-instant: still a move, not a jump cut
  };
  window.KUIPER_SEQUENCE = SEQUENCE;              // one place to tune it, from the console or from a test
  let arrived = false;                            // set by the first pulse of the visit, never reset

  // THE GATHERING. A module's namespace cannot be written to, so ASM is swapped for a copy of itself
  // whose MOVE_MS is asked for every frame. Everything else on it is the module's own function.
  let ASM_MOVE_MS = 2000;
  const gatherMs = () => (arrived && SEQUENCE.after.gatherMs != null) ? SEQUENCE.after.gatherMs : ASM_MOVE_MS;
  try {
    asmReady.then(() => {
      if (typeof ASM === 'undefined' || !ASM || ASM.__kuiperClock) return;
      ASM_MOVE_MS = ASM.MOVE_MS;
      const shim = Object.assign({}, ASM);
      Object.defineProperty(shim, 'MOVE_MS', { get: gatherMs, enumerable: true });
      shim.__kuiperClock = true;
      ASM = shim;
    });
  } catch (_) {}

  // THE RING SWEEP. isolate() sets the length of the sweep from the program it is about to run; this
  // shortens it AFTER the sweep has been set up, so the program's own declared seconds are untouched
  // and the arrival - the first one - runs at its full length.
  try {
    if (typeof isolate === 'function') {
      const origIsolate = isolate;
      isolate = function (name) {
        const out = origIsolate.apply(this, arguments);
        if (!arrived) arrived = true;
        else if (typeof isoAnim !== 'undefined' && isoAnim && SEQUENCE.after.sweepMs != null) isoAnim.ms = SEQUENCE.after.sweepMs;
        return out;
      };
    }
  } catch (_) {}
  ...
  /* THE CHIPS ARE ONE BUTTON. A scrolling row of eight was still a row: it held the top of the screen
     and the band the drawing is laid into started under it. There is ONE small button now, "layouts",
     and the presets themselves are in the pop-up it opens - the same pop-up, no new widget. */
  #presets{top:20px !important;left:10px !important;flex-wrap:nowrap !important;overflow-x:auto;overflow-y:hidden;
  ...
  #chipanswer{display:none !important}
  #presets button{min-height:26px;padding:2px 9px;font-size:11px}
  #chipmore.pulse{animation:kpulse 1s ease-out 1}
  ...
  #sldnames span.danger{position:fixed;left:0 !important;right:0 !important;top:0 !important;
    ...
    text-align:center;padding:2px 8px;font-size:9.5px !important;line-height:1.5;letter-spacing:.04em;
    background:#160b0a;border-bottom:1px solid #4a2019;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}

  /* THE SAME RED LINE BEFORE ANYTHING IS DRAWN. The engine writes its warning beside a study, so the
     first screen - the disc, nothing fired - carried no warning at all. This line is the chrome's own,
     it is there from the first paint, and it steps aside the moment the engine's own line appears. */
  #kdev{position:fixed;left:0;right:0;top:0;z-index:9;text-align:center;padding:2px 8px;
    font:9.5px/1.5 ui-monospace,Menlo,Consolas,monospace;font-weight:600;letter-spacing:.04em;color:#ff5a4f;
    background:#160b0a;border-bottom:1px solid #4a2019;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}

  /* THE KEY, COLLAPSED, ON EVERY SIZE. The pinned lines under the drawing become one line above the
     bar with "more". On a desk they used to stay pinned, and the band the drawing is laid into was
     shortened by whatever they took - eighty pixels of key under a picture that had asked for the
     screen. One line above the bar says the same thing and costs fifteen. */
  ...
  #keyline{display:none;align-items:baseline;gap:8px;font:10px/1.35 ui-monospace,Menlo,Consolas,monospace;
    color:#8b93a7;padding:0 0 3px;border-bottom:1px solid #141a26;margin-bottom:3px}
  ...
  /* THE BAR, AND THE FOOT THAT HOLDS IT. Everything below the drawing - the key line, the line that
     says what this is not, and the bar - is a hundred pixels, not two hundred. The drawing gets the
     rest, on a telephone and on a desk. */
  #pilot{margin-top:4px;padding-top:4px}
  #foot{padding:5px 12px !important}
  #foot .short{margin-top:3px;font-size:9px;line-height:1.3}
  ...
  #kt{padding:.5rem .7rem;border-bottom:1px solid #1b2030;background:#0c1017;max-height:46vh;overflow:auto}
  #say.min #kt{display:none}
  #say #saybody{min-height:110px}
  ...
  @media (max-width:700px){
    #foot{padding:4px 10px !important}
    #foot .short{margin-top:3px;font-size:8.5px;line-height:1.25}
    #presets{top:20px !important;gap:5px}
    #presets button{min-height:26px;padding:2px 9px;font-size:11px}
    #pilot input{min-height:36px !important;padding:.35rem .8rem}
    #say.open:not(.max){left:0 !important;right:0 !important;top:auto !important;
      ...
      max-height:60vh !important;height:auto !important;transform:none !important;
      border-radius:10px 10px 0 0;border-left:0;border-right:0;resize:none}
    #say.open:not(.max) #kt{max-height:none}
  }`;
  ...
  function boot() {
    ...
    const shut = () => { sayEl.classList.remove('open', 'max', 'min'); relayout(); };
    const loosen = () => { sayEl.style.left = ''; sayEl.style.top = ''; sayEl.style.width = '';
      sayEl.style.maxHeight = ''; sayEl.style.transform = ''; };
    // THE CARD OPENS AS ITS TITLE BAR. A card that opens full is the drawing gone, and that is what a
    // reader saw the moment they touched the bar to type. FOLDED it is one line - the wafer's own title
    // bar, which already expands on a tap and is still draggable - so the picture stays whole and the
    // card is one tap away, never two. A reader who ASKS for the commands still gets them in full; the
    // page never opens itself full over the drawing.
    const openSheet = (title, folded) => {
      if (title) { const t = $('saytitle'); if (t) t.textContent = title; }
      sayEl.classList.toggle('min', !!folded);
      if (PHONE()) loosen();                                    // the sheet's place is in the stylesheet
      sayEl.classList.add('open');
      relayout();
      try { if (window.__placeSay && !PHONE() && !folded) window.__placeSay(); } catch (_) {}
    };
    ...
    kt.append(el('p', 'kdim', 'Pick a preset below, or type a few plain words in the bar and press Enter.'));

    // THE PRESETS LIVE HERE NOW. They are the page's own chips, CARRIED, not rebuilt: every listener the
    // programs cartridge put on them is still on them. The face keeps one button that opens this.
    const presetRow = el('div', 'krow'); presetRow.append(el('p', 'klab', 'the presets')); kt.append(presetRow);
    ...
    // TYPE IT AND FIRE IT, exactly as a reader would. The word goes into the bar and the bar's own
    // Enter carries it, so a menu entry and a typed line take ONE path into the engine, never two.
    const typed = (text) => { if (!pin) return false; pin.value = text;
      pin.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true })); openSheet(null, true); return true; };
    ...
    const COMMANDS = [
      ...
      // ONE MORE COMMAND, NO NEW WIDGET: the systems of the estate, listed in this same window. Tapping
      // one fires its pulse through the engine and shuts the sheet, so the disc is what is seen.
      ['systems', 'the estate', 'systems', () => { showSystems(); return true; }],
      ['substation', 'the estate', 'substation', () => typed('fire substation')],
      ['neighbours', 'the estate', 'neighbours', () => typed('fire neighbours')],
      ['pulse', 'the estate', 'pulse', () => typed('fire pulse')]
    ];

    // THE LIST OF SYSTEMS, built into this window's own tools the first time it is asked for.
    let sysBox = null;
    function showSystems() {
      const S = window.KUIPER_SYSTEMS; if (!S) return false;
      if (!sysBox) {
        sysBox = el('details'); sysBox.append(el('summary', null, 'the systems of the estate'));
        const b = el('div', 'kkey'); sysBox.append(b);
        for (const s of S.all) {
          const line = el('button', null, s.id + '  ' + s.system + '  · ' + s.files_total + ' files');
          line.type = 'button'; line.dataset.sys = s.id;
          line.style.display = 'block'; line.style.textAlign = 'left'; line.style.width = '100%';
          line.addEventListener('click', e => { e.stopPropagation(); S.fire(s.id); shut(); });
          b.append(line);
        }
        kt.append(sysBox);
      }
      sysBox.open = true; openSheet();
      try { sysBox.scrollIntoView({ block: 'nearest' }); } catch (_) {}
      return true;
    }
    ...
    function fixChips() {
      if (!chips) return;
      if (!mine) { mine = el('button', 'konly', 'layouts'); mine.type = 'button'; mine.id = 'chipmore';
      ...
      if (!answerChip) { answerChip = el('button', 'konly', 'answer'); answerChip.type = 'button'; answerChip.id = 'chipanswer';
        ...
        answerChip.addEventListener('click', e => { e.stopPropagation(); openSheet(); }); }
      // every preset chip the page draws is MOVED into the pop-up, where the one button opens it
      const own = Array.from(chips.children).filter(b => b.id !== 'chipapps' && b !== mine && b !== answerChip);
      for (const b of own) { b.style.display = ''; presetRow.append(b); }
      // the button is the FIRST thing in the row, and it is the only one on the face
      if (chips.firstElementChild !== mine) chips.prepend(mine);
      if (answerChip.parentNode !== chips) chips.append(answerChip);
    }
    fixChips();

    // THE WARNING IS ON THE FIRST SCREEN. It is put up once, and hidden only while the engine's own
    // red line is on the page, so a reader never sees the sentence twice and never sees it not at all.
    const dev = el('div', null, 'Not real cable or array sizing - consult a design engineer. DEVELOPMENT ENVIRONMENT: estimates from a model, nothing measured.');
    dev.id = 'kdev';
    try { document.body.appendChild(dev); } catch (_) {}
    const devWatch = () => { try { const own = document.querySelector('#sldnames span.danger');
      dev.style.display = (own && own.getClientRects().length) ? 'none' : 'block'; } catch (_) {} };
    devWatch();
    try { setInterval(devWatch, 1000); } catch (_) {}
    ...
    pin.addEventListener('focus', () => openSheet(haveAnswer ? null : 'commands', true));
    ...
    if (foot) foot.addEventListener('keydown', e => {
      ...
      if (!q) return;
      // "system <id or name>" fires that one system's pulse at once. "systems" alone opens the list,
      // through the ordinary command below, so there is one way in and not two.
      if (/^system\s+\S/.test(q) && window.KUIPER_SYSTEMS) {
        e.stopPropagation(); e.preventDefault();
        window.KUIPER_SYSTEMS.fire(q.replace(/^system\s+/, '')); shut(); pin.value = '';
        return;
      }
    ...
    try {
      if (typeof sayOpen === 'function') {
        ...
        sayOpen = function (title, nodes, beside) {
          ...
          groupBoxes();
          readKey();
          if (mine) { mine.classList.remove('pulse'); void mine.offsetWidth; mine.classList.add('pulse'); }
        // THE ANSWER ARRIVES AS A TITLE BAR. It used to be taken off the screen altogether, which kept
        // the drawing whole and put the answer two taps away behind the layouts button. Folded, it is
        // on the page, it says what it is, and ONE tap on it opens it in full.
        if (!wasOpen) { sayEl.classList.remove('max'); sayEl.classList.add('open', 'min'); relayout(); }
    ...
    function readKey() {
      ...
      const lines = layer ? Array.from(layer.querySelectorAll('span.pinned'))
        .filter(s => !s.classList.contains('danger'))
        .map(s => (s.textContent || '').trim()).filter(Boolean) : [];
      keyBody.replaceChildren(...(lines.length ? lines.map(t => el('p', null, t))
        : [el('p', null, 'The key appears with the drawing.')]));
      keyText.textContent = lines.find(t => /ohm/.test(t)) || lines[0] || '';
      applyKey();
    }
    function applyKey() {
      ...
      keyRow.classList.toggle('on', has && keyOn);
      relayout();
    }
    ...
    try {
      if (typeof shapeBand === 'function') {
        ...
        window.shapeBand = function (reserve) {
          ...
          // FULL SCREEN. Not eighty-five per cent of it: the whole viewport. The canvas is already fixed
          // to all four edges, and the band the drawing is laid into is now that same rectangle, corner
          // to corner. The red line, the one button, the key line and the bar keep their own backgrounds
          // and sit OVER the picture; nothing is given a strip of its own any more.
          b.left = 0; b.right = innerWidth; b.top = 0; b.bottom = innerHeight;
          // A CARD OPENED IN FULL ON A PHONE STILL TAKES THE BOTTOM, because the reader asked for it.
          // Folded to its title bar it takes nothing.
          if (isOpen() && !sayEl.classList.contains('min') && PHONE()) { const r = sayEl.getBoundingClientRect();

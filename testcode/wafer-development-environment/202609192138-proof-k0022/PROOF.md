...
- Links on the card are now as short as they can be: the application, "GitHub" as one word, and
  www.globalgrid2050.com whenever there is nothing else to offer. No card is a dead end, and no
  link reads like a sentence.
- Work on the drawing engine now leads to the London Underground drawing it makes, instead of only
  to its history. The cable geometry tool now opens from its own code as part of CABLES.
- Nine applications can be opened from the card. Each was checked before release: its work is
  public and its address answers.
...
Score 20 of 20, 2026-09-19T21:27:36Z.

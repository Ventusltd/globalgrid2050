...
function fireCommand(text, rec){
  ...
  if (out.name === 'string') { const row = el('div', 'viewrow');
    ...
    nodes.push(row);
    if (out.electrical_order) nodes.push(el('p', 'dimtext', 'Electrical order: ' + out.electrical_order)); }
...
function amberName(tag){
  ...
  if (tag.startsWith('short')) return 'the circuit is open here: no current anywhere in this string';
...
function makeSldNames(lay, net){
  ...
  if (net.kind === 'string') { const d = net.g.__draw || stringDrawing(net.g);
    const out = d.words.map(w => ({ x:w.x, y:w.y, dy:0, dx:0, text:w.text, warn:false, note:!!w.note }));
    ...
    if (d.open) out.push({ x:d.bounds.x0 + (d.bounds.x1 - d.bounds.x0) * 0.5, y:d.bounds.y0, dy:0.12,
      text:'the circuit is open: ' + d.g.short.length + ' links do not reach, so no current flows anywhere in this string', warn:true });
    return out; }
...
function placeSldNames(){
  ...
  list.forEach((n, i) => { const sp = el.children[i]; if (!sp) return;
    ...
    if (px0 < -260 || px0 > innerWidth + 260 || py0 < -60 || py0 > innerHeight + 60) { sp.style.visibility = 'hidden'; return; }
...
function stringOpenView(){
  const D = sldShapes; if (!D || D.opened) return; D.opened = true;
  const R = Math.sqrt(SPACE), f = BODY_RADIUS_MAX, span = stringViewSpanM() * D.perM;
  const z = Math.max(1e-6, innerWidth * 0.94 / (span * R * f));
  if (typeof flyTo === 'function') flyTo((D.near.x + span * 0.5) * R * f, D.near.y * R * f, z);
}
function stringWholeView(){
  const D = sldShapes; if (!D) return;
  const R = Math.sqrt(SPACE), f = BODY_RADIUS_MAX, b = D.bounds;
  const w = Math.max(1e-6, b.x1 - b.x0), h = Math.max(1e-6, b.y1 - b.y0);
  const z = Math.min(innerWidth * 0.94 / (w * R * f), innerHeight * 0.70 / (h * R * f));
  if (typeof flyTo === 'function') flyTo((b.x0 + b.x1) / 2 * R * f, (b.y0 + b.y1) / 2 * R * f, Math.max(1e-6, z));
}
...
function stringDrawing(g){
  ...
  g.links.forEach((l, i) => {
    ...
    if (l.reaches) { const t = Math.min(0.92, Math.max(0.08, lp / Math.max(1e-6, g.pair)));
      ...
      put({ role:'gap', order:o, pot, node, pts:[[p0[0], p0[1]], [p1[0], p1[1]]], link:l, says,
            short:l.need - g.pair, mid:[(p0[0] + p1[0]) / 2, (p0[1] + p1[1]) / 2] }); }
  });
  ...
  words.push({ x:g.row / 2, y:botLane - 1.15, text:CABLE_NOTE, note:true });
  ...
  return { kind:'string', shapes, marks, words, perM:S, open, g, volts:vString, earth_node:'earth',
...
function drawSldShapes(){
  ...
  for (const s of D.shapes) {
    ...
    path(s.pts); ctx.lineWidth = w + (lit ? 1.6 : fused ? 0.7 : 0.8); ctx.strokeStyle = lit ? '#eaf6ff' : (s.open ? SHAPE_WARN : SHAPE_EDGE); ctx.stroke();
    path(s.pts); ctx.lineWidth = w; ctx.strokeStyle = fused ? potColour(s.pot === undefined ? 0 : s.pot) : (s.open ? '#3a3020' : SHAPE_BLACK); ctx.stroke();
    if (!fused) { path(s.pts); ctx.lineWidth = hair; ctx.strokeStyle = s.open ? SHAPE_WARN : SHAPE_PENCIL; ctx.stroke(); } }

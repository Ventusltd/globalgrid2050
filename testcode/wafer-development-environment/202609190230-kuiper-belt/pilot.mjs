...
(async () => {
  ...
  const qd = new URLSearchParams(location.search).get('draw'); const start = qd ? (['logo', 'clock'].includes(qd) ? qd : 'draw ' + qd) : document.querySelector('meta[name="wafer-start"]')?.content; if (start) { /* fire as soon as the wafer is actually there, instead of waiting a fixed guess: poll for window.__wafer every 80 ms, give it one short grace period, cap at 6 s */ for (let w = 0; w < 75 && !window.__wafer; w++) await new Promise(r => setTimeout(r, 80)); await new Promise(r => setTimeout(r, 350)); const m = /^(land|visit|go|open)\s+(\S+)/.exec(start); if (m) { await run('app ' + m[2]); log(`This page stops here. To enter ${m[2]}, type: land ${m[2]} — space brings you back to the wafer.`); } else await run(start); sayEl.classList.add('min'); /* the wafer is the main event: the card starts folded to its title bar */ }
})();

...
// zoom — replaces the memory-hungry pattern of loading whole-country GeoJSON
// files (motorways, trunk roads, primary roads, mainline rail) for line layers.
// If the pmtiles shim is missing, plain GeoJSON layers still work; only tile
// layers will silently fail (logged), so the rest of the map keeps running.
if (typeof window.pmtiles !== 'undefined' && window.pmtiles.Protocol) {
    const _pmtilesProtocol = new window.pmtiles.Protocol();
    maplibregl.addProtocol('pmtiles', _pmtilesProtocol.tile);
} else {
    console.warn('[PMTiles] protocol shim not loaded — tile layers (motorways, trunk_roads, primary_roads, mainline_rail) will not render. Add the pmtiles script tag before ventus-corev9engine.js.');
}
...
async function hydrateLayer(layerId) {
    ...
    if (!layerConfig) { updateUIState(layerId, 'FAIL'); return; }

    // PMTiles vector-tile layers don't need fetching — tiles stream on demand.
    // The source was already added at startup; nothing to hydrate.
    if (typeof layerConfig.url === 'string' && layerConfig.url.startsWith('pmtiles://')) {
        state.loaded = true;
        state.loading = false;
        updateUIState(layerId, 'TILES');
        return;
    }
...
map.on('load', () => {
    ...
    GRID_CONFIG.forEach(group => {
        group.layers.forEach(layer => {
            ...
            // ── PMTiles vector-tile branch ───────────────────────────────
            // A layer is a vector-tile layer iff its url starts with `pmtiles://`.
            // We add a `vector` source pointing at the .pmtiles file, then add a
            // styled layer that references a `source-layer` (the named layer baked
            // into the tiles by tippecanoe — by default, the input filename's stem).
            // The `source-layer` can be overridden via `layer.sourceLayer`.
            if (typeof layer.url === 'string' && layer.url.startsWith('pmtiles://')) {
                const srcId = `src-${layer.id}`;
                if (!map.getSource(srcId)) {
                    map.addSource(srcId, { type: 'vector', url: layer.url });
                }
                const sourceLayerName = layer.sourceLayer || layer.id;
                ...
                if (layer.filter)  tileLayerObject.filter  = layer.filter;
                if (layer.minzoom) tileLayerObject.minzoom = layer.minzoom;
                if (layer.maxzoom) tileLayerObject.maxzoom = layer.maxzoom;
                map.addLayer(tileLayerObject);
                allLayerIds.push(`l-${layer.id}`);
                // Tile sources stream tiles on demand — no fetch step, no count.
                // Mark as loaded so hydrateLayer short-circuits and the UI label
                // doesn't sit on WAIT forever.
                if (RUNTIME_STATE[layer.id]) {
                    RUNTIME_STATE[layer.id].loaded = true;
                    RUNTIME_STATE[layer.id].loading = false;
                }
                updateUIState(layer.id, 'TILES');
                return;
            }

            // ── GeoJSON branch (original behaviour) ──────────────────────

# GlobalGrid2050 V7 Application Architecture

V7 is the separated testing workspace for the GlobalGrid2050 Solar BESS Topology system.

It was created from the stable V5 toolset. V5 remains the working baseline. V7 is where applications are separated, documented, modularised and tested one controlled step at a time.
...
The repository philosophy is the governing doctrine. The V7 docs explain how that doctrine is applied inside this workspace.
...
V7 exists to convert the previous flat V5 toolset into a cleaner separated application workspace.
...
`index.html` links to the 4 separated V7 apps.
...
Copied from V5 and isolated inside its own folder. The app title says V7 but internal files still use `gis-sld-v5` names. This is intentional for now. The first V7 objective is safe separation, not wholesale renaming.
...
Copied from V5 and isolated. It still loads V5 named CSS and JS files inside the V7 folder.
...
Copied from V5 and isolated. It still uses V5 named CSS and JS files inside the V7 folder.
...
That report confirms the extracted V7 CSS and JavaScript rebuild exactly to the original V7 inline logic and match the original V5 baseline.
...
Do not rename or restructure this app until the V7 separation and cable geometry modularisation have been manually validated in browser.
...
```text
1. V5 remains stable.
2. V7 is the testing and modularisation workspace.
3. No broad rewrite.
...
11. Browser testing must compare V7 against the relevant V5 baseline.
12. If uncertain, stop and refer to /PHILOSOPHY.md and the V7 docs.
```
...
```text
1. Manually open the V7 launcher.
2. Open all 4 V7 apps.
3. Compare the cable geometry visualiser against the V5 original.
4. Confirm export JSON and copy snapshot still work.
5. Add a testing phase label to the V7 launcher page if desired.
```

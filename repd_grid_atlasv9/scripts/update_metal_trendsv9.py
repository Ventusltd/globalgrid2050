...
# From /scripts/ up to Root, targeting the file directly
FILE = Path(__file__).parent.parent / "copper_and_aluminium_prices_historic_trends.md"
...
def get_market_data():
    ...
    # Fetch Copper (HG=F is per pound, multiply by 2204.62 for tonne)
    try:
        r = requests.get(
            "https://query1.finance.yahoo.com/v9/finance/chart/HG=F",
            timeout=10,
    ...
    # Fetch Aluminium (ALI=F is per tonne)
    try:
        r = requests.get(
            "https://query1.finance.yahoo.com/v9/finance/chart/ALI=F",
            timeout=10,
            headers={"User-Agent": "Mozilla/5.0"}
        )
...
def main():
    ...
    usd_to_gbp = 1 / d["gbp_usd"]
    usd_to_eur = d["gbp_eur"] / d["gbp_usd"]

    cu_gbp = d["cu_usd"] * usd_to_gbp
    cu_eur = d["cu_usd"] * usd_to_eur

    al_gbp = d["al_usd"] * usd_to_gbp
    al_eur = d["al_usd"] * usd_to_eur

    # ---------------------------------------------------------
    # PART 1: The Static Top Section (Includes Plotly JS)
    # ---------------------------------------------------------
    PART_1 = """---
layout: page
...
"""
    ...
    # ---------------------------------------------------------
    # PART 2: The Dynamic API Injection
    # ---------------------------------------------------------
    PART_2 = f"""
...
|:---|---:|---:|---:|
| **Copper** | ${d['cu_usd']:,.0f} | €{cu_eur:,.0f} | £{cu_gbp:,.0f} |
| **Aluminium** | ${d['al_usd']:,.0f} | €{al_eur:,.0f} | £{al_gbp:,.0f} |

---
"""

    # ---------------------------------------------------------
    # PART 3: The Static Historic Bottom Table
    # ---------------------------------------------------------
    PART_3 = """
...
"""
    ...
    full_md = PART_1 + PART_2 + PART_3

    with open(FILE, "w", encoding="utf-8") as f:
        f.write(full_md)
        
    print(f"✅ Successfully updated Historic Copper/Aluminium Trends at: {FILE}")

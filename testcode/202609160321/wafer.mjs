/* wafer.mjs - the bands-by-usage law, generated. Variation bands-by-usage-lit.
 *
...
export function place(key, rad, len, fam, maxKey) {
  const r = R*(1-Math.floor(rad*8)/8) + shell(key)*B*1.5;
  const a = key * GOLDEN;
...
function draw() {
  ...
  footEl.textContent = 'law: bands-by-usage - Usage bands. ' + shown.toLocaleString() +
...
fetch(LAWS_INDEX).then(r => r.ok ? r.json() : null).then(ix => {
  ...
  for (const law of ix.laws) {
    ...
    if (law.id === 'bands-by-usage') a.setAttribute('aria-current', 'page');
    nav.append(a);
  }
  nav.hidden = false;
 ...
 */
window.__wafer = {
  law: 'bands-by-usage', variant: 'bands-by-usage-lit', place, lawsIndex: LAWS_INDEX,

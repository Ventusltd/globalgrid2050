...
- The card is now ordered for the person who decides, not the person who codes. Tap a dot and the
  first and biggest thing is the application that work belongs to: its name, one plain sentence
  saying what it does for a project, and OPEN. The whole tile is the button.
- Under it, whose work this is, then one plain line saying what was done and the day it was done.
- The code is still there, one tap further in, under "See the code behind this": the line, ten
  above and ten below, and the link to its history. Nothing technical sits above that line.
- Where a piece of work cannot be shown to belong to an application, the card says so, offers no
  application, and points to the front page of the site instead.
...
Score 20 of 20, 2026-09-19T21:02:35Z.

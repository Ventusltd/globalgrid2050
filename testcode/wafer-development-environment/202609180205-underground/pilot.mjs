...
cmds.draw = async function(spec){
  const what = (spec || '').trim().toLowerCase();
  if (!/^(underground|tube|london)/.test(what)) return log('draw underground — the one network this version carries');
  if (!W()) return log('the wafer is not ready');
  const N = await loadNet(); if (!N) return log('draw underground: underground-network.json missing');
  await W().scope(null); const m = W().state().scopeN;
  log(`draw underground: ${N.stations.length} stations, ${N.edges.length} edges; ${m.toLocaleString('en-GB')} lines placed along them by ${N.law}`);
  await W().blend(networkTargets(N, m)); hud('underground');

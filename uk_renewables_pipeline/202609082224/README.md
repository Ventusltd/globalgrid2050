# GlobalGrid2050 UK Renewables Pipeline 202609082224 (V9.10)

## Status

Candidate dated 8 September 2026. Parent `202609081016` (V9.9) is the exact frozen
parent; its REPD spine, the v9.8 interconnector product, its fixtures and every
inherited module are carried byte-for-byte and proven so by `tests/run_v9_10.sh`.

**V9.10 bounds the ACTIONS column so a sentence cannot widen it**, and removes
the offshore sentence V9.9 printed on every offshore row. The MAP link is
unchanged: every row opens the canonical Atlas receiver, and an offshore row
still arrives on the offshore cable engine in GridAtlas 202609080850.

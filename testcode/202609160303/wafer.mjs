/* wafer.mjs - the core law, generated. Variation core-lit.
 *
...
function draw() {
  ...
  footEl.textContent = 'law: core - Density. ' + shown.toLocaleString() +
...
fetch(LAWS_INDEX).then(r => r.ok ? r.json() : null).then(ix => {
  ...
  for (const law of ix.laws) {
    ...
    if (law.id === 'core') a.setAttribute('aria-current', 'page');
    nav.append(a);
  }
  nav.hidden = false;
 ...
 */
window.__wafer = {
  law: 'core', variant: 'core-lit', place, lawsIndex: LAWS_INDEX,

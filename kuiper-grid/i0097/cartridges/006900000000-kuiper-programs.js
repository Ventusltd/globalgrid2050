...
function stringDrawing(g){
  ...
  // HOW CABLE IS REALLY CLIPPED, NOT HOW A ROPE HANGS. The sagging parabolas read as tall arches that
  // crossed one another over and over between M2 and M4, and at telephone size that stretch was a knot.
  // A cable on a table is not hung: it leaves its box, goes a short way to a CLIP LINE that runs
  // parallel to the line of boxes, runs ALONG that line, and turns into the box it mates with. There
  // are two clip lines and no more - one above the boxes for the links on the way out, one below for
  // the links on the way back - so an out link can never meet a return link, and two links on the same
  // line are end to end, never on top of each other. The only place a cable meets anything now is where
  // it enters its own box. Corners are rounded, because a square corner is a busbar, not a cable.
  const bendR = Math.min(0.16, boxH * 0.55);
  ...
  // ONE HEIGHT FOR ALL OF THEM, and it is the largest height every link can afford out of its own spare
  // cable. Picking a height per link would make as many clip lines as there are links, which is the
  // mess this replaced.
  const pairAll = c.lead_plus_m + c.lead_minus_m;
  let clipOff = 0.20;
  for (const l of g.links) { const Ac2 = g.plusPt(l.from), Bc2 = g.minusPt(l.to);
    const span = Math.hypot(Bc2[0] - Ac2[0], Bc2[1] - Ac2[1]);
    if (pairAll >= span) clipOff = Math.min(clipOff, Math.max(0.05, (pairAll - span) * 0.40)); }
  clipOff = Math.max(0.05, clipOff);
  const clipY = (yMid, up) => yMid + (up ? 1 : -1) * (boxH / 2 + clipOff);
  g.links.forEach((l, i) => {
    ...
    const ux = dx / L, uy = dy / L, nx = -uy * (l.back ? -1 : 1), ny = ux * (l.back ? -1 : 1);
    ...
    if (pair >= L) {
      // OUT OF THE BOX, UP OR DOWN A SHORT WAY, ALONG THE CLIP LINE, AND IN. Out links take the line
      // above the boxes, return links the one below, so the two families never meet.
      const up = !l.back;
      const cy = up ? Math.max(clipY(Ac[1], true), clipY(Bc[1], true))
                    : Math.min(clipY(Ac[1], false), clipY(Bc[1], false));
      const Ar = port ? [faceX(Ac[0], Bc[0]), Ac[1]] : [Ac[0], Ac[1] + (up ? 1 : -1) * boxH / 2];
      const Br = port ? [faceX(Bc[0], Ac[0]), Bc[1]] : [Bc[0], Bc[1] + (up ? 1 : -1) * boxH / 2];
      const raw = [Ar];
      if (port) { const sa = Ar[0] >= Ac[0] ? 1 : -1, sb = Br[0] >= Bc[0] ? 1 : -1;
        raw.push([Ar[0] + sa * stub, Ar[1]], [Ar[0] + sa * stub, cy], [Br[0] + sb * stub, cy], [Br[0] + sb * stub, Br[1]]); }
      else { if (Math.abs(Ar[0] - Br[0]) > 1e-6) raw.push([Ar[0], cy], [Br[0], cy]); else raw.push([Ar[0], cy]); }
      raw.push(Br);
      let pts = round3(raw, bendR);
      // THE SPARE CABLE IS A LOOP AT THE CONNECTOR, NOT HEIGHT IN THE RUN. An installer leaves the slack
      // as one turn of cable at the pair that is mated; he does not raise the whole run to swallow it.
      const rt = polyLen(pts), t = Math.min(0.92, Math.max(0.08, lp / pair));
      const j = polyAt(pts, rt.cum, rt.total, t), tn = polyAt(pts, rt.cum, rt.total, Math.min(1, t + 0.02));
      let tvx = tn[0] - j[0], tvy = tn[1] - j[1];
      const tl = Math.max(1e-6, Math.hypot(tvx, tvy)); tvx /= tl; tvy /= tl;
      const head = polySlice(pts, 0, t); let tail = polySlice(pts, t, 1);
      const spare = pair - rt.total;
      if (spare > 0.06) { const r = Math.min(spare / 6.2832, boxH * 0.9, 0.30);
        const cxx = j[0] - tvy * r, cyy = j[1] + tvx * r, a0 = Math.atan2(j[1] - cyy, j[0] - cxx), loop = [];
        for (let q = 1; q <= 16; q++) { const a = a0 + 6.2832 * q / 16;
          loop.push([cxx + Math.cos(a) * r, cyy + Math.sin(a) * r]); }
        tail = [[j[0], j[1]]].concat(loop, tail); }
  ...
  for (const s of D.shapes) {
    ...
    if (D.wiring) {
      ...
      for (const [r0, r1] of runs) { const seg = polySlice(s.pts, r0, r1); if (seg.length < 2) continue;
        path(seg); ctx.lineWidth = core + 1.8; ctx.strokeStyle = casing; ctx.stroke();

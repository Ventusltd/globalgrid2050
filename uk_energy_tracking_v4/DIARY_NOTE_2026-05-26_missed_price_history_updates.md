...
That meant the normal chart moved to the larger historical dataset, while the full screen chart remained on the V4 captured JSON file only and therefore still showed 4 records.
...
```text
Every future V4 price history patch must inspect index.md, price-history-ui.js and price-history-fullscreen.js together.
...
```text
scripts/patch_v4_price_range_fullscreen_fix.py
```

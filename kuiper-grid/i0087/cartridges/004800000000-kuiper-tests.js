...
async function buttonsSelftest(){
  ...
  const viewBtns = Array.from(document.querySelectorAll('#firemenu .fm-view button'));
...
async function isolateSelftest(){
  ...
  { const box = document.getElementById('firebox'), menu = document.getElementById('firemenu');
    T.the_bar_is_on_the_screen = onScreen(box);
    T.the_bar_is_empty = !!box && box.value === '';
    T.the_bar_shows_no_command = !!box && !/\{|fire\s/i.test(box.placeholder || '');
    T.no_application_button_on_the_face = !btns.some(onScreen);
    T.no_list_on_the_face = !menu || getComputedStyle(menu).display === 'none';
    if (box) { box.click(); await new Promise(r => setTimeout(r, 120)); }
    T.the_bar_opens_the_popup = !!menu && menu.classList.contains('open');
    // a pop-up is a thing you scroll: a row is reachable when scrolling it into the pop-up puts it on the screen
    const reach = el => { if (!el) return false; if (el.scrollIntoView) el.scrollIntoView({ block:'center' }); return onScreen(el); };
    T.every_application_is_in_the_popup = btns.length === programs.length && btns.every(reach);
    dispatchEvent(new KeyboardEvent('keydown', { key:'Escape' })); await new Promise(r => setTimeout(r, 120));
    T.escape_shuts_the_popup = !!menu && !menu.classList.contains('open');
    T.T1_the_face_is_bare_and_the_popup_holds_every_application = programs.length > 0
      && T.the_bar_is_on_the_screen && T.the_bar_is_empty && T.the_bar_shows_no_command
      && T.no_application_button_on_the_face && T.no_list_on_the_face
      && T.the_bar_opens_the_popup && T.every_application_is_in_the_popup && T.escape_shuts_the_popup; }

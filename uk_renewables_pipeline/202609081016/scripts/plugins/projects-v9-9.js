...
function mapActionHtmlV9_5_1(project) {
  ...
  /* WHAT AN OFFSHORE ROW'S MAP BUTTON OPENS ON, SAID IN THE CELL.

     Since GridAtlas v9.154 an offshore wind arrival is answered by a separate
     engine. The onshore engine measures to the nearest mapped substations
     within 40 km; for an array 60 km out at sea that returns another
     company's offshore platform, which is an answer to the wrong question. The
     offshore engine instead reads NESO's connection registers - the
     Transmission Entry Capacity and Embedded registers, published by the body
     that issues the connection - binds the project to the substation it is
     contracted to, at whatever distance that is, draws the connection in gold
     and frames both ends. Hornsea 3 lands on Norwich Main; Berwick Bank on
     Branxton.

     The sentence is deliberately the same for every offshore row, because the
     button does the same thing for every offshore row and the Atlas says on
     arrival which of three things happened: the connection is drawn; the
     register names a substation this map cannot yet place, usually one not yet
     built; or the project is in neither register. No version number is printed
     here - the route is read from the engine's own contract, and a number typed
     into this file would drift the day the engine moves. */
  const offshoreNote = project.technology === "wind_offshore"
    ? `<div class="map-note">Opens on the offshore cable engine: where NESO's connection register names `
      + `the substation this project is contracted to, the Atlas draws that connection and frames it. `
      + `Nearest-substation distances are shown as nearest, never as the connection.</div>`
    : "";
  if (atlasCentresOnRepdPointV9_7(project)) return link + offshoreNote;
  return `${link}${offshoreNote}<div class="map-note">REPD published no coordinate for this record. `

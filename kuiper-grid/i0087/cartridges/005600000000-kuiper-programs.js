...
function unisolate(quiet){ labels(true); sldNow = null; sldNames = null; { const el = document.getElementById('sldnames'); if (el) el.replaceChildren(); }
  if (typeof cablesGone === 'function') cablesGone();
...
function makeSldBody(st){
  ...
  // THE CABLES ARE BUILT FROM THE SAME WIRES, THROUGH THE SAME FIT. Nothing else draws a string's cables.
  cableLayer(); sldCables = net.kind === 'string' ? stringCables(net.g, fit) : null;
  nameLayer();
...
function makeSldNames(lay, net){
  ...
  if (net.kind === 'string') { const i = first('inv'), r = first('ret'), out = [];
    ...
    // WHAT THE RED IS FOR, said once, under the drawing. Red and black here are how the drawing is read;
    // in the field both conductors are black cable and only the polarity marking tells them apart.
    { let lowest = Infinity; for (let j = 0; j < lay.tag.length; j++) lowest = Math.min(lowest, lay.unit[2*j+1]);
      if (isFinite(lowest)) out.push({ x:0, y:lowest, dy:-0.085, text:CABLE_NOTE, warn:false, note:true }); }
    return out; }
...
function placeSldNames(){
  ...
  if (el.childElementCount !== sldNames.length) {
    el.replaceChildren(...sldNames.map(n => { const sp = document.createElement('span');
      sp.textContent = n.text; if (n.warn) sp.className = 'warn'; else if (n.note) sp.className = 'note'; return sp; }));
  }
...
// EVERY CONDUCTOR IS ONE WIRE, AND THE WIRE KNOWS ITS POLARITY. The dots and the solid cables are both
// made from this one list, so a line can never sit anywhere but on the dots it was made with. A wire is
// a run of segments in ELECTRICAL order, from the end nearest the + side to the end nearest the - side:
//   home, minus    the home run to the string's - end
//   link           one conductor made of TWO leads: the + lead of one module and the - lead of the next
//   home/ret, plus the home run from the string's + end; a sequential string lays it the length of the row
function stringWires(g){
...
function stringPieces(g){
  const P = [], w = stringWires(g), X = w.X, yb = w.yb;
  ...
  for (const wire of w.wires) for (const s of wire.segs) line(wire.tag, s[4], s[0], s[1], s[2], s[3]);
  return P;
}
// THE SAME WIRES AS SOLID CABLE, cut into the lengths the two leads really are. A link is ONE conductor
// made of TWO leads that meet at a connector: the + lead covers lead_plus_m of the distance the link has
// to span and the - lead covers lead_minus_m of it, and where they meet is marked with a tick. When they
// do not reach, what is left between them IS the shortfall, and it is drawn as the gap it is.
function stringCables(g, fit){
  const w = stringWires(g), F = (x, y) => [(x - fit.mx) * fit.k, (y - fit.my) * fit.k], out = [];
  for (const wire of w.wires) {
    const pts = [F(wire.segs[0][0], wire.segs[0][1])];
    for (const s of wire.segs) pts.push(F(s[2], s[3]));
    let plusTo = 1, minusFrom = 1, dash = false, tick = false;
    if (wire.pol === 'minus') { plusTo = 0; minusFrom = 0; }
    else if (wire.pol === 'link') { const l = wire.link, need = Math.max(1e-6, l.need), lp = g.c.lead_plus_m, lm = g.c.lead_minus_m;
      if (l.reaches) { plusTo = minusFrom = Math.min(0.9, Math.max(0.1, lp / Math.max(1e-6, g.pair))); tick = true; }
      else { plusTo = Math.min(0.88, Math.max(0.05, lp / need)); minusFrom = Math.min(0.97, Math.max(plusTo + 0.03, 1 - lm / need)); dash = true; } }
    out.push({ tag:wire.tag, pol:wire.pol, pts, plusTo, minusFrom, dash, tick });
  }
  return out;
}
// ===== THE CABLES AS SOLID LINES, ON A LAYER THAT FOLLOWS THE CAMERA ============================
// The dots say where the work is; a cable drawing says WHICH CONDUCTOR IS WHICH. The two are one picture:
// every line here is built from the same wire list the dots were laid along, and put on the screen with
// the same arithmetic the words overlay uses, so zooming, dragging, resizing and turning the device can
// never move one without the other. The layer takes no pointer, is redrawn after every frame the shell
// draws, and is wiped the moment the diagram goes.
// RED IS FOR READING THIS DRAWING AND NOTHING ELSE. On site both conductors are black cable and are told
// apart by polarity markers; that is the note under the picture, and it is not a footnote.
// BLACK ON A NEAR-BLACK SKY IS INVISIBLE, so the black cable is given a hairline light edge: what reads is
// a black cable with an outline, not a grey cable.
// THE CABLE IS DRAWN NARROWER THAN A DOT IS WIDE, so the dots it is laid along still show along its
// edges: the pulse still runs, and a link whose leads do not reach still reads AMBER under its cable.
// Its own light edge is amber there too, because the colour is the warning and a reader should not have
// to find one amber dot behind a black line to see it.
const CABLE_RED = '#ff4b3a', CABLE_BLACK = '#04060a', CABLE_EDGE = '#66748a', CABLE_EDGE_SHORT = '#ffb454', CABLE_TICK = '#d8eef7';
const CABLE_NOTE = 'black cables only with polarity marking in the field';
let sldCables = null;
function cableLayer(){
  let el = document.getElementById('sldcables');
  if (!el) { el = document.createElement('canvas'); el.id = 'sldcables'; el.setAttribute('aria-hidden', 'true');
    const st = document.createElement('style');
    st.textContent = '#sldcables{position:fixed;inset:0;z-index:2;pointer-events:none}';
    document.head.append(st);
    const c = document.getElementById('c');
    if (c && c.parentNode) c.parentNode.insertBefore(el, c.nextSibling); else document.body.append(el); }
  return el;
}
function cablePoint(p){ const R = Math.sqrt(SPACE), f = BODY_RADIUS_MAX;
...
function polySlice(pts, u0, u1){ const m = polyLen(pts); if (m.total < 1e-9 || u1 <= u0 + 1e-6) return [];
...
function cablesGone(){ sldCables = null; const el = document.getElementById('sldcables'); if (!el) return;
  const ctx = el.getContext && el.getContext('2d');
  if (ctx) { ctx.setTransform(1, 0, 0, 1, 0, 0); ctx.clearRect(0, 0, el.width, el.height); } }
function drawSldCables(){
  const el = document.getElementById('sldcables'); if (!el) return;
  ...
  if (!sldCables || !sldCables.length) return;
  if (typeof body === 'undefined' || !body || !body.sld || !body.shown || bodyAnim || !sldNow) return;
  const w = Math.max(1.2, Math.min(6.5, bodyDotPx() * 1.3));
  ctx.lineCap = 'round'; ctx.lineJoin = 'round';
  const stroke = (pts, colour, dash, edge) => { if (pts.length < 2) return;
    const s = pts.map(cablePoint), path = () => { ctx.beginPath(); ctx.moveTo(s[0][0], s[0][1]);
      for (let i = 1; i < s.length; i++) ctx.lineTo(s[i][0], s[i][1]); };
    ctx.setLineDash(dash ? [w * 3.6, w * 2.2] : []);
    if (edge) { path(); ctx.lineWidth = w + 1.3; ctx.strokeStyle = edge; ctx.stroke(); }
    path(); ctx.lineWidth = w; ctx.strokeStyle = colour; ctx.stroke(); };
  for (const c of sldCables) {
    const edge = c.dash ? CABLE_EDGE_SHORT : CABLE_EDGE;
    if (c.plusTo > 0.001) stroke(polySlice(c.pts, 0, c.plusTo), CABLE_RED, c.dash, c.dash ? edge : null);
    if (c.minusFrom < 0.999) stroke(polySlice(c.pts, c.minusFrom, 1), CABLE_BLACK, c.dash, edge);
    if (c.tick) { const m = polyLen(c.pts);
      const a = cablePoint(polyAt(c.pts, m.cum, m.total, c.plusTo - 0.03)), b = cablePoint(polyAt(c.pts, m.cum, m.total, c.plusTo + 0.03));
      const mid = cablePoint(polyAt(c.pts, m.cum, m.total, c.plusTo));
      const dx = b[0] - a[0], dy = b[1] - a[1], d = Math.hypot(dx, dy) || 1, nx = -dy / d, ny = dx / d, r = w * 1.35;
      ctx.setLineDash([]); ctx.beginPath(); ctx.moveTo(mid[0] - nx * r, mid[1] - ny * r); ctx.lineTo(mid[0] + nx * r, mid[1] + ny * r);
      ctx.lineWidth = Math.max(1, w * 0.55); ctx.strokeStyle = CABLE_TICK; ctx.stroke(); } }
  ctx.setLineDash([]);
}
// AFTER EVERY FRAME THE SHELL DRAWS, not on a loop of this file's own and not on a guessed list of events.
// The shell tells whoever asks that it has drawn; the words and the cables are then put where the dots are.
try { if (window.__wafer && window.__wafer.onDraw) window.__wafer.onDraw.add(function(){ try { placeSldNames(); drawSldCables(); } catch (_) {} }); } catch (_) {}

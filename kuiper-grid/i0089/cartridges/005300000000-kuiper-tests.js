...
async function buttonsSelftest(){
  ...
  // THE CONTROLS ARE IN THE POP-UP NOW. The face is the disc and one bar; the applications and the four
  // view commands live inside #firemenu, and a button answers .click() wherever it is. The row in the foot
  // is taken when it is still there, so this check reads the same before and after the dressing changes.
  const viewBtns = Array.from(document.querySelectorAll('.viewrow button, #firemenu .fm-view button'));
  const keyBtns = viewBtns.length ? viewBtns : Array.from(document.querySelectorAll('#keys button'));
  const controls = Array.from(document.querySelectorAll('#apps button')).concat(keyBtns).filter(b => b.getAttribute('data-k') !== '/');
...
async function isolateSelftest(){
  ...
  // THE FACE IS THE DISC AND ONE BAR (Vikram, 20 Sept: "ALL CLUTTER TEXT AND BUTTON GONE, USE POP UP FOR
  // EVERYTHING"). So the rule this checks is the opposite of the old one: with the pop-up shut NO application
  // button is on the screen and the bar IS; one touch of the bar opens the pop-up; then every application is
  // reachable in it, and Escape shuts it again. The bar must be EMPTY, and must never show a command.
  // THE WAFER'S OWN COMPONENT, ON THE KUIPER. Vikram's rule: the wafer development environment page is the
  // look, and every feature evolves that renderer. So what this asks is what the wafer has: one full width
  // typed bar at the very bottom (#pilot, #pin), the examples picker above it (#eg, #egs), the preset chips
  // along the top (#presets), and one window for whatever was asked (#say, with #saybar, #saymin, #saymax,
  // #sayclose and #saybody). The face carries nothing else: no row of applications, no list, no JSON in
  // the bar. The applications are in the window, behind the chip that opens them, and Escape shuts it.
  { const box = document.getElementById('pin'), sayEl = document.getElementById('say');
    const picker = document.getElementById('egs'), chips = document.getElementById('presets');
    T.the_wafers_parts_are_all_here = ['pilot', 'pin', 'eg', 'egs', 'presets', 'say', 'saybar', 'saytitle', 'saymin', 'saymax', 'sayclose', 'saybody']
      .filter(id => !document.getElementById(id));
    T.the_bar_is_on_the_screen = onScreen(box);
    T.the_bar_is_empty = !!box && box.value === '';
    T.the_bar_shows_no_json = !!box && !/[{}]/.test(box.placeholder || '');
    T.the_picker_offers_sentences = !!picker && picker.options.length > 1 && !/[{}]/.test(picker.options[1].textContent);
    T.the_chips_are_on_the_screen = !!chips && chips.children.length > 1 && onScreen(chips.children[0]);
    T.no_chip_shows_a_command = !!chips && !Array.from(chips.children).some(b => /[{}]|fire\s/i.test(b.textContent));
    T.no_application_button_on_the_face = !btns.some(onScreen);
    T.the_window_is_shut = !!sayEl && !sayEl.classList.contains('open');
    const chipApps = document.getElementById('chipapps');
    if (chipApps) { chipApps.click(); await new Promise(r => setTimeout(r, 150)); }
    T.the_window_opens_on_the_applications = !!sayEl && sayEl.classList.contains('open');
    const reach = el => { if (!el) return false; if (el.scrollIntoView) el.scrollIntoView({ block:'center' }); return onScreen(el); };
    T.every_application_is_in_the_window = btns.length === programs.length && btns.every(reach);
    // the window is the wafer's window: it minimises, it maximises and it closes
    document.getElementById('saymin').click(); const minned = sayEl.classList.contains('min');
    document.getElementById('saymin').click();
    document.getElementById('saymax').click(); const maxed = sayEl.classList.contains('max');
    document.getElementById('saymax').click();
    T.the_window_minimises_and_maximises = minned && maxed;
    dispatchEvent(new KeyboardEvent('keydown', { key:'Escape' })); await new Promise(r => setTimeout(r, 150));
    T.escape_shuts_the_window = !!sayEl && !sayEl.classList.contains('open');
    T.T1_the_face_is_bare_and_the_wafers_bar_holds_every_application = programs.length > 0
      && T.the_wafers_parts_are_all_here.length === 0
      && T.the_bar_is_on_the_screen && T.the_bar_is_empty && T.the_bar_shows_no_json
      && T.the_picker_offers_sentences && T.the_chips_are_on_the_screen && T.no_chip_shows_a_command
      && T.no_application_button_on_the_face && T.the_window_is_shut
      && T.the_window_opens_on_the_applications && T.every_application_is_in_the_window
      && T.the_window_minimises_and_maximises && T.escape_shuts_the_window; }
  ...
  T.T25_the_row_is_the_seven_applications_in_order = T.the_row.slice(0, 7).join('|') === 'GRIDATLAS|PIPELINE NEWS|CABLES|REAL SYSTEMS|SPIDER SANDBOX|GRID ENGINE|PERIODIC TABLE';   // what the menu adds comes after the seven
  T.T25b_what_the_menu_adds_is_named_and_comes_last = T.the_row.slice(7).every(b => typeof MENU !== 'undefined' && MENU.some(m => m.program.button === b));

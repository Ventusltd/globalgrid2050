...
(async () => {
  ...
  const qd = new URLSearchParams(location.search).get('draw'); const start = qd ? (['logo', 'clock'].includes(qd) ? qd : 'draw ' + qd) : document.querySelector('meta[name="wafer-start"]')?.content; if (start) { /* fire as soon as the wafer is actually there, instead of waiting a fixed guess: poll for window.__wafer every 80 ms, give it one short grace period, cap at 6 s */ for (let w = 0; w < 75 && !window.__wafer; w++) await new Promise(r => setTimeout(r, 80)); /* The header must describe THIS page, not the wafer in the abstract. The estate's real size was measured on 2026-09-19 by twelve runners over every blob in git history; the wafer holds a subset of it as numbered dust. State both, never one as the other. */ try { const c = document.getElementById('count'); if (c) c.textContent = '66 repositories · 124,905,502 lines measured across all git history · 42,549 blobs · this wafer draws 250,174 of them as dust'; } catch {} await new Promise(r => setTimeout(r, 350)); const m = /^(land|visit|go|open)\s+(\S+)/.exec(start); if (m) { await run('app ' + m[2]); log(`This page stops here. To enter ${m[2]}, type: land ${m[2]} — space brings you back to the wafer.`); } else await run(start); sayEl.classList.add('min'); /* the wafer is the main event: the card starts folded to its title bar */ }
})();
...
const netCache = {};
const NETWORKS = { grid: 'the grid: 400 kV, 132 kV and every substation', underground: 'the London Underground', grid400: 'the 400 kV grid', grid132: 'the 132 kV network', substations: 'every substation', shotwick: 'Shotwick Solar Farm and its grid', uk: 'the British Isles', world: 'the world', trench: 'a cable trench in flat formation (a stated arrangement, not a design)', fault: 'a fault study: the current paths, by pandapower (IEC 60909) on a published example network', cable132: 'single core 132 kV XLPE, 76/132(145) kV, 1600 mm2 aluminium Class 2 Milliken compacted; 50.3 mm over conductor, 53.3 mm over conductor screen, 17 mm XLPE giving 87.3 mm over insulation; read from the UK Power Networks schedule of technical particulars; layers beyond the insulation not yet drawn', cable: 'a single core 800 mm2 aluminium 19/33 kV cable in cross section, 3086 nodes, overall 63.0 mm, drawn from stated inputs and not from any standard', kuiper: 'the Kuiper belt: all 66 repositories, every tracked file including duplicates, 19,257 bodies from the estate'  };

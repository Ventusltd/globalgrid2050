...
function newsMatches(item) {
  ...
  if (state.newsMode === "RELEVANT" && item.canonical_relevant !== true && item.priority_uk !== true) return false;
...
export function drawNewsV9_5_1() {
  ...
  stories.innerHTML = rows.map((item) => {
    ...
    const capacity = Number(item.canonical_capacity_mw ?? item.capacity_mw ?? item.capacity_mw_max ?? 0);
    const quality = item.priority_uk === true
      ? `<span class="news-quality relevant">UK SOURCE P${Number(item.source_priority || 0)}</span> · discovery only · no project signal`

#!/usr/bin/env python3
"""
Add Atlas V8 link to V6 dashboard.

This script does not move or copy the Atlas V8 app.
It only adds a V6 dashboard card and a supporting engineering function note.
"""
...
V6_INDEX = ROOT / "solar-bess-topology-v6" / "index.html"
ATLAS_INDEX = ROOT / "repd_grid_atlasv8" / "index.html"
REPORTS = ROOT / "gridbot_reports"
REPORT = REPORTS / "v6_add_atlas_v8_dashboard_link.md"
...
FUNCTION_ITEM = '''
            <div class="function-item"><h3>15. Atlas V8 wider grid analysis</h3><p>Open the standalone Atlas V8 app for wider UK grid, infrastructure, project pipeline and spatial screening analysis alongside the V6 solar BESS tools.</p></div>'''
...
def main() -> int:
    if not V6_INDEX.exists():
        raise SystemExit(f"Missing V6 dashboard: {V6_INDEX}")
    ...
    if "../repd_grid_atlasv8/" not in html:
        if CARD_MARKER not in html:
            raise SystemExit("Cable Geometry Visualiser card marker not found in V6 dashboard")
    ...
    if "15. Atlas V8 wider grid analysis" not in html:
        if FUNCTION_MARKER not in html:
            raise SystemExit("Commercial engineering linkage function marker not found in V6 dashboard")
    ...
    report = "\n".join([
        "# V6 Add Atlas V8 Dashboard Link",
        "",
        f"UTC created: {timestamp}",
        "",
        "## Purpose",
        "",
        "Add a V6 dashboard link to the existing standalone Atlas V8 application without moving or duplicating the Atlas folder.",
        "",
        "## Target",
        "",
        "- Dashboard: `solar-bess-topology-v6/index.html`",
    ...
    write(REPORT, report)
    print(f"V6 dashboard Atlas V8 link update complete. Report: {REPORT.relative_to(ROOT)}")
    return 0

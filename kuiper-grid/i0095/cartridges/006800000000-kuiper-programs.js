...
function stringDrawing(g){
  ...
  g.links.forEach((l, i) => {
    ...
    // OUT ABOVE THE BOX LINE, BACK BELOW IT - the way the owner draws it by hand. The bow direction used
    // to be flipped by l.back, but a return link ALSO travels the other way, so ux flips too and the two
    // flips cancelled: out and back both bowed upward and crossed each other over and over between M2 and
    // M4. The side is decided in the PAGE's frame, not the cable's: a return link bows down, an outward one
    // up, whichever way along the row it runs. Nothing else moves - the sag h, and so the arc length, is
    // still exactly the lead pair, and both ends still land on the faces of the boxes they name.
    const want = l.back ? -1 : 1;
    let nx = -uy, ny = ux;
    const side = Math.abs(ny) > 1e-9 ? ny : nx;
    if (side * want < 0) { nx = -nx; ny = -ny; }
    ...
    if (pair >= L) {
      ...
      const cp = [(A[0] + B[0]) / 2 + nx * h * 2, (A[1] + B[1]) / 2 + ny * h * 2];

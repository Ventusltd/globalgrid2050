...
function stringDrawing(g){
  ...
  // A LEAD IS NOT AN ARCH, AND IT IS NOT SCATTERED EITHER. It leaves its own box, runs behind the
  // modules and plugs into the box it mates with, and the line drawn here is THAT ROUTE, AT ITS TRUE
  // LENGTH: 0.84 m of lead is 0.84 m of line at this drawing's scale.
  // HOW AN INSTALLER CLIPS CABLE, WHICH IS THE WHOLE OF IT. A cable on the back of a table is not hung
  // and it does not wander: it leaves the box face SQUARE ON, runs the short way to a CLIP LINE that
  // runs parallel to the row of boxes, runs ALONG that line, and turns square into the box it mates
  // with. Corners are rounded, because a square corner is a busbar and not a cable.
  // TWO CLIP LINES AND NO MORE: one clearly ABOVE the row of boxes for the links on the way out, one
  // clearly BELOW it for the links on the way back, so an out link can never meet a return link. Each
  // line stands at least CLIP_CLEAR of a MODULE HEIGHT off the row. That distance is the point: a
  // drawing that put the lines a few centimetres off the boxes turned the row into one thick smudge in
  // which nothing could be followed, and no cable may hug the boxes it is not going into.
  // PARALLEL RUNS ARE BESIDE EACH OTHER, NEVER ON TOP OF EACH OTHER. Two links whose runs share any
  // stretch of the same clip line are put in their own LANES, one cable spacing apart, chosen by the
  // stretch each one really occupies, so a return line carrying several cables reads as several cables.
  // Colour stays with POLARITY, not with direction: red is the + lead and black the -.
  // THE SPARE CABLE IS ONE SMALL SERVICE LOOP AT THE MATED CONNECTOR, which is where a jointer really
  // leaves it. It is not spent raising the run, because the run's height is the clip line's and belongs
  // to the table, not to this link.
  const stub = Math.max(0.13, wX * 0.22), lip = boxW * 0.60;
  const CLIP_CLEAR = 0.18, CABLE_SPACING = Math.max(0.06, boxH * 0.5);
  const clipBase = Math.max(CLIP_CLEAR * wY, boxH / 2 + 0.08);
  const bendR = Math.min(0.13, boxH * 0.55, CABLE_SPACING * 0.9);
  // ROUNDED CORNERS, the same little fillet at every turn of every clipped cable.
  const round3 = (pp, r2) => { if (pp.length < 3) return pp.slice(); const out = [pp[0]];
  ...
  // THE LANE A RUN GETS: the lowest one on its own clip line that nothing already on it overlaps.
  const lanes = {};
  const laneFor = (key, a, b) => { const lo = Math.min(a, b) - CABLE_SPACING * 0.5, hi = Math.max(a, b) + CABLE_SPACING * 0.5;
    const rowsK = lanes[key] || (lanes[key] = []);
    for (let n = 0; n < rowsK.length; n++) if (rowsK[n].every(s => hi <= s[0] || lo >= s[1])) { rowsK[n].push([lo, hi]); return n; }
    rowsK.push([[lo, hi]]); return rowsK.length - 1; };
  const faceX = (bx, towards) => bx + (towards >= bx ? 1 : -1) * boxW / 2;
  const faceY = (by, towards) => by + (towards >= by ? 1 : -1) * boxH / 2;
  // WHERE THE BOXES ARE, so no cable is ever drawn through one it does not plug into. The rectangles
  // are the ones already drawn above, read back, so there is one truth and not a second copy.
  const BOX_MARGIN = Math.max(0.03, boxW * 0.18);
  const boxRects = shapes.filter(s => s.role === 'box' && s.pts).map(s => {
  ...
  // HOW FAR A RUN ALONG A CLIP LINE MAY GO BEFORE IT MUST STOP: up to the near face of the first box
  // that is not its own, with a clear margin, and never one millimetre past it.
  const freeRun = (x, y, dirx, mine) => { let best = Infinity;
    for (const r of boxRects) { if (r.node === mine) continue;
      if (y < r.y0 - BOX_MARGIN || y > r.y1 + BOX_MARGIN) continue;
      const far = (dirx > 0 ? r.x1 + BOX_MARGIN : r.x0 - BOX_MARGIN - 0) ;
      if ((far - x) * dirx <= 0) continue;                       // the box is behind this run
      const near = dirx > 0 ? r.x0 - BOX_MARGIN : r.x1 + BOX_MARGIN;
      best = Math.min(best, Math.max(0, (near - x) * dirx)); }
    return best; };
  // A SERVICE LOOP OF EXACTLY THE SPARE CABLE, coiled at the connector the way a jointer leaves it.
  const loopAt = (p, tx, ty, side, extra) => { const rr = extra / (2 * 18 * Math.sin(Math.PI / 18)), ring = [];
    const cxx = p[0] - ty * side * rr, cyy = p[1] + tx * side * rr, a0 = Math.atan2(p[1] - cyy, p[0] - cxx);
    for (let q = 1; q <= 18; q++) { const a = a0 + 6.2832 * side * q / 18; ring.push([cxx + Math.cos(a) * rr, cyy + Math.sin(a) * rr]); }
    ring[ring.length - 1] = [p[0], p[1]];
    laneHi = Math.max(laneHi, cyy + rr); laneLo = Math.min(laneLo, cyy - rr);
    return ring; };
  ...
  g.links.forEach((l, i) => {
    ...
    // OUT ABOVE, RETURN BELOW: an out link is clipped to the line above the row of boxes and a return
    // link to the line below it, so two cables sharing a module are told apart by which line they are
    // on and not by a guess.
    ...
    if (pair >= L) {
      // SQUARE OUT OF THE BOX, ALONG THE CLIP LINE, AND SQUARE IN. Both ends start inside the face of
      // the box they name, so no cable begins in the air and none begins on a box it does not belong to.
      const up = !l.back, sgn = up ? 1 : -1, face = boxH * 0.47;
      const Ar = [Ac[0], Ac[1] + sgn * face], Br = [Bc[0], Bc[1] + sgn * face];
      const base = up ? Math.max(Ac[1], Bc[1]) + clipBase : Math.min(Ac[1], Bc[1]) - clipBase;
      const lane = laneFor((up ? 'o' : 'b') + Math.round(base * 200), Ac[0], Bc[0]);
      const cy = base + sgn * lane * CABLE_SPACING;
      // THE ROUTE MAY NEVER BE LONGER THAN THE CABLE. If the clip line stands so far off that the two
      // leads could not reach along it, the line is brought down to the height they CAN pay for, and
      // if even that is too far the pair runs straight. A drawing that shows more cable than the string
      // has is a lie, and this one would rather be plain than untrue.
      const dxAB = Math.abs(Ar[0] - Br[0]);
      const needH = h => Math.abs(h - Ar[1]) + Math.abs(h - Br[1]) + dxAB;
      let cyU = cy;
      if (needH(cyU) > pair - 0.02) {
        const room = (pair - 0.02 - dxAB) / 2, mid = (Ar[1] + Br[1]) / 2;
        const lim = up ? Math.max(Ar[1], Br[1]) : Math.min(Ar[1], Br[1]), h = mid + sgn * room;
        cyU = sgn * (h - lim) >= 0 ? h : null; }
      const raw = cyU === null ? [Ar, Br]
                : (dxAB > 1e-6 ? [Ar, [Ar[0], cyU], [Br[0], cyU], Br] : [Ar, [Ar[0], cyU], Br]);
      const pts = round3(raw, bendR);
      for (const q of pts) { laneHi = Math.max(laneHi, q[1]); laneLo = Math.min(laneLo, q[1]); }
      // THE LENGTH IS STILL THE CABLE'S OWN. What the clipped route does not spend is not lost and it is
      // not hidden: it is ONE small service loop at the connector that is mated, the way it is left on
      // site, so the line a finger follows is still lead-plus-lead long.
      const m2 = polyLen(pts), t = Math.min(0.92, Math.max(0.08, lp / pair));
      const j = polyAt(pts, m2.cum, m2.total, t), tan = polyAt(pts, m2.cum, m2.total, Math.min(1, t + 0.02));
      let tvx = tan[0] - j[0], tvy = tan[1] - j[1];
      const tl = Math.max(1e-6, Math.hypot(tvx, tvy)); tvx /= tl; tvy /= tl;
      // EACH LEAD IS ITS OWN TRUE LENGTH, not half a route each. The route is split where the two
      // connectors mate, and what each lead has over and above its share of the route is coiled AT that
      // connector as its own service loop - the + lead's loop one side, the - lead's the other.
      let head = polySlice(pts, 0, t); let tail = polySlice(pts, t, 1);
      const hL = polyLen(head).total, tL = polyLen(tail).total;
      if (lp - hL > 0.02) head = head.concat(loopAt(j, tvx, tvy, 1, lp - hL));
      if (lm - tL > 0.02) tail = [[j[0], j[1]]].concat(loopAt(j, tvx, tvy, -1, lm - tL), tail);
      poly('lead', o, head, { node, pot, plus:true, link:l, says, hit:true, from:fromId, to:'mate' + i, metres:lp });
      poly('lead', o, tail, { node, pot, plus:false, link:l, says, hit:true, from:'mate' + i, to:toId, metres:lm });
      put({ role:'mate', order:o, pot, node, pts:[[j[0], j[1]]], link:l, says, mated:true, along:[tvx, tvy] });
    } else {
      // NEITHER LEAD REACHES. Each is drawn straight out from its own box, at its own length, and stops.
      // NEITHER LEAD REACHES, AND A LEAD THAT DOES NOT REACH IS STILL A CLIPPED CABLE. It leaves its
      // own box square, rises to its own clip line, runs ALONG that line towards the box it cannot get
      // to, and STOPS in clear air before the first box it does not belong to - never inside one, and
      // never across the gap. What it still has in hand at that stop is coiled there, so the line is
      // its full declared length and the gap between the two connectors is the honest shortfall.
      const up = !l.back, sgn = up ? 1 : -1, face = boxH * 0.47;
      const stubOf = (Pc, mine, towardsX, metres, plus) => {
        const y0s = Pc[1] + sgn * face;
        const base = Pc[1] + sgn * clipBase;
        const lane = laneFor((up ? 'o' : 'b') + Math.round(base * 200) + 's', Pc[0], Pc[0] + (towardsX - Pc[0]) * 0.5);
        const cyS = base + sgn * lane * CABLE_SPACING;
        const rise = Math.abs(cyS - Pc[1]);
        let ptsS, endT = [0, 0];
        if (rise >= metres) { const e = [Pc[0], Pc[1] + sgn * metres]; ptsS = [Pc, e]; endT = [0, sgn]; }
        else {
          const dirx = towardsX >= Pc[0] ? 1 : -1;
          const run = Math.min(metres - rise, Math.abs(towardsX - Pc[0]) * 0.45, Math.max(0, freeRun(Pc[0], cyS, dirx, mine)));
          const e = [Pc[0] + dirx * run, cyS];
          ptsS = round3(run > 1e-6 ? [Pc, [Pc[0], cyS], e] : [Pc, [Pc[0], cyS]], bendR);
          endT = run > 1e-6 ? [dirx, 0] : [0, sgn];
        }
        const have = polyLen(ptsS).total, endP = ptsS[ptsS.length - 1];
        if (metres - have > 0.02) ptsS = ptsS.concat(loopAt(endP, endT[0], endT[1], plus ? 1 : -1, metres - have));
        for (const q of ptsS) { laneHi = Math.max(laneHi, q[1]); laneLo = Math.min(laneLo, q[1]); }
        return { ptsS, endP, endT };
      };
      const sa = stubOf(Ac, 'M' + l.from, Bc[0], lp, true);
      const sb = stubOf(Bc, 'M' + l.to, Ac[0], lm, false);
      poly('lead', o, sa.ptsS, { node, pot, plus:true, link:l, says, hit:true, open:true, from:fromId, to:'open' + i, metres:lp });
      poly('lead', o, sb.ptsS.slice().reverse(), { node, pot, plus:false, link:l, says, hit:true, open:true, from:'open' + i, to:toId, metres:lm });
      put({ role:'plug', order:o, pot, node, pts:[[sa.endP[0], sa.endP[1]]], male:true, along:sa.endT });
      put({ role:'plug', order:o, pot, node, pts:[[sb.endP[0], sb.endP[1]]], male:false, along:sb.endT });
  ...
  const round4 = (pp, r2) => { const out = [pp[0]];
    for (let i2 = 1; i2 + 1 < pp.length; i2++) { const a2 = pp[i2 - 1], b2 = pp[i2], c2 = pp[i2 + 1];
      ...
      out.push(s2); }
    out.push(pp[pp.length - 1]); return out; };
  // THE HOME RUNS SPEAK THE SAME LANGUAGE: square out of the box face, along a line of their own well
  // clear of the row, and square into the inverter. They too end INSIDE the box they name.
  const nEdge = [xn, nP[1] + boxH * 0.47];
  const pEdge = [xp, pP[1] - boxH * 0.47];

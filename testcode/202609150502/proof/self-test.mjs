...
check('registered lens contracts and pure deterministic bounds',()=>{for(const L of [ring,table]){for(const key of ['id','from','wants','draws','hint','layout','edges','labels'])assert.ok(key in L);L.layout(core,view,a);L.layout(core,view,b);assert.deepEqual(a,b);assert.ok(a.every(Number.isFinite));}ring.layout(core,view,a);for(let i=0;i<core.N;i++){assert.ok(a[2*i]>=0&&a[2*i]<=view.w);assert.ok(a[2*i+1]>=0&&a[2*i+1]<=view.h);}const source=fs.readFileSync(new URL('ring.mjs',root),'utf8');assert.ok(!/\bfetch\s*\(|#[a-fA-F0-9]{6}\b|history\.|state\.\w+\s*=/.test(source));});

...
(function (root) {
  ...
  function build(inp) {
    ...
    c.box_spacing_m = Math.round(Math.abs(fP0 - fM0) * c.width_m * 1e6) / 1e6;
    ...
    const home_m = 2 * c.near_end_m + 4 * (+c.termination_extra_m || 0), array_m = field_m - 2 * c.near_end_m;
    ...
    (() => { if (Math.round(+c.n_boxes) === 1 && Math.abs(fP0 - fM0) > 1e-6)
        return no_('lead_only_from_terminal_box', 'this module has ONE junction box, so both leads leave that box and the box spacing is zero; the + and the − are asked for ' + c.box_spacing_m.toFixed(3) + ' m apart. A module lead can only leave a junction box that carries one.');

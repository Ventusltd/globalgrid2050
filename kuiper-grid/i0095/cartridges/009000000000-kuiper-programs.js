...
function makeFireBar(){
  ...
  pin.addEventListener('keydown', e => {
    ...
    // BARE WORDS ON A SUBSTATION. With one station's map already on the screen, "neighbours", "pulse",
    // "outage <far station name or id>" and "back" fire that same station again with the one input
    // changed, exactly as the chips do. Anywhere else the same words fall through to the search.
    if (lastFire && lastFire.ran && lastFire.name === 'substation' && lastFire.inputs && lastFire.inputs.site
        && (/^(neighbours|pulse|back)$/i.test(q) || /^outage\s+\S/i.test(q))) {
      const want = q.toLowerCase(), si = Object.assign({}, lastFire.inputs);
      delete si.neighbours; delete si.pulse; delete si.outage;
      if (want === 'neighbours') si.neighbours = 1;
      else if (want === 'pulse') si.pulse = 1;
      else if (want !== 'back') si.outage = q.replace(/^outage\s+/i, '').trim();
      pin.value = '';
      fireCommand('fire substation ' + JSON.stringify(si), null); return; }
...
function fireCommand(text, rec, inPlace){
  ...
  lastFire = { ran:true, name:out.name, family:out.family, numbers:out.numbers, inputs:out.inputs || {} };
...
(function (root) {
  ...
  // ---- TAPPING A FAR STATION WALKS THERE ---------------------------------------------------------
  // The far end was only ever NAMED. A map you cannot walk is a picture. The engine's own hit-testing
  // for shapes (shapeAsk) finds the mark under the finger; a tap is a press and a release in the same
  // place, so a pan is never mistaken for a tap. A far end with a netlist re-fires this same command
  // for that site; one without SAYS SO in words rather than doing nothing.
  try {
    let tapAt = null;
    root.addEventListener('pointerdown', function (e) { tapAt = [e.clientX, e.clientY, Date.now()]; }, { passive:true });
    root.addEventListener('pointerup', function (e) {
      try {
        const t = tapAt; tapAt = null;
        if (!t || Math.hypot(e.clientX - t[0], e.clientY - t[1]) > 9 || Date.now() - t[2] > 700) return;
        if (typeof sldShapes === 'undefined' || !sldShapes || !sldShapes.substation) return;
        if (typeof shapeAsk !== 'function') return;
        const s = shapeAsk(e.clientX, e.clientY);
        if (!s || s.role !== 'sldstation' || !s.far) return;
        const label = String(s.farLabel || s.far);
        if ((DATA().netlists || {})[s.far]) {
          if (typeof fireCommand === 'function') fireCommand('fire substation {"site":"' + label.replace(/"/g, '') + '"}', null);
        } else {
          sldShapes.saying = label + ' is named at the end of this circuit but has no netlist here yet, so it cannot be opened.';

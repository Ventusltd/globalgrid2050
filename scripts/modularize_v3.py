...
def is_already_modular(html: str) -> bool:
    return "V3 modular app scripts" in html and all(name in html for name in MODULE_ORDER)
...
def slice_section(script: str, start_marker: str, end_marker: str | None) -> str:
    ...
    end = len(script) if end_marker is None else script.find(end_marker, start + len(start_marker))
    if end < 0:
...
def write_modules(script: str) -> None:
    ...
    for filename, (start_marker, end_marker) in MODULES.items():
        ...
        (APP_DIR / filename).write_text('"use strict";\n\n' + body, encoding="utf-8")
...
def verify() -> None:
    ...
    bad = [item for item in ["gis-sld-v2", "Financial Sandbox V2", "__V3_MODULE_SCRIPT_TAGS__"] if item in html]
    if bad:
...
def main() -> int:
    ...
    html = normalise_identity(HTML_PATH.read_text(encoding="utf-8"))
    html_without_script, script = extract_inline_script(html)
    write_modules(script)
    if script:
        html = html_without_script.replace("__V3_MODULE_SCRIPT_TAGS__", module_tags())
        HTML_PATH.write_text(html, encoding="utf-8")
    else:
        HTML_PATH.write_text(html_without_script, encoding="utf-8")
    remove_v2_duplicates()
    verify()
    print("[modularize_v3] V3 modularisation verified")
    return 0

/* wafer.mjs - the inward-by-length-and-usage law, generated. Variation inward-by-length-and-usage-lit.
 *
...
const R = 585.5, B = 30, LAT = 597;
const POINT = 1.3, COLOUR = 'radiation';
let ORDER = 'descending';   /* let, not const: drawWith(order) re-renders the same camera */
...
export function place(key, rad, len, fam, maxKey) {
  const r = R*(1-Math.min(1,(len/160)*rad*2)) + shell(key)*B;
  const a = key * GOLDEN;
...
function draw() {
  ...
  footEl.textContent = 'law: inward-by-length-and-usage - Length x usage. ' + shown.toLocaleString() +
    ' of ' + keys.length.toLocaleString() + ' on screen - pack ' + meta.built_utc +
    ' - identity is the key, not the coordinate' +
    ' - across plain/fine/lit this law varies in placement and weight';
}
...
fetch(LAWS_INDEX).then(r => r.ok ? r.json() : null).then(ix => {
  ...
  for (const law of ix.laws) {
    ...
    if (law.id === 'inward-by-length-and-usage') a.setAttribute('aria-current', 'page');
    nav.append(a);
  }
  nav.hidden = false;
 ...
 */
window.__wafer = {
  law: 'inward-by-length-and-usage', variant: 'inward-by-length-and-usage-lit', place, lawsIndex: LAWS_INDEX,

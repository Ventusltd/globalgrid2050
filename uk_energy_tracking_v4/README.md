# UK Energy Tracking V4

## Purpose

`uk_energy_tracking_v4` is the isolated development twin of the stable UK live grid tracker.

The stable tracker remains the reference point. V4 is used for new transport energy work, DESNZ road fuel prices, EV charging comparison logic and experimental UI changes.

## Design rule

V4 must mirror the stable tracker where it provides core grid tracking, but must remain independent where new features are being developed.

The stable tracker must not be touched when testing V4 changes.

## Where V4 mirrors the stable tracker

V4 keeps the same core dashboard concepts:
...
V4 also mirrors the stable tracker update cadence:
...
It uses the same automation pattern as the stable tracker, but writes only into the V4 folder.

## Where V4 is independent
...
V4 has its own live JSON feeds:
...
The V4 energy script writes to:

```text
uk_energy_tracking_v4/live_grid_energy.json
```

The V4 price script writes to:

```text
uk_energy_tracking_v4/live_grid_price.json
```

This means V4 can fail without corrupting the stable tracker data files.

## Current convergence

V4 is converging with the stable tracker in these areas:
...
V4 diverges in these areas:

- V4 has transport energy logic
- V4 includes DESNZ road fuel price cards
- V4 includes Brent crude to pump price explanatory logic
- V4 includes fuel duty and VAT source links
- V4 includes EV charging tariff placeholders
- V4 embeds the Atlas V8 reference map while the exact EV layer path is verified

## Transport energy mandate

The V4 transport layer is intended to compare:
...
V4 is where pump and EV work should continue.

The EV charging tariff file is currently a curated placeholder until reliable operator tariff values are inserted.

The Atlas V8 EV layer is currently embedded as a reference iframe. The exact reusable EV charging layer path should be verified before copying any map layer logic into V4.

## Workflow operations

Manual V4 grid update:

```text
Actions -> fetch_uk_energy_and_prices_v2 -> Run workflow -> slice = both
```

Scheduled V4 grid update:

```text
cron: */5 * * * *
```

V4 transport patch workflow:

```text
.github/workflows/patch_uk_energy_tracking_v4_transport.yml
```

V4 isolation workflow:

```text
.github/workflows/isolate_uk_energy_tracking_v4.yml
```

## Recovery rule

If V4 breaks, compare against the stable tracker:
...
Do not patch the stable tracker when diagnosing V4.

...
The last 24 hours of Git commits were reviewed to understand why the original 5 minute live update behaviour appears to have weakened after oil prices, V4 isolation, DESNZ fuel and EV charging work were added.
...
V4 also recovered at least once after a failed push:

```text
Automated UK grid update V4 (both): 2026-05-25 09:12 UTC
```

This proves the V4 Python update scripts can run and can write V4 JSON feeds.
...
1. The stable tracker workflow and V4 workflow both write to `main` frequently.
...
6. At least one V4 run reached the commit stage but failed during `git push` with a GitHub internal server error.
...
The original 5 minute system was simpler. It mostly had one frequent writer. After V4 and oil work, the repository now has multiple workflows pushing into the same branch. Even if each workflow is logically correct, they compete at Git level and can cause rejected pushes, stale checkouts or delayed commits.
...
Create a V4 only workflow hardening patch that does not touch the stable tracker.

The patch should:

```text
keep V4 offset from stable
use origin based push rather than explicit tokenised URL where possible
add retry around pull and push
stage only V4 JSON outputs
avoid running documentation or repo structure workflows as part of grid updates
```

### Recovery rule

If V4 breaks again, compare against stable but patch only V4:

```text
uk_energy_tracking_v4/index.md
uk_energy_tracking_v4/*.json
scripts/*_v2.py
...
```text
V4 should behave identically to stable for live grid values.
```
...
## Diary entry: 2026-05-25 V4 clone created

V4 was cloned from V2 as a controlled experimental build.
...
V4 becomes the diary led experimental version for price history, graphs, diagnostics and competitor tracking comparison.
```
...
## Diary entry: 2026-05-25 16:46 IST tracker comparison and V4 production focus
...
This is now the current production focus for experimental development. V4 inherits the protected twin logic from V2 but becomes the main diary led build for price history, graphing, diagnostics, competitor tracking comparison and independent validation of electricity price accuracy.

### Current production focus

V4 should become the active proving layer for live electricity price validation while the stable tracker remains untouched.
...
Create a V4 only electricity price history system.
...
Potential V4 only files:

```text
uk_energy_tracking_v4/electricity_price_history.csv
uk_energy_tracking_v4/electricity_price_history.json
uk_energy_tracking_v4/electricity_price_history.xlsx or Excel compatible CSV export
```

Potential UI additions inside V4 only:
...
V4 remains the controlled test bed. Vikram reviews and approves promotion only after the V4 feature works cleanly.

## Diary entry: 2026-05-25 V4 price history table dark UI patch

Purpose:

```text
repair the electricity price history table UI on mobile and desktop
keep the V4 development tracker isolated
...
```text
The V4 price history table rendered with a white table background while the rest of the SCADA page remained dark.
...
insert a normal stylesheet link for /uk_energy_tracking_v4/price-history-ui.css
...
## Diary entry: 2026-05-25 V4 price history correctness patch

Purpose:

```text
correct V4 price history graph and table behaviour without touching the stable tracker
```

Patch method:

```text
keep /uk_energy_tracking_v4/electricity_price_history.json as the active captured history source
...
## Diary entry: 2026-05-25 V4 price history full screen chart patch
...
load V4 captured electricity price history JSON
...
## Diary entry: 2026-05-26 post patch review of V4 price history UI
...
The V4 page now has the external stylesheet link for:
...
This means V4 is no longer silently requesting a missing CSV path during normal chart operation.
...
are the V4 captured evidence trail built from observed values over time.
...
The chart loads the same V4 captured JSON file and applies the currently selected range before opening. This means the main selector still governs the full screen view.
...
The full screen JavaScript is compact and functional but not as readable as the main V4 script. A later maintenance patch should unminify it into a cleaner form before adding more features.
...
The full screen chart does not yet downsample very large retained records. This is fine for the current small V4 captured dataset, but a 12 month or 10 year half hourly record will require level of detail or sampling logic.

### Next recommended technical patch

Do not add more visual features before hardening the V4 automation path.

The next recommended patch should be:

```text
V4 workflow hardening patch
```
...
stage only V4 JSON and price history output files
...
The current V4 price history UI should now be treated as a working prototype layer. Further development should be split into 3 separate future tracks:
...
## Diary preflight: 2026-05-26 00:27 UTC GridBot V4 workflow start

```text
feature: merge_system
purpose: prepare merged historical Elexon System Price and captured Market Index view
rule: read WORK_DIARY before changing files, commit diary preflight first, then run the selected V4 patch
stable tracker: do not touch uk_energy_tracking/
```


## Diary entry: 2026-05-26 V4 merged price source patch

Merged the existing Elexon historical system price CSV with the V4 captured live Market Index trail. The chart and dropdown now use historical context plus new live captured rows, while the live gauge remains separate. System Prices and Market Index Prices remain labelled as different price products.

## Diary entry: 2026-05-26 merge system workflow verification

Verification after running GridBot V4 consolidated price history with feature merge_system.

Confirmed commits:

```text
4ab2123acf11bef22c906aba53d2c753b87e2d81 GridBot V4 diary preflight
4325b77e5e0775feca4f69b3964e6c10edefa94e GridBot V4 price history patch
```

Confirmed result:

```text
uk_energy_tracking_v4/price-history-ui.js now enables the CSV feed
CSV_URL now points to /data/electricity/elexon_system_prices_half_hourly.csv
mergeSystemAndCapturedRows now merges Elexon System Price rows with V4 captured Market Index rows
source card now reports Historical Elexon System Prices plus V4 captured Market Index when CSV rows exist
...
```text
The V4 chart and dropdown should now move from the small captured only record count to the much larger historical Elexon System Price record set plus new live captured Market Index records.
...
## Diary entry: 2026-05-26 V4 range and full screen correction amended before run

Amended before execution after code review. The patch now aligns full screen with the inline chart by loading both the historical Elexon system price CSV and the V4 captured JSON. It adds From and To date controls capped to 60 days, changes rolling cutoffs to millisecond based UTC comparisons, surfaces priceHealth warnings before carbon values in the table health column and highlights warned rows. It also fixes the label so the panel is not described as captured Market Index only when historical System Prices are present.


## Diary entry: 2026-05-26 V4 selected range axis and fullscreen simplification patch
...
### V4 UI controls planned and partially prepared

The V4 price history UI should support:
...
```text
scripts/patch_v4_price_lazy_selectors.py
```

The workflow prepared for applying it is:

```text
.github/workflows/patch_v4_price_lazy_selectors.yml
```

The workflow to run is:

```text
Patch V4 price lazy selectors
```
...
The patch should be checked carefully after running because earlier V4 price history work had a repeated problem where the inline chart was corrected but the full screen chart retained old source logic. Future verification must always inspect both:

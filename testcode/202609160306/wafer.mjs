/* wafer.mjs - the bands-by-key law, generated. Variation bands-by-key-lit.
 *
 * Keys grouped into bands of ten thousand: history as concentric shells.
 *
...
export function place(key, rad, len, fam, maxKey) {
  const r = (Math.floor(key/10000)+1)*R/35 + shell(key)*B;
  const a = key * GOLDEN;
...
function draw() {
  ...
  footEl.textContent = 'law: bands-by-key - Issue bands. ' + shown.toLocaleString() +
...
fetch(LAWS_INDEX).then(r => r.ok ? r.json() : null).then(ix => {
  ...
  for (const law of ix.laws) {
    ...
    if (law.id === 'bands-by-key') a.setAttribute('aria-current', 'page');
    nav.append(a);
  }
  nav.hidden = false;
 ...
 */
window.__wafer = {
  law: 'bands-by-key', variant: 'bands-by-key-lit', place, lawsIndex: LAWS_INDEX,

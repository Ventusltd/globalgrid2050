...
const RELEASE_URL = "contracts/release.v9.6.json";

function invariant(condition, message) {
  if (!condition) throw new Error(`V9.6 canonical projects: ${message}`);
}
...
export async function loadCanonicalProjectsV9_6() {
  const [release, model] = await Promise.all([fetchRelease(), loadCanonicalProjectsV9_1()]);
  invariant(release.release === "9.6", "release contract mismatch");
  invariant(release.frozen_parent?.release === "9.5.1", "frozen-parent release mismatch");
  ...
  return Object.freeze({
    ...
    metadata: Object.freeze({ ...model.metadata, ui_release: "9.6" }),

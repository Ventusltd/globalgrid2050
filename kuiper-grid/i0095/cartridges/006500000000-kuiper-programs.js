...
function stringDrawing(g){
  ...
  g.links.forEach((l, i) => {
    ...
    const outA = port ? (Bc[0] >= Ac[0] ? 1 : -1) : (Bc[1] >= Ac[1] ? 1 : -1);
    const outB = port ? (Ac[0] >= Bc[0] ? 1 : -1) : (Ac[1] >= Bc[1] ? 1 : -1);
    const A = port ? [faceX(Ac[0], Bc[0]), Ac[1]] : [Ac[0], faceY(Ac[1], Bc[1])];
    const B = port ? [faceX(Bc[0], Ac[0]), Bc[1]] : [Bc[0], faceY(Bc[1], Ac[1])];
    put({ role:'landing', order:(2 * i + 2) / chain, pts:[[A[0], A[1]]], plus:true });
    put({ role:'landing', order:(2 * i + 2) / chain, pts:[[B[0], B[1]]], plus:false });
    const P0 = port ? [A[0] + lip * outA, A[1]] : [A[0], A[1] + lip * outA];
    const Q0 = port ? [B[0] + lip * outB, B[1]] : [B[0], B[1] + lip * outB];
    ...
    let arc;
    // SAG WITHIN ONE MODULE. An arch that leaves its own module tangles with the tier above it, and on
    // a table that is unreadable. A cable on a table does not fly; it lies on the glass.
    if (sameRow) { const sag = Math.min(wY * 0.44, (rows === 1 ? 0.55 : boxH * 1.1) + d * 0.13 + (i % 3) * 0.08);
    ...
    const full = [A, P0, P].concat(arc.slice(1, arc.length - 1)).concat([Q, Q0, B]);
    for (const q of full) { laneHi = Math.max(laneHi, q[1]); laneLo = Math.min(laneLo, q[1]); }
  ...
  put({ role:'landing', order:0, pts:[[nEdge[0], nEdge[1]]], plus:false });
  put({ role:'landing', order:1, pts:[[pEdge[0], pEdge[1]]], plus:true });
...
function drawSldShapes(){
  ...
  for (const s of D.shapes) {
    ...
    if (s.role === 'landing') { const q = S(s.pts[0]);

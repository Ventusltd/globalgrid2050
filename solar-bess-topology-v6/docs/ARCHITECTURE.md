...
V5 is now the active development branch of the GIS SLD Financial Sandbox. V3 remains the safety baseline.
...
`indexforgis-sld-v5.html` is the main GIS and finance control surface.
...
`module-layout-v5.html` is intentionally separate from the main GIS finance app.
...
The top of `indexforgis-sld-v5.html` loads external libraries first:
...
`gis-sld-v5-ui.js` is the final boot module. It wires the controls, calls `initMap()` and starts the app.
...
User changes input
|
|-- gis-sld-v5-ui.js catches input or change event
|   |
|   |-- recalcDebounced or computeAndDraw
|       |
|       |-- gis-sld-v5-calculations.js calculates physical and electrical stats
|       |   |-- DC megawatts peak
|       |   |-- AC megawatts
|       |   |-- module count
|       |   |-- block count
...
|       |   |-- logistics
|       |
|       |-- gis-sld-v5-finance.js calculates finance
|       |   |-- revenue
|       |   |-- CAPEX
|       |   |-- BESS value
|       |   |-- development finance
|       |   |-- warnings
|       |
|       |-- gis-sld-v5-drawing.js builds GeoJSON
|       |   |-- point of connection
|       |   |-- customer substation
|       |   |-- export cable
...
|       |
|       |-- gis-sld-v5-map.js renders GeoJSON layers
|       |
|       |-- gis-sld-v5-ui-core.js updates technical summary and legend
```
...
The geometry engine is in `gis-sld-v5-drawing.js`.
...
`gis-sld-v5-calculations.js` calculates the physical and electrical project summary.
...
`gis-sld-v5-finance.js` calculates:
...
`gis-sld-v5-export.js` exports the current topology to GeoJSON.
...
Injected by `gis-sld-v5-ui.js` near the draw button:
...
|
|-- module-layout-v5.js reads inputs
|   |
...
## 18. Safe development rules for V5

1. V3 must remain untouched unless explicitly requested.
2. V5 is now the active development copy.
...
10. Detailed design should remain a later paid layer unless deliberately brought into V5 as optional toggled layers.
...
```text
1. Some comments still reference V3 history because V5 was cloned from V3.
...
The V5 app currently has 2 related but separate tools:

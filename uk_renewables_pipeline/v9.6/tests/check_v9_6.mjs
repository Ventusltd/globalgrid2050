...
assert.equal(contract.ui_contract.mobile_project_rendering, "all matching rows");
assert.equal(contract.ui_contract.mobile_news_rendering, "all matching headlines");
assert.equal(contract.ui_contract.mobile_table_horizontal_scroll_required, true);
assert.equal(contract.ui_contract.mobile_visual_baseline, "V1/V8/V9");
...
assert.match(html, /styles\/v9-6\.css\?v=9\.6-v1-mobile/);
assert.match(html, /scripts\/app-v9-6\.js\?v=9\.6-v1-mobile/);
assert.match(css, /\.tablewrap\s*\{[^}]*overflow-x:\s*auto/s);
assert.match(css, /\.tablewrap table\s*\{[^}]*min-width:\s*1850px/s);
assert.match(css, /\.tablewrap \.hide-mobile\s*\{[^}]*display:\s*table-cell/s);
assert.doesNotMatch(css, /\.tablewrap tr\s*\{[^}]*display:\s*grid/s);
assert.match(projectsPlugin, /const visible = filtered/);
assert.match(projectsPlugin, /state\.filtered = filtered/);
assert.doesNotMatch(projectsPlugin, /MOBILE_PROJECT_BATCH/);
...
assert.match(newspaperPlugin, /const visible = rows/);
assert.doesNotMatch(newspaperPlugin, /MOBILE_NEWS_BATCH/);
...
console.log("V9.6: PASS (V9.5.1 frozen; 7,680 loaded; 100–500 MW = 476; V1-style mobile table scroll)");

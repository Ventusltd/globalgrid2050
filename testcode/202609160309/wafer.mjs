/* wafer.mjs - the radius-by-length law, generated. Variation radius-by-length-lit.
 *
...
fetch(LAWS_INDEX).then(r => r.ok ? r.json() : null).then(ix => {
  ...
  for (const law of ix.laws) {
    ...
    if (law.id === 'radius-by-length') a.setAttribute('aria-current', 'page');
    nav.append(a);
  }
  nav.hidden = false;
 ...
 */
window.__wafer = {
  law: 'radius-by-length', variant: 'radius-by-length-lit', place, lawsIndex: LAWS_INDEX,

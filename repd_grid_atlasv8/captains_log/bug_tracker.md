...
**Atlas:** repd_grid_atlasv8 — Ventus OS V7 Experimental
**Engine:** ventus-corev8engine.js
...
**Root Cause:**
During messy reverts of the `bess_operational` implementation, the constant `ZONE_DRAW_DEFAULT_KM` was declared twice in `ventus-corev8engine.js`. JavaScript strict mode (enforced by module scope) throws a fatal SyntaxError on duplicate `const`, halting all execution before the map initialises.

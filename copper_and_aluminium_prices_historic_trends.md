...
## ⚡ Live Daily Market Prices (Auto-Updated)

<div id="live-price-table-container" style="background-color: #1a1a1a; padding: 20px; border-radius: 8px; border: 1px solid #333; margin-bottom: 40px; overflow-x: auto;">
  <table style="width:100%; text-align: left; border-collapse: collapse; font-family: sans-serif; color: #e0e0e0; min-width: 500px;">
    <thead>
      <tr style="border-bottom: 2px solid #444;">
        <th style="padding: 12px; font-weight: bold;">Metal</th>
        <th style="padding: 12px; font-weight: bold;">USD per tonne</th>
        <th style="padding: 12px; font-weight: bold;">EUR per tonne</th>
        <th style="padding: 12px; font-weight: bold; color: #888;">Last Updated</th>
      </tr>
    </thead>
    <tbody id="live-price-body">
      <tr><td colspan="4" style="padding: 12px; text-align: center; color: #888;">Fetching latest market data... ⏳</td></tr>
    </tbody>
  </table>
</div>

<script>
  // Fetch the JSON generated by your GitHub Action and populate the table
  fetch('latest_prices.json')
    ...
    .then(data => {
      document.getElementById('live-price-body').innerHTML = `
        <tr style="border-bottom: 1px solid #333; background-color: #121212;">
          <td style="padding: 12px; font-weight: bold; color: #FF007F; font-size: 1.1em;">Copper</td>
          <td style="padding: 12px; font-size: 1.1em;">$${data.copper_usd.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</td>
          <td style="padding: 12px; font-size: 1.1em;">€${data.copper_eur.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</td>
          <td style="padding: 12px; font-size: 0.85em; color: #888;">${data.last_updated}</td>
        </tr>
        <tr style="background-color: #1a1a1a;">
          <td style="padding: 12px; font-weight: bold; color: #00FFFF; font-size: 1.1em;">Aluminium</td>
          <td style="padding: 12px; font-size: 1.1em;">$${data.aluminium_usd.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</td>
          <td style="padding: 12px; font-size: 1.1em;">€${data.aluminium_eur.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</td>
          <td style="padding: 12px; font-size: 0.85em; color: #888;">${data.last_updated}</td>
        </tr>
      `;
    })
    .catch(error => {
      document.getElementById('live-price-body').innerHTML = `
        <tr><td colspan="4" style="padding: 12px; color: #FF007F; text-align: center;">⚠️ Unable to load live prices right now. Awaiting next GitHub Action run.</td></tr>
      `;
...
<script>
  ...
  // 5. Layout configs (Dark Theme, Legend Top-Left)
  const layoutBase = {
    hovermode: 'x unified',
    autosize: true, 
